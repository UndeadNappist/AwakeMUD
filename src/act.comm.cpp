...
extern bool can_send_act_to_target(struct char_data *ch, bool hide_invisible,
                                   struct obj_data *obj, void *vict_obj,
                                   struct char_data *to, int type);
...
ACMD(do_say) {
  ...
  if (!*argument)
  ...
  else if (subcmd != SCMD_OSAY && !PLR_FLAGGED(ch, PLR_MATRIX) &&
           !char_can_make_noise(ch))
    ...
    if (ch->in_veh) {
      if (subcmd == SCMD_OSAY) {
        ...
        for (tmp = ch->in_veh->people; tmp; tmp = tmp->next_in_veh) {
          ...
          if (can_send_act_to_target(ch, FALSE, NULL, NULL, tmp, TO_ROOM)) {
            // They're a valid target, so send the message with a raw
            // perform_act() call.
            store_message_to_history(
                tmp->desc, COMM_CHANNEL_OSAYS,
                str_dup(perform_act(buf, ch, NULL, NULL, tmp)));
          }
        }
      } else {
          ...
          if (tmp != ch) {
            if (to) {
              if (to == tmp)
                ...
                sprintf(buf2, " to %s",
                        CAN_SEE(tmp, to)
                            ? (found_mem(GET_MEMORY(tmp), to)
                                   ? CAP(found_mem(GET_MEMORY(tmp), to)->mem)
                                   : GET_NAME(to))
                            : "someone");
            }
            if (success > 0) {
              ...
              if (suc > 0 || IS_NPC(tmp))
                sprintf(buf, "$z says%s in %s, \"%s^n\"", (to ? buf2 : ""),
                        skills[GET_LANGUAGE(ch)].name, argument);
              else
                sprintf(buf, "$z speaks%s in a language you don't understand.",
                        (to ? buf2 : ""));
            } else
            ...
            // Note: includes act()
            store_message_to_history(
                tmp->desc, COMM_CHANNEL_SAYS,
                str_dup(act(buf, FALSE, ch, NULL, tmp, TO_VICT)));
          }
      }
    } else {
      /** new code by WASHU **/
      if (subcmd == SCMD_OSAY) {
        ...
        for (tmp = (ch->in_veh ? ch->in_veh->people : ch->in_room->people); tmp;
    ...
    if (PRF_FLAGGED(ch, PRF_NOREPEAT))
      ...
      if (subcmd == SCMD_OSAY) {
        ...
        if (to)
          sprintf(buf2, " to %s",
                  CAN_SEE(ch, to)
                      ? (found_mem(GET_MEMORY(ch), to)
                             ? CAP(found_mem(GET_MEMORY(ch), to)->mem)
                             : GET_NAME(to))
                      : "someone");
...
ACMD(do_exclaim) {
  ...
  if (ch->in_veh) {
    for (struct char_data *tmp = ch->in_veh->people; tmp;
         tmp = tmp->next_in_veh) {
      // Replicate act() in a way that lets us capture the message.
      if (can_send_act_to_target(ch, FALSE, NULL, NULL, tmp, TO_ROOM)) {
        // They're a valid target, so send the message with a raw perform_act()
        // call.
        store_message_to_history(
            tmp->desc, COMM_CHANNEL_SAYS,
            str_dup(perform_act(buf, ch, NULL, NULL, tmp)));
    ...
    for (struct char_data *tmp =
             (ch->in_veh ? ch->in_veh->people : ch->in_room->people);
...
void perform_tell(struct char_data *ch, struct char_data *vict, char *arg) {
  ...
  store_message_to_history(vict->desc, COMM_CHANNEL_TELLS,
                           str_dup(act(buf, FALSE, ch, 0, vict, TO_VICT)));

  if (PRF_FLAGGED(ch, PRF_NOREPEAT))
    send_to_char(OK, ch);
  else {
    sprintf(buf, "^rYou tell $n, '%s'^n", arg);
    store_message_to_history(ch->desc, COMM_CHANNEL_TELLS,
                             str_dup(act(buf, FALSE, vict, 0, ch, TO_VICT)));
  }
...
ACMD(do_tell) {
  ...
  if (PRF_FLAGGED(vict, PRF_NOTELL))
    ...
    act("$E's editing right now, try again later.", FALSE, ch, 0, vict,
        TO_CHAR);
  else
...
ACMD(do_spec_comm) {
  ...
  if (!*buf || !*buf2) {
    ...
    if (ch->in_veh->cspeed > SPEED_IDLE) {
      send_to_char(
          "You're going to hit your head on a lamppost if you try that.\r\n",
          ch);
      return;
    }
    ...
    if ((vict = get_char_room_vis(ch, buf))) {
      ...
      if (success > 0) {
        ...
        store_message_to_history(
            ch->desc, COMM_CHANNEL_SAYS,
            str_dup(act(buf, FALSE, ch, NULL, vict, TO_CHAR)));
        suc = success_test(GET_SKILL(vict, GET_LANGUAGE(ch)), 4);
        if (suc > 0)
          sprintf(buf, "From within %s^n, $z says to you in %s, \"%s^n\"\r\n",
                  GET_VEH_NAME(ch->in_veh), skills[GET_LANGUAGE(ch)].name,
                  buf2);
        else
          sprintf(buf,
                  "From within %s^n, $z speaks in a language you don't "
                  "understand.\r\n",
        ...
        sprintf(buf, "$z mumbles incoherently from %s.\r\n",
                GET_VEH_NAME(ch->in_veh));
      store_message_to_history(
          vict->desc, COMM_CHANNEL_SAYS,
          str_dup(act(buf, FALSE, ch, NULL, vict, TO_VICT)));
    } else {
    ...
    ch->in_veh = last_veh;
             ...
             !((veh = get_veh_list(buf, ch->in_room->vehicles, ch)) &&
               subcmd == SCMD_WHISPER))
    send_to_char(NOPERSON, ch);
  else if (vict == ch)
    send_to_char("You can't get your mouth close enough to your ear...\r\n",
           ...
           !(IS_ASTRAL(vict) || PLR_FLAGGED(vict, PLR_PERCEIVE) ||
             IS_DUAL(vict)))
    ...
    if (veh) {
      ...
      sprintf(buf, "You lean into a %s and say, \"%s\"", GET_VEH_NAME(veh),
              buf2);
      send_to_char(buf, ch);
      store_message_to_history(ch->desc, COMM_CHANNEL_SAYS, str_dup(buf));
      for (vict = veh->people; vict; vict = vict->next_in_veh) {
        if (success > 0) {
          ...
          if (suc > 0)
            sprintf(
                buf,
                "From outside, $z^n says into the vehicle in %s, \"%s^n\"\r\n",
                skills[GET_LANGUAGE(ch)].name, buf2);
          else
            sprintf(buf, "From outside, $z^n speaks into the vehicle in a "
                         "language you don't understand.\r\n");
        } else
          sprintf(
              buf,
              "From outside, $z mumbles incoherently into the vehicle.\r\n");
        store_message_to_history(
            vict->desc, COMM_CHANNEL_SAYS,
            str_dup(act(buf, FALSE, ch, NULL, vict, TO_VICT)));
      }
    ...
    store_message_to_history(vict->desc, COMM_CHANNEL_SAYS,
                             str_dup(act(buf, FALSE, ch, 0, vict, TO_VICT)));
    if (PRF_FLAGGED(ch, PRF_NOREPEAT))
      send_to_char(OK, ch);
    else {
      sprintf(buf, "You %s $N, \"%s%s^n\"", action_sing, buf2,
              (subcmd == SCMD_WHISPER) ? "" : "?");
      store_message_to_history(ch->desc, COMM_CHANNEL_SAYS,
                               str_dup(act(buf, FALSE, ch, 0, vict, TO_CHAR)));
    }
...
ACMD(do_page) {
  ...
  if (IS_NPC(ch))
    ...
    if ((vict = get_char_vis(ch, arg)) != NULL) {
      ...
      store_message_to_history(vict->desc, COMM_CHANNEL_PAGES,
                               str_dup(act(buf, FALSE, ch, 0, vict, TO_VICT)));
      if (PRF_FLAGGED(ch, PRF_NOREPEAT))
        send_to_char(OK, ch);
      else
        store_message_to_history(
            ch->desc, COMM_CHANNEL_PAGES,
            str_dup(act(buf, FALSE, ch, 0, vict, TO_CHAR)));
      return;
    } else
...
ACMD(do_radio) {
  ...
  for (i = 0; !radio && i < NUM_WEARS; i++)
    if (GET_EQ(ch, i)) {
      if (GET_OBJ_TYPE(GET_EQ(ch, i)) == ITEM_RADIO)
        ...
        for (struct obj_data *obj2 = GET_EQ(ch, i)->contains; obj2;
  ...
  if (!*one) {
    ...
    if (i > MAX_RADIO_FREQUENCY) {
      sprintf(buf, "$p cannot center a frequency higher than %d MHz.",
              MAX_RADIO_FREQUENCY);
      act(buf, FALSE, ch, radio, 0, TO_CHAR);
    } else if (i < MIN_RADIO_FREQUENCY) {
      sprintf(buf, "$p cannot center a frequency lower than %d MHz.",
              MIN_RADIO_FREQUENCY);
      act(buf, FALSE, ch, radio, 0, TO_CHAR);
    } else {
    ...
    if ((i = atoi(two))) {
      ...
      if (!GET_OBJ_VAL(radio, (cyberware ? 6 : (vehicle ? 5 : 3))))
        act("$p's crypt mode is already disabled.", FALSE, ch, radio, 0,
            TO_CHAR);
      else {
...
ACMD(do_broadcast) {
  ...
  if (frequency > 0) {
    if (crypt) {
      sprintf(buf, "^y\\%s^y/[%d MHz, %s](CRYPTO-%d): %s^N", voice, frequency,
              skills[GET_LANGUAGE(ch)].name, crypt, argument);
      sprintf(buf2,
              "^y\\Garbled Static^y/[%d MHz, Unknown](CRYPTO-%d): ***ENCRYPTED "
              "DATA***^N",
              frequency, crypt);
      sprintf(buf4,
              "^y\\Unintelligible Voice^y/[%d MHz, Unknown](CRYPTO-%d): %s",
              frequency, crypt, buf3);
      sprintf(
          buf3,
          "^y\\%s^y/[%d MHz, Unknown](CRYPTO-%d): (something incoherent...)^N",
          voice, frequency, crypt);
    } else {
      sprintf(buf, "^y\\%s^y/[%d MHz, %s]: %s^N", voice, frequency,
              skills[GET_LANGUAGE(ch)].name, argument);
      sprintf(buf2, "^y\\%s^y/[%d MHz, Unknown]: %s^N", voice, frequency,
              argument);
      sprintf(buf4, "^y\\Unintelligible Voice^y/[%d MHz, Unknown]: %s",
              frequency, buf3);
      sprintf(buf3, "^y\\%s^y/[%d MHz, Unknown]: (something incoherent...)^N",
              voice, frequency);
    }

  } else {
    if (crypt) {
      sprintf(buf, "^y\\%s^y/[All Frequencies, %s](CRYPTO-%d): %s^N", voice,
              skills[GET_LANGUAGE(ch)].name, crypt, argument);
      sprintf(buf2,
              "^y\\Garbled Static^y/[All Frequencies, Unknown](CRYPTO-%d): "
              "***ENCRYPTED DATA***^N",
              crypt);
      sprintf(buf4,
              "^y\\Unintelligible Voice^y/[All Frequencies, "
              "Unknown](CRYPTO-%d): %s",
              crypt, buf3);
      sprintf(buf3,
              "^y\\%s^y/[All Frequencies, Unknown](CRYPTO-%d): (something "
              "incoherent...)^N",
              voice, crypt);
    } else {
      sprintf(buf, "^y\\%s^y/[All Frequencies, %s]: %s^N", voice,
              skills[GET_LANGUAGE(ch)].name, argument);
      sprintf(buf2, "^y\\%s^y/[All Frequencies, Unknown]: %s^N", voice,
              argument);
      sprintf(buf4, "^y\\Unintelligible Voice^y/[All Frequencies, Unknown]: %s",
              buf3);
      sprintf(
          buf3,
          "^y\\%s^y/[All Frequencies, Unknown]: (something incoherent...)^N",
          voice);
    }
  }
  ...
  if (PRF_FLAGGED(ch, PRF_NOREPEAT))
    ...
    store_message_to_history(ch->desc, COMM_CHANNEL_RADIO,
                             str_dup(act(buf, FALSE, ch, 0, 0, TO_CHAR)));
  if (!ROOM_FLAGGED(get_ch_in_room(ch), ROOM_SOUNDPROOF))
    for (d = descriptor_list; d; d = d->next) {
      if (!d->connected && d != ch->desc && d->character &&
          !PLR_FLAGS(d->character)
               .AreAnySet(PLR_WRITING, PLR_MAILING, PLR_EDITING, PLR_MATRIX,
                          ENDBIT) &&
        ...
        if (!IS_NPC(d->character) && !IS_SENATOR(d->character)) {
          ...
          if (d->character->in_veh &&
              (radio = GET_MOD(d->character->in_veh, MOD_RADIO)))
            vehicle = TRUE;

          for (obj = d->character->cyberware; obj && !radio;
          ...
          for (obj = d->character->carrying; obj && !radio;
               obj = obj->next_content)
            if (GET_OBJ_TYPE(obj) == ITEM_RADIO)
              radio = obj;
          ...
          for (obj = ch->in_veh ? ch->in_veh->contents : ch->in_room->contents;
               obj && !radio; obj = obj->next_content)
            if (GET_OBJ_TYPE(obj) == ITEM_RADIO &&
          ...
          if (radio) {
            ...
            if (i == 0 || ((i != -1 && frequency != -1) &&
                           !(frequency >= (i - j) && frequency <= (i + j))))
              continue;
            if (decrypt >= crypt) {
              if (to_room) {
                if (success > 0 || IS_NPC(ch))
                  if (suc > 0 || IS_NPC(ch))
                    if (ROOM_FLAGGED(get_ch_in_room(d->character),
                                     ROOM_NO_RADIO))
                      store_message_to_history(
                          d, COMM_CHANNEL_RADIO,
                          str_dup(
                  ...
                  store_message_to_history(
                      d, COMM_CHANNEL_RADIO,
                      str_dup(act(buf3, FALSE, ch, 0, d->character, TO_VICT)));
              } else {
                if (success > 0 || IS_NPC(ch))
                  if (suc > 0 || IS_NPC(ch))
                    if (ROOM_FLAGGED(get_ch_in_room(d->character),
              ...
              store_message_to_history(
                  d, COMM_CHANNEL_RADIO,
                  str_dup(act(buf2, FALSE, ch, 0, d->character, TO_VICT)));
          }
        } else if (IS_SENATOR(d->character) &&
                   !PRF_FLAGGED(d->character, PRF_NORADIO))
          store_message_to_history(
              d, COMM_CHANNEL_RADIO,
              str_dup(act(buf, FALSE, ch, 0, d->character, TO_VICT)));
      }
    }
...
ACMD(do_gen_comm) {
  ...
  static int channels[] = {PRF_DEAF, PRF_NONEWBIE, PRF_NOOOC, PRF_NORPE,
                           PRF_NOHIRED};
  ...
  if ((PLR_FLAGGED(ch, PLR_NOSHOUT) && subcmd == SCMD_SHOUT) ||
      ...
      (!(PLR_FLAGGED(ch, PLR_RPE) || IS_SENATOR(ch)) &&
       subcmd == SCMD_RPETALK) ||
      (!(PRF_FLAGGED(ch, PRF_QUEST) || IS_SENATOR(ch)) &&
       subcmd == SCMD_HIREDTALK)) {
  ...
  if (subcmd == SCMD_NEWBIE) {
    if (IS_NPC(ch)) {
      ...
      return;
               ...
               !PRF_FLAGGED(ch, PRF_NEWBIEHELPER)) {
  ...
  if (PRF_FLAGGED(ch, PRF_NOREPEAT))
    ...
    for (tmp = ch->in_veh ? ch->in_veh->people : ch->in_room->people; tmp;
      ...
      if (tmp != ch) {
        if (success > 0) {
          ...
          if (suc > 0 || IS_NPC(tmp))
            sprintf(buf, "%s$z shouts in %s, \"%s^n\"", com_msgs[subcmd][3],
                    skills[GET_LANGUAGE(ch)].name, argument);
          else
        ...
        store_message_to_history(
            tmp->desc, COMM_CHANNEL_SHOUTS,
            str_dup(act(buf, FALSE, ch, NULL, tmp, TO_VICT)));
      }

    sprintf(buf1, "%sYou shout, \"%s^n\"", com_msgs[subcmd][3], argument);
    // Note that this line invokes act().
    store_message_to_history(ch->desc, COMM_CHANNEL_SHOUTS,
                             str_dup(act(buf1, FALSE, ch, 0, 0, TO_CHAR)));

    was_in = ch->in_room;
    if (ch->in_veh) {
      ch->in_room = get_ch_in_room(ch);
      sprintf(buf1, "%sFrom inside %s, $z %sshouts, '%s^n'",
              com_msgs[subcmd][3], GET_VEH_NAME(ch->in_veh),
      ...
      for (tmp = ch->in_room->people; tmp; tmp = tmp->next_in_room) {
        ...
        if (can_send_act_to_target(ch, FALSE, NULL, NULL, tmp, TO_ROOM)) {
          ...
          store_message_to_history(
              tmp->desc, COMM_CHANNEL_SHOUTS,
              str_dup(perform_act(buf1, ch, NULL, NULL, tmp)));
        }
      }
    } else {
        ...
        for (tmp = get_ch_in_room(ch)->people; tmp; tmp = tmp->next_in_room)
          if (tmp != ch) {
            if (success > 0) {
              ...
              sprintf(buf, "$z shouts incoherently.");
            if (IS_NPC(ch))
              sprintf(buf, "%s$z shouts, \"%s^n\"", com_msgs[subcmd][3],
                      argument);
    ...
    for (d = descriptor_list; d != NULL; d = d->next) {
      if (!d->character ||
          (d->connected != CON_PLAYING &&
           !PRF_FLAGGED(d->character, PRF_MENUGAG)) ||
          PLR_FLAGGED(d->character, PLR_WRITING) ||
          PRF_FLAGGED(d->character, PRF_NOOOC) ||
          PLR_FLAGGED(d->character, PLR_AUTH) ||
          found_mem(GET_IGNORE(d->character), ch))
        continue;
    ...
    sprintf(buf, "%s%s ^W[^rRPE^W]^r %s^n", com_msgs[subcmd][3],
    ...
    sprintf(buf, "%s%s ^y[^YHIRED^y]^Y %s^n", com_msgs[subcmd][3],
            GET_CHAR_NAME(ch), argument);
    send_to_char(ch, "%s\r\n", buf);
    ...
    sprintf(buf, "%s%s |]newbie[| %s^n", com_msgs[subcmd][3], GET_CHAR_NAME(ch),
            argument);
    send_to_char(ch, "%s\r\n", buf);
  ...
  for (i = descriptor_list; i; i = i->next)
    if (!i->connected && i != ch->desc && i->character &&
        ...
        !PLR_FLAGS(i->character)
             .AreAnySet(PLR_WRITING, PLR_MAILING, PLR_EDITING, ENDBIT) &&
        !IS_PROJECT(i->character) &&
        !(ROOM_FLAGGED(get_ch_in_room(i->character), ROOM_SOUNDPROOF) &&
          subcmd == SCMD_SHOUT)) {
      if (subcmd == SCMD_NEWBIE &&
          !(PLR_FLAGGED(i->character, PLR_NEWBIE) || IS_SENATOR(i->character) ||
            PRF_FLAGGED(i->character, PRF_NEWBIEHELPER)))
        continue;
        ...
        store_message_to_history(
            i, channel,
            str_dup(act(buf, FALSE, ch, 0, i->character, TO_VICT | TO_SLEEP)));
      }
    }
}
...
ACMD(do_language) {
  ...
  if (!*arg) {
    ...
    for (i = SKILL_ENGLISH; i <= SKILL_FRENCH; i++)
      if ((GET_SKILL(ch, i)) > 0) {
        sprintf(buf, "%-20s %-17s", skills[i].name,
  ...
  if ((lannum = find_skill_num(arg)) &&
      (lannum >= SKILL_ENGLISH && lannum <= SKILL_FRENCH))
...
void add_phone_to_list(struct obj_data *obj) {
  ...
  if (GET_OBJ_TYPE(obj) == ITEM_CYBERWARE)
    cyber = TRUE;
  sprintf(buf, "%04d%04d", GET_OBJ_VAL(obj, (cyber ? 3 : 0)),
...
ACMD(do_phone) {
  ...
  if (subcmd == SCMD_ANSWER) {
    ...
    if (!k) {
      send_to_char("The phone is picked up and a polite female voice says, "
    ...
    if (k->persona) {
      send_to_icon(
          k->persona,
          "A small telephone symbol blinks in the top left of your view.\r\n");
    } else {
      tch = k->phone->carried_by ? k->phone->carried_by : k->phone->worn_by;
      if (!tch && k->phone->in_obj)
        tch = k->phone->in_obj->carried_by ? k->phone->in_obj->carried_by
                                           : k->phone->in_obj->worn_by;
      if (tch) {
        if (GET_POS(tch) == POS_SLEEPING) {
          if (success_test(GET_WIL(tch),
                           4)) { // Why does it take a successful Willpower test
                                 // to hear your phone?
            GET_POS(tch) = POS_RESTING;
            send_to_char("You are woken by your phone ringing.\r\n", tch);
            if (!GET_OBJ_VAL(k->phone, 3))
              act("$n is startled awake by the ringing of $s phone.", FALSE,
                  tch, 0, 0, TO_ROOM);
        ...
        // Edge case: A phone inside a container inside a container won't ring.
        // But do we even want it to?
      }
    }
    ...
    if (success_test(GET_SKILL(ch, GET_LANGUAGE(ch)), 4) > 0) {
      sprintf(buf, "^Y%s on the other end of the line says in %s, \"%s\"",
              voice, skills[GET_LANGUAGE(ch)].name, argument);
      sprintf(buf2, "$z says into $s phone in %s, \"%s\"",
              skills[GET_LANGUAGE(ch)].name, argument);
    } else {
    ...
    if (phone->dest->persona && phone->dest->persona->decker &&
        phone->dest->persona->decker->ch)
      store_message_to_history(
          ch->desc, COMM_CHANNEL_PHONE,
          str_dup(act(buf, FALSE, ch, 0, phone->dest->persona->decker->ch,
                      TO_DECK)));
    else {
      tch = phone->dest->phone->carried_by;
      if (!tch)
    ...
    if (tch) {
      if (success_test(GET_SKILL(tch, GET_LANGUAGE(ch)), 4) > 0)
        ...
        store_message_to_history(
            tch->desc, COMM_CHANNEL_PHONE,
            str_dup(act("^Y$v speaks in a language you don't understand.",
                        FALSE, ch, 0, tch, TO_VICT)));
    }
    if (!cyber) {
      for (tch = ch->in_veh ? ch->in_veh->people : ch->in_room->people; tch;
           tch = ch->in_veh ? tch->next_in_veh : tch->next_in_room)
        if (tch != ch) {
          if (success_test(GET_SKILL(tch, GET_LANGUAGE(ch)), 4) > 0)
            store_message_to_history(
                tch->desc, COMM_CHANNEL_SAYS,
                str_dup(act(buf2, FALSE, ch, 0, tch, TO_VICT)));
          else
            store_message_to_history(
                tch->desc, COMM_CHANNEL_SAYS,
                str_dup(act("$z speaks into $s phone in a language you don't "
                            "understand.",
                            FALSE, ch, 0, tch, TO_VICT)));
        }
    }
    ...
    if (active) {
      ...
      if (active)
        ...
        if (!phone) {
          ...
          sprintf(buf,
                  "%s would have crashed the mud, whats up with their phone.",
                  GET_CHAR_NAME(ch));
    ...
    if (phone && phone->dest) {
      if (phone->dest->connected && phone->connected)
        ...
        sprintf(ENDOF(buf), "Incoming call from: %08d\r\n",
                phone->dest->number);
    }
    send_to_char(buf, ch);
  }
    ...
    if (k->dest && !k->connected) {
      ...
      if (tch) {
        if (GET_POS(tch) == POS_SLEEPING) {
          if (GET_POS(tch) == POS_SLEEPING) {
            if (success_test(GET_WIL(tch),
                             4)) { // Why does it take a successful Willpower
                                   // test to hear your phone?
              GET_POS(tch) = POS_RESTING;
              send_to_char("You are woken by your phone ringing.\r\n", tch);
              if (!GET_OBJ_VAL(k->phone, 3))
                act("$n is startled awake by the ringing of $s phone.", FALSE,
...
void send_message_history_to_descriptor(struct descriptor_data *d, int channel,
                                        int maximum,
                                        const char *channel_string) {
  // Precondition: No screwy pointers. Allow for NPCs to be passed (we ignore
  // them).
  if (d == NULL) {
    // mudlog("SYSERR: Null descriptor passed to
    // send_message_history_to_descriptor.", NULL, LOG_SYSLOG, TRUE);
    return;
  }
  ...
  if (channel < 0 || channel >= NUM_COMMUNICATION_CHANNELS) {
    sprintf(buf, "SYSERR: Channel %d is not within bounds 0 <= channel < %d.",
  ...
  if (maximum > NUM_MESSAGES_TO_RETAIN) {
    sprintf(buf,
            "SYSERR: send_message_history_to_descriptor asked to send %d "
            "messages, but max message history is %d.",
  ...
  for (nodeStruct<const char *> *currnode = d->message_history[channel].Tail();
       currnode; currnode = currnode->prev) {
    // If they've requested a maximum of X, then skip 20-X of the oldest
    // messages before we start sending them.
    if (skip-- > 0)
      continue;
...
void display_message_history_help(struct char_data *ch) {
  send_to_char(
      "You have past messages available for the following channels:\r\n", ch);
  ...
  send_to_char("\r\nSyntax: HISTORY <channel name> [optional number of "
...
void raw_message_history(struct char_data *ch, int channel, int quantity) {
  ...
  case COMM_CHANNEL_HIRED:
    send_message_history_to_descriptor(ch->desc, channel, quantity,
                                       "over the Hired Talk channel");
    break;
  case COMM_CHANNEL_NEWBIE:
    send_message_history_to_descriptor(ch->desc, channel, quantity,
                                       "over the Newbie channel");
    break;
  case COMM_CHANNEL_OOC:
    send_message_history_to_descriptor(ch->desc, channel, quantity,
                                       "over the OOC channel");
    break;
  ...
  default:
    sprintf(buf,
            "SYSERR: Unrecognized channel/subcmd %d provided to "
            "raw_message_history's channel switch.",
            channel);
    mudlog(buf, NULL, LOG_SYSLOG, TRUE);
    send_to_char(ch, "Sorry, this command appears to have an error. Staff have "
                     "been notified.\r\n");
    break;
  }
}
...
ACMD(do_switched_message_history) {
  ...
  if (*argument) {
    ...
    if (quantity < 1) {
      send_to_char(
          "You must specify a number of history messages greater than 0.\r\n",
          ch);
      return;
    }
  }
