...
extern bool trainable_attribute_is_maximized(struct char_data *ch,
                                             int attribute);
    ...
    *blood_messages[] =
        {
            ...
            "^rCripes, there's blood EVERYWHERE. You can taste it in the "
            "air.\r\n",
            "^rThe walls are practically painted red with blood.\r\n",
            "^rBlood drips from the walls and ceiling, and covers the floor an "
            "inch deep.\r\n",
            "^rThere is gore, blood, guts and parts everywhere. The stench is "
            "horrible.\r\n",
            "^rThe gore is indescribable, and you feel numb and light "
            "headed.\r\n", /* 10 */
            "If you see this, alert an immort that the blood level is too "
            "high.\r\n"};

/* end blood stuff */

char *make_desc(struct char_data *ch, struct char_data *i, char *buf, int act,
                bool dont_capitalize_a_an) {
  ...
  if (!IS_NPC(i) &&
      ...
      (act == 2 ||
       success_test(
           GET_INT(ch) + GET_POWER(ch, ADEPT_IMPROVED_PERCEPT),
           (GET_EQ(i, WEAR_HEAD) ? GET_OBJ_VAL(GET_EQ(i, WEAR_HEAD), 7) : 0) +
               (GET_EQ(i, WEAR_FACE) ? GET_OBJ_VAL(GET_EQ(i, WEAR_FACE), 7)
                                     : 0)) < 1)) {
    int conceal =
    ...
    if (GET_EQ(i, WEAR_ABOUT))
      sprintf(buf + strlen(buf), " wearing %s",
              decapitalize_a_an(GET_OBJ_NAME(GET_EQ(i, WEAR_ABOUT))));
    else if (GET_EQ(i, WEAR_BODY))
      sprintf(buf + strlen(buf), " wearing %s",
              decapitalize_a_an(GET_OBJ_NAME(GET_EQ(i, WEAR_BODY))));
    else if (GET_EQ(i, WEAR_UNDER))
      sprintf(buf + strlen(buf), " wearing %s",
              decapitalize_a_an(GET_OBJ_NAME(GET_EQ(i, WEAR_UNDER))));
  } else {
    struct remem *mem;
    if (!act) {
      strcpy(buf, dont_capitalize_a_an ? decapitalize_a_an(CAP(GET_NAME(i)))
                                       : CAP(GET_NAME(i)));
  ...
  if (!IS_NPC(i) && PRF_FLAGGED(ch, PRF_LONGWEAPON) && GET_EQ(i, WEAR_WIELD))
    sprintf(ENDOF(buf), ", wielding %s",
            decapitalize_a_an(GET_OBJ_NAME(GET_EQ(i, WEAR_WIELD))));
  ...
  *buf = '\0';
  if ((mode == 0) && object->text.room_desc) {
    ...
    if (GET_OBJ_TYPE(object) == ITEM_FOCUS &&
        GET_OBJ_VAL(object, 9) == GET_IDNUM(ch))
      sprintf(ENDOF(buf), " ^Y(Geas)^n");
  } else if (GET_OBJ_NAME(object) &&
    ...
    if (GET_OBJ_TYPE(object) == ITEM_HOLSTER) {
      ...
      if (GET_OBJ_VAL(object, 3) == 1 &&
          ((object->worn_by && object->worn_by == ch) ||
           (object->in_obj && object->in_obj->worn_by &&
            object->in_obj->worn_by == ch)))
        strcat(buf, " ^Y(Ready)");
    } else if (GET_OBJ_TYPE(object) == ITEM_WORN && object->contains &&
  ...
  if ((mode == 7 || mode == 8) && !PRF_FLAGGED(ch, PRF_COMPACT))
    if (GET_OBJ_TYPE(object) == ITEM_WORN && object->contains) {
      for (struct obj_data *cont = object->contains; cont;
  ...
  *buf = '\0';
  ...
  if (vehicle->damage >= 10) {
    ...
    if (vehicle->type == VEH_BIKE && vehicle->people &&
        CAN_SEE(ch, vehicle->people))
      sprintf(buf, "%s%s sitting on ", buf,
              CAP(found_mem(GET_MEMORY(ch), vehicle->people)
  ...
  for (i = list; i; i = i->next_content) {
    ...
    if (ch->in_veh && i->in_room != ch->in_veh->in_room) {
      if (ch->in_veh->cspeed > SPEED_IDLE) {
        if (get_speed(ch->in_veh) >= 200) {
          if (!success_test(GET_INT(ch) + GET_POWER(ch, ADEPT_IMPROVED_PERCEPT),
                            7))
            continue;
          else if (get_speed(ch->in_veh) < 200 &&
                   get_speed(ch->in_veh) >= 120) {
            if (!success_test(
                    GET_INT(ch) + GET_POWER(ch, ADEPT_IMPROVED_PERCEPT), 6))
              continue;
            else if (get_speed(ch->in_veh) < 120 &&
                     get_speed(ch->in_veh) >= 60) {
              if (!success_test(
                      GET_INT(ch) + GET_POWER(ch, ADEPT_IMPROVED_PERCEPT), 5))
                continue;
              else if (!success_test(GET_INT(ch) +
      ...
      if (ch->char_specials.rigging->cspeed > SPEED_IDLE) {
        if (get_speed(ch->char_specials.rigging) >= 240) {
          if (!success_test(GET_INT(ch) + GET_POWER(ch, ADEPT_IMPROVED_PERCEPT),
                            6))
            continue;
          else if (get_speed(ch->char_specials.rigging) < 240 &&
                   get_speed(ch->char_specials.rigging) >= 180) {
            if (!success_test(
                    GET_INT(ch) + GET_POWER(ch, ADEPT_IMPROVED_PERCEPT), 5))
              continue;
            else if (get_speed(ch->char_specials.rigging) < 180 &&
                     get_speed(ch->char_specials.rigging) >= 90) {
              if (!success_test(
                      GET_INT(ch) + GET_POWER(ch, ADEPT_IMPROVED_PERCEPT), 4))
                continue;
              else if (!success_test(GET_INT(ch) +
                                         GET_POWER(ch, ADEPT_IMPROVED_PERCEPT),
                                     3))
    ...
    while (i->next_content) {
      if (i->item_number != i->next_content->item_number ||
          ...
          (i->restring && !i->next_content->restring) ||
          (!i->restring && i->next_content->restring) ||
          (i->restring && i->next_content->restring &&
           strcmp(i->restring, i->next_content->restring)))
        break;
      if (CAN_SEE_OBJ(ch, i)) {
        num++;
      }
...
void diag_char_to_char(struct char_data *i, struct char_data *ch) {
  ...
  if (phys >= 100 ||
      (GET_TRADITION(i) == TRAD_ADEPT && phys >= 0 &&
  ...
  if (phys <= 0)
    strcat(buf, " and is unconscious.\r\n");
           ...
           (GET_TRADITION(i) == TRAD_ADEPT && ment >= 0 &&
            ((100 - ment) / 10) <=
                (GET_POWER(i, ADEPT_PAIN_RESISTANCE) -
...
void look_at_char(struct char_data *i, struct char_data *ch) {
  ...
  if (((GET_EQ(i, WEAR_HEAD) && GET_OBJ_VAL(GET_EQ(i, WEAR_HEAD), 7) > 1) ||
    ...
    if (i != ch && GET_HEIGHT(i) > 0 && GET_WEIGHT(i) > 0) {
      ...
      if (weight > 4) {
        sprintf(buf,
                "$e looks to be about %0.1f meters tall and "
                "appears to weigh about %d kg.",
                height, weight);
        act(buf, FALSE, i, 0, ch, TO_VICT);
      } else {
        sprintf(buf,
                "$e looks to be about %0.1f meters tall and "
                "appears to barely weigh anything.",
                height);
        act(buf, FALSE, i, 0, ch, TO_VICT);
      }
    }
    ...
    if (IS_DUAL(ch) || IS_ASTRAL(ch)) {
      ...
      if (GET_GRADE(i)) {
        ...
        if (GET_METAMAGIC(i, META_MASKING)) {
          if (!GET_GRADE(ch) ||
              success_test(GET_MAG(ch) / 100, GET_MAG(i) / 100) < targ) {
            if (IS_SET(GET_MASKING(i), MASK_INIT) ||
                IS_SET(GET_MASKING(i), MASK_COMPLETE))
              init = FALSE;
            if (IS_SET(GET_MASKING(i), MASK_DUAL) ||
                IS_SET(GET_MASKING(i), MASK_COMPLETE))
              dual = FALSE;
          }
        }
  ...
  if (found) {
    ...
    for (j = 0; j < NUM_WEARS; j++)
      if (GET_EQ(i, j) && CAN_SEE_OBJ(ch, GET_EQ(i, j))) {
        if (GET_OBJ_TYPE(GET_EQ(i, j)) == ITEM_HOLSTER &&
            GET_OBJ_VAL(GET_EQ(i, j), 0) == 2) {
        ...
        } else if ((j == WEAR_BODY || j == WEAR_LARM || j == WEAR_RARM ||
                    j == WEAR_WAIST) &&
                   GET_EQ(i, WEAR_ABOUT)) {
          if (success_test(GET_INT(ch) + GET_POWER(ch, ADEPT_IMPROVED_PERCEPT),
                           4 + GET_OBJ_VAL(GET_EQ(i, WEAR_ABOUT), 7)) >= 2) {
            send_to_char(where[j], ch);
            show_obj_to_char(GET_EQ(i, j), ch, 1);
          }
        } else if (j == WEAR_UNDER &&
                   (GET_EQ(i, WEAR_ABOUT) || GET_EQ(i, WEAR_BODY))) {
          if (success_test(GET_INT(ch) + GET_POWER(ch, ADEPT_IMPROVED_PERCEPT),
                           6 +
                               (GET_EQ(i, WEAR_ABOUT)
                                    ? GET_OBJ_VAL(GET_EQ(i, WEAR_ABOUT), 7)
                                    : 0) +
                               (GET_EQ(i, WEAR_BODY)
                                    ? GET_OBJ_VAL(GET_EQ(i, WEAR_BODY), 7)
                                    : 0)) >= 2) {
            send_to_char(where[j], ch);
            show_obj_to_char(GET_EQ(i, j), ch, 1);
          }
          ...
          if (success_test(GET_INT(ch) + GET_POWER(ch, ADEPT_IMPROVED_PERCEPT),
                           2 + GET_OBJ_VAL(GET_EQ(i, WEAR_ABOUT), 7)) >= 2) {
            send_to_char(where[j], ch);
            show_obj_to_char(GET_EQ(i, j), ch, 1);
          }
        } else if ((j == WEAR_RANKLE || j == WEAR_LANKLE) &&
                   (GET_EQ(i, WEAR_ABOUT) || GET_EQ(i, WEAR_LEGS))) {
          if (success_test(GET_INT(ch) + GET_POWER(ch, ADEPT_IMPROVED_PERCEPT),
                           5 +
                               (GET_EQ(i, WEAR_ABOUT)
                                    ? GET_OBJ_VAL(GET_EQ(i, WEAR_ABOUT), 7)
                                    : 0) +
                               (GET_EQ(i, WEAR_LEGS)
                                    ? GET_OBJ_VAL(GET_EQ(i, WEAR_LEGS), 7)
                                    : 0)) >= 2) {
            send_to_char(where[j], ch);
            show_obj_to_char(GET_EQ(i, j), ch, 1);
          }
  ...
  *buf2 = '\0';
  for (tmp_obj = i->cyberware; tmp_obj; tmp_obj = tmp_obj->next_content)
    ...
    case CYB_BALANCETAIL:
      ...
      break;
    case CYB_EYES:
      if (GET_EQ(i, WEAR_EYES) ||
          (GET_EQ(i, WEAR_HEAD) && GET_OBJ_VAL(GET_EQ(i, WEAR_HEAD), 7) > 1))
        continue;
      if ((IS_SET(GET_OBJ_VAL(tmp_obj, 3), EYE_OPTMAG1) ||
           IS_SET(GET_OBJ_VAL(tmp_obj, 3), EYE_OPTMAG2) ||
           IS_SET(GET_OBJ_VAL(tmp_obj, 3), EYE_OPTMAG3)) &&
...
void list_one_char(struct char_data *i, struct char_data *ch) {
    ...
    if (DISPLAY_HELPFUL_STRINGS_FOR_MOB_FUNCS) {
      if (mob_index[GET_MOB_RNUM(i)].func) {
        ...
        if (mob_index[GET_MOB_RNUM(i)].func == teacher) {
          sprintf(
              ENDOF(buf),
              "^y...%s looks willing to help you practice your skills.^n\r\n",
              HSSH(i));
        }
        if (mob_index[GET_MOB_RNUM(i)].func == metamagic_teacher) {
          ...
          if (GET_TRADITION(ch) != TRAD_MUNDANE)
            sprintf(ENDOF(buf),
                    "^y...%s looks willing to help you train in metamagic "
                    "techniques.^n\r\n",
                    HSSH(i));
        }
        if (mob_index[GET_MOB_RNUM(i)].func == adept_trainer) {
          ...
          if (GET_TRADITION(ch) == TRAD_ADEPT)
            sprintf(
                ENDOF(buf),
                "^y...%s looks willing to help you train your powers.^n\r\n",
                HSSH(i));
        }
        if (mob_index[GET_MOB_RNUM(i)].func == spell_trainer) {
          ...
          if (GET_TRADITION(ch) != TRAD_MUNDANE &&
            ...
            sprintf(ENDOF(buf),
                    "^y...%s looks willing to help you learn new spells.^n\r\n",
                    HSSH(i));
        }
      ...
      if (mob_index[GET_MOB_RNUM(i)].sfunc &&
          mob_index[GET_MOB_RNUM(i)].sfunc != mob_index[GET_MOB_RNUM(i)].func) {
        ...
        if (mob_index[GET_MOB_RNUM(i)].sfunc == teacher) {
          sprintf(
              ENDOF(buf),
              "^y...%s%s looks willing to help you practice your skills.^n\r\n",
              HSSH(i), mob_index[GET_MOB_RNUM(i)].func ? " also" : "");
        }
        if (mob_index[GET_MOB_RNUM(i)].sfunc == metamagic_teacher) {
          ...
          if (GET_TRADITION(ch) != TRAD_MUNDANE)
            sprintf(ENDOF(buf),
                    "^y...%s%s looks willing to help you train in metamagic "
                    "techniques.^n\r\n",
                    HSSH(i), mob_index[GET_MOB_RNUM(i)].func ? " also" : "");
        }
        if (mob_index[GET_MOB_RNUM(i)].sfunc == adept_trainer) {
          ...
          if (GET_TRADITION(ch) == TRAD_ADEPT)
            sprintf(
                ENDOF(buf),
                "^y...%s%s looks willing to help you train your powers.^n\r\n",
                HSSH(i), mob_index[GET_MOB_RNUM(i)].func ? " also" : "");
        }
        if (mob_index[GET_MOB_RNUM(i)].sfunc == spell_trainer) {
          ...
          if (GET_TRADITION(ch) != TRAD_MUNDANE &&
            ...
            sprintf(
                ENDOF(buf),
                "^y...%s%s looks willing to help you learn new spells.^n\r\n",
                HSSH(i), mob_index[GET_MOB_RNUM(i)].func ? " also" : "");
        }
  ...
  if (IS_AFFECTED(i, AFF_INVISIBLE) || IS_AFFECTED(i, AFF_IMP_INVIS) ||
      IS_AFFECTED(i, AFF_SPELLINVIS) || IS_AFFECTED(i, AFF_SPELLIMPINVIS))
  ...
  if (GET_QUI(i) <= 0)
    ...
    if (FIGHTING(i)) {
      ...
      strcat(buf, " is here, fighting ");
      if ((ch->in_veh && ch->in_veh == FIGHTING_VEH(i)) ||
          (ch->char_specials.rigging &&
           ch->char_specials.rigging == FIGHTING_VEH(i)))
        strcat(buf, "YOU!");
      else {
...
void list_char_to_char(struct char_data *list, struct char_data *ch) {
  ...
  // Show room's characters to character. Done this way because
  // list_char_to_char should have been split for vehicles but wasn't.
  for (i = list; i; i = i->next_in_room) {
    // Skip them if they're invisible to us, or if they're us and we're not
    // rigging.
    if (!CAN_SEE(ch, i) || !(ch != i || ch->char_specials.rigging) ||
        (ch->in_veh && i->in_veh == ch->in_veh)) {
      continue;
    }
  ...
  *buf = '\0';
  ...
  for (door = 0; door < NUM_OF_DIRS; door++) {
    if (EXIT(ch, door) && EXIT(ch, door)->to_room &&
        EXIT(ch, door)->to_room != &world[0]) {
      if (GET_REAL_LEVEL(ch) >= LVL_BUILDER) {
        sprintf(
            buf2, "%-5s - [%5ld] %s%s\r\n", dirs[door],
            EXIT(ch, door)->to_room->number, EXIT(ch, door)->to_room->name,
        ...
        if (!IS_ASTRAL(ch) && !LIGHT_OK(ch))
          strcat(buf2, "Too dark to tell.\r\n");
        else {
          if (IS_SET(EXIT(ch, door)->exit_info, EX_CLOSED)) {
            SPECIAL(call_elevator);
            SPECIAL(elevator_spec);
            // If it leads into an elevator shaft or car from a landing, it's an
            // elevator door. If it leads into a landing from an elevator shaft
            // or car, it's an elevator door.
            if (EXIT(ch, door)->to_room &&
                (((ROOM_FLAGGED(EXIT(ch, door)->to_room, ROOM_ELEVATOR_SHAFT) ||
                   EXIT(ch, door)->to_room->func == elevator_spec) &&
                  get_ch_in_room(ch)->func == call_elevator) ||
                 ((ROOM_FLAGGED(get_ch_in_room(ch), ROOM_ELEVATOR_SHAFT) ||
                   get_ch_in_room(ch)->func == elevator_spec) &&
                  EXIT(ch, door)->to_room->func == call_elevator))) {
              strcat(buf2, "A pair of closed elevator doors");
            } else
              sprintf(ENDOF(buf2), "A closed %s",
...
ACMD(do_exits) { disp_long_exits(ch, FALSE); }
    ...
    if (world[i].blood > 0) {
      ...
      if (!ROOM_FLAGGED(&world[i], ROOM_INDOORS)) {
        if (weather_info.sky == SKY_RAINING) {
          world[i].blood--;
          sprintf(buf,
                  "%s the blood staining this area washes away in the rain.",
                  world[i].blood > 3
                      ? "Some of"
                      : (world[i].blood > 1 ? "Most of" : "The last of"));
          send_to_room(buf, &world[i]);
        } else if (weather_info.sky == SKY_LIGHTNING) {
          world[i].blood -= 2;
          sprintf(buf,
                  "%s the blood staining this area washes away in the pounding "
                  "rain.",
                  world[i].blood > 4
                      ? "Some of"
                      : (world[i].blood > 1 ? "Most of" : "The last of"));
...
void look_in_veh(struct char_data *ch) {
  ...
  if (!(AFF_FLAGGED(ch, AFF_RIG) || PLR_FLAGGED(ch, PLR_REMOTE))) {
    if (!ch->in_veh->in_room && !ch->in_veh->in_veh) {
      sprintf(buf, "SYSERR: Character %s is not in a room or vehicle.",
              GET_CHAR_NAME(ch));
    ...
    send_to_char(ch, ch->vfront ? ch->in_veh->inside_description
                                : ch->in_veh->rear_description);
    CCHAR = "^g";
    CGLOB = KGRN;
  ...
  if (!ch->in_room || PLR_FLAGGED(ch, PLR_REMOTE)) {
    ...
    if (!veh->in_room && !veh->in_veh) {
      sprintf(buf, "SYSERR: Vehicle %s is not in a room or vehicle.",
              GET_VEH_NAME(veh));
      mudlog(buf, ch, LOG_SYSLOG, TRUE);
      send_to_char("ALERT AN IMM!!\r\n", ch);
      return;
...
void look_at_room(struct char_data *ch, int ignore_brief) {
  ...
  if (!ch->in_room) {
    send_to_char("Your mind is blasted by the eldritch horrors of the unknown "
                 "void you're drifting in.\r\n",
                 ch);
    sprintf(buf, "SYSERR: %s tried to look at their room but is nowhere!",
            GET_CHAR_NAME(ch));
    mudlog(buf, ch, LOG_SYSLOG, TRUE);
    return;
  }
  ...
  if ((PRF_FLAGGED(ch, PRF_ROOMFLAGS) && GET_REAL_LEVEL(ch) >= LVL_BUILDER)) {
    ROOM_FLAGS(ch->in_room)
        .PrintBits(buf, MAX_STRING_LENGTH, room_bits, ROOM_MAX);
    sprintf(buf2, "^C[%5ld] %s [ %s ]^n\r\n", GET_ROOM_VNUM(ch->in_room),
            GET_ROOM_NAME(ch->in_room), buf);
    send_to_char(buf2, ch);
  } else
    send_to_char(ch, "^C%s^n\r\n", GET_ROOM_NAME(ch->in_room), ch);

  // TODO: Why is this code here? If you're in a vehicle, you do look_in_veh()
  // above right?
  ...
  if (GET_BACKGROUND_COUNT(ch->in_room) && (IS_ASTRAL(ch) || IS_DUAL(ch))) {
    if (GET_BACKGROUND_AURA(ch->in_room) == AURA_POWERSITE) {
      ...
      case 1:
        send_to_char(
            "^CAstral energy seems to be slightly concentrated here.^n\r\n",
            ch);
        break;
      case 2:
        send_to_char(
            "^CAstral energy seems to be mildly concentrated here.^n\r\n", ch);
        break;
      case 3:
      ...
      case 4:
        send_to_char(
            "^CAstral energy seems to be heavily concentrated here.^n\r\n", ch);
        break;
      case 5:
        send_to_char("^CThe power of the astral energy here is nearly "
                     "overwhelming.^n\r\n",
                     ch);
        break;
      default:
        send_to_char("^RA horrific quantity of astral energy tears at your "
                     "senses.^n\r\n",
                     ch);
        sprintf(buf,
                "SYSERR: '%s' (%ld) has powersite background count %d, which "
                "is greater than displayable max of 5.",
  ...
  if (!ROOM_FLAGGED(ch->in_room, ROOM_INDOORS)) {
    if (IS_WATER(ch->in_room)) {
      if (weather_info.sky >= SKY_RAINING) {
        send_to_char(
            ch, "^cThe water around you is dimpled by the falling rain.^n\r\n");
      }
    } else {
      if (weather_info.sky >= SKY_RAINING) {
        send_to_char(
            ch, "^cRain splashes into the puddles around your feet.^n\r\n");
      } else if (weather_info.lastrain < 5) {
        send_to_char(
            ch, "^cThe ground is wet, it must have rained recently.^n\r\n");
      }
    }
  }
  if (ch->in_room->poltergeist[0])
    send_to_char(
        "^cAn invisible force is whipping small objects around the area.^n\r\n",
        ch);
  ...
  if (ROOM_FLAGGED(ch->in_room, ROOM_ELEVATOR_SHAFT)) {
    ...
    for (int index = 0; !match && index < num_elevators; index++) {
      // Iterate through floors to see if a given floor matches the shaft's
      // vnum.
      for (int floor = 0; !match && floor < elevator[index].num_floors;
           floor++) {
        // Check for a match.
        if (elevator[index].floor[floor].shaft_vnum == ch->in_room->number) {
          ...
          if (world[real_room(elevator[index].room)].rating == floor)
            send_to_char("^RThe massive bulk of an elevator car fills the "
                         "hoistway, squeezing you aside.^n\r\n",
                         ch);
          match = TRUE;
        }
      }
...
void look_in_direction(struct char_data *ch, int dir) {
  if (EXIT(ch, dir)) {
    ...
    if (IS_SET(EXIT(ch, dir)->exit_info, EX_DESTROYED) &&
        EXIT(ch, dir)->keyword)
      send_to_char(
          ch, "The %s is destroyed.\r\n", fname(EXIT(ch, dir)->keyword),
          !strcmp(fname(EXIT(ch, dir)->keyword), "doors") ? "are" : "is");
    else if (IS_SET(EXIT(ch, dir)->exit_info, EX_CLOSED) &&
             EXIT(ch, dir)->keyword)
      send_to_char(ch, "The %s %s closed.\r\n", fname(EXIT(ch, dir)->keyword),
                   !strcmp(fname(EXIT(ch, dir)->keyword), "doors") ? "are"
                                                                   : "is");
...
void look_in_obj(struct char_data *ch, char *arg, bool exa) {
  ...
  if (ch->in_veh) {
    bits = generic_find(arg, FIND_OBJ_INV | FIND_OBJ_ROOM | FIND_OBJ_EQUIP, ch,
                        &dummy, &obj);
  ...
  if (veh) {
    ...
    if (veh->cspeed > SPEED_IDLE) {
      if (success_test(GET_INT(ch) + GET_POWER(ch, ADEPT_IMPROVED_PERCEPT),
        ...
        send_to_char(
            ch, "It's moving too fast for you to get a good look inside.\r\n");
        return;
      }
    } else {
...
void look_at_target(struct char_data *ch, char *arg) {
  ...
  if (!ch->in_veh || (ch->in_veh && !ch->vfront)) {
    found_veh = get_veh_list(
        arg, ch->in_veh ? ch->in_veh->carriedvehs : ch->in_room->vehicles, ch);
    if (found_veh) {
  ...
  if (ch->in_room &&
      (desc = find_exdesc(arg, ch->in_room->ex_description)) != NULL) {
...
void do_probe_veh(struct char_data *ch, struct veh_data *k) {
  sprintf(buf, "Name: '^y%s^n', Aliases: %s\r\n", k->short_description,
          k->name);
  sprintf(ENDOF(buf),
          "It is a ^c%s^n with handling ^c%d^n, a top speed of ^c%d^n, and raw "
          "acceleration of ^c%d^n.\r\n",
          veh_type[k->type], k->handling, k->speed, k->accel);
  sprintf(ENDOF(buf),
          "It has a body rating of ^c%d^n and rating-^c%d^n vehicular armor. "
          "It seats ^c%d^n up front and ^c%d^n in the back.\r\n",
          k->body, k->armor, k->seating[1], k->seating[0]);
  sprintf(ENDOF(buf),
          "Its signature rating is ^c%d^n, and its NERP pilot rating is "
          "^c%d^n.\r\n",
          k->sig, k->pilot);
  sprintf(ENDOF(buf),
          "It has ^c%d^n slots in its autonav and carrying capacity of ^c%d^n "
          "(%d in use).\r\n",
...
void do_probe_object(struct char_data *ch, struct obj_data *j) {
  ...
  bool has_pockets = FALSE, added_extra_carriage_return = FALSE,
       has_smartlink = FALSE;
  ...
  sprintf(ENDOF(buf),
          "It is %s ^c%s^n that weighs ^c%.2f^n kilos. It is made of ^c%s^n "
          "with a durability of ^c%d^n.\r\n",
          AN(buf1), buf1, GET_OBJ_WEIGHT(j),
          material_names[(int)GET_OBJ_MATERIAL(j)], GET_OBJ_BARRIER(j));

  if (strcmp(j->obj_flags.wear_flags.ToString(), "0") != 0) {
    j->obj_flags.wear_flags.PrintBits(buf2, MAX_STRING_LENGTH, wear_bits,
                                      NUM_WEARS);
    sprintf(ENDOF(buf),
            "It can be worn or equipped at the following wear location(s):\r\n "
            " ^c%s^n\r\n",
            buf2);
  } else {
  ...
  case ITEM_LIGHT:
    if (GET_OBJ_VAL(j, 2) == -1) {
      ...
      sprintf(ENDOF(buf), "It has ^c%d^n hours of light left.",
              GET_OBJ_VAL(j, 2));
    }
    break;
  case ITEM_FIREWEAPON:
    sprintf(
        ENDOF(buf),
        "It is a ^c%s^n that requires ^c%d^n strength to use in combat.\r\n",
        GET_OBJ_VAL(j, 5) == 0 ? "Bow" : "Crossbow", GET_OBJ_VAL(j, 6));
    if (GET_OBJ_VAL(j, 2)) {
      sprintf(ENDOF(buf), "It has a damage code of ^c(STR+%d)%s%s^n",
              GET_OBJ_VAL(j, 2), wound_arr[GET_OBJ_VAL(j, 1)],
              !IS_DAMTYPE_PHYSICAL(get_weapon_damage_type(j)) ? "( stun)" : "");
    } else {
      sprintf(ENDOF(buf), "It has a damage code of ^c(STR)%s%s^n",
              wound_arr[GET_OBJ_VAL(j, 1)],
              !IS_DAMTYPE_PHYSICAL(get_weapon_damage_type(j)) ? " (stun)" : "");
    }
    sprintf(ENDOF(buf), "and requires the ^c%s^n skill to use.",
  ...
  case ITEM_WEAPON:
    ...
    if (IS_GUN(GET_OBJ_VAL((j), 3))) {
      if (GET_OBJ_VAL(j, 5) > 0) {
        sprintf(ENDOF(buf),
                "It is a ^c%d-round %s^n that uses the ^c%s^n skill to fire. "
                "Its damage code is ^c%d%s%s^n.",
                GET_OBJ_VAL(j, 5), weapon_type[GET_OBJ_VAL(j, 3)],
                skills[GET_OBJ_VAL(j, 4)].name, GET_OBJ_VAL(j, 0),
                wound_arr[GET_OBJ_VAL(j, 1)],
                !IS_DAMTYPE_PHYSICAL(get_weapon_damage_type(j)) ? " (stun)"
                                                                : "");
      } else {
        sprintf(ENDOF(buf),
                "It is %s ^c%s^n that uses the ^c%s^n skill to fire. Its "
                "damage code is ^c%d%s%s^n.",
                AN(weapon_type[GET_OBJ_VAL(j, 3)]),
      ...
      for (int i = ACCESS_LOCATION_TOP; i <= ACCESS_LOCATION_UNDER; i++) {
        if (GET_OBJ_VAL(j, i) > 0 && real_object(GET_OBJ_VAL(j, i)) > 0 &&
          ...
          case ACCESS_SMARTLINK:
            has_smartlink = TRUE;
            sprintf(ENDOF(buf),
                    "\r\nA Smartlink%s attached to the %s provides ^c%d^n to "
                    "target numbers.",
                    GET_OBJ_VAL(access, 2) < 2 ? "" : "-II",
                    gun_accessory_locations[mount_location],
                    (GET_OBJ_VAL(j, 1) == 1 || GET_OBJ_VAL(access, 2) < 2)
                        ? -2
                        : -4);
            break;
          case ACCESS_SCOPE:
            if (GET_OBJ_AFFECT(access).IsSet(AFF_LASER_SIGHT)) {
              sprintf(ENDOF(buf),
                      "\r\nA laser sight attached to the %s provides ^c-1^n to "
                      "target numbers.",
                      gun_accessory_locations[mount_location]);
            } else {
          ...
          case ACCESS_GASVENT:
            sprintf(ENDOF(buf),
                    "\r\nA gas vent installed in the %s provides ^c%d^n "
                    "round%s worth of recoil compensation.",
                    gun_accessory_locations[mount_location],
                    -GET_OBJ_VAL(access, 2),
                    -GET_OBJ_VAL(access, 2) > 1 ? "s'" : "'s");
            break;
          case ACCESS_SHOCKPAD:
            sprintf(ENDOF(buf),
                    "\r\nA shock pad installed on the %s provides ^c1^n "
                    "round's worth of recoil compensation.",
                    gun_accessory_locations[mount_location]);
            break;
          case ACCESS_SILENCER:
            sprintf(ENDOF(buf),
                    "\r\nThe silencer installed on the %s will muffle this "
                    "weapon's report.",
                    gun_accessory_locations[mount_location]);
            break;
          case ACCESS_SOUNDSUPP:
            sprintf(ENDOF(buf),
                    "\r\nThe suppressor installed on the %s will muffle this "
                    "weapon's report.",
                    gun_accessory_locations[mount_location]);
            break;
          ...
          case ACCESS_BIPOD:
            sprintf(ENDOF(buf),
                    "\r\nA bipod installed on the %s provides ^c%d^n round%s "
                    "worth of recoil compensation when fired from prone.",
                    gun_accessory_locations[mount_location],
                    RECOIL_COMP_VALUE_BIPOD,
                    RECOIL_COMP_VALUE_BIPOD > 1 ? "s'" : "'s");
            break;
          case ACCESS_TRIPOD:
            sprintf(ENDOF(buf),
                    "\r\nA tripod installed on the %s provides ^c%d^n round%s "
                    "worth of recoil compensation when fired from prone.",
                    gun_accessory_locations[mount_location],
                    RECOIL_COMP_VALUE_TRIPOD,
                    RECOIL_COMP_VALUE_TRIPOD > 1 ? "s'" : "'s");
            break;
          case ACCESS_BAYONET:
            if (mount_location != ACCESS_LOCATION_UNDER) {
              strcat(buf, "\r\n^RYour bayonet has been mounted in the wrong "
                          "location and is nonfunctional. Alert an imm.");
            } else {
              sprintf(ENDOF(buf),
                      "\r\nA bayonet attached to the %s allows you to use the "
                      "Pole Arms skill when defending from melee attacks.",
                      gun_accessory_locations[mount_location]);
            }
            break;
          default:
            sprintf(
                buf1,
                "SYSERR: Unknown accessory type %d passed to do_probe_object()",
                GET_OBJ_VAL(access, 1));
            log(buf1);
            break;
          }
          // Tack on affect and extra flags to the attachment.
          if (strcmp(GET_OBJ_AFFECT(access).ToString(), "0") != 0) {
            GET_OBJ_AFFECT(access).PrintBits(buf2, MAX_STRING_LENGTH,
          ...
          if (strcmp(GET_OBJ_EXTRA(access).ToString(), "0") != 0) {
            GET_OBJ_EXTRA(access).PrintBits(buf2, MAX_STRING_LENGTH, extra_bits,
                                            ITEM_EXTRA_MAX);
            sprintf(ENDOF(buf),
                    "\r\n ^- It provides the following extra features: ^c%s^n",
                    buf2);
          }
        }
      }
    ...
    // Melee weapons.
      ...
      if (GET_OBJ_VAL(j, 2) != 0) {
        sprintf(ENDOF(buf),
                "It is a ^c%s^n that uses the ^c%s^n skill to attack with. Its "
                "damage code is ^c(STR%s%d)%s%s^n.",
                weapon_type[GET_OBJ_VAL(j, 3)], skills[GET_OBJ_VAL(j, 4)].name,
                GET_OBJ_VAL(j, 2) < 0 ? "" : "+", GET_OBJ_VAL(j, 2),
                wound_arr[GET_OBJ_VAL(j, 1)],
                !IS_DAMTYPE_PHYSICAL(get_weapon_damage_type(j)) ? " (stun)"
                                                                : "");
      } else {
      ...
      if (GET_WEAPON_REACH(j)) {
        sprintf(ENDOF(buf), "\r\nIt grants %d meters of reach when wielded.",
                GET_WEAPON_REACH(j));
      }
    }
    break;
  ...
  case ITEM_WORN:
    ...
    if (GET_OBJ_VAL(j, 0) > 0) {
      sprintf(ENDOF(buf1), "%s^c%d^n holster%s", (has_pockets ? ", " : ""),
              GET_OBJ_VAL(j, 0), GET_OBJ_VAL(j, 0) > 1 ? "s" : "");
      has_pockets = TRUE;
    }
    if (GET_OBJ_VAL(j, 1) > 0) {
      sprintf(ENDOF(buf1), "%s^c%d^n magazine%s", (has_pockets ? ", " : ""),
              GET_OBJ_VAL(j, 1), GET_OBJ_VAL(j, 1) > 1 ? "s" : "");
      has_pockets = TRUE;
    }
    if (GET_OBJ_VAL(j, 4) > 0) {
      sprintf(ENDOF(buf1), "%s^c%d^n miscellaneous small item%s",
              (has_pockets ? ", " : ""), GET_OBJ_VAL(j, 4),
              GET_OBJ_VAL(j, 4) > 1 ? "s" : "");
      has_pockets = TRUE;
    }
    ...
    sprintf(ENDOF(buf),
            "It provides ^c%d^n ballistic armor and ^c%d^n impact armor. Its "
            "concealability rating is ^c%d^n.",
  ...
  case ITEM_DOCWAGON:
    sprintf(ENDOF(buf), "It is a ^c%s^n contract that ^c%s bonded%s^n.",
            ...
            GET_OBJ_VAL(j, 1) ? "is" : "has not been",
            GET_OBJ_VAL(j, 1)
                ? (GET_OBJ_VAL(j, 1) == GET_IDNUM(ch) ? " to you"
                                                      : " to someone else")
                : " to anyone yet");
    break;
  case ITEM_CONTAINER:
    sprintf(ENDOF(buf), "It can hold a maximum of ^c%d^n kilograms.",
            GET_OBJ_VAL(j, 0));
    break;
  case ITEM_DRINKCON:
  case ITEM_FOUNTAIN:
  ...
  case ITEM_MONEY:
    if (!GET_OBJ_VAL(j, 1))
      sprintf(ENDOF(buf), "It is a stack of cash worth ^c%d^n nuyen.",
              GET_OBJ_VAL(j, 0));
    else
      sprintf(ENDOF(buf),
              "It is a ^c%s^n-secured ^ccredstick^n loaded with ^c%d^n nuyen.",
              (GET_OBJ_VAL(j, 2) == 1
                   ? "6-digit PIN"
                   : (GET_OBJ_VAL(j, 2) == 2 ? "thumbprint" : "retina")),
              GET_OBJ_VAL(j, 0));
    break;
  case ITEM_KEY:
    ...
    break;
  case ITEM_FOOD:
    sprintf(ENDOF(buf), "It provides ^c%d^n units of nutrition when eaten.",
            GET_OBJ_VAL(j, 0));
    break;
  case ITEM_QUIVER:
    if (GET_OBJ_VAL(j, 1) >= 0 && GET_OBJ_VAL(j, 1) <= 3) {
      sprintf(ENDOF(buf), "It can hold up to ^c%d^n ^c%s%s^n.",
              GET_OBJ_VAL(j, 0), projectile_ammo_types[GET_OBJ_VAL(j, 1)],
              GET_OBJ_VAL(j, 0) > 1 ? "s" : "");
    } else {
      sprintf(ENDOF(buf), "It can hold up to ^c%d^n ^cundefined projectiles^n.",
              GET_OBJ_VAL(j, 0));
    }
    break;
  case ITEM_PATCH:
    sprintf(ENDOF(buf), "It is a ^crating-%d %s^n patch.", GET_OBJ_VAL(j, 1),
            patch_names[GET_OBJ_VAL(j, 0)]);
    break;
  case ITEM_CYBERDECK:
    sprintf(ENDOF(buf),
            "MPCP: ^c%d^n, Hardening: ^c%d^n, Active: ^c%d^n, Storage: ^c%d^n, "
            "Load: ^c%d^n.",
  ...
  case ITEM_PROGRAM:
    sprintf(
        ENDOF(buf), "It is a ^crating-%d %s^n program, ^c%d^n units in size.",
        GET_OBJ_VAL(j, 1), programs[GET_OBJ_VAL(j, 0)].name, GET_OBJ_VAL(j, 2));
    if (GET_OBJ_VAL(j, 0) == SOFT_ATTACK)
      sprintf(ENDOF(buf), " Its damage code is ^c%s^n.",
              wound_name[GET_OBJ_VAL(j, 3)]);
    break;
  case ITEM_BIOWARE:
    sprintf(
        ENDOF(buf),
        "It is a ^crating-%d %s%s^n that uses ^c%.2f^n index when installed.",
        GET_OBJ_VAL(j, 1),
        GET_OBJ_VAL(j, 2) || GET_OBJ_VAL(j, 0) >= BIO_CEREBRALBOOSTER
            ? "cultured "
            : "",
  ...
  case ITEM_CYBERWARE:
    sprintf(ENDOF(buf),
            "It is a ^crating-%d %s-grade %s^n that uses ^c%.2f^n essence when "
            "installed.",
            GET_OBJ_VAL(j, 1), decap_cyber_grades[GET_OBJ_VAL(j, 2)],
            decap_cyber_types[GET_OBJ_VAL(j, 0)],
            ((float)GET_OBJ_VAL(j, 4) / 100));
    break;
  case ITEM_WORKSHOP:
    sprintf(ENDOF(buf), "It is a ^c%s^n designed for ^c%s^n.",
            GET_OBJ_VAL(j, 1) ? GET_OBJ_VAL(j, 1) == 3 ? "Facility" : "Workshop"
                              : "Kit",
            workshops[GET_OBJ_VAL(j, 0)]);
    break;
  case ITEM_FOCUS:
    sprintf(ENDOF(buf), "It is a ^c%s^n focus of force ^c%d^n.",
            foci_type[GET_OBJ_VAL(j, 0)], GET_OBJ_VAL(j, 1));
    break;
  case ITEM_SPELL_FORMULA:
    sprintf(ENDOF(buf), "It is a ^cforce-%d %s^n designed for ^c%s^n mages.",
            GET_OBJ_VAL(j, 0), spells[GET_OBJ_VAL(j, 1)].name,
            GET_OBJ_VAL(j, 2) ? "Shamanic" : "Hermetic");
    break;
  case ITEM_PART:
    sprintf(ENDOF(buf), "It is %s ^c%s^n designed for MPCP ^c%d^n decks.",
            AN(parts[GET_OBJ_VAL(j, 0)].name), parts[GET_OBJ_VAL(j, 0)].name,
            GET_OBJ_VAL(j, 2));
    break;
  case ITEM_CUSTOM_DECK:
    sprintf(ENDOF(buf),
            "You should EXAMINE this deck, or jack in and view its SOFTWARE.");
    break;
  case ITEM_DRUG:
    sprintf(ENDOF(buf), "It is a dose of ^c%s^n.",
            drug_types[GET_OBJ_VAL(j, 0)].name);
    break;
  case ITEM_MAGIC_TOOL:
    sprintf(ENDOF(buf), "It is a ^crating-^c%d %s^n.", GET_OBJ_VAL(j, 1),
            magic_tool_types[GET_OBJ_VAL(j, 0)]);
    break;
  case ITEM_RADIO:
    sprintf(ENDOF(buf),
            "It has a ^c%d/5^n range and can encrypt and decrypt signals up to "
            "crypt level ^c%d^n.",
            GET_OBJ_VAL(j, 1), GET_OBJ_VAL(j, 2));
    break;
  case ITEM_GUN_ACCESSORY:
    if (GET_OBJ_VAL(j, 1) == ACCESS_SMARTLINK ||
        GET_OBJ_VAL(j, 1) == ACCESS_GASVENT) {
      sprintf(ENDOF(buf),
              "It is a ^crating-%d %s^n that attaches to the ^c%s^n.",
              GET_OBJ_VAL(j, 2), gun_accessory_types[GET_OBJ_VAL(j, 1)],
              gun_accessory_locations[GET_OBJ_VAL(j, 0)]);
    } else {
      sprintf(ENDOF(buf), "It is %s ^c%s^n that attaches to the ^c%s^n.",
              AN(gun_accessory_types[GET_OBJ_VAL(j, 1)]),
              gun_accessory_types[GET_OBJ_VAL(j, 1)],
              gun_accessory_locations[GET_OBJ_VAL(j, 0)]);
    }
  ...
  case ITEM_RCDECK:
    ...
    break;
  case ITEM_CHIP:
    sprintf(ENDOF(buf), "It grants the skill ^c%s^n at rating ^c%d^n.",
            skills[GET_OBJ_VAL(j, 0)].name, GET_OBJ_VAL(j, 1));
    break;
  case ITEM_HOLSTER:
    sprintf(ENDOF(buf), "It is designed for a ^c%s^n.",
            holster_types[GET_OBJ_VAL(j, 0)]);
    break;
  case ITEM_DECK_ACCESSORY:
    if (GET_OBJ_VAL(j, 0) == TYPE_FILE) {
      sprintf(ENDOF(buf), "This file requires ^c%d^n units of space.",
              GET_OBJ_VAL(j, 2));
    } else if (GET_OBJ_VAL(j, 0) == TYPE_UPGRADE) {
      if (GET_OBJ_VAL(j, 1) == 3) {
        sprintf(
            ENDOF(buf),
            "This cyberdeck upgrade affects ^c%s^n with a rating of ^c%d^n.",
            deck_accessory_upgrade_types[GET_OBJ_VAL(j, 1)], GET_OBJ_VAL(j, 2));
        if (GET_OBJ_VAL(j, 1) == 5) {
          sprintf(ENDOF(buf), "\r\nIt is designed for MPCP rating ^c%d^n.",
                  GET_OBJ_VAL(j, 3));
        }
      } else {
      ...
      sprintf(ENDOF(buf),
              "This computer has ^c%d^n units of active memory and ^c%d^n "
      ...
      sprintf(ENDOF(buf), "This pack of parts contains ^c%d^n units of ^c%s^n.",
              GET_OBJ_COST(j),
              GET_OBJ_VAL(j, 1) == 0 ? "general parts" : "memory chips");
    } else {
      sprintf(
          buf2,
          "Error: Unknown ITEM_DECK_ACCESSORY type %d passed to probe command.",
          GET_OBJ_VAL(j, 0));
      log(buf2);
    }
    break;
  case ITEM_MOD:
    sprintf(ENDOF(buf), "It is %s ^c%s^n upgrade",
            AN(mod_types[GET_OBJ_VAL(j, 0)].name),
            mod_types[GET_OBJ_VAL(j, 0)].name);

    // Val 1
    if (GET_OBJ_VAL(j, 0) == TYPE_MOUNT) {
      sprintf(ENDOF(buf), " that adds %s ^c%s^n.",
              AN(mount_types[GET_OBJ_VAL(j, 1)]),
              mount_types[GET_OBJ_VAL(j, 1)]);
    } else {
      sprintf(ENDOF(buf), " that takes up ^c%d^n load space.",
              GET_OBJ_VAL(j, 1));
    }
    ...
    if (GET_OBJ_VAL(j, 0) == MOD_ENGINE) {
      ...
      sprintf(ENDOF(buf), "\r\nIt is %s ^c%s^n engine.",
              AN(engine_type[GET_OBJ_VAL(j, 2)]),
              engine_type[GET_OBJ_VAL(j, 2)]);
    } else if (GET_OBJ_VAL(j, 0) == MOD_RADIO) {
      // radio range 0-5
      sprintf(ENDOF(buf),
              "\r\nIt has a ^c%d/5^n range and can encrypt and decrypt signals "
              "up to crypt level ^c%d^n.",
              GET_OBJ_VAL(j, 2), GET_OBJ_VAL(j, 3));
    } else {
      sprintf(ENDOF(buf), "\r\nIt functions at rating ^c%d^n.",
              GET_OBJ_VAL(j, 2));
    }
    ...
    sprintf(
        ...
        GET_OBJ_VAL(j, 4) == 0
            ? "vehicles"
            : GET_OBJ_VAL(j, 4) == 1 ? "drones" : "all types of vehicles",
        mod_name[GET_OBJ_VAL(j, 6)]);
    break;
  case ITEM_DESIGN:
    if (GET_OBJ_VAL(j, 0) == 5) {
      sprintf(ENDOF(buf),
              "This design is for a ^crating-%d %s (%s)^n program. It requires "
              "^c%d^n units of storage.\r\n",
              GET_OBJ_VAL(j, 1), programs[GET_OBJ_VAL(j, 0)].name,
              wound_name[GET_OBJ_VAL(j, 2)],
              (GET_OBJ_VAL(j, 1) * GET_OBJ_VAL(j, 1)) *
                  attack_multiplier[GET_OBJ_VAL(j, 2)]);
    } else {
      sprintf(ENDOF(buf),
              "This design is for a ^crating-%d %s^n program. It requires "
              "^c%d^n units of storage.\r\n",
              GET_OBJ_VAL(j, 1), programs[GET_OBJ_VAL(j, 0)].name,
              (GET_OBJ_VAL(j, 1) * GET_OBJ_VAL(j, 1)) *
                  programs[GET_OBJ_VAL(j, 0)].multiplier);
    }
    break;
  case ITEM_GUN_AMMO:
    if (GET_OBJ_VAL(j, 3))
      send_to_char(ch, "It has %d/%d %s round%s of %s ammunition left.\r\n",
                   GET_OBJ_VAL(j, 0), GET_OBJ_VAL(j, 0) + GET_OBJ_VAL(j, 3),
                   ammo_type[GET_OBJ_VAL(j, 2)].name,
                   GET_OBJ_VAL(j, 0) != 1 ? "s" : "",
                   weapon_type[GET_OBJ_VAL(j, 1)]);
    else
      send_to_char(ch, "It has %d %s round%s of %s ammunition left.\r\n",
                   GET_OBJ_VAL(j, 0), ammo_type[GET_OBJ_VAL(j, 2)].name,
                   GET_OBJ_VAL(j, 0) != 1 ? "s" : "",
                   weapon_type[GET_OBJ_VAL(j, 1)]);
    break;
  ...
  case ITEM_PHONE:
    ...
    break;
  default:
    strcat(buf, "This item type has no probe string. Contact the staff to "
                "request one.");
    break;
  }

  if (GET_OBJ_AFFECT(j).IsSet(AFF_LASER_SIGHT) && has_smartlink) {
    sprintf(ENDOF(buf), "\r\n\r\n^yWARNING:^n Your smartlink overrides your "
                        "laser sight-- the laser will not function.");
  }
  ...
  if (strcmp(j->obj_flags.bitvector.ToString(), "0") != 0) {
    j->obj_flags.bitvector.PrintBits(buf2, MAX_STRING_LENGTH, affected_bits,
                                     AFF_MAX);
    sprintf(ENDOF(buf),
            "This object provides the following flags when used: ^c%s^n\r\n",
            buf2);
  }

  if (strcmp(GET_OBJ_EXTRA(j).ToString(), "0") != 0) {
    GET_OBJ_EXTRA(j).PrintBits(buf2, MAX_STRING_LENGTH, extra_bits,
                               ITEM_EXTRA_MAX);
    sprintf(ENDOF(buf),
            "This object has the following extra features: ^c%s^n\r\n", buf2);
  }

  sprintf(buf1, "This object modifies your character in the following ways "
                "when used:\r\n  ^c");
  ...
  if (IS_OBJ_STAT(j, ITEM_NERPS)) {
    strcat(buf, "^YIt has been flagged NERPS, indicating it has no special "
                "coded effects.^n\r\n");
  }
...
ACMD(do_examine) {
  ...
  if (!ch->in_veh || (ch->in_veh && !ch->vfront)) {
    ...
    if (found_veh) {
      if (subcmd == SCMD_PROBE) {
        // If they don't own the vehicle and the hood isn't open, they can't
        // view the stats.
        if (GET_IDNUM(ch) != found_veh->owner && !found_veh->hood) {
          send_to_char("You can only see the OOC stats for vehicles you own or "
                       "vehicles that have popped hoods.\r\n",
                       ch);
          return;
        }
  ...
  if (subcmd == SCMD_PROBE) {
    generic_find(arg, FIND_OBJ_INV | FIND_OBJ_EQUIP, ch, &tmp_char,
                 &tmp_object);

    if (tmp_object) {
      do_probe_object(ch, tmp_object);
    } else {
      send_to_char("You're not carrying any such object, and there are no "
                   "vehicles like that here.\r\n",
                   ch);
    }
    return;
  } else {
  ...
  if (tmp_object) {
    if (GET_OBJ_TYPE(tmp_object) == ITEM_CONTAINER ||
      ...
      if (!tmp_object->contains)
        ...
        if (exobj) {
          int q = GET_OBJ_VAL(tmp_object, 0) - (int)(GET_OBJ_WEIGHT(exobj)),
              x = (int)(GET_OBJ_WEIGHT(tmp_object) - GET_OBJ_WEIGHT(exobj));
          extract_obj(exobj);
          if (x >= q * .9)
      ...
      for (i = 7; i < 10; ++i)
        if (GET_OBJ_VAL(tmp_object, i) >= 0) {
          sprintf(buf1, "There is %s attached to the %s of it.\r\n",
                  GET_OBJ_VAL(tmp_object, i) > 0
                      ? short_object(GET_OBJ_VAL(tmp_object, i), 2)
                      : "nothing",
      ...
      if (tmp_object->contains)
        sprintf(buf,
                "It contains %d round%s of %s ammunition, and can hold %d "
                "round%s.\r\n",
                MIN(GET_OBJ_VAL(tmp_object->contains, 9),
                    GET_OBJ_VAL(tmp_object, 5)),
                (GET_OBJ_VAL(tmp_object->contains, 9) != 1 ? "s" : ""),
                ammo_type[GET_OBJ_VAL(tmp_object->contains, 2)].name,
                GET_OBJ_VAL(tmp_object, 5),
                (GET_OBJ_VAL(tmp_object, 5) != 1 ? "s" : ""));
      else
        sprintf(buf,
                "It does not contain any ammunition, but looks to hold %d "
                "round%s.\r\n",
                GET_OBJ_VAL(tmp_object, 5),
                (GET_OBJ_VAL(tmp_object, 5) != 1 ? "s" : ""));
      ...
      send_to_char(ch, "It has %d %s round%s left.\r\n",
                   ...
                   ammo_type[GET_OBJ_VAL(tmp_object, 2)].name,
      ...
      if (GET_OBJ_VAL(tmp_object, 3))
        send_to_char(ch, "It has %d/%d %s round%s of %s ammunition left.\r\n",
                     GET_OBJ_VAL(tmp_object, 0),
                     GET_OBJ_VAL(tmp_object, 0) + GET_OBJ_VAL(tmp_object, 3),
                     ammo_type[GET_OBJ_VAL(tmp_object, 2)].name,
                     GET_OBJ_VAL(tmp_object, 0) != 1 ? "s" : "",
                     weapon_type[GET_OBJ_VAL(tmp_object, 1)]);
      else
        send_to_char(ch, "It has %d %s round%s of %s ammunition left.\r\n",
                     ...
                     GET_OBJ_VAL(tmp_object, 0) != 1 ? "s" : "",
                     weapon_type[GET_OBJ_VAL(tmp_object, 1)]);
    } else if (GET_OBJ_VNUM(tmp_object) == 119 &&
               GET_OBJ_VAL(tmp_object, 0) == GET_IDNUM(ch))
      sprintf(buf, "The card contains %d nuyen.\r\n",
              GET_OBJ_VAL(tmp_object, 1));
    else if (GET_OBJ_TYPE(tmp_object) == ITEM_DECK_ACCESSORY &&
             GET_OBJ_VAL(tmp_object, 0) == TYPE_COOKER) {
      if (!tmp_object->contains)
        sprintf(buf, "The small LED is currently off.\r\n");
      else if (GET_OBJ_VAL(tmp_object, 9))
        sprintf(buf,
                "The small LED is currently orange, indicating activity.\r\n");
      else if (GET_OBJ_TIMER(tmp_object->contains) == -1)
        sprintf(buf, "The small LED is currently flashed red, indicating a "
                     "failed encode.\r\n");
      else
        sprintf(buf, "The small LED is currently green, indicating a "
                     "successful encode.\r\n");
      send_to_char(buf, ch);
    } else if (GET_OBJ_TYPE(tmp_object) == ITEM_PROGRAM &&
               (GET_OBJ_VAL(tmp_object, 0) >= SOFT_ASIST_COLD ||
                GET_OBJ_VAL(tmp_object, 0) < SOFT_SENSOR)) {
      ...
      send_to_char(buf, ch);
               ...
               GET_OBJ_VAL(tmp_object, 0) == TYPE_PARTS)
      send_to_char(ch,
                   "There seems to be about %d nuyen worth of %s there.\r\n",
                   GET_OBJ_COST(tmp_object),
                   GET_OBJ_VAL(tmp_object, 1) ? "chips" : "parts");
      ...
      if (!GET_OBJ_VAL(tmp_object, 9) && GET_OBJ_VAL(tmp_object, 0) > 0) {
        ...
        if (success_test(skill, target) > 0) {
          ...
          for (struct obj_data *soft = tmp_object->contains; soft;
      ...
      case TYPE_LIBRARY_SPELL:
        send_to_char(ch,
                     "It seems to have enough material to be classified as "
                     "rating %d.\r\n",
                     GET_OBJ_VAL(tmp_object, 1));
        break;
      }
      ...
      if (GET_OBJ_TIMER(tmp_object) < 0) {
        if (GET_OBJ_VAL(tmp_object, 8) == GET_IDNUM(ch)) {
          ...
          if (GET_OBJ_VAL(tmp_object, 6))
            send_to_char(
                ch, "You are about %d%% done.\r\n",
                (int)(((float)timeleft / (float)GET_OBJ_VAL(tmp_object, 7)) *
                          -100 +
                      100));
          else
          ...
          send_to_char(ch, "It doesn't seem to be completed.\r\n");
      } else if (GET_SKILL(ch, SKILL_SORCERY)) {
        send_to_char(ch,
                     "It is a rating %d spell formula describing %s. It is "
                     "designed for use by %s mages.\r\n",
                     GET_OBJ_VAL(tmp_object, 0),
                     spells[GET_OBJ_VAL(tmp_object, 1)].name,
                     GET_OBJ_VAL(tmp_object, 2) == 1 ? "shamanic" : "hermetic");
      }
      ...
      if (GET_DOCWAGON_BONDED_IDNUM(tmp_object)) {
        if (GET_DOCWAGON_BONDED_IDNUM(tmp_object) == GET_IDNUM(ch))
          send_to_char(
              ch, "It has been bonded to your biometric signature%s.\r\n\r\n",
              tmp_object->worn_by != ch
                  ? ", but ^yit won't function until you wear it^n"
                  : "");
        else
          send_to_char("It has been activated by someone else and ^ywill not "
                       "function for you^n.\r\n\r\n",
                       ch);
      } else {
        send_to_char("It has not been BONDed yet, and ^ywill not function "
                     "until it is^n.\r\n\r\n",
                     ch);
      }
    }
    if (GET_OBJ_VNUM(tmp_object) > 1) {
      ...
      if (targ <= 0)
        ...
        int skill = get_skill(ch, SKILL_POLICE_PROCEDURES, targ),
            suc = success_test(skill, targ);
        if (suc <= 0)
          strcat(buf, "is completely legal");
        else {
          if (GET_LEGAL_NUM(tmp_object) <= 3)
            ...
            strcat(buf, "could be considered illegal");
        }
      }
      send_to_char(ch, "%s.\r\n", buf);
    }
...
ACMD(do_pool) {
  ...
  if (PRF_FLAGGED(ch, PRF_SCREENREADER)) {
    sprintf(pools,
            "  Dodge: %d%s\r\n  Body: %d\r\n  Offense: %d\r\n  Total Combat "
            "Dice: %d\r\n",
            GET_DEFENSE(ch), buf, GET_BODY(ch), GET_OFFENSE(ch),
            GET_COMBAT(ch));
  } else {
    sprintf(pools,
            "  Combat: %d     (Dodge: %d%s      Body: %d     Offense: %d)\r\n",
            GET_COMBAT(ch), GET_DEFENSE(ch), buf, GET_BODY(ch),
            GET_OFFENSE(ch));
  }
  if (GET_ASTRAL(ch) > 0)
  ...
  if (GET_MAGIC(ch) > 0) {
    if (PRF_FLAGGED(ch, PRF_SCREENREADER)) {
      ...
      sprintf(ENDOF(pools),
              "  Spell: %d      (Casting: %d    Drain: %d    Defense: %d",
              GET_MAGIC(ch), GET_CASTING(ch), GET_DRAIN(ch), GET_SDEFENSE(ch));
      if (GET_METAMAGIC(ch, META_REFLECTING) == 2)
  ...
  if (AFF_FLAGGED(ch, AFF_PRONE))
    ...
    case POS_FIGHTING:
      if (FIGHTING(ch))
        ...
        sprintf(position_string, "fighting %s.",
                GET_VEH_NAME(FIGHTING_VEH(ch)));
      else
...
const char *get_vision_string(struct char_data *ch,
                              bool ascii_friendly = FALSE) {
  ...
  if (ascii_friendly) {
    if (AFF_FLAGGED(ch, AFF_DETECT_INVIS) &&
        get_ch_in_room(ch)->silence[0] <= 0)
      return "You have ultrasonic vision.";
  } else {
    if (AFF_FLAGGED(ch, AFF_DETECT_INVIS)) {
      if (get_ch_in_room(ch)->silence[0] > 0)
        return "Your ultrasonic vision is being suppressed by a field of "
               "silence here.\r\n";
      else
        return "You have ultrasonic vision.\r\n";
    }
  ...
  if (GET_TRADITION(ch) == TRAD_MUNDANE)
    ...
    if (GET_MAG(ch) != ch->real_abils.mag)
      sprintf(ENDOF(buf2), "Magic: %d (base magic %d)\r\n",
              (int)(GET_MAG(ch) / 100),
              MAX(0, (int)(ch->real_abils.mag / 100)));
    else
...
const char *get_plaintext_score_equipment(struct char_data *ch) {
  ...
  sprintf(ENDOF(buf2),
          "You are carrying %.2f kilos. Your maximum carry weight is %d.\r\n",
...
const char *get_plaintext_score_misc(struct char_data *ch) {
  ...
  if (IS_AFFECTED(ch, AFF_INVISIBLE) || IS_AFFECTED(ch, AFF_IMP_INVIS) ||
      IS_AFFECTED(ch, AFF_SPELLINVIS) || IS_AFFECTED(ch, AFF_SPELLIMPINVIS))
  ...
  playing_time =
      real_time_passed(time(0) + ch->player.time.played, ch->player.time.logon);
  ...
  if (AFF_FLAGGED(ch, AFF_RIG) || PLR_FLAGGED(ch, PLR_REMOTE)) {
    ...
    if (PRF_FLAGGED(ch, PRF_SCREENREADER)) {
      sprintf(screenreader_buf,
              ...
              GET_WIL(ch), veh->body, veh->armor, veh->autonav, veh->handling,
              veh->speed, veh->accel, veh->sig, veh->sensor);
    } else {
      sprintf(buf, "You are rigging %s.\r\n", GET_VEH_NAME(veh));
      sprintf(ENDOF(buf),
              "Damage:^R%3d/10^n      Mental:^B%3d(%2d)^n\r\n"
              "  Reaction:%3d      Int:%3d\r\n"
              "       Wil:%3d      Bod:%3d\r\n"
              "     Armor:%3d  Autonav:%3d\r\n"
    ...
    if (*argument) {
      ...
      if (*(score_switches[cmd_index].cmd) == '\n') {
        send_to_char("Sorry, that's not a valid score type. Available score "
                     "types are:\r\n",
                     ch);
        for (cmd_index = 0; *(score_switches[cmd_index].cmd) != '\n';
             cmd_index++)
    ...
    if (pain_editor) {
      strcat(buf, "^L/^b/  ^L///-\\  ^wcharacter sheet                         "
                  "                  ^b/^L/\r\n");
    } else {
      strcat(buf, "^L/^b/  ^L///-\\  ^wcharacter sheet           ^LPhysical "
                  "Damage Overflow: ");
      if (physical < 0)
    ...
    playing_time = real_time_passed(time(0) + ch->player.time.played,
    ...
    if (ch->desc != NULL && ch->desc->original != NULL) {
      if (PLR_FLAGGED(ch->desc->original, PLR_MATRIX))
        ...
        sprintf(out_of_body_string, "You are occupying the body of %s.",
                GET_NAME(ch));
    } else {
    ...
    sprintf(ENDOF(buf), "^b/^L/  ^L\\\\@//                                     "
                        "                       ^L/^b/\r\n");
    sprintf(ENDOF(buf), "^L/^b/   ^L`^                                         "
                        "                     ^b/^L/\r\n");
    sprintf(ENDOF(buf), "^b/^L/                                                "
                        "                   ^L/^b/\r\n");
    sprintf(ENDOF(buf),
            "^L/^b/ ^nBody          ^w%2d (^W%2d^w)    Height: ^W%0.2f^w "
            "meters   Weight: ^W%3d^w kilos  ^b/^L/\r\n",
            GET_REAL_BOD(ch), GET_BOD(ch), ((float)GET_HEIGHT(ch) / 100),
            GET_WEIGHT(ch));
    sprintf(ENDOF(buf),
            "^b/^L/ ^nQuickness     ^w%2d (^W%2d^w)    Encumbrance: ^W%3.2f^w "
            "kilos carried, ^W%3d^w max ^L/^b/\r\n",
            GET_REAL_QUI(ch), GET_QUI(ch), IS_CARRYING_W(ch), CAN_CARRY_W(ch));
    sprintf(ENDOF(buf),
            "^L/^b/ ^nStrength      ^w%2d (^W%2d^w)    Current session length "
            "^W%2d^w days, ^W%2d^w hours.^b/^L/\r\n",
            GET_REAL_STR(ch), GET_STR(ch), playing_time.day,
            playing_time.hours);
    sprintf(ENDOF(buf),
            "^b/^L/ ^nCharisma      ^w%2d (^W%2d^w)    ^wKarma ^B[^W%7.2f^B] "
            "^wRep ^B[^W%4d^B] ^rNotor ^r[^R%4d^r]  ^L/^b/\r\n",
            GET_REAL_CHA(ch), GET_CHA(ch), ((float)GET_KARMA(ch) / 100),
            GET_REP(ch), GET_NOT(ch));
    sprintf(ENDOF(buf),
            "^L/^b/ ^nIntelligence  ^w%2d (^W%2d^w)    ^r%-41s^b/^L/\r\n",
            GET_REAL_INT(ch), GET_INT(ch), get_vision_string(ch, TRUE));
    sprintf(
        ENDOF(buf),
        "^b/^L/ ^nWillpower     ^w%2d (^W%2d^w)    ^nYou are %-33s^L/^b/\r\n",
        GET_REAL_WIL(ch), GET_WIL(ch), get_position_string(ch));
    sprintf(ENDOF(buf),
            "^L/^b/ ^nEssence       ^g[^w%5.2f^g]    ^W%-18s                   "
            "    ^b/^L/\r\n",
            ((float)GET_ESS(ch) / 100), invisibility_string);
#ifdef ENABLE_HUNGER
    sprintf(ENDOF(buf),
            "^b/^L/ ^nBioware Index ^B[^w%5.2f^B]    ^n%-15s                   "
            "       ^L/^b/\r\n",
            ((float)GET_INDEX(ch) / 100),
            GET_COND(ch, COND_FULL) == 0 ? "You are hungry." : "");
    sprintf(ENDOF(buf),
            "^L/^b/ ^nEssence Index ^W[^w%5.2f^W]    ^n%-16s                   "
            "      ^b/^L/\r\n",
            (((float)GET_ESS(ch) / 100) + 3),
            GET_COND(ch, COND_THIRST) == 0 ? "You are thirsty." : "");
#else
    sprintf(ENDOF(buf),
            "^b/^L/ ^nBioware Index ^B[^w%5.2f^B]    ^n%-15s                   "
            "       ^L/^b/\r\n",
...
#endif
    sprintf(ENDOF(buf),
            "^b/^L/ ^nMagic         ^w%2d (^W%2d^w)    ^g%-20s                 "
            "    ^L/^b/\r\n",
            MAX(0, ((int)ch->real_abils.mag / 100)), ((int)GET_MAG(ch) / 100),
            GET_COND(ch, COND_DRUNK) > 10 ? "You are intoxicated." : "");
    sprintf(ENDOF(buf),
            "^L/^b/ ^nReaction      ^w%2d (^W%2d^w)    ^c%-41s^b/^L/\r\n",
            GET_REAL_REA(ch), GET_REA(ch), out_of_body_string);
    sprintf(ENDOF(buf),
            "^b/^L/ ^nInitiative^w   [^W%2d^w+^W%d^rd6^n]    ^n%-32s         "
            "^L/^b/\r\n",
            GET_REA(ch), 1 + GET_INIT_DICE(ch), shaman_string);
    sprintf(ENDOF(buf),
            "^L/^b/ ^nArmor     ^w[ ^W%2d^rB^w/ ^W%2d^rI^w]    ^L%-17s^n       "
            "                 ^b/^L/\r\n",
            GET_BALLISTIC(ch), GET_IMPACT(ch),
            AFF_FLAGGED(ch, AFF_SNEAK) ? "You are sneaking." : "");
    sprintf(ENDOF(buf),
            "^b/^L/ ^nNuyen     ^w[^W%'9ld^w]    %11s                          "
            "    ^L/^b/\r\n",
...
ACMD(do_bioware) {
  ...
  for (obj = ch->bioware; obj != NULL; obj = obj->next_content) {
    sprintf(buf, "%-30s Rating: %-2d     Body Index: %0.2f\r\n",
            GET_OBJ_NAME(obj), GET_OBJ_VAL(obj, 1),
            ((float)GET_OBJ_VAL(obj, 4) / 100));
    send_to_char(buf, ch);
  }
}
    ...
    if (GET_EQ(ch, i)) {
      if (i == WEAR_WIELD || i == WEAR_HOLD) {
        if (IS_OBJ_STAT(GET_EQ(ch, i), ITEM_TWOHANDS))
          send_to_char(hands[2], ch);
        else if (GET_OBJ_TYPE(GET_EQ(ch, i)) ==
                 ITEM_WEAPON) { // wielding something?
          if (i == WEAR_WIELD)
            send_to_char(
                wielding_hands[(int)ch->char_specials.saved.left_handed], ch);
          else
            send_to_char(wielding_hands[!ch->char_specials.saved.left_handed],
                         ch);
        } else { // just held
          if (i == WEAR_WIELD)
...
ACMD(do_time) {
  ...
  for (check = ch->cyberware; check; check = check->next_content)
    if (GET_OBJ_VAL(check, 0) == CYB_MATHSSPU ||
        (GET_OBJ_VAL(check, 0) == CYB_EYES &&
         IS_SET(GET_OBJ_VAL(check, 3), EYE_CLOCK)))
      subcmd = SCMD_PRECISE;
  ...
  if (subcmd == SCMD_NORMAL)
    sprintf(buf, "%d o'clock %s, %s, %s %d, %d.\r\n", hour, pm ? "PM" : "AM",
            weekdays[(int)time_info.weekday], month_name[month - 1], day,
            time_info.year);
  else
...
ACMD(do_weather) {
  static const char *sky_look[] = {"cloudless", "cloudy", "rainy",
...
ACMD(do_index) {
  ...
  if (!*temp) {
    ...
    // No prepare_quotes since this is guaranteed to be a single alphanumeric
    // character.
    sprintf(query, "SELECT name FROM help_topic WHERE name LIKE '%c%%'",
            letter);
  }
  ...
  if (mysql_wrapper(mysql, query)) {
    send_to_char(
        "Apologies, but it seems the help system is currently offline.\r\n",
        ch);
    mudlog("WARNING: Failed to return all help topics with 'index' command.",
           ch, LOG_SYSLOG, TRUE);
    return;
  }
...
void display_help(char *help, const char *arg, struct char_data *ch) {
  ...
  *help = '\0';
  ...
  if (mysql_wrapper(mysql, query)) {
    ...
    sprintf(buf3,
            "WARNING: Failed to return help topic about %s. See server log for "
            "MYSQL error.",
            arg);
  ...
  sprintf(query,
          "SELECT * FROM help_topic WHERE name LIKE '%%%s%%' ORDER BY name ASC",
          buf);
  if (mysql_wrapper(mysql, query)) {
    // If we don't find it here either, we know the file doesn't exist-- failure
    // condition.
  ...
  // If we have too many rows, try to refine the search to just files starting
  // with the search string.
  else if (x > 5) {
    // Prepare a just-in-case response with the topics that were found in the
    // overbroad search.
    sprintf(help,
            "%d articles returned, please narrow your search. The topics found "
            "were:\r\n",
            x);
    while ((row = mysql_fetch_row(res))) {
    ...
    // Try a lookup with just files that have the search string at the start of
    // their title.
    sprintf(query,
            "SELECT * FROM help_topic WHERE name LIKE '%s%%' ORDER BY name ASC",
            buf);
    if (mysql_wrapper(mysql, query)) {
      // We hit an error with this followup search, so we just return our
      // pre-prepared string with the search that succeeded.
      sprintf(buf3,
              "Overbroad helpfile search combined with follow-up lookup "
              "failure (%d articles): %s.",
              x, arg);
      mudlog(buf3, ch, LOG_HELPLOG, TRUE);
      return;
    }
    ...
    if (mysql_num_rows(res) == 1) {
      ...
      // We didn't find any candidate rows, or we found too many. They'll get
      // the pre-prepared string, and we'll log this one.
...
#define WHO_FORMAT                                                             \
  "format: who [sort] [quest] [pker] [immortal] [mortal] [hardcore] [ooc] "    \
  "[newbie]\r\n"
...
ACMD(do_who) {
  ...
  bool mortal = FALSE, hardcore = FALSE, quest = FALSE, pker = FALSE,
       immort = FALSE, ooc = FALSE, newbie = FALSE;
  int output_header;
  ...
  *buf = '\0';
  ...
  for (; sort != 0; sort > 0 ? sort-- : sort++) {
    ...
    for (d = descriptor_list; d; d = d->next) {
      ...
      if ((mortal && IS_SENATOR(tch)) || (immort && !IS_SENATOR(tch)))
        continue;
      ...
      if (PRF_FLAGGED(tch, PRF_SHOWGROUPTAG) && GET_PGROUP_MEMBER_DATA(tch) &&
          GET_PGROUP(tch)) {
        sprintf(buf2, "%10s :^N %s%s^N%s%s%s %s^n",
                ...
                GET_CHAR_NAME(tch), GET_TITLE(tch));
      } else {
      ...
      if (GET_PGROUP_MEMBER_DATA(tch) && GET_PGROUP(tch) &&
          !(GET_PGROUP(tch)->is_secret()) && GET_PGROUP_MEMBER_DATA(ch) &&
      ...
      if (PRF_FLAGGED(tch, PRF_AFK))
        strcat(buf1, " (AFK)");
      if (PLR_FLAGGED(tch, PLR_RPE) &&
          (level > LVL_MORTAL || PLR_FLAGGED(ch, PLR_RPE)))
        strcat(buf1, " ^R(RPE)^n");

      if (PRF_FLAGGED(tch, PRF_NEWBIEHELPER) &&
          (level > LVL_MORTAL || PRF_FLAGGED(ch, PRF_NEWBIEHELPER)))
  ...
  for (d = descriptor_list; d; d = d->next) {
    ...
    *(timeptr + 8) = '\0';
    ...
    if (d->character && GET_CHAR_NAME(d->character)) {
      if (d->original)
        sprintf(line, format, d->desc_num, GET_CHAR_NAME(d->original), state,
                idletime, timeptr);
      else
        sprintf(line, format, d->desc_num, GET_CHAR_NAME(d->character), state,
                idletime, timeptr);
    } else
...
void print_object_location(int num, struct obj_data *obj, struct char_data *ch,
  ...
  if (strlen(buf) >= 7500)
    return;
  if (num > 0)
    sprintf(buf + strlen(buf), "O%3d. %-25s - ", num, GET_OBJ_NAME(obj));
  else
        ...
        if (i && CAN_SEE(ch, i) && (i->in_room || i->in_veh)) {
          if (d->original)
            if (d->character->in_veh)
              sprintf(
                  buf + strlen(buf),
                  "%-20s - [%6ld] %s^n (switched as %s) (in %s)\r\n",
                  GET_CHAR_NAME(i), GET_ROOM_VNUM(get_ch_in_room(d->character)),
                  GET_ROOM_NAME(get_ch_in_room(d->character)),
                  GET_NAME(d->character), GET_VEH_NAME(d->character->in_veh));
            else
            ...
            sprintf(buf + strlen(buf), "%-20s - [%6ld] %s^n (in %s)\r\n",
                    GET_CHAR_NAME(i), GET_ROOM_VNUM(get_ch_in_room(i)),
                    GET_ROOM_NAME(get_ch_in_room(i)), GET_VEH_NAME(i->in_veh));
          else
    ...
    for (i = character_list; i; i = i->next)
      if (CAN_SEE(ch, i) && (i->in_room || i->in_veh) &&
        ...
        sprintf(buf + strlen(buf), "M%3d. %-25s - [%5ld] %s^n", ++num,
                GET_NAME(i), GET_ROOM_VNUM(get_ch_in_room(i)),
                GET_ROOM_NAME(get_ch_in_room(i)));
        if (i->in_veh) {
...
ACMD(do_consider) {
  ...
  if (IS_NPC(victim)) {
    ...
    if (GET_MAG(ch) >= 100 &&
        (IS_NPC(ch) || (GET_TRADITION(ch) == TRAD_HERMETIC ||
...
ACMD(do_commands) {
  ...
  // Note: Providing an argument to COMMANDS used to list the commands available
  // to someone else. This seems rather pointless to me, so I'm changing the
  // behavior to filter based on the prefix you provide. -LS
  ...
  if (!*arg && PRF_FLAGGED(ch, PRF_SCREENREADER)) {
    send_to_char(
        ch,
        "The full list of %s is several hundred lines long. We recommend "
        "filtering the list by typing %s <prefix>, which will return"
        " all %s that begin with the specified prefix. If you wish to see all "
        "%s, type %s ALL.\r\n",
        // Here we go...
  ...
  if (*arg && !str_cmp(arg, "all")) {
    ...
    sprintf(buf,
            "The following %s beginning with '%s' are available to you:\r\n",
  ...
  if (PLR_FLAGGED(ch, PLR_MATRIX)) {
    for (no = 1, cmd_num = 1;; cmd_num++) {
      ...
      if (mtx_info[cmd_num].minimum_level >= 0 &&
          ((!IS_NPC(vict) &&
            GET_REAL_LEVEL(vict) >= mtx_info[cmd_num].minimum_level) ||
           (IS_NPC(vict) && vict->desc->original &&
            GET_REAL_LEVEL(vict->desc->original) >=
                mtx_info[cmd_num].minimum_level))) {
    ...
    for (no = 1, cmd_num = 1;; cmd_num++) {
      ...
      if (rig_info[cmd_num].minimum_level >= 0 &&
          ((!IS_NPC(vict) &&
            GET_REAL_LEVEL(vict) >= rig_info[cmd_num].minimum_level) ||
           (IS_NPC(vict) && vict->desc->original &&
            GET_REAL_LEVEL(vict->desc->original) >=
                rig_info[cmd_num].minimum_level))) {
    ...
    for (no = 1, cmd_num = 1; cmd_num < num_of_cmds; cmd_num++) {
      ...
      if (cmd_info[i].minimum_level >= 0 &&
          ((!IS_NPC(vict) &&
            GET_REAL_LEVEL(vict) >= cmd_info[i].minimum_level) ||
           (IS_NPC(vict) && vict->desc->original &&
            GET_REAL_LEVEL(vict->desc->original) >=
...
ACMD(do_scan) {
  ...
  bool specific = FALSE, infra, lowlight, onethere, anythere = FALSE,
       done = FALSE;
  int i = 0, j, dist = 3;
  struct room_data *was_in = NULL, *x = NULL;

  if (AFF_FLAGGED(ch, AFF_DETECT_INVIS) &&
      !(PLR_FLAGGED(ch, PLR_REMOTE) || AFF_FLAGGED(ch, AFF_RIG))) {
  ...
  infra =
      ((PRF_FLAGGED(ch, PRF_HOLYLIGHT) || CURRENT_VISION(ch) == THERMOGRAPHIC)
           ? TRUE
           : FALSE);
  lowlight = ((PRF_FLAGGED(ch, PRF_HOLYLIGHT) || CURRENT_VISION(ch) == LOWLIGHT)
                  ? TRUE
                  : FALSE);
    ...
    for (i = 0; i < NUM_OF_DIRS; ++i) {
      if (CAN_GO(ch, i)) {
        ...
        if (!((!infra && light_level(EXIT(ch, i)->to_room) == LIGHT_FULLDARK) ||
              ((!infra || !lowlight) &&
               (light_level(EXIT(ch, i)->to_room) == LIGHT_MINLIGHT ||
                light_level(EXIT(ch, i)->to_room) == LIGHT_PARTLIGHT)))) {
          strcpy(buf1, "");
          for (list = EXIT(ch, i)->to_room->people; list;
               list = list->next_in_room)
            if (CAN_SEE(ch, list)) {
              if (in_veh) {
                if (in_veh->cspeed > SPEED_IDLE) {
                  if (get_speed(in_veh) >= 200) {
                    if (success_test(GET_INT(ch) +
                                         GET_POWER(ch, ADEPT_IMPROVED_PERCEPT),
                                     7) < 1)
                      continue;
                    else if (get_speed(in_veh) < 200 &&
                             get_speed(in_veh) >= 120) {
                      if (success_test(
                              GET_INT(ch) +
                                  GET_POWER(ch, ADEPT_IMPROVED_PERCEPT),
                              6) < 1)
          ...
          for (veh = EXIT(ch, i)->to_room->vehicles; veh; veh = veh->next_veh) {
            ...
            sprintf(ENDOF(buf1), "  %s (%s)\r\n", GET_VEH_NAME(veh),
                    (get_speed(veh) ? "Moving" : "Stationary"));
            onethere = TRUE;
            anythere = TRUE;
          }
        }
...
ACMD(do_status) {
  ...
  if (GET_DRUG_AFFECT(targ) && GET_DRUG_STAGE(targ) > 0)
    send_to_char(ch, "  %s (%s)\r\n", drug_types[GET_DRUG_AFFECT(targ)].name,
                 GET_DRUG_STAGE(targ) == 1 ? "Up" : "Down");
  ...
  if (GET_SUSTAINED_NUM(targ)) {
    ...
    int i = 1;
    for (struct sustain_data *sust = GET_SUSTAINED(targ); sust;
         sust = sust->next)
...
ACMD(do_recap) {
  if (!GET_QUEST(ch))
    ...
    sprintf(
        buf, "%s told you: \r\n%s",
        GET_NAME(mob_proto + real_mobile(quest_table[GET_QUEST(ch)].johnson)),
