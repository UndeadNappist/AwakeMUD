...
extern void damage_door(struct char_data *ch, struct room_data *room, int dir,
                        int power, int type);
extern void perform_wear(struct char_data *, struct obj_data *, int);
extern void perform_get_from_container(struct char_data *, struct obj_data *,
                                       struct obj_data *, int);
...
int messageless_find_door(struct char_data *ch, char *type, char *dir,
                          const char *cmdname) {
  int door;
...
bool perform_hit(struct char_data *ch, char *argument, const char *cmdname) {
  ...
  if (ch->in_veh) {
    send_to_char(ch,
                 "You can only drive by or use mounts inside a vehicle.\r\n");
    return TRUE;
  }
  ...
  if (weapon && (GET_OBJ_TYPE(weapon) != ITEM_WEAPON &&
    ...
    send_to_char(
        ch,
        "You can't figure out how to attack while using %s as a weapon.\r\n",
  ...
  if (!*arg) {
    ...
    if (wielded) {
      if (GET_OBJ_TYPE(wielded) == ITEM_FIREWEAPON) {
        ...
        damage_door(ch, ch->in_room, dir, (int)(GET_WEAPON_POWER(wielded) / 2),
                    DAMOBJ_PIERCE);
        return TRUE;
        ...
        default:
          if (EXIT(ch, dir)->keyword) {
            ...
            act("You can't damage the door with $p!", FALSE, ch, wielded, 0,
                TO_CHAR);
          return TRUE;
        }
      WAIT_STATE(ch, PULSE_VIOLENCE * 2);
      ...
      damage_door(ch, ch->in_room, dir,
                  (int)((GET_STR(ch) + GET_WEAPON_POWER(wielded)) / 2), type);
    } else {
  ...
  if (veh) {
    ...
    if (!PRF_FLAGGED(ch, PRF_PKER) && veh->owner &&
    ...
    if (!FIGHTING(ch)) {
      ...
      if (GET_EQ(ch, WEAR_WIELD) &&
        ...
        sprintf(buf, "%s aims %s right at your ride!\r\n", GET_NAME(ch),
                GET_OBJ_NAME(GET_EQ(ch, WEAR_HOLD)));
        send_to_veh(buf, veh, NULL, TRUE);
      } else {
  ...
  if (vict == ch) {
    ...
    act("$N is just such a good friend, you simply can't hit $M.", FALSE, ch, 0,
        vict, TO_CHAR);
  else {
    ...
    if (IS_ASTRAL(vict) &&
        !(IS_DUAL(ch) || IS_ASTRAL(ch) || PLR_FLAGGED(ch, PLR_PERCEIVE))) {
    ...
    if (!CH_IN_COMBAT(ch)) {
      ...
      if (GET_EQ(ch, WEAR_WIELD) &&
          GET_OBJ_TYPE(GET_EQ(ch, WEAR_WIELD)) == ITEM_WEAPON) {
        act("You aim $p at $N!", FALSE, ch, GET_EQ(ch, WEAR_WIELD), vict,
            TO_CHAR);
        act("$n aims $p straight at you!", FALSE, ch, GET_EQ(ch, WEAR_WIELD),
            vict, TO_VICT);
      } else if (GET_EQ(ch, WEAR_HOLD) &&
                 GET_OBJ_TYPE(GET_EQ(ch, WEAR_HOLD)) == ITEM_WEAPON) {
        act("You aim $p at $N!", FALSE, ch, GET_EQ(ch, WEAR_HOLD), vict,
            TO_CHAR);
        act("$n aims $p straight at you!", FALSE, ch, GET_EQ(ch, WEAR_HOLD),
            vict, TO_VICT);
      } else {
    ...
    if (!(vict = get_char_room_vis(ch, arg)))
      ...
      if (!IS_NPC(vict)) {
        sprintf(buf2, "%s raw killed by %s. {%s (%ld)}", GET_CHAR_NAME(vict),
                GET_NAME(ch), vict->in_room->name, vict->in_room->number);
...
ACMD(do_flee) {
  ...
  for (int tries = 0; tries < 6; tries++) {
    ...
    if (CAN_GO(ch, attempt) &&
        (!IS_NPC(ch) || !ROOM_FLAGGED(ch->in_room->dir_option[attempt]->to_room,
                                      ROOM_NOMOB))) {
      // Supply messaging and put the character into a wait state to match wait
      // state in perform_move.
      act("$n panics, and attempts to flee!", TRUE, ch, 0, 0, TO_ROOM);
      WAIT_STATE(ch, PULSE_VIOLENCE * 2);

      // If the character is fighting in melee combat, they must pass a test to
      // escape.
      if (GET_POS(ch) >= POS_FIGHTING && FIGHTING(ch) &&
          !AFF_FLAGGED(ch, AFF_PRONE)) {
        if (!success_test(GET_QUI(ch), GET_QUI(FIGHTING(ch)))) {
          act("$N cuts you off as you try to escape!", TRUE, ch, 0,
              FIGHTING(ch), TO_CHAR);
          act("You lunge forward and block $n's escape.", TRUE, ch, 0,
              FIGHTING(ch), TO_VICT);
          act("$N lunges forward and blocks $n's escape.", TRUE, ch, 0,
...
ACMD(do_kick) {
  ...
  if (dir >= NUM_OF_DIRS) {
    if (!(vict = get_char_room_vis(ch, arg)))
      ...
      act("$n tries to kick $s own ass... not too bright, is $e?", TRUE, ch, 0,
          0, TO_ROOM);
    } else {
      act("You sneak up behind $N and kick $M straight in the ass!", FALSE, ch,
          0, vict, TO_CHAR);
      act("$n sneaks up behind you and kicks you in the ass!\r\n"
          "That's gonna leave a mark...",
          FALSE, ch, 0, vict, TO_VICT);
      act("$n sneaks up behind $N and gives $M a swift kick in the ass!", TRUE,
  ...
  if (CH_IN_COMBAT(ch))
    ...
    if (EXIT(ch, dir)->keyword) {
      send_to_char(ch, "You show the %s the bottom of %s!\r\n",
                   fname(EXIT(ch, dir)->keyword),
                   GET_EQ(ch, WEAR_FEET) ? GET_OBJ_NAME(GET_EQ(ch, WEAR_FEET))
                                         : "your foot");
      sprintf(buf, "$n kicks the %s.", fname(EXIT(ch, dir)->keyword));
      act(buf, TRUE, ch, 0, 0, TO_ROOM);
    } else {
      send_to_char(ch, "You show the door the bottom of %s!\r\n",
                   GET_EQ(ch, WEAR_FEET) ? GET_EQ(ch, WEAR_FEET)->text.name
                                         : "your foot");
...
ACMD(do_retract) {
  ...
  } else if (!(GET_CYBERWARE_TYPE(cyber) == CYB_HANDRAZOR ||
               GET_CYBERWARE_TYPE(cyber) == CYB_HANDSPUR ||
               GET_CYBERWARE_TYPE(cyber) == CYB_HANDBLADE ||
               GET_CYBERWARE_TYPE(cyber) == CYB_FOOTANCHOR ||
               GET_CYBERWARE_TYPE(cyber) == CYB_FIN))
    send_to_char("That cyberware isn't retractable.\r\n", ch);
  else if ((GET_CYBERWARE_TYPE(cyber) == CYB_HANDRAZOR ||
            GET_CYBERWARE_TYPE(cyber) == CYB_HANDSPUR ||
            GET_CYBERWARE_TYPE(cyber) == CYB_HANDBLADE) &&
           !IS_SET(GET_CYBERWARE_FLAGS(cyber), CYBERWEAPON_RETRACTABLE))
  ...
  if (!(weapon = GET_EQ(ch, WEAR_WIELD)) ||
    ...
    if (WEAPON_CAN_USE_FIREMODE(weapon, MODE_SS))
      send_to_char(ch, "%s%s^n\r\n",
                   GET_WEAPON_FIREMODE(weapon) == MODE_SS ? "^R" : "",
                   fire_mode[MODE_SS]);
    if (WEAPON_CAN_USE_FIREMODE(weapon, MODE_SA))
      send_to_char(ch, "%s%s^n\r\n",
                   GET_WEAPON_FIREMODE(weapon) == MODE_SA ? "^R" : "",
                   fire_mode[MODE_SA]);
    if (WEAPON_CAN_USE_FIREMODE(weapon, MODE_BF))
      send_to_char(ch, "%s%s^n\r\n",
                   GET_WEAPON_FIREMODE(weapon) == MODE_BF ? "^R" : "",
                   fire_mode[MODE_BF]);
    if (WEAPON_CAN_USE_FIREMODE(weapon, MODE_FA))
      send_to_char(ch, "%s%s (%d rounds)^n\r\n",
                   GET_WEAPON_FIREMODE(weapon) == MODE_FA ? "^R" : "",
                   fire_mode[MODE_FA], GET_WEAPON_FULL_AUTO_COUNT(weapon));
  } else {
    skip_spaces(&argument);
