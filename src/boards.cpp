...
void BoardInit(void) {
  ...
  if (will_die) {
    // Is this true, though? I don't see anything in the logic that would cause
    // this. -Lucien
    log("WARNING: Missing boards were found (vnums above). Entire board system "
        "will be disabled.");
  }

  log("INFO: Loading boards. Any boards that log errors here will be "
...
void Board_write_message(int board_type, struct obj_data *terminal,
  ...
  if (!access_level(ch, WRITE_LVL(board_type))) {
    send_to_char(ch,
                 "You do not have the qualifications to post on the %s.\r\n",
  ...
  if (num_of_msgs[board_type] >= MAX_BOARD_MESSAGES)
    for (i = 0; i < num_of_msgs[board_type]; i++)
      if (MSG_LEVEL(board_type, i) <
          LVL_BUILDER) { /* <-- fixes annoyance. --Py */
...
int Board_reply_message(int board_type, struct obj_data *terminal,
  ...
  if (!access_level(ch, WRITE_LVL(board_type)) ||
    ...
    send_to_char(ch,
                 "You do not have the qualifications to reply on the %s.\r\n",
                 fname(terminal->text.keywords));
    return 1;
  }
  /* Prevent Astral beings from writing to boards */
  ...
  msg--;
  if (MSG_SLOTNUM(board_type, msg) < 0 ||
      MSG_SLOTNUM(board_type, msg) >= INDEX_SIZE) {
  ...
  if (all) {
    ...
    if (MAX(second - first + 1, 0) == 1) {
      ...
      sprintf(buf, "There are %d files on the %s in the specified range.\r\n",
              MAX(second - first + 1, 0), fname(terminal->text.keywords));
    }
...
int Board_show_board(int board_type, struct obj_data *terminal,
  ...
  sprintf(
      buf,
      "This is a computer bulletin board system.\r\n"
...
int Board_remove_msg(int board_type, struct obj_data *terminal,
  ...
  if (!access_level(ch, REMOVE_LVL(board_type)) &&
    ...
    send_to_char("You do not have the correct qualifications to remove other "
                 "people's files.\r\n",
                 ch);
    return 1;
  }
  ...
  for (d = descriptor_list; d; d = d->next)
    if (!d->connected &&
        d->str == &(msg_storage[MSG_SLOTNUM(board_type, ind)])) {
      send_to_char(
