...
const char *pc_race_types[] = {
    "Undef", "Undefined", "Human",      "Dwarf",     "Elf",      "Ork",
    "Troll", "Cyclops",   "Koborokuru", "Fomori",    "Menehune", "Hobgoblin",
    "Giant", "Gnome",     "Oni",        "Wakyambi",  "Ogre",     "Minotaur",
    "Satyr", "Night-One", "Dragon",     "Elemental", "Spirit",   "\n"};
...
int resource_table[2][8] = {
    {500, 5000, 20000, 90000, 200000, 400000, 650000, 1000000},
    {-5, 0, 5, 10, 15, 20, 25, 30}};
int magic_cost[4] = {0, 30, 25, 25};
const char *magic_table[4] = {"None", "Full Magician", "Aspected Magician",
                              "Adept"};

void set_attributes(struct char_data *ch, int magic) {
  // If the character is a magic user, their magic is equal to their essence
  // (this is free).
  if (magic) {
    GET_REAL_MAG(ch) = 600;
  } else {
  ...
  for (int attr = BOD; attr <= WIL; attr++) {
    GET_REAL_ATT(ch, attr) =
        MAX(1, racial_attribute_modifiers[(int)GET_RACE(ch)][attr] + 1);
  }

  // Subtract the cost of making all stats 1 from the character's available
  // attribute-training points.
  int attribute_point_cost =
      get_minimum_attribute_points_for_race(GET_RACE(ch));
  GET_ATT_POINTS(ch) -= attribute_point_cost;
  send_to_char(ch,
               "You spend %d attribute points to raise your attributes to "
               "their minimums.\r\n",
...
void priority_menu(struct descriptor_data *d) {
  ...
  SEND_TO_Q("\r\nChoose a priority (A-E) to set (? for help, p to continue): ",
            d);
}
...
void init_char_sql(struct char_data *ch) {
  ...
  sprintf(
      buf,
      "INSERT INTO pfiles (idnum, name, password, race, gender, Rank, Voice,"
      "Physical_Keywords, Physical_Name, Whotitle, Height, Weight, Host,"
      "Tradition, Born, Background, Physical_LookDesc, Matrix_LookDesc, "
      "Astral_LookDesc, LastD) VALUES ('%ld', '%s', '%s', %d, '%d',"
      "'%d', '%s', '%s', '%s', '%s', '%d', '%d', '%s', '%d', '%ld', '%s', "
      "'%s', '%s', '%s', %ld);",
      GET_IDNUM(ch), GET_CHAR_NAME(ch), GET_PASSWD(ch), GET_RACE(ch),
      GET_SEX(ch), MAX(1, GET_LEVEL(ch)),
      prepare_quotes(buf2, ch->player.physical_text.room_desc,
                     sizeof(buf2) / sizeof(buf2[0])),
      GET_KEYWORDS(ch), GET_NAME(ch), GET_WHOTITLE(ch), GET_HEIGHT(ch),
      GET_WEIGHT(ch), ch->player.host, GET_TRADITION(ch), ch->player.time.birth,
      "A blank slate.", "A nondescript person.\r\n",
      "A nondescript entity.\r\n", "A nondescript entity.\r\n", time(0));
  mysql_wrapper(mysql, buf);
  if (PLR_FLAGGED(ch, PLR_AUTH)) {
    sprintf(buf,
            "INSERT INTO pfiles_chargendata (idnum, AttPoints, SkillPoints, "
            "ForcePoints) VALUES"
            "('%ld', '%d', '%d', '%d');",
            GET_IDNUM(ch), GET_ATT_POINTS(ch), GET_SKILL_POINTS(ch),
            GET_FORCE_POINTS(ch));
    mysql_wrapper(mysql, buf);
  }
  if (GET_TRADITION(ch) != TRAD_MUNDANE) {
    sprintf(
        buf,
        "INSERT INTO pfiles_magic (idnum, Totem, TotemSpirit, Aspect) VALUES"
        "('%ld', '%d', '%d', '%d');",
        GET_IDNUM(ch), GET_TOTEM(ch), GET_TOTEMSPIRIT(ch), GET_ASPECT(ch));
    mysql_wrapper(mysql, buf);
  }
  sprintf(buf, "INSERT INTO pfiles_drugdata (idnum) VALUES (%ld)",
          GET_IDNUM(ch));
  mysql_wrapper(mysql, buf);
  if (GET_LEVEL(ch) > 0) {
    sprintf(buf,
            "INSERT INTO pfiles_immortdata (idnum, InvisLevel, IncogLevel, "
            "Zonenumber, Poofin, Poofout) VALUES ("
            "%ld, %d, %d, %d, '%s', '%s');",
            GET_IDNUM(ch), GET_INVIS_LEV(ch), GET_INCOG_LEV(ch),
...
static void start_game(descriptor_data *d) {
  ...
  set_character_skill(d->character, SKILL_ENGLISH,
                      STARTING_LANGUAGE_SKILL_LEVEL, FALSE);
  ...
  if (PLR_FLAGGED(d->character, PLR_AUTH)) {
    sprintf(buf, "%s [%s] new player.", GET_CHAR_NAME(d->character), d->host);
...
void ccr_totem_menu(struct descriptor_data *d) {
  ...
  for (int i = 1; i < NUM_TOTEMS; i++) {
    sprintf(ENDOF(buf), "\r\n  %s[%2d] %-20s^n", valid_totem(CH, i) ? "" : "^R",
            i, totem_types[i]);
    if (++i < NUM_TOTEMS)
      sprintf(ENDOF(buf), "%s[%2d] %-20s^n", valid_totem(CH, i) ? "" : "^R", i,
              totem_types[i]);
    if (++i < NUM_TOTEMS)
      sprintf(ENDOF(buf), "%s[%2d] %-20s^n", valid_totem(CH, i) ? "" : "^R", i,
              totem_types[i]);
...
void points_menu(struct descriptor_data *d) {
  ...
  sprintf(buf,
          "  1) Attributes : ^c%8d^n (^c%2d^n Points)\r\n"
          ...
          "Choose an area to change points on(p to continue): ",
          d->ccr.pr[PO_ATTR] / 2, d->ccr.pr[PO_ATTR], d->ccr.pr[PO_SKILL],
          d->ccr.pr[PO_SKILL], resource_table[0][d->ccr.pr[PO_RESOURCES]],
          resource_table[1][d->ccr.pr[PO_RESOURCES]],
          magic_table[d->ccr.pr[PO_MAGIC]], magic_cost[d->ccr.pr[PO_MAGIC]],
          pc_race_types[(int)GET_RACE(d->character)], d->ccr.pr[PO_RACE],
          d->ccr.points);
  SEND_TO_Q(buf, d);
}
...
int get_minimum_attribute_points_for_race(int race) {
  // Initialize the returned value to 6, which is the minimum the player must
  // spend to raise all from 0 to 1.
  int minimum_attribute_points = 6;

  // Cycle through all attributes.
  for (int attr = BOD; attr <= WIL; attr++) {
    if (racial_attribute_modifiers[race][attr] < 0) {
      // If the attribute modifier is negative, the player must spend that many
      // points to raise it to 0.
...
// Returns the maximum attribute points you can spend on a given race to max
// them out.
int get_maximum_attribute_points_for_race(int race) {
  int amount = 0;
  for (int attr = BOD; attr <= WIL; attr++) {
    amount += racial_limits[race][0][attr] -
              (max(1, racial_attribute_modifiers[race][attr] + 1));
  }
  return amount;
}
...
void create_parse(struct descriptor_data *d, const char *arg) {
  ...
  int minimum_attribute_points, maximum_attribute_points,
  ...
  case CCR_PO_ATTR:
    ...
    maximum_attribute_points =
        get_maximum_attribute_points_for_race(GET_RACE(CH)) +
        minimum_attribute_points;
    available_attribute_points = (int)(d->ccr.points / 2);

    if (i < minimum_attribute_points) {
      send_to_char(CH,
                   "You need a minimum of %d points in attributes.\r\n"
                   "Enter desired number of attribute points (^c%d^n "
                   "available, minimum %d, maximum %d):",
                   minimum_attribute_points, available_attribute_points,
                   minimum_attribute_points, maximum_attribute_points);
      break;
      ...
      send_to_char(CH,
                   ...
                   "Enter desired number of attribute points (^c%d^n "
                   "available, minimum %d, maximum %d):",
                   available_attribute_points, minimum_attribute_points,
                   maximum_attribute_points);
      break;
    }
    ...
    if (i > maximum_attribute_points) {
      send_to_char(
          CH, "The maximum number of attribute points for your race is %d.\r\n",
          maximum_attribute_points);
  ...
  case CCR_PO_SKILL:
    if (i < 0)
      send_to_char(CH,
                   "You cannot have less than 0 points of skill. Enter desired "
                   "number of skill points (^c%d^n available):",
                   d->ccr.points);
    else if (i > d->ccr.points)
      send_to_char(CH,
                   "You do not have enough points for that. Enter desired "
                   "number of skill points (^c%d^n available):",
                   d->ccr.points);
    else {
  ...
  case CCR_PO_RESOURCES:
    ...
    if (i > 7 || i < 0)
      send_to_char(
          CH,
          "Invalid number. Enter desired amount of nuyen (^c%d^n available): ",
          d->ccr.points);
    else if (resource_table[1][i] > d->ccr.points)
      send_to_char(CH,
                   "You do not have enough points for that. Enter desired "
                   "amount of nuyen (^c%d^n available):",
                   d->ccr.points);
    else {
  ...
  case CCR_PO_MAGIC:
    ...
    if (i > 3 || i < 0)
      send_to_char(
          CH, "Invalid number. Enter desired magic level (^c%d^n available): ",
          d->ccr.points);
    else if (magic_cost[i] > d->ccr.points)
      send_to_char(CH,
                   "You do not have enough points for that. Enter desired "
                   "magic level(^c%d^n available):",
                   d->ccr.points);
    else {
      d->ccr.pr[PO_MAGIC] = i;
  ...
  case CCR_POINTS:
    ...
    case '2':
      ...
      send_to_char(CH,
                   "Enter desired number of skill points (^c%d^n available): ",
                   d->ccr.points);
    ...
    case '3':
      ...
      for (int x = 0; x < 8; x++)
        sprintf(ENDOF(buf), " %d) %8d nuyen   (%2d points)\r\n ", x + 1,
                resource_table[0][x], resource_table[1][x]);
      SEND_TO_Q(buf, d);
      send_to_char(CH,
                   "Enter desired amount of nuyen points (^c%d^n available): ",
                   d->ccr.points);
    ...
    case '4':
      ...
      for (int x = 0; x < 4; x++)
        sprintf(ENDOF(buf), " %d) %18s (%2d points)\r\n ", x + 1,
                magic_table[x], magic_cost[x]);
      SEND_TO_Q(buf, d);
      send_to_char(
          CH, "Enter desired magic level (^c%d^n available): ", d->ccr.points);
    ...
    case 'P':
      ...
      if (d->ccr.points > 0) {
        send_to_char("You still have points remaining to allocate! You should "
                     "spend these before finishing creation.\r\n\r\n",
                     CH);
        points_menu(d);
        break;
      }
      ...
      if (d->ccr.points < 0) {
        send_to_char(
            "You cannot finish creation with a negative point balance. Please "
            "lower one of your selections first.\r\n\r\n",
            CH);
        sprintf(buf,
                "SYSERR: Character %s attained negative point balance %d "
                "during creation.",
                GET_NAME(CH), d->ccr.points);
  ...
  case CCR_TYPE:
    ...
    case '2':
      ...
      d->ccr.pr[PO_ATTR] =
          get_minimum_attribute_points_for_race(GET_RACE(CH)) * 2;
      d->ccr.points -= d->ccr.pr[PO_ATTR];
  ...
  case CCR_RACE:
    ...
    GET_EQ(d->character, WEAR_BODY) =
        read_object(shirts[number(0, 4)], VIRTUAL);
    ...
    // Check to see if the radio exists (real_object() returns -1 if it can't
    // find an object with this vnum).
  ...
  case CCR_TOTEM:
    ...
    if (GET_TOTEM(CH) == TOTEM_WOLF || GET_TOTEM(CH) == TOTEM_SNAKE ||
        GET_TOTEM(CH) == TOTEM_GATOR || GET_TOTEM(CH) == TOTEM_FOX ||
        GET_TOTEM(CH) == TOTEM_FISH || GET_TOTEM(CH) == TOTEM_TURTLE ||
        GET_TOTEM(CH) == TOTEM_GECKO || GET_TOTEM(CH) == TOTEM_GOOSE ||
        GET_TOTEM(CH) == TOTEM_HORSE || GET_TOTEM(CH) == TOTEM_LIZARD ||
        GET_TOTEM(CH) == TOTEM_OTTER)
      ...
      if (ok) {
        ...
        if (d->ccr.pr[0] == PR_MAGIC || d->ccr.pr[1] == PR_MAGIC) {
          if (GET_RACE(d->character) == RACE_GNOME) {
            ...
            d->ccr.mode = CCR_TRADITION;
            SEND_TO_Q("\r\nFollow [h]ermetic, [s]hamanic or som[a]tic (Adept) "
                      "tradition? ",
                      d);
          }
          return;
  ...
  case CCR_TRADITION:
    ...
    case 'h':
      ...
      if ((d->ccr.pr[5] = -1 && d->ccr.pr[PO_MAGIC] == 2) ||
          (d->ccr.pr[5] != -1 && d->ccr.pr[1] == PR_MAGIC)) {
        GET_FORCE_POINTS(CH) = 35;
        ccr_aspect_menu(d);
      } else {
        GET_FORCE_POINTS(CH) = 25;
    ...
    default:
      SEND_TO_Q("\r\nThat's not a tradition!\r\n"
                "Follow [h]ermetic, [s]hamanic or som[a]tic tradition? ",
                d);
      break;
    }
    break;
