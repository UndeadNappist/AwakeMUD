...
char *colorize(struct descriptor_data *d, const char *str,
...
#endif
{
  ...
  while ((pos < argc) && (*(argv[pos]) == '-')) {
    ...
    case 's':
      ...
      break;
    default:
      log_vfprintf("SYSERR: Unknown option -%c in argument string.",
                   *(argv[pos] + 1));
      break;
    }
    pos++;
...
void copyover_recover() {
  ...
  for (;;) {
    ...
    /* Why would you create a new character, then immediately discard it and
     * load a new one from the DB? */
...
#define RLIMIT_NOFILE RLIMIT_OFILE
...

  {
    ...
    if (limit.rlim_max == RLIM_INFINITY)
      ...
      max_descs = MIN((unsigned)MAX_PLAYERS + NUM_RESERVED_DESCS,
                      (unsigned)limit.rlim_max);
...
void alarm_handler(int signal) {
  log("alarm_handler: Alarm signal has been triggered. Killing the MUD to "
      "break out of a suspected infinite loop.\r\n");
  raise(SIGSEGV);
}
...
void game_loop(int mother_desc) {
  ...
  while (!circle_shutdown) {
    ...
    /* Reset the timer on our alarm. If this amount of time in seconds elapses
     * without this being called again, invoke alarm_handler() and die. */
    alarm(10);
    ...
    if (descriptor_list == NULL) {
      ...
      if ((select(mother_desc + 1, &input_set, (fd_set *)0, (fd_set *)0, NULL) <
           0)) {
        if (errno == EINTR) {
          // Suppressed this log since we get woken up regularly by alarm while
          // idle. log("Waking up to process signal.");
        } else
          perror("Select coma");
    ...
    do {
...

      {
        ...
        if (usage_pcnt >= 100) {
          ...
          snprintf(buf, sizeof(buf),
                   "Pulse usage %f >= 100%%. Trace info:", usage_pcnt);
          log(buf);
      ...
      /* just in case, re-calculate after PERF logging to figure out for how
       * long we have to sleep */
      gettimeofday(&now, (struct timezone *)0);
      timespent = timediff(&now, &last_time);
    ...
    if (select(maxdesc + 1, &input_set, &output_set, &exc_set, &zero_time) <
        0) {
      perror("Select poll");
      return;
    }
      ...
      for (d = descriptor_list; d; d = next_d) {
        ...
        if ((--(d->wait) <= 0) && get_from_q(&d->input, comm, &aliased)) {
          ...
          if (d->str) /* writing boards, mail, etc.   */
            ...
            command_interpreter(
                d->character, comm,
                GET_CHAR_NAME(d->character)); /* send it to interpreter */
          }
        }
      }
    }
    ...
    /* give each descriptor an appropriate prompt */
    {
      ...
      for (d = descriptor_list; d; d = d->next) {
        /* KaVir's protocol snippet. The last sent data was OOB, so do NOT draw
         * the prompt */
      ...
      if (time_info.hours == 17) {
        for (i = 0; i <= top_of_world; i++) {
          if (ROOM_FLAGGED(&world[i], ROOM_STREETLIGHTS)) {
            send_to_room(
                "A streetlight hums faintly, flickers, and turns on.\r\n",
                &world[i]);
          }
        }
      }
...
void keepalive(struct descriptor_data *d) {
  char keepalive[] = {(char)IAC, (char)NOP, (char)0};
...
void make_prompt(struct descriptor_data *d) {
  ...
  if (d->str) {
    if (D_PRF_FLAGGED(d, PRF_SCREENREADER)) {
      write_to_descriptor(
          d->descriptor,
          "Compose mode, write the at symbol on new line to quit. ");
    } else {
      write_to_descriptor(d->descriptor,
                          "Compose mode, write @ on new line to quit]: ");
    }
        ...
        if (*prompt == '@' && *(prompt + 1)) {
          ...
          case 'A':
            if (GET_EQ(d->character, WEAR_WIELD)) {
              ...
              if (IS_GUN(GET_OBJ_VAL(GET_EQ(d->character, WEAR_WIELD), 3)))
                ...
                case MODE_FA:
                  sprintf(str, "FA(%d)",
                          GET_OBJ_TIMER(GET_EQ(d->character, WEAR_WIELD)));
                  break;
                default:
                  sprintf(str, "NA");
                }
          ...
          case 'g': // current ammo
            if (GET_EQ(d->character, WEAR_WIELD) &&
              ...
              if (GET_EQ(d->character, WEAR_WIELD)->contains) {
                sprintf(
                    str, "%d",
                    MIN(GET_OBJ_VAL(GET_EQ(d->character, WEAR_WIELD), 5),
                        GET_OBJ_VAL(GET_EQ(d->character, WEAR_WIELD)->contains,
                                    9)));
              } else
                sprintf(str, "0");
            else
          ...
          case 'G': // max ammo
            if (GET_EQ(d->character, WEAR_WIELD) &&
                IS_GUN(GET_OBJ_VAL(GET_EQ(d->character, WEAR_WIELD), 3)))
              sprintf(str, "%d",
                      GET_OBJ_VAL(GET_EQ(d->character, WEAR_WIELD), 5));
            else
              sprintf(str, "0");
            break;
          case 'h': // hacking pool
          ...
          case 'm': // current mental
            ...
            for (struct obj_data *bio = ch->bioware; bio;
                 bio = bio->next_content)
              if (GET_OBJ_VAL(bio, 0) == BIO_DAMAGECOMPENSATOR)
                physical = MIN(10, physical + GET_OBJ_VAL(bio, 1));
              else if (GET_OBJ_VAL(bio, 0) == BIO_PAINEDITOR &&
                       GET_OBJ_VAL(bio, 3)) {
                physical = 10;
                break;
              }
          ...
          case 'r':
            if (ch->persona && ch->persona->decker->deck)
              sprintf(str, "%d",
                      GET_OBJ_VAL(ch->persona->decker->deck, 3) -
                          GET_OBJ_VAL(ch->persona->decker->deck, 5));
            else
              sprintf(str, "0");
            break;
          case 'R':
          ...
          case 's': // current ammo
            if (GET_EQ(d->character, WEAR_HOLD) &&
              ...
              if (GET_EQ(d->character, WEAR_HOLD)->contains) {
                sprintf(str, "%d",
                        MIN(GET_OBJ_VAL(GET_EQ(d->character, WEAR_HOLD), 5),
                            GET_OBJ_VAL(
                                GET_EQ(d->character, WEAR_HOLD)->contains, 9)));
              } else
                sprintf(str, "0");
            else
              sprintf(str, "0");
          ...
          case 'S': // max ammo
            if (GET_EQ(d->character, WEAR_HOLD) &&
                IS_GUN(GET_OBJ_VAL(GET_EQ(d->character, WEAR_HOLD), 3)))
              sprintf(str, "%d",
                      GET_OBJ_VAL(GET_EQ(d->character, WEAR_HOLD), 5));
            else
              sprintf(str, "0");
            break;
          case 't': // magic pool
...
int new_descriptor(int s) {
  ...
  if (sockets_connected >= max_players) {
    write_to_descriptor(desc, "Sorry, Awakened Worlds is full right now... try "
                              "again later!  :-)\r\n");
    close(desc);
    return 0;
  }
  ...
  // Once the descriptor has been fully created and added to any lists, it's
  // time to negotiate:
...
int process_input(struct descriptor_data *t) {
  ...
  // Initialize our temporary buffer, which is used by KaVir protocol code for
  // reading out their crap.
  ...
  do {
    ...
    if ((bytes_read = read(t->descriptor, &temporary_buffer,
                           MAX_PROTOCOL_BUFFER - 1)) < 0) {
    ...
    // Push the data for processing through KaVir's protocol code. Resize
    // bytes_read to account for the stripped control chars.
    ProtocolInput(t, temporary_buffer, bytes_read, t->inbuf);
#ifdef DEBUG_PROTOCOL
    log_vfprintf("Parsed '%s' to '%s', changing length from %d to %lu.",
                 temporary_buffer, t->inbuf + buf_length, bytes_read,
                 strlen(t->inbuf + buf_length));
#endif
    bytes_read = strlen(t->inbuf + buf_length);

    // We parsed out all the data-- must have been pure control codes. Break
    // out.
    if (bytes_read == 0)
      return 0;
...
#ifdef SOMETHING_THAT_DOESNT_EXIST_JUST_SO_XCODE_WONT_CHOKE_ON_SEEING_TWO_END_BRACKETS_WHILE_NOT_UNDERSTANDING_THE_IFDEFS_THAT_SHIELD_THEM
    {
      ...
      // note: if you define the above, you'll have to figure out protocol
      // handling yourself-- see below.
    }
    while (nl_pos == NULL)
      ;
#else
  ...
  int perform_subst(struct descriptor_data * t, char *orig, char *subst) {
    ...
    *(second++) = '\0';
    ...
    if (((strpos - orig) + strlen(first)) < strlen(orig))
      strncat(New, strpos + strlen(first),
              (MAX_INPUT_LENGTH - strlen(New) - 1));
  ...
  void free_editing_structs(descriptor_data * d, int state) {
    if (d->edit_obj) {
      if (d->connected == CON_PART_CREATE || d->connected == CON_AMMO_CREATE ||
  ...
  void close_socket(struct descriptor_data * d) {
    ...
    if (d->character) {
      ...
      if (d->connected == CON_PART_CREATE || d->connected == CON_SPELL_CREATE ||
          d->connected == CON_PLAYING ||
          (d->connected >= CON_SPELL_CREATE &&
        ...
        sprintf(buf, "Closing link to: %s. (%s)", GET_CHAR_NAME(d->character),
        ...
        sprintf(buf, "Losing player: %s.",
                GET_CHAR_NAME(d->character) ? GET_CHAR_NAME(d->character)
                                            : "<null>");
        mudlog(buf, d->character, LOG_CONNLOG, TRUE);
...
void nonblock(int s) {
  ...
  void unrestrict_game(int Empty) {
    ...
    mudlog("Received SIGUSR2 - completely unrestricting game (emergency)", NULL,
           LOG_SYSLOG, TRUE);
    ban_list = NULL;
    restrict = 0;
  }
  ...
  void free_up_memory(int Empty) {
    mudlog("Warning: Received signal, Freeing up Memory", NULL, LOG_SYSLOG,
           TRUE);
    if (!Mem->ClearStacks()) {
...
sigfunc *my_signal(int signo, sigfunc *func) {
  ...
  char *colorize(struct descriptor_data * d, const char *str, bool skip_check) {
    // Big ol' buffer so that even if the entire string is color codes, we can
    // still store it all plus a terminal \0. -LS
    ...
    if (!str || !*str) {
      ...
      sprintf(colorize_error_buf, "SYSERR: Received empty string to colorize()
    for descriptor %d (orig %s, char %s).", d->descriptor, d->original ?
    GET_NAME(d->original) : "(null)", d->character ? GET_CHAR_NAME(d->character)
    : "(null)"); mudlog(colorize_error_buf, NULL, LOG_SYSLOG, TRUE);
      strcpy(buffer, "(null)");
      return buffer;
    }

    if (!D_PRF_FLAGGED(d, PRF_NOCOLOR) && (skip_check || d->character)) // ok
    but why do we care if they have a character?
    {
      while(*str) {
        if (*str == '^') {
  ...
  void send_to_icon(struct matrix_icon * icon, const char *const messg, ...) {
    if (!icon || !icon->decker || !icon->decker->ch ||
        !icon->decker->ch->desc || !messg)
      return;
  ...
  void send_to_host(vnum_t room, const char *messg, struct matrix_icon *icon,
                    bool needsee) {
  ...
  void send_to_veh(const char *messg, struct veh_data *veh,
                   struct char_data *ch, bool torig, ...) {
    struct char_data *i;
  ...
  void send_to_veh(const char *messg, struct veh_data *veh,
                   struct char_data *ch, struct char_data *cha, bool torig) {
    struct char_data *i;
  ...
  void send_to_room(const char *messg, struct room_data *room) {
    ...
    if (messg && room) {
      for (i = room->people; i; i = i->next_in_room)
        if (i->desc)
          if (!(PLR_FLAGGED(i, PLR_REMOTE) || PLR_FLAGGED(i, PLR_MATRIX)) &&
              AWAKE(i))
            SEND_TO_Q(messg, i->desc);
...
#define CHECK_NULL(pointer, expression)                                        \
  ...
  const char *get_voice_perceived_by(struct char_data * speaker,
    ...
    if (IS_NPC(speaker))
      ...
      if (IS_SENATOR(listener)) {
        sprintf(voice_buf, "%s(%s)", speaker->player.physical_text.room_desc,
  ...
  // now returns the composed line, in case you need to capture it for some
  // reason
  const char *perform_act(const char *orig, struct char_data *ch,
    ...
    for (;;) {
      if (*orig == '$') {
        ...
        case 'z':
          ...
          // If we've gotten here, the speaker is an invisible player or staff
          // member.
          else {
            // Since $z is only used for speech, and since NPCs can't be
            // remembered, we display their name when NPCs speak.
            if (IS_NPC(ch))
              i = GET_NAME(ch);
            else {
              if (IS_SENATOR(ch)) {
                ...
                bool masked = FALSE;
                for (struct obj_data *obj = ch->cyberware; obj;
                ...
                if (masked) {
                  ...
                  char *voice = strip_ending_punctuation_new(
  ...
  bool can_send_act_to_target(struct char_data * ch, bool hide_invisible,
                              struct obj_data *obj, void *vict_obj,
...
#define SENDOK(ch)                                                             \
  ((ch)->desc && AWAKE(ch) &&                                                  \
   ...
   (STATE(ch->desc) != CON_SPELL_CREATE))
    ...
    return SENDOK(to) && !(hide_invisible && ch && !CAN_SEE(to, ch)) &&
           (to != ch) &&
           !(PLR_FLAGGED(to, PLR_REMOTE) || PLR_FLAGGED(to, PLR_MATRIX)) &&
           (type == TO_ROOM || type == TO_ROLLS || (to != vict_obj));

#undef SENDOK
  }

  /* Condition checking for perform_act(). Returns the string generated by p_a()
     provided that the type is TO_CHAR, TO_VICT, or TO_DECK. If no string is
     generated or type is other, returns NULL. */
...
#define SENDOK(ch)                                                             \
  ((ch)->desc && (AWAKE(ch) || sleep) &&                                       \
   ...
   (STATE(ch->desc) != CON_SPELL_CREATE))
    ...
    if (type == TO_CHAR) {
      if (ch && SENDOK(ch) &&
          !(PLR_FLAGGED(ch, PLR_REMOTE) || PLR_FLAGGED(ch, PLR_MATRIX)))
    ...
    if (type == TO_VEH_ROOM)
      ...
      sprintf(
          buf,
          "Invocation: act('%s', '%d', char_data, obj_data, vict_obj, '%d').",
          str, hide_invisible, type);
      if (ch) {
        sprintf(ENDOF(buf), "\r\nch: %s, in_room %s, in_veh %s",
                GET_CHAR_NAME(ch),
                ch->in_room ? GET_ROOM_NAME(ch->in_room) : "n/a",
      ...
      if (obj) {
        sprintf(ENDOF(buf), "\r\nobj: %s, in_room %s, in_veh %s",
                GET_OBJ_NAME(obj),
                obj->in_room ? GET_ROOM_NAME(obj->in_room) : "n/a",
...
#undef SENDOK
  ...
  void check_memory_canaries() {
    ...
    for (int world_index = 0; world_index < top_of_world; world_index++) {
      ...
      for (int direction_index = 0; direction_index <= DOWN;
           direction_index++) {
        if (world[world_index].dir_option[direction_index])
          assert(world[world_index].dir_option[direction_index]->canary ==
                 CANARY_VALUE);
      }
