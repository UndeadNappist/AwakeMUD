...
struct obj_data *objList::FindObj(struct char_data *ch, char *name, int num) {
  ...
  while (temp && (i <= num)) {
    if (isname(name, temp->data->text.keywords) &&
        CAN_SEE_OBJ(ch, temp->data) && (++i == num))
      return temp->data;
    temp = temp->next;
  }
...
void objList::UpdateObjsIDelete(const struct obj_data *proto, int rnum,
                                int new_rnum) {
  static nodeStruct<struct obj_data *> *temp;
  static struct obj_data old;
...
void objList::UpdateCounters(void) {
  ...
  mysql_wrapper(mysql,
                "SELECT message FROM trideo_broadcast ORDER BY RAND() LIMIT 1");
  ...
  for (temp = head; temp; temp = next) {
    ...
    if (!OBJ) {
      mudlog("SYSERR: UpdateCounters encountered a non-existent object.", NULL,
             LOG_SYSLOG, TRUE);
      continue;
    }
    ...
    if (GET_OBJ_TYPE(OBJ) == ITEM_PROGRAM &&
        GET_OBJ_VAL(OBJ, 0) == SOFT_EVALUATE) {
      if (!GET_OBJ_VAL(OBJ, 5)) {
        GET_OBJ_VAL(OBJ, 5) = time(0);
        GET_OBJ_VAL(OBJ, 6) = GET_OBJ_VAL(OBJ, 5);
      } else if (GET_OBJ_VAL(OBJ, 5) < time(0) - SECS_PER_REAL_DAY &&
    ...
    // Send out the trideo messages. We assume anything that is a trideo box is
    // not anything else.
    ...
    if (GET_OBJ_TYPE(OBJ) == ITEM_WORKSHOP && GET_WORKSHOP_UNPACK_TICKS(OBJ)) {
      ...
      for (ch = OBJ->in_veh ? OBJ->in_veh->people : OBJ->in_room->people; ch;
      ...
      // If there were no characters in the room working on it, clear its
      // pack/unpack counter.
      if (!ch) {
        sprintf(buf,
                "A passerby rolls %s eyes and quickly re-%spacks the "
                "half-packed $P.",
    ...
    if (GET_OBJ_TYPE(OBJ) == ITEM_DECK_ACCESSORY &&
        GET_OBJ_VAL(OBJ, 0) == TYPE_COOKER && OBJ->contains &&
        GET_OBJ_VAL(OBJ, 9) > 0) {
      if (--GET_OBJ_VAL(OBJ, 9) < 1) {
        struct obj_data *chip = OBJ->contains;
        act("$p beeps loudly, signaling completion.", FALSE, 0, OBJ, 0,
            TO_ROOM);
    ...
    // In-game this is a UCAS dress shirt. I suspect the files haven't been
    // updated.
    /*
    ...
    */
    ...
    if (OBJ->in_room && !OBJ->in_obj && !OBJ->carried_by &&
        !OBJ->obj_flags.quest_id &&
        ((GET_OBJ_TYPE(OBJ) == ITEM_GUN_MAGAZINE && !GET_OBJ_VAL(OBJ, 9)) ||
         (GET_OBJ_TYPE(OBJ) == ITEM_MONEY && !GET_OBJ_VAL(OBJ, 0))) &&
        ++GET_OBJ_TIMER(OBJ) == 3) {
      act("$p is lost on the ground.", TRUE, temp->data->in_room->people, OBJ,
          0, TO_CHAR);
      next = temp->next;
      extract_obj(OBJ);
      continue;
    ...
    if (IS_OBJ_STAT(OBJ, ITEM_CORPSE)) {
      if (GET_OBJ_TIMER(OBJ) > 1) {
        ...
        if (OBJ->carried_by)
          ...
          act("$p is taken away by the coroner.", TRUE,
              temp->data->in_room->people, temp->data, 0, TO_ROOM);
          act("$p is taken away by the coroner.", TRUE,
              temp->data->in_room->people, temp->data, 0, TO_CHAR);
        }
...
void objList::RemoveObjNum(int num) {
  ...
  for (temp = head; temp; temp = next) {
    ...
    if (GET_OBJ_RNUM(temp->data) == num) {
      if (temp->data->carried_by)
        act("$p disintegrates.", FALSE, temp->data->carried_by, temp->data, 0,
            TO_CHAR);
      else if (temp->data->worn_by)
        act("$p disintegrates.", FALSE, temp->data->carried_by, temp->data, 0,
            TO_CHAR);
      else if (temp->data->in_room && temp->data->in_room->people) {
        act("$p disintegrates.", TRUE, temp->data->in_room->people, temp->data,
            0, TO_ROOM);
        act("$p disintegrates.", TRUE, temp->data->in_room->people, temp->data,
            0, TO_CHAR);
      }
