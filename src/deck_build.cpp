...
int get_chip_cost(int type, int rating, int mpcp) {
  int ratingcost =
      (int)(rating * rating * programs[parts[type].software].multiplier * 0.5);
  int mpcpcost =
      (int)(mpcp * mpcp * programs[parts[type].software].multiplier * 0.5);
  switch (type) {
  case PART_ACTIVE:
    return rating * 2;
  case PART_STORAGE:
...
// Enforce certain restrictions on values in parts. Why it was done here instead
// of in the setter logic is anyone's guess.
...
void partbuild_main_menu(struct descriptor_data *d) {
  ...
  send_to_char(CH, "2) Type: ^c%-40s^n     Parts Cost: ^c%d nuyen^n\r\n",
               parts[GET_PART_TYPE(PART)].name,
               get_part_cost(GET_PART_TYPE(PART), GET_PART_RATING(PART),
                             GET_PART_TARGET_MPCP(PART)));
  send_to_char(CH,
               "3) MPCP Designed For: ^c%-30d^n  Chips Cost: ^c%d nuyen^n\r\n",
               GET_PART_TARGET_MPCP(PART),
               get_chip_cost(GET_PART_TYPE(PART), GET_PART_RATING(PART),
                             GET_PART_TARGET_MPCP(PART)));
...
void partbuild_disp_types(struct descriptor_data *d) {
  ...
  for (; x < NUM_PARTS - 1; x += 2)
    send_to_char(CH, "%2d) %-28s%s%2d) %s\r\n", x, parts[x].name,
                 D_PRF_FLAGGED(d, PRF_SCREENREADER) ? "\r\n\r\n" : "  ", x + 1,
                 parts[x + 1].name);
  if (x < NUM_PARTS)
...
void pbuild_parse(struct descriptor_data *d, const char *arg) {
  ...
  case DEDIT_MAIN:
    ...
    case '3':
      if (parts[GET_PART_TYPE(PART)].design >= 0 ||
          GET_PART_TYPE(PART) == PART_ACTIVE ||
          GET_PART_TYPE(PART) == PART_STORAGE ||
          GET_PART_TYPE(PART) == PART_MATRIX_INTERFACE) {
    ...
    case 'q':
    case 'Q':
      GET_PART_CHIP_COST(PART) =
          get_chip_cost(GET_PART_TYPE(PART), GET_PART_RATING(PART),
                        GET_PART_TARGET_MPCP(PART));
      GET_PART_PART_COST(PART) =
          get_part_cost(GET_PART_TYPE(PART), GET_PART_RATING(PART),
                        GET_PART_TARGET_MPCP(PART));
    ...
    case '4':
      ...
      // Explicit fallthrough-- we only allow option 4 if the part can accept a
      // rating in the first place. fall through
    default:
  ...
  case DEDIT_RATING:
    ...
    case PART_IO:
      if (number < 1 || number > GET_PART_TARGET_MPCP(PART) * 100) {
        send_to_char(CH, "I/O must be between 1 and %d. Enter Speed of I/O: ",
                     GET_PART_TARGET_MPCP(PART) * 100);
        return;
      }
      break;
    case PART_STORAGE:
      if (number < 1 || number > GET_PART_TARGET_MPCP(PART) * 600) {
        send_to_char(
            CH, "Memory must be between 1 and %d. Enter Amount of Memory: ",
            GET_PART_TARGET_MPCP(PART) * 600);
        return;
      }
      break;
    case PART_ACTIVE:
      if (number < 1 || number > GET_PART_TARGET_MPCP(PART) * 250) {
        send_to_char(
            CH, "Memory must be between 1 and %d. Enter Amount of Memory: ",
            GET_PART_TARGET_MPCP(PART) * 250);
        return;
      }
      break;
    case PART_RESPONSE:
      if (number < 0 ||
          number > MIN(3, (int)(GET_PART_TARGET_MPCP(PART) / 4))) {
        send_to_char(CH,
                     "Response Increase must be between 1 and %d. Enter "
                     "Response Increase: ",
    ...
    case PART_SATELLITE:
      if (number < 1 || number > GET_PART_TARGET_MPCP(PART)) {
        send_to_char(CH,
                     "Part ratings must be between 1 and the MPCP of the "
                     "target deck (%d). Enter Rating: ",
                     GET_PART_TARGET_MPCP(PART));
        return;
      }
      break;
  ...
  case DEDIT_MPCP:
    if (number > GET_SKILL(CH, SKILL_BR_COMPUTER))
      send_to_char(CH,
                   "You can't create a part for a deck with more MPCP than "
                   "your Computer B/R Skill (%d). Enter Target MPCP: ",
                   GET_SKILL(CH, SKILL_BR_COMPUTER));
    else if (number < 1 || number > MAX_CUSTOM_MPCP_RATING)
      send_to_char(CH,
                   "Target MPCP must be between 1 and %d. Enter Target MPCP: ",
                   MAX_CUSTOM_MPCP_RATING);
    else {
...
ACMD(do_cook) {
  ...
  if (ch->in_veh) {
    if (ch->vfront) {
      send_to_char("There's not enough space up here-- try switching to the "
                   "rear of the vehicle.\r\n",
                   ch);
      return;
    } else if (!ch->in_veh->flags.IsSet(VFLAG_WORKSHOP)) {
      send_to_char(
          "This vehicle doesn't have any power outlets for you to use.\r\n",
          ch);
      return;
    }
  }
  for (struct obj_data *comp = ch->in_veh ? ch->in_veh->contents
                                          : ch->in_room->contents;
       comp; comp = comp->next_content)
    if (GET_OBJ_TYPE(comp) == ITEM_DECK_ACCESSORY &&
        GET_OBJ_VAL(comp, 0) == TYPE_COMPUTER && comp->contains)
  ...
  if (!chip)
    ...
    for (cooker = ch->in_veh ? ch->in_veh->contents : ch->in_room->contents;
         cooker; cooker = cooker->next_content)
      if (GET_OBJ_TYPE(cooker) == ITEM_DECK_ACCESSORY &&
          GET_OBJ_VAL(cooker, 0) == TYPE_COOKER && !cooker->contains)
        break;
    if (!cooker) {
    ...
    for (struct obj_data *obj = ch->in_veh ? ch->in_veh->contents
                                           : ch->in_room->contents;
         obj; obj = obj->next_content)
      if (GET_OBJ_TYPE(obj) == ITEM_DECK_ACCESSORY &&
          GET_OBJ_VAL(obj, 0) == TYPE_PARTS && GET_OBJ_VAL(obj, 1) &&
    ...
    if (!paid) {
      send_to_char(ch, "You don't have a large enough optical chip to encode "
                       "that onto.\r\n");
      return;
    }
    ...
    int skill = get_skill(ch, SKILL_BR_COMPUTER, target) +
                MIN(GET_SKILL(ch, SKILL_BR_COMPUTER), GET_OBJ_VAL(cooker, 0));
    ...
    if (access_level(ch, LVL_ADMIN)) {
      send_to_char("You use your admin powers to greatly accelerate the "
                   "cooking time.\r\n",
                   ch);
      GET_OBJ_TIMER(chip) = 0;
      GET_OBJ_VAL(cooker, 9) = 1;
    }
    act("The light on $p turns orange as it starts to cook the chip.", TRUE, ch,
        cooker, 0, TO_ROOM);
    act("The light on $p turns orange as it starts to cook the chip.", TRUE, ch,
        cooker, 0, TO_CHAR);
  }
...
void part_design(struct char_data *ch, struct obj_data *part) {
  ...
  if (!GET_OBJ_VAL(part, 3) || GET_OBJ_TYPE(part) != ITEM_PART)
    ...
    int target = GET_OBJ_VAL(part, 2) / 2,
        skill = get_skill(ch, SKILL_CYBERTERM_DESIGN, target);
    GET_OBJ_VAL(part, 3) = GET_OBJ_VAL(part, 2) * 2;
    GET_OBJ_VAL(part, 5) = success_test(skill, target) << 1;
    GET_OBJ_VAL(part, 7) = GET_IDNUM(ch);
    if (access_level(ch, LVL_ADMIN)) {
      send_to_char("You use your admin powers to greatly accelerate the design "
                   "process.\r\n",
                   ch);
      GET_OBJ_VAL(part, 3) = 1;
...
ACMD(do_build) {
  ...
  if (!(obj = get_obj_in_list_vis(ch, arg1, ch->carrying))) {
    if (!(obj = get_obj_in_list_vis(ch, arg1, ch->in_room->contents))) {
      if (!str_cmp(arg1, "circle")) {
        for (obj = ch->in_room->contents; obj; obj = obj->next_content)
          if (GET_OBJ_TYPE(obj) == ITEM_MAGIC_TOOL &&
            ...
            send_to_char(
                "There is already a lodge or a hermetic circle here.\r\n", ch);
            return;
          }
        if (!*arg2 || !*arg3)
      ...
      if (!GET_OBJ_VAL(obj, 9))
        ...
        act("$n continues to draw the hermetic circle.", FALSE, ch, 0, 0,
            TO_ROOM);
      } else
  ...
  if (GET_PART_TYPE(obj) == PART_MPCP && GET_CYBERDECK_MPCP(deck)) {
    ...
    if (GET_PART_TARGET_MPCP(obj) != GET_CYBERDECK_MPCP(deck))
      for (; temp; temp = temp->next_content)
        if (GET_OBJ_TYPE(temp) == ITEM_PART &&
            GET_PART_TARGET_MPCP(obj) == GET_PART_TARGET_MPCP(temp))
          break;
    if (!temp) {
      send_to_char(ch, "%s is not designed for the same MPCP as %s.\r\n",
                   capitalize(GET_OBJ_NAME(obj)),
                   decapitalize_a_an(GET_OBJ_NAME(deck)));
      return;
    }
  }
  if (GET_OBJ_TYPE(obj) != ITEM_PART)
    ...
    send_to_char(ch,
                 "%s isn't a custom deck; how exactly did you plan to build %s "
                 "into it?\r\n",
    ...
    send_to_char(ch, "You must make a design for %s first.\r\n",
           ...
           (GET_CYBERDECK_MPCP(deck) &&
            GET_PART_TARGET_MPCP(obj) != GET_CYBERDECK_MPCP(deck)) &&
    ...
    send_to_char(ch, "%s is not designed for the same MPCP as %s.\r\n",
                 capitalize(GET_OBJ_NAME(obj)),
                 decapitalize_a_an(GET_OBJ_NAME(deck)));
  else if (GET_PART_BUILDER_IDNUM(obj) != GET_IDNUM(ch) &&
    ...
    if (has_kit(ch, TYPE_MICROTRONIC))
      kit_rating = TYPE_KIT;
    if ((workshop = find_workshop(ch, TYPE_MICROTRONIC)) &&
        GET_OBJ_VAL(workshop, 0) > kit_rating)
    ...
    if (kit_rating < parts[GET_PART_TYPE(obj)].tools) {
      ...
      if (kit_rating == TYPE_SHOP)
        ...
        target -= 3;
    } else if (parts[GET_OBJ_VAL(obj, 0)].tools == TYPE_SHOP &&
               kit_rating == TYPE_FACILITY)
      target--;
    if (GET_PART_TYPE(obj) == PART_BOD || GET_PART_TYPE(obj) == PART_SENSOR ||
        GET_PART_TYPE(obj) == PART_MASKING ||
        GET_PART_TYPE(obj) == PART_EVASION) {
      int total = GET_PART_RATING(obj);
      for (struct obj_data *part = deck->contains; part;
           part = part->next_content)
        if (GET_OBJ_TYPE(part) == ITEM_PART &&
            (GET_PART_TYPE(part) == PART_BOD ||
             GET_PART_TYPE(part) == PART_SENSOR ||
             GET_PART_TYPE(part) == PART_MASKING ||
             GET_PART_TYPE(part) == PART_EVASION))
          total += GET_OBJ_VAL(part, 1);
      if (total > GET_CYBERDECK_MPCP(deck) * 3) {
        send_to_char(ch,
                     "The total persona program rating for MPCP %d decks is "
                     "limited to %d; that would make it %d.\r\n",
                     GET_CYBERDECK_MPCP(deck), GET_CYBERDECK_MPCP(deck) * 3,
                     total);
        return;
      }
    }
    for (struct obj_data *part = deck->contains; part;
         part = part->next_content)
      if (GET_OBJ_TYPE(part) == TYPE_PARTS &&
          (GET_PART_TYPE(part) == GET_PART_TYPE(obj) ||
           (GET_PART_TYPE(part) == PART_ASIST_HOT &&
            GET_PART_TYPE(obj) == PART_ASIST_COLD) ||
           (GET_PART_TYPE(part) == PART_ASIST_COLD &&
            GET_PART_TYPE(obj) == PART_ASIST_HOT))) {
        send_to_char(
            ch, "You have already installed a part of that type in there.\r\n");
        return;
      }
    if (GET_OBJ_VAL(obj, 4) > 0) {
      ...
      for (struct obj_data *part = deck->contains; part;
           part = part->next_content)
        if (GET_OBJ_TYPE(part) == TYPE_PARTS &&
            (GET_OBJ_VAL(part, 0) == GET_OBJ_VAL(obj, 0) ||
             (GET_OBJ_VAL(part, 0) == PART_ASIST_HOT &&
              GET_OBJ_VAL(obj, 0) == PART_ASIST_COLD) ||
             (GET_OBJ_VAL(part, 0) == PART_ASIST_COLD &&
              GET_OBJ_VAL(obj, 0) == PART_ASIST_HOT))) {
          send_to_char(
              ch,
              "You have already installed a part of that type in there.\r\n");
          return;
        }
      if (GET_OBJ_VAL(obj, 4) == -1) {
        ...
        if (parts[GET_OBJ_VAL(obj, 0)].software) {
          for (soft = ch->carrying; soft; soft = soft->next_content)
            if (GET_OBJ_VAL(soft, 0) == parts[GET_OBJ_VAL(obj, 0)].software &&
              ...
              break;
          if (!soft) {
            send_to_char(ch, "You need to program and cook the required "
                             "software for that first.\r\n");
            return;
          }
        }
        struct obj_data *chips = NULL, *part = NULL;
        for (struct obj_data *find = ch->in_room->contents; find;
             find = find->next_content)
          if (GET_OBJ_TYPE(find) == ITEM_DECK_ACCESSORY &&
              GET_OBJ_VAL(find, 0) == TYPE_PARTS) {
            if (GET_OBJ_VAL(find, 1) &&
                GET_OBJ_COST(find) >= GET_OBJ_VAL(obj, 9))
              chips = find;
            else if (!GET_OBJ_VAL(find, 1) &&
                     GET_OBJ_COST(find) >= GET_OBJ_VAL(obj, 8))
              part = find;
          }
        for (struct obj_data *find = ch->carrying; find;
             find = find->next_content)
          if (GET_OBJ_TYPE(find) == ITEM_DECK_ACCESSORY &&
              GET_OBJ_VAL(find, 0) == TYPE_PARTS) {
            if (GET_OBJ_VAL(find, 1) &&
        ...
        if (GET_OBJ_VAL(obj, 8)) {
          if (!part) {
            send_to_char(ch,
                         "You don't have enough materials for that part.\r\n");
            return;
          } else {
          ...
          if (!chips) {
            send_to_char(
                ch, "You don't have enough optical chips for that part.\r\n");
            return;
          } else {
...
ACMD(do_progress) {
  if (AFF_FLAGS(ch).IsSet(AFF_CIRCLE)) {
    send_to_char(
        ch,
        "The hermetic circle you are working on is about %d%% completed.\r\n",
        (int)(((float)((GET_OBJ_VAL(ch->char_specials.programming, 1) * 60) -
                       GET_OBJ_VAL(ch->char_specials.programming, 9)) /
               (float)((GET_OBJ_VAL(ch->char_specials.programming, 1) != 0
                            ? GET_OBJ_VAL(ch->char_specials.programming, 1)
                            : 1) *
                       60)) *
              100));
  } else if (AFF_FLAGS(ch).IsSet(AFF_LODGE)) {
    send_to_char(
        ch, "The lodge you are working on is about %d%% completed.\r\n",
        (int)(((float)(((GET_OBJ_VAL(ch->char_specials.programming, 1) != 0
                             ? GET_OBJ_VAL(ch->char_specials.programming, 1)
                             : 1) *
                        300) -
                       GET_OBJ_VAL(ch->char_specials.programming, 9)) /
               (float)(GET_OBJ_VAL(ch->char_specials.programming, 1) * 300)) *
              100));
    ...
    for (struct obj_data *obj = ch->in_room->contents; obj;
      ...
      if (GET_OBJ_TYPE(obj) == ITEM_WORKSHOP && GET_OBJ_VAL(obj, 3)) {
        send_to_char(ch,
                     "You are about %d%% of the way through%spacking %s.\r\n",
                     (int)((float)((3.0 - GET_OBJ_VAL(obj, 3)) / 3) * 100),
                     GET_OBJ_VAL(obj, 2) ? " " : " un", GET_OBJ_NAME(obj));
        break;
      }
  } else if (AFF_FLAGS(ch).IsSet(AFF_BONDING)) {
    send_to_char(ch, "You are about %d%% of the way through bonding %s.\r\n",
                 (int)(((float)(GET_OBJ_VAL(GET_BUILDING(ch), 1) -
                                GET_OBJ_VAL(GET_BUILDING(ch), 9)) /
                        (GET_OBJ_VAL(GET_BUILDING(ch), 1) != 0
                             ? GET_OBJ_VAL(GET_BUILDING(ch), 1)
                             : 1) *
                        60) *
                       100),
    ...
    send_to_char(ch,
                 "You are about %d%% of the way through programming %s.\r\n",
                 (int)(((float)(GET_OBJ_TIMER(GET_BUILDING(ch)) -
                                GET_OBJ_VAL(GET_BUILDING(ch), 5)) /
                        GET_OBJ_TIMER(GET_BUILDING(ch))) *
                       100),
                 GET_OBJ_NAME(GET_BUILDING(ch)));
  } else if (AFF_FLAGS(ch).IsSet(AFF_PART_BUILD)) {
    send_to_char(ch, "You are about %d%% of the way through building %s.\r\n",
                 (int)(((float)(GET_OBJ_VAL(GET_BUILDING(ch), 10) -
                                GET_OBJ_VAL(GET_BUILDING(ch), 4)) /
                        GET_OBJ_VAL(GET_BUILDING(ch), 10)) *
                       100),
                 GET_OBJ_NAME(GET_BUILDING(ch)));
    ...
    send_to_char(ch, "You are about %d%% of the way through designing %s.\r\n",
                 (int)(((float)(GET_OBJ_TIMER(GET_BUILDING(ch)) -
                                GET_OBJ_VAL(GET_BUILDING(ch), 4)) /
                        (GET_OBJ_TIMER(GET_BUILDING(ch)) != 0
                             ? GET_OBJ_TIMER(GET_BUILDING(ch))
                             : 1) *
                        100)),
                 GET_OBJ_NAME(GET_BUILDING(ch)));
    ...
    send_to_char(
        ch, "You are about %d%% of the way through the conjuring process.\r\n",
        (int)((float)(ch->char_specials.conjure[2] /
                      ch->char_specials.conjure[3]) *
              100));
    ...
    send_to_char(ch, "You are about %d%% done designing %s.\r\n",
                 (int)(((float)timeleft /
                        (float)GET_OBJ_VAL(ch->char_specials.programming, 7)) *
                           -100 +
                       100),
                 GET_OBJ_NAME(ch->char_specials.programming));
