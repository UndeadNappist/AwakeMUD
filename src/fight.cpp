...
void damage_door(struct char_data *ch, struct room_data *room, int dir,
                 int power, int type);
void damage_obj(struct char_data *ch, struct obj_data *obj, int power,
                int type);
...
void explode(struct char_data *ch, struct obj_data *weapon,
...
extern int can_wield_both(struct char_data *, struct obj_data *,
                          struct obj_data *);
...
extern void cast_spell(struct char_data *ch, int spell, int sub, int force,
                       char *arg);
extern char *get_player_name(vnum_t id);

extern void dominator_mode_switch(struct char_data *ch, struct obj_data *obj,
                                  int mode);
...
void appear(struct char_data *ch) {
  AFF_FLAGS(ch).RemoveBits(AFF_IMP_INVIS, AFF_INVISIBLE, AFF_HIDE, ENDBIT);

  // TODO: Go through all sustained spells in the game and remove those that are
  // causing this character to be invisible.
...
void update_pos(struct char_data *victim) {
  ...
  if ((GET_PHYSICAL(victim) >= 100) && (GET_POS(victim) > POS_STUNNED))
    ...
    return;
  } else if ((int)(GET_PHYSICAL(victim) / 100) <=
             -GET_REAL_BOD(victim) +
                 (GET_BIOOVER(victim) > 0 ? GET_BIOOVER(victim) : 0))
...
void check_killer(struct char_data *ch, struct char_data *vict) {
  ...
  if (!IS_NPC(vict) && !PLR_FLAGS(vict).AreAnySet(PLR_KILLER, ENDBIT) &&
      ...
      !PLR_FLAGGED(attacker, PLR_KILLER) && attacker != vict &&
      !IS_SENATOR(attacker)) {
    PLR_FLAGS(attacker).SetBit(PLR_KILLER);

    sprintf(buf, "PC Killer bit set on %s for initiating attack on %s at %s.",
            GET_CHAR_NAME(attacker), GET_CHAR_NAME(vict), vict->in_room->name);
...
void set_fighting(struct char_data *ch, struct char_data *vict, ...) {
  ...
  if (!(AFF_FLAGGED(ch, AFF_MANNING) || PLR_FLAGGED(ch, PLR_REMOTE) ||
  ...
  if (!ch->in_veh) {
    if (ch->followers)
      for (k = ch->followers; k; k = k->next)
        if (PRF_FLAGGED(k->follower, PRF_ASSIST) &&
            k->follower->in_room == ch->in_room && !k->follower->in_veh &&
            !FIGHTING(k->follower) && AWAKE(k->follower))
          do_assist(k->follower, buf, 0, 0);
    if (ch->master) {
      if (PRF_FLAGGED(ch->master, PRF_ASSIST) &&
          ch->master->in_room == ch->in_room && !ch->master->in_veh &&
...
void set_fighting(struct char_data *ch, struct veh_data *vict) {
  ...
  if (ch->master) {
    if (PRF_FLAGGED(ch->master, PRF_ASSIST) &&
        ch->master->in_room == ch->in_room && !FIGHTING(ch->master) &&
        AWAKE(ch->master))
      do_assist(ch->master, buf, 0, 0);
    for (k = ch->master->followers; k; k = k->next)
      if (PRF_FLAGGED(k->follower, PRF_ASSIST) &&
          k->follower->in_room == ch->in_room && k->follower != ch &&
          !FIGHTING(k->follower) && AWAKE(k->follower))
        do_assist(k->follower, buf, 0, 0);
  }
}
...
void make_corpse(struct char_data *ch) {
  ...
  corpse->text.look_desc =
      str_dup("What once was living is no longer. Poor sap.\r\n");
  ...
  if (IS_NPC(ch)) {
    ...
    mob_proto[GET_MOB_RNUM(ch)].mob_specials.value_death_nuyen +=
        credits + nuyen;
...
void death_cry(struct char_data *ch) {
  ...
  for (door = 0; door < NUM_OF_DIRS; door++) {
    if (CAN_GO(ch, door)) {
      ...
      act("Somewhere close, you hear someone's death cry!", FALSE, ch, 0, 0,
          TO_ROOM);
      ch->in_room = was_in;
    }
  }
  ...
  if (IS_ELEMENTAL(ch) && GET_ACTIVE(ch)) {
    for (struct descriptor_data *d = descriptor_list; d; d = d->next)
      if (d->character && GET_IDNUM(d->character) == GET_ACTIVE(ch)) {
        for (struct spirit_data *spirit = GET_SPIRIT(d->character); spirit;
             spirit = spirit->next)
        ...
        send_to_char(
            d->character,
            "%s is disrupted and returns to the metaplanes to heal.\r\n",
            CAP(GET_NAME(ch)));
        break;
      }
  }
  if (IS_ASTRAL(ch)) {
    ...
    if (!IS_NPC(ch)) {
          ...
          if (GET_OBJ_VAL(bio, 5) > 0) {
            for (i = 0; i < MAX_OBJ_AFFECT; i++)
              affect_modify(ch, bio->affected[i].location,
                            bio->affected[i].modifier, bio->obj_flags.bitvector,
                            FALSE);
            GET_OBJ_VAL(bio, 5) = 0;
          }
          break;
      ...
      if (PLR_FLAGGED(ch, PLR_AUTH))
        ...
        default:
          sprintf(buf,
                  "SYSERR: Bad jurisdiction type %d in room %ld encountered in "
                  "raw_kill() while transferring %s (%ld).",
                  GET_JURISDICTION(ch->in_room), ch->in_room->number,
...
void death_penalty(struct char_data *ch) {
  ...
  if (!IS_NPC(ch) && !PLR_FLAGGED(ch, PLR_NEWBIE) && GET_REAL_BOD(ch) > 1 &&
    ...
    sprintf(
        buf,
        "%s lost a point of attribute %d.  Total Karma Earned from %d to %d.",
...
void die(struct char_data *ch) {
  ...
  if (!((IS_NPC(ch) && MOB_FLAGGED(ch, MOB_INANIMATE)) || IS_PROJECT(ch) ||
        IS_SPIRIT(ch) || IS_ELEMENTAL(ch))) {
    increase_blood(temp_room);
    act("^rBlood splatters everywhere!^n", FALSE, ch, 0, 0, TO_ROOM);
    if (!GET_BACKGROUND_COUNT(temp_room) ||
        GET_BACKGROUND_AURA(temp_room) == AURA_PLAYERCOMBAT) {
...
ACMD(do_die) {
  ...
  /* log it */
  sprintf(buf, "%s gave up the will to live. {%s (%ld)}", GET_CHAR_NAME(ch),
...
int calc_karma(struct char_data *ch, struct char_data *vict) {
  ...
  if (ch && IS_PROJECT(ch))
    ch = ch->desc->original;
  if (ch &&
      (GET_RACE(vict) == RACE_SPIRIT || GET_RACE(vict) == RACE_ELEMENTAL) &&
      GET_IDNUM(ch) == GET_ACTIVE(vict))
    return 0;

  base = (int)((1.2 *
                (GET_REAL_BOD(vict) + GET_BALLISTIC(vict) + GET_IMPACT(vict))) +
  ...
  base += (!ch ? 0 : (!number(0, 2) ? number(0, 5) : 0 - number(0, 5)));
...
char *replace_string(const char *str, const char *weapon_singular,
                     const char *weapon_plural, const char *weapon_different) {
  static char buf[256];
  char *cp;
...
void dam_message(int dam, struct char_data *ch, struct char_data *victim,
  ...
  static struct dam_weapon_type {
    ...
    const char *to_victim;
      ...
      {"$n grazes $N as $e #W $M.", /* 1: 1..2  */
       "You graze $N as you #w $M.", "$n grazes you as $e #W you."},
  ...
  buf = replace_string(
      dam_weapons[msgnum].to_room, attack_hit_text[w_type].singular,
      attack_hit_text[w_type].plural, attack_hit_text[w_type].different);
  for (witness = victim->in_room->people; witness;
       witness = witness->next_in_room)
    if (witness != ch && witness != victim &&
        !PRF_FLAGGED(witness, PRF_FIGHTGAG) && SENDOK(witness))
      perform_act(buf, ch, NULL, victim, witness);
  if (ch->in_room != victim->in_room && !PLR_FLAGGED(ch, PLR_REMOTE))
    for (witness = ch->in_room->people; witness;
         witness = witness->next_in_room)
      if (witness != ch && witness != victim &&
          !PRF_FLAGGED(witness, PRF_FIGHTGAG) && SENDOK(witness))
        perform_act(buf, ch, NULL, victim, witness);
  ...
  strcpy(buf1, "^y");
  buf = replace_string(
      dam_weapons[msgnum].to_char, attack_hit_text[w_type].singular,
      attack_hit_text[w_type].plural, attack_hit_text[w_type].different);
  strcat(buf1, buf);
  if (SENDOK(ch))
  ...
  strcpy(buf1, "^r");
  buf = replace_string(
      dam_weapons[msgnum].to_victim, attack_hit_text[w_type].singular,
      attack_hit_text[w_type].plural, attack_hit_text[w_type].different);
  strcat(buf1, buf);
...
void weapon_scatter(struct char_data *ch, struct char_data *victim,
  ...
  if (i <= total) { // hits a victim
    ...
    if (vict && (IS_NPC(vict) || (!IS_NPC(vict) && vict->desc))) {
      ...
      damage_total = convert_damage(
          stage((2 - success_test(GET_BOD(vict) + GET_BODY(vict), power)),
                damage_total));
    ...
    if (i < 3) {
      ...
      damage_door(NULL, victim->in_room, dir[i], GET_OBJ_VAL(weapon, 0),
                  DAMOBJ_PROJECTILE);
      return;
    }
  }
...
void damage_equip(struct char_data *ch, struct char_data *victim, int power,
  ...
  int i = number(
      0, attacktype == TYPE_FIRE || attacktype == TYPE_ACID ? NUM_WEARS : 60);
...
void damage_door(struct char_data *ch, struct room_data *room, int dir,
                 int power, int type) {
  if (!room || dir < NORTH || dir > DOWN || !room->dir_option[dir] ||
      !room->dir_option[dir]->keyword ||
  ...
  rev = rev_dir[dir];
  if (opposite && opposite->dir_option[rev] &&
      opposite->dir_option[rev]->to_room == room)
    ok = TRUE;
  ...
  if (ch && IS_SET(type, DAMOBJ_CRUSH) && GET_TRADITION(ch) == TRAD_ADEPT &&
      GET_POWER(ch, ADEPT_SMASHING_BLOW))
    power += MAX(0, success_test(GET_SKILL(ch, SKILL_UNARMED_COMBAT), 4));
  if (IS_GUN(type))
    sprintf(buf,
            "You hear gunshots and the sound of bullets impacting the %s.\r\n",
  ...
  // Compose the damage message, but don't send it-- it can be overwritten by
  // the destruction message if necessary.
  if (power < rating) {
...
void damage_obj(struct char_data *ch, struct obj_data *obj, int power,
  ...
  if (IS_SET(type, DAMOBJ_MANIPULATION) || type == DAMOBJ_FIRE ||
      type == DAMOBJ_ACID || type == DAMOBJ_LIGHTNING) {
  ...
  switch (type) {
  case DAMOBJ_ACID:
    if ((GET_OBJ_TYPE(obj) == ITEM_GYRO || GET_OBJ_TYPE(obj) == ITEM_WORN) &&
        success == 2)
      switch (number(0, 4)) {
      case 0:
  ...
  case DAMOBJ_EXPLODE:
    if (GET_OBJ_TYPE(obj) == ITEM_GUN_MAGAZINE && success == 2 && vict) {
      ...
      act("One of $n's magazines ignites, spraying bullets about!", FALSE, vict,
          obj, 0, TO_ROOM);
      ...
      dam = convert_damage(
          stage(-success_test(GET_BOD(vict) + GET_BODY(vict), target), dam));
    ...
    if (GET_OBJ_TYPE(obj) == ITEM_WEAPON && success == 2 &&
      ...
      if (vict) {
        ...
        act("A $p carried by $n is set off by the fire!", FALSE, vict, obj, 0,
            TO_ROOM);
        explode(NULL, obj, obj->in_room);
      } else if (obj->in_room) {
        sprintf(buf, "%s is set off by the flames!", CAP(obj->text.name));
...
void docwagon_message(struct char_data *ch) {
  ...
  case SPIRIT_HEARTH:
    sprintf(buf,
            "A DocWagon employee suddenly appears, transports %s's body "
            "to\r\nsafety, and rushes away.",
            GET_NAME(ch));
  ...
  case SPIRIT_SEA:
    sprintf(buf,
            "A DocWagon armored speedboat arrives, loading %s's body "
    ...
    break;
  default:
    sprintf(buf, "A DocWagon helicopter flies in, taking %s's body to safety.",
            GET_NAME(ch));
    act(buf, FALSE, ch, 0, 0, TO_ROOM);
    sprintf(buf, "A DocWagon helicopter flies in, taking %s's body to safety.",
            GET_CHAR_NAME(ch));
...
void docwagon(struct char_data *ch) {
  ...
  for (i = 0; (i < NUM_WEARS && !docwagon); i++)
    if (GET_EQ(ch, i) && GET_OBJ_TYPE(GET_EQ(ch, i)) == ITEM_DOCWAGON &&
        GET_DOCWAGON_BONDED_IDNUM(GET_EQ(ch, i)) == GET_IDNUM(ch))
  ...
  if (success_test(GET_DOCWAGON_CONTRACT_GRADE(docwagon) + 1,
    ...
    if (ch->persona) {
      ...
      for (struct char_data *temp = ch->in_room->people; temp;
    ...
    char_from_room(ch);
    char_to_room(ch, &world[i]);
    creds = MAX((number(8, 12) * 500 / GET_DOCWAGON_CONTRACT_GRADE(docwagon)),
                (int)(GET_NUYEN(ch) / 10));
    send_to_char(ch, "DocWagon demands %d nuyen for your rescue.\r\n", creds);
    if ((GET_NUYEN(ch) + GET_BANK(ch)) < creds) {
      send_to_char("Not finding sufficient payment, your DocWagon contract was "
                   "retracted.\r\n",
                   ch);
...
void check_adrenaline(struct char_data *ch, int mode) {
  ...
  for (pump = ch->bioware; pump && GET_OBJ_VAL(pump, 0) != BIO_ADRENALPUMP;
       pump = pump->next_content)
    ;
  if (!pump)
    return;
  if (GET_OBJ_VAL(pump, 5) == 0 && mode == 1) {
    ...
    if (GET_OBJ_VAL(pump, 5) == 0) {
      for (i = 0; i < MAX_OBJ_AFFECT; i++)
        affect_modify(ch, pump->affected[i].location,
                      pump->affected[i].modifier, pump->obj_flags.bitvector,
                      FALSE);
      GET_OBJ_VAL(pump, 5) = -number(80, 100);
      send_to_char(
          "Your body softens and relaxes as the adrenaline wears off.\r\n", ch);
      dam = convert_damage(stage(-success_test(GET_BOD(ch) + GET_BODY(ch),
                                               (int)(GET_OBJ_VAL(pump, 6) / 2)),
                                 DEADLY));
...
#define WRITE_DEATH_MESSAGE(format_string)                                     \
  sprintf(buf2, format_string, GET_CHAR_NAME(vict),                            \
          GET_ROOM_NAME(vict->in_room), GET_ROOM_VNUM(vict->in_room))
#define WRITE_DEATH_MESSAGE_WITH_AGGRESSOR(format_string)                      \
  sprintf(buf2, format_string, GET_CHAR_NAME(vict),                            \
          ch == vict ? "Misadventure(?)" : GET_NAME(ch),                       \
          GET_ROOM_NAME(vict->in_room), GET_ROOM_VNUM(vict->in_room))
void gen_death_msg(struct char_data *ch, struct char_data *vict,
                   int attacktype) {
  switch (attacktype) {
  case TYPE_SUFFERING:
    switch (number(0, 4)) {
  ...
  case TYPE_EXPLOSION:
    ...
    case 0:
      ...
      break;
    case 1:
      WRITE_DEATH_MESSAGE_WITH_AGGRESSOR(
          "%s was blown to bits. [%s] {%s (%ld)}");
      break;
    case 2:
    ...
    case 3:
      WRITE_DEATH_MESSAGE_WITH_AGGRESSOR(
          "%s was incinerated by an explosion. [%s] {%s (%ld)}");
      break;
    case 4:
      WRITE_DEATH_MESSAGE_WITH_AGGRESSOR(
          "%s got blown to hell by %s. {%s (%ld)}");
      break;
    }
    break;
  case TYPE_SCATTERING:
    ...
    case 0:
      WRITE_DEATH_MESSAGE_WITH_AGGRESSOR(
          "%s accidentally (?) killed by %s. {%s (%ld)}");
      break;
    case 1:
      WRITE_DEATH_MESSAGE_WITH_AGGRESSOR(
          "Shouldn't have been standing there, should you %s? [%s] {%s (%ld)}");
      break;
    case 2:
    ...
    case 3:
      sprintf(
          buf2,
          "%s's stray bullet caught %s in the heart. What a shame. {%s (%ld)}",
          ch == vict ? "???" : GET_NAME(ch), GET_CHAR_NAME(vict),
          vict->in_room->name, vict->in_room->number);
      break;
    case 4:
      WRITE_DEATH_MESSAGE_WITH_AGGRESSOR(
          "A random bullet killed a random person -- poor %s. [%s] {%s (%ld)}");
      break;
    }
    break;
  case TYPE_FALL:
    ...
    case 3:
      WRITE_DEATH_MESSAGE(
          "What %s would have given for a safety net... {%s (%ld)}");
      break;
    case 4:
      WRITE_DEATH_MESSAGE(
          "The ground can be such an unforgiving thing. Right, %s? {%s (%ld)}");
      break;
    }
    break;
  case TYPE_DROWN:
    ...
    case 1:
      WRITE_DEATH_MESSAGE(
          "%s sleeps with the fishes. Involuntarily. {%s (%ld)}");
      break;
    case 2:
      WRITE_DEATH_MESSAGE(
          "The water had a fight with %s's lungs. The water won. {%s (%ld)}");
      break;
    case 3:
      WRITE_DEATH_MESSAGE(
          "Water doesn't seem so harmless now, does it %s? {%s (%ld)}");
      break;
    case 4:
  ...
  case TYPE_BIOWARE:
    ...
    case 0:
      sprintf(buf2,
              "%s just hasn't been taking %s medication.  Oops. "
              "{%s (%ld)}",
              GET_CHAR_NAME(vict),
    ...
    case 3:
      WRITE_DEATH_MESSAGE(
          "Still think the bioware was worth it, %s? {%s (%ld)}");
      break;
    case 4:
      WRITE_DEATH_MESSAGE(
          "Maybe %s got a defective piece of bioware... {%s (%ld)}");
      break;
    }
    break;
  case TYPE_RECOIL:
    ...
    case 0:
      WRITE_DEATH_MESSAGE(
          "You're meant to hit *other* people with that whip, %s. {%s (%ld)}");
      break;
    case 1:
      sprintf(buf2, "%s lopped off %s own head.  Oops. {%s (%ld)}",
              GET_CHAR_NAME(vict), HSHR(vict), vict->in_room->name,
              vict->in_room->number);
      break;
    case 2:
      WRITE_DEATH_MESSAGE(
          "%s, watch out for your whi....nevermind. {%s (%ld)}");
      break;
    case 3:
      WRITE_DEATH_MESSAGE(
          "THWAP!  Wait.....was that *your* whip, %s?!? {%s (%ld)}");
      break;
    case 4:
      sprintf(buf2, "%s's whip didn't agree with %s. {%s (%ld)}",
              GET_CHAR_NAME(vict), HMHR(vict), vict->in_room->name,
              vict->in_room->number);
      break;
    }
    break;
  case TYPE_RAM:
    WRITE_DEATH_MESSAGE_WITH_AGGRESSOR(
        "%s was made into a hood ornament by %s. {%s (%ld)}");
    break;
  case TYPE_DUMPSHOCK:
    WRITE_DEATH_MESSAGE(
        "%s couldn't quite manage a graceful logoff. {%s (%ld)}");
    break;
  case TYPE_BLACKIC:
  ...
  case TYPE_POLTERGEIST:
    WRITE_DEATH_MESSAGE(
        "%s is a pansy who got killed by the poltergeist spell. {%s (%ld)}");
    break;
  case TYPE_ELEVATOR:
    WRITE_DEATH_MESSAGE(
        "%s got crushed by a moving elevator. Ouch. {%s (%ld)}");
    break;
  case TYPE_CRASH:
    switch (number(0, 1)) {
    case 0:
  ...
  default:
    if (ch == vict)
      sprintf(buf2, "%s died (cause uncertain-- damage type %d). {%s (%ld)}",
              GET_CHAR_NAME(vict), attacktype, vict->in_room->name,
              vict->in_room->number);
    else
      switch (number(0, 5)) {
      case 0:
      ...
      case 1:
        WRITE_DEATH_MESSAGE_WITH_AGGRESSOR(
            "%s wiped out of existence by %s. {%s (%ld)}");
        break;
      case 2:
        WRITE_DEATH_MESSAGE_WITH_AGGRESSOR(
            "%s's life terminated by %s. {%s (%ld)}");
        break;
      case 3:
      ...
      case 4:
        WRITE_DEATH_MESSAGE_WITH_AGGRESSOR(
            "%s transformed into a corpse by %s. {%s (%ld)}");
        break;
      case 5:
...
#define IS_RANGED(eq)                                                          \
  (GET_OBJ_TYPE(eq) == ITEM_FIREWEAPON ||                                      \
   (GET_OBJ_TYPE(eq) == ITEM_WEAPON && (IS_GUN(GET_OBJ_VAL(eq, 3)))))

#define RANGE_OK(ch)                                                           \
  ((GET_EQ(ch, WEAR_WIELD) &&                                                  \
    GET_OBJ_TYPE(GET_EQ(ch, WEAR_WIELD)) == ITEM_WEAPON &&                     \
    IS_RANGED(GET_EQ(ch, WEAR_WIELD))) ||                                      \
   (GET_EQ(ch, WEAR_HOLD) &&                                                   \
    GET_OBJ_TYPE(GET_EQ(ch, WEAR_HOLD)) == ITEM_WEAPON &&                      \
    IS_RANGED(GET_EQ(ch, WEAR_HOLD))))

#define IS_SINGLE(eq)                                                          \
  (GET_OBJ_TYPE(eq) == ITEM_WEAPON && GET_OBJ_VAL(eq, 11) == MODE_SS)

#define IS_SEMI(eq)                                                            \
  (GET_OBJ_TYPE(eq) == ITEM_WEAPON && GET_OBJ_VAL(eq, 11) == MODE_SA)

#define IS_BURST(eq)                                                           \
  (GET_OBJ_TYPE(eq) == ITEM_WEAPON && GET_OBJ_VAL(eq, 11) == MODE_BF)

#define IS_AUTO(eq)                                                            \
  (GET_OBJ_TYPE(eq) == ITEM_WEAPON && GET_OBJ_VAL(eq, 11) == MODE_FA)
...
// Basically ripped the logic from damage(). Used to adjust combat messages for
// edge cases.
bool can_hurt(struct char_data *ch, struct char_data *victim, int attacktype) {
  if (IS_NPC(victim)) {
    ...
    if (attacktype == TYPE_EXPLOSION &&
        (IS_ASTRAL(victim) || MOB_FLAGGED(victim, MOB_IMMEXPLODE)))
      return false;

    // Whatever the hell protection this is.
    if (((!IS_NPC(ch) && IS_SENATOR(ch) && !access_level(ch, LVL_ADMIN)) ||
         (IS_NPC(ch) && ch->master && AFF_FLAGGED(ch, AFF_CHARM) &&
          !IS_NPC(ch->master) && IS_SENATOR(ch->master) &&
          !access_level(ch->master, LVL_ADMIN))) &&
        !vnum_from_non_connected_zone(GET_MOB_VNUM(victim))) {
      return false;
    }
  } else {
    // Known ignored edge case: if the player is not a killer but would become a
    // killer because of this action.
...
bool damage(struct char_data *ch, struct char_data *victim, int dam,
            int attacktype, bool is_physical) {
  ...
  if (!ch) {
    mudlog("SYSERR: NULL ch passed to damage()! Aborting call, nobody takes "
           "damage this time.",
           victim, LOG_SYSLOG, TRUE);
    return FALSE;
  }
  if (!victim) {
    mudlog("SYSERR: NULL victim passed to damage()! Aborting call, nobody "
           "takes damage this time.",
           ch, LOG_SYSLOG, TRUE);
    return FALSE;
  }
  ...
  sprintf(rbuf, "Damage (%s vs %s, %s %d): ", GET_CHAR_NAME(ch),
          GET_CHAR_NAME(victim), wound_name[MIN(DEADLY, MAX(0, dam))],
          attacktype);
  if (((!IS_NPC(ch) && IS_SENATOR(ch) && !access_level(ch, LVL_ADMIN)) ||
       (IS_NPC(ch) && ch->master && AFF_FLAGGED(ch, AFF_CHARM) &&
        !IS_NPC(ch->master) && IS_SENATOR(ch->master) &&
  ...
  if (victim != ch) {
    ...
    if (GET_POS(victim) > POS_STUNNED && !FIGHTING(victim)) {
      ...
      if (MOB_FLAGGED(victim, MOB_MEMORY) && !IS_NPC(ch) && (!IS_SENATOR(ch)))
        remember(victim, ch);
    }
  }
  ...
  if (ch != victim) {
    if (attacktype == TYPE_SCATTERING && !IS_NPC(ch) && !IS_NPC(victim)) {
      // Unless both chars are PK, or victim is KILLER, deal no damage and abort
      // without flagging anyone as KILLER.
      if (!(PLR_FLAGGED(victim, PLR_KILLER) ||
            (PRF_FLAGGED(ch, PRF_PKER) && PRF_FLAGGED(victim, PRF_PKER)))) {
        dam = -1;
  ...
  if (ch != victim && !IS_NPC(victim) && !(victim->desc)) {
    if (!CH_IN_COMBAT(victim)) {
      ...
      GET_PHYSICAL(victim) =
          MAX(100, GET_PHYSICAL(victim) - (int)(GET_MAX_PHYSICAL(victim) / 2));
      GET_MENTAL(victim) =
          MAX(100, GET_MENTAL(victim) - (int)(GET_MAX_MENTAL(victim) / 2));
  ...
  if (attacktype != TYPE_BIOWARE) {
    for (bio = victim->bioware; bio; bio = bio->next_content) {
      ...
      else if (GET_OBJ_VAL(bio, 0) == BIO_TRAUMADAMPNER &&
  ...
  if (dam > 0) {
    if (is_physical) {
      ...
      if (!pain && trauma &&
          (!comp || (comp && GET_MENTAL(victim) > 0 &&
                     1000 - GET_PHYSICAL(victim) > comp))) {
  ...
  if (GET_SUSTAINED_NUM(victim)) {
    ...
    if (GET_POS(victim) < POS_LYING) {
      for (struct sustain_data *sust = GET_SUSTAINED(victim); sust;
           sust = next) {
        next = sust->next;
      ...
      for (struct sustain_data *sust = GET_SUSTAINED(victim); sust;
        ...
        next = sust->next;
        if (sust->caster && !sust->focus && !sust->spirit)
          if (success_test(GET_SKILL(victim, SKILL_SORCERY),
                           sust->force + damage_modifier(victim, buf)) < 1)
            end_sustained_spell(victim, sust);
      }
    }
  }
  ...
  if (attacktype > 0 && ch != victim && attacktype < TYPE_HAND_GRENADE) {
    ...
    if (dam > 5) {
      send_to_char(victim,
                   "^RYou slam into the %s at speed, the impact reshaping your "
                   "body in ways you do not appreciate.^n\r\n",
                   ROOM_FLAGGED(victim->in_room, ROOM_INDOORS) ? "floor"
                                                               : "ground");
      sprintf(buf3,
              "^R$n slams into the %s at speed, the impact reshaping $s body "
              "in horrific ways.^n\r\n",
  ...
  if ((ch->in_room != victim->in_room) && total_miss &&
      attacktype < TYPE_SUFFERING && attacktype >= TYPE_PISTOL)
    weapon_scatter(ch, victim, GET_EQ(ch, WEAR_WIELD));

  if (victim->bioware && GET_POS(victim) > POS_STUNNED && dam > 0 &&
      ch != victim)
  ...
  case POS_DEAD:
    if (IS_NPC(victim))
      ...
      act("$n slumps in a pile. You hear sirens as a DocWagon rushes in and "
          "grabs $m.",
          FALSE, victim, 0, 0, TO_ROOM);
    send_to_char("You feel the world slip in to darkness, you better hope a "
                 "wandering DocWagon finds you.\r\n",
                 victim);
    break;
  default: /* >= POSITION SLEEPING */
    ...
    if (GET_PHYSICAL(victim) < (GET_MAX_PHYSICAL(victim) >> 2))
      send_to_char(victim,
                   "%sYou wish that your wounds would stop "
                   "^RBLEEDING^r so much!%s\r\n",
                   CCRED(victim, C_SPR), CCNRM(victim, C_SPR));

    if (MOB_FLAGGED(victim, MOB_WIMPY) && !AFF_FLAGGED(victim, AFF_PRONE) &&
        ch != victim && GET_PHYSICAL(victim) < (GET_MAX_PHYSICAL(victim) >> 2))
      do_flee(victim, "", 0, 0);
    ...
    if ((IS_NPC(victim) || victim->desc) && ch != victim &&
      ...
      if (PLR_FLAGGED(ch, PLR_AUTH)) {
        if (IS_AFFECTED(ch, AFF_GROUP)) {
          ...
          if (IS_NPC(victim)) {
            extern struct char_data *mob_proto;
            mob_proto[GET_MOB_RNUM(victim)].mob_specials.value_death_karma +=
                exp;
          }
...
bool process_has_ammo(struct char_data *ch, struct obj_data *wielded,
                      bool deduct_one_round) {
  int i;
  bool found = FALSE;
  struct obj_data *obj, *cont;

  if (!ch || !wielded) {
    mudlog("SYSERR: process_has_ammo received null value. Returning TRUE.", ch,
           LOG_SYSLOG, TRUE);
    return TRUE;
  }
      ...
      if (GET_EQ(ch, i) && GET_OBJ_TYPE(GET_EQ(ch, i)) == ITEM_QUIVER)
        for (obj = GET_EQ(ch, i)->contains; obj; obj = obj->next_content)
          if (GET_OBJ_TYPE(obj) == ITEM_MISSILE &&
              GET_OBJ_VAL(obj, 0) == GET_OBJ_VAL(wielded, 5)) {
            if (deduct_one_round) {
  ...
  if (GET_OBJ_TYPE(wielded) == ITEM_WEAPON &&
      IS_GUN(GET_WEAPON_ATTACK_TYPE(wielded))) {
    // First, check if they're manning a turret-- if they are, special handling
    // is required.
    if (AFF_FLAGGED(ch, AFF_MANNING)) {
      ...
      // It's a player. Look through their gun and locate any ammo boxes in
      // there.
      for (struct obj_data *obj = wielded->in_obj->contains; obj;
  ...
  sprintf(buf,
          "Got to end of process_has_ammo (%s, %s (%ld)) with no result. "
          "Returning true.",
...
int check_smartlink(struct char_data *ch, struct obj_data *weapon) {
  ...
  for (int i = 7; !mod && i < 10; i++) {
    if (GET_OBJ_VAL(weapon, i) > 0 && real_object(GET_OBJ_VAL(weapon, i)) > 0 &&
      ...
      for (obj = ch->cyberware; !mod && obj; obj = obj->next_content)
        if (GET_OBJ_VAL(obj, 0) == CYB_SMARTLINK &&
            (GET_OBJ_VAL(obj, 1) == 1 || GET_OBJ_VAL(access, 2) < 2)) {
          // Smartlink plus cyber found-- full value.
          return 2;
        }
      if (!mod && GET_EQ(ch, WEAR_EYES) &&
...
void astral_fight(struct char_data *ch, struct char_data *vict) {
  ...
  if ((IS_PROJECT(ch) || PLR_FLAGGED(ch, PLR_PERCEIVE)) && wielded &&
      GET_OBJ_TYPE(wielded) == ITEM_WEAPON &&
      GET_OBJ_VAL(wielded, 3) < TYPE_TASER && GET_OBJ_VAL(wielded, 7) &&
      GET_OBJ_VAL(wielded, 8) && GET_OBJ_VAL(wielded, 9) == GET_IDNUM(ch))
    focus = TRUE;
  else if (wielded) {
    stop_fighting(ch);
    return;
  ...
  if (IS_PROJECT(ch) && (ch != vict) && PLR_FLAGGED(vict, PLR_PERCEIVE) &&
    ...
    sprintf(
        buf,
        "PC Killer bit set on %s (astral) for initiating attack on %s at %s.",
  ...
  if (((w_type == TYPE_HIT) || (w_type == TYPE_BLUDGEON) ||
       (w_type == TYPE_PUNCH) || (w_type == TYPE_TASER) ||
       (w_type == TYPE_CRUSH) || (w_type == TYPE_POUND)) &&
      (GET_MENTAL(vict) >= 100))
    is_physical = FALSE;
  else
    is_physical = TRUE;
...
void combat_message_process_ranged_response(struct char_data *ch, rnum_t rnum) {
  for (struct char_data *tch = world[rnum].people; tch;
    ...
    if (IS_NPC(tch)) {
      ...
      if (!IS_NPC(ch) && CH_IN_COMBAT(ch) &&
          ...
          !CH_IN_COMBAT(tch) &&
          !(FIGHTING(ch) ? (IS_NPC(FIGHTING(ch)) &&
        ...
        if (number(0, 6) >= 2) {
          ...
          if (ranged_response(ch, tch) && tch->in_room == was_in) {
            act("$n aims $s weapon at a distant threat!", FALSE, tch, 0, ch,
                TO_ROOM);
          }
        }
      }
...
void combat_message(struct char_data *ch, struct char_data *victim,
                    struct obj_data *weapon, int damage, int burst) {
  char buf[MAX_MESSAGE_LENGTH], buf1[MAX_MESSAGE_LENGTH],
      buf2[MAX_MESSAGE_LENGTH], buf3[MAX_MESSAGE_LENGTH],
      been_heard[MAX_STRING_LENGTH], temp[20];
  struct obj_data *obj = NULL;
  rnum_t room1 = 0, room2 = 0, rnum = 0;

  if (burst <= 1) {
    if (GET_OBJ_VAL(weapon, 4) == SKILL_SHOTGUNS ||
        GET_OBJ_VAL(weapon, 4) == SKILL_ASSAULT_CANNON)
  ...
  if (ch->in_room == victim->in_room ||
      (ch->in_veh && ch->in_veh->in_room == victim->in_room)) {
    // Same-room messaging.
    static char vehicle_message[50];

    if (ch->in_veh)
      sprintf(vehicle_message, "From inside %s, ",
    ...
    if (damage < 0) {
      switch (number(1, 3)) {
      case 1:
        sprintf(buf1, "^r%s$n fires a %s^r at you but you manage to dodge.^n",
                vehicle_message, buf);
        sprintf(buf2, "^yYou fire a %s^y at $N but $E manages to dodge.^n",
                buf);
        sprintf(buf3, "%s$n fires a %s^n at $N but $E manages to dodge.",
                vehicle_message, buf);
        break;
      case 2:
        sprintf(buf1, "^r%s$n fires a %s^r at you but you easily dodge.^n",
                vehicle_message, buf);
        sprintf(buf2, "^yYou fire a %s^y at $N but $E easily dodges.^n", buf);
        sprintf(buf3, "%s$n fires a %s^n at $N but $E easily dodges.",
                vehicle_message, buf);
        break;
      case 3:
        sprintf(buf1,
                "^r%s$n fires a %s^r at you but you move out of the way.^n",
                vehicle_message, buf);
        sprintf(buf2, "^yYou fire a %s^y at $N but $E moves out of the way.^n",
                buf);
        sprintf(buf3, "%s$n fires a %s^n at $N but $E moves out of the way.",
                vehicle_message, buf);
        break;
      }
    } else if (damage == 0) {
      switch (number(1, 2)) {
      case 1:
        sprintf(buf1, "^r%s$n fires a %s^r at you but your armour holds.^n",
                vehicle_message, buf);
        sprintf(buf2,
                "^yYou fire a %s^y at $N but it doesn't seem to hurt $M.^n",
                buf);
        sprintf(buf3, "%s$n fires a %s^n at $N but it doesn't seem to hurt $M.",
                vehicle_message, buf);
        break;
      case 2:
        sprintf(buf1,
                "^r%s$n fires a %s^r at you but you roll with the impact.^n",
                vehicle_message, buf);
        sprintf(buf2, "^yYou fire a %s^y at $N but $E rolls with the impact.^n",
                buf);
        sprintf(buf3, "%s$n fires a %s^n at $N but $E rolls with the impact.",
                vehicle_message, buf);
        break;
      }
      ...
      sprintf(buf1, "^r%s$n massacres you with a %s^r.^n", vehicle_message,
              buf);
      ...
      switch (number(1, 2)) {
      case 1:
        sprintf(buf1, "^r%s$n puts you down with a deadly %s^r.^n",
                vehicle_message, buf);
        sprintf(buf2, "^yYou put $N down with a deadly %s^y.^n", buf);
        sprintf(buf3, "%s$n puts $N down with a deadly %s^n.", vehicle_message,
                buf);
        break;
      case 2:
        sprintf(buf1, "^r%s$n sublimates you with a deadly %s^r.^n",
                vehicle_message, buf);
        sprintf(buf2, "^yYou sublimate $N with a deadly %s^y.^n", buf);
        sprintf(buf3, "%s$n sublimates $N with a deadly %s^n.", vehicle_message,
                buf);
        break;
      }
    }
    ...
    if (damage < 0) {
      switch (number(1, 3)) {
      case 1:
        sprintf(buf1, "^r$n fires a %s^r at you but you manage to dodge.^n",
                buf);
        sprintf(buf2, "^yYou fire a %s^y at $N but $E manages to dodge.^n",
                buf);
      ...
      case 2:
        ...
        break;
      case 3:
        sprintf(buf1, "^r$n fires a %s^r at you but you move out of the way.^n",
                buf);
        sprintf(buf2, "^yYou fire a %s^y at $N but $E moves out of the way.^n",
                buf);
        sprintf(buf3, "$n fires a %s^n at $N but $E moves out of the way.",
                buf);
        break;
      }
    } else if (damage == 0) {
      ...
      case 1:
        ...
        sprintf(buf2,
                "^yYou fire a %s^y at $N but it doesn't seem to hurt $M.^n",
                buf);
        sprintf(buf3, "$n fires a %s^n at $N but it doesn't seem to hurt $M.",
                buf);
        break;
      case 2:
        sprintf(buf1,
                "^r$n fires a %s^r at you but you roll with the impact.^n",
                buf);
        sprintf(buf2, "^yYou fire a %s^y at $N but $E rolls with the impact.^n",
                buf);
        sprintf(buf3, "$n fires a %s^n at $N but $E rolls with the impact.",
                buf);
        break;
      }
    } else if (damage == LIGHT) {
  ...
  // If the player has a silencer or suppressor, restrict the propagation of the
  // gunshot.
  ...
  for (int i = 7; i < 10; i++) {
    if (GET_OBJ_VAL(weapon, i) > 0 &&
        ...
        (obj = &obj_proto[rnum]) && GET_OBJ_TYPE(obj) == ITEM_GUN_ACCESSORY)
      if (GET_OBJ_VAL(obj, 1) == ACCESS_SILENCER ||
          GET_OBJ_VAL(obj, 1) == ACCESS_SOUNDSUPP) {
        has_suppressor = TRUE;
        break;
      }
  }

  sprintf(been_heard, ".%ld.", real_room(ch->in_room->number));

  // Initialize gunshot queue.
  std::queue<rnum_t> room_queue;
  std::queue<rnum_t> secondary_room_queue;

  // Scan the shooter's room's exits and add notifications to any valid adjacent
  // rooms. 'Valid' is defined as 'exists and is not flagged as silent'.
  for (int door1 = 0; door1 < NUM_OF_DIRS; door1++) {
    if (ch->in_room->dir_option[door1] &&
        (room1 = real_room(ch->in_room->dir_option[door1]->to_room->number)) !=
            NOWHERE &&
        !(world[room1].silence[0])) {
      ...
      if (has_suppressor) {
        // Special case: If they're using a suppressed weapon, print a
        // muffled-gunfire message and terminate.
        send_to_room("You hear muffled gunshots nearby.\r\n", &world[room1]);
        combat_message_process_ranged_response(ch, room1);
        strcat(been_heard, temp);
      } else {
        // Send gunshot notifications to the selected room. Process guard/helper
        // responses.
        ...
        for (int door2 = 0; door2 < NUM_OF_DIRS; door2++)
          if (world[room1].dir_option[door2] &&
              (room2 = real_room(
                   world[room1].dir_option[door2]->to_room->number)) !=
                  NOWHERE &&
              !(world[room2].silence[0]))
  ...
  // Scan the list of near-adjacent rooms and send messages to all non-heard
  // ones. Add their exits to the secondary queue.
  while (!room_queue.empty()) {
    ...
    for (int door = 0; door < NUM_OF_DIRS; door++)
      if (world[room1].dir_option[door] &&
          (room2 = real_room(world[room1].dir_option[door]->to_room->number)) !=
              NOWHERE &&
          !(world[room2].silence[0]))
...
bool is_char_too_tall(struct char_data *ch) {
  ...
  return ROOM_FLAGGED(ch->in_room, ROOM_INDOORS) &&
         GET_HEIGHT(ch) >= ch->in_room->z * 100;
}

int calculate_vision_penalty(struct char_data *ch, struct char_data *victim) {
  // TODO: Why would someone handling a vehicle or manning a turret negate
  // visibility penalties?
  // !(ch->in_veh || PLR_FLAGGED(ch, PLR_REMOTE) || AFF_FLAGGED(ch,
  // AFF_MANNING))
  int modifier = 0;
  if (!CAN_SEE(ch, victim)) {
    modifier = 8;
    if (AFF_FLAGGED(ch, AFF_DETECT_INVIS) ||
        GET_POWER(ch, ADEPT_BLIND_FIGHTING))
      modifier = 4;
  }
  return modifier;
}

// todo: single shot weaps can only be fired once per combat phase-- what does
// this mean for us?

struct combat_data *populate_cyberware(struct char_data *ch,
                                       struct combat_data *cd) {
...
bool weapon_has_bayonet(struct obj_data *weapon) {
  // Precondition: Weapon must exist, must be a weapon-class item, and must be a
  // gun.
  ...
  return (GET_WEAPON_ATTACH_UNDER_VNUM(weapon) &&
          (attach_obj = real_object(GET_WEAPON_ATTACH_UNDER_VNUM(weapon))) >
              0 &&
...
int get_melee_skill(struct combat_data *cd) {
  ...
  if (cd->weapon && GET_OBJ_TYPE(cd->weapon) == ITEM_WEAPON) {
    // If you're wielding a gun, you get to beat your opponent with it (or stab
    // them, if it has a bayonet).
    if (cd->weapon_is_gun) {
...
void hit(struct char_data *attacker, struct char_data *victim,
         struct obj_data *weap, struct obj_data *vict_weap) {
  // Load our Dominator weapon spec, because that's totally canon and kosher
  // right?
  ...
  // Set the base TNs to accommodate get_skill() changing the TNs like the
  // special snowflake it is.
  att->tn = 4;
  def->tn = 4;
  ...
  if (att->weapon && (GET_OBJ_TYPE(att->weapon) != ITEM_WEAPON &&
                      GET_OBJ_TYPE(att->weapon) != ITEM_FIREWEAPON)) {
    send_to_char(att->ch,
                 "You struggle to figure out how to attack while using %s as a "
                 "weapon!\r\n",
                 decapitalize_a_an(GET_OBJ_NAME(att->weapon)));
    return;
  }

  // Precondition: If you're asleep, paralyzed, or out of ammo, you don't get to
  // fight. Note the use of the deducting has_ammo here.
  if (!AWAKE(att->ch) || GET_QUI(att->ch) <= 0 ||
      !has_ammo(att->ch, att->weapon))
    return;

  // Short-circuit: If you're wielding an activated Dominator, you don't care
  // about all these pesky rules.
  if (att->weapon && GET_OBJ_SPEC(att->weapon) == weapon_dominator) {
    if (GET_LEVEL(def->ch) > GET_LEVEL(att->ch)) {
      send_to_char(
          att->ch,
          "As you aim your weapon at %s, a dispassionate feminine voice "
          "states: \"^cThe target's Crime Coefficient is below 100. %s is not "
          "a target for enforcement. The trigger has been locked.^n\"\r\n",
    ...
    case WEAP_TASER:
      ...
      act("Your crackling shot of energy slams into $N, disabling $M.", FALSE,
          att->ch, 0, def->ch, TO_CHAR);
      act("A crackling shot of energy erupts from $n's Dominator and slams "
          "into $N, disabling $M!",
          FALSE, att->ch, 0, def->ch, TO_NOTVICT);
      act("A crackling shot of energy erupts from $n's Dominator and slams "
          "into you! Your vision fades as your muscles lock up.",
          FALSE, att->ch, 0, def->ch, TO_VICT);
      damage(att->ch, def->ch, 15, TYPE_SUFFERING, FALSE);
      break;
    case WEAP_HEAVY_PISTOL:
      // Lethal? Kill your target.
      act("A ball of coherent light leaps from your Dominator, tearing into "
          "$N. With a scream, $E crumples, bubbles, and explodes in a shower "
          "of gore!",
          FALSE, att->ch, 0, def->ch, TO_CHAR);
      act("A ball of coherent light leaps from $n's Dominator, tearing into "
          "$N. With a scream, $E crumples, bubbles, and explodes in a shower "
          "of gore!",
          FALSE, att->ch, 0, def->ch, TO_NOTVICT);
      act("A ball of coherent light leaps from $n's Dominator, tearing into "
          "you! A horrible rending sensation tears through you as your vision "
          "fades.",
          FALSE, att->ch, 0, def->ch, TO_VICT);
      die(def->ch);
      break;
    case WEAP_CANNON:
      // Decomposer? Don't just kill your target-- if they're a player,
      // disconnect them.
      act("A roaring column of force explodes from your Dominator, erasing $N "
          "from existence!",
          FALSE, att->ch, 0, def->ch, TO_CHAR);
      act("A roaring column of force explodes from $n's Dominator, erasing $N "
          "from existence!",
          FALSE, att->ch, 0, def->ch, TO_NOTVICT);
      act("A roaring column of force explodes from $n's Dominator, erasing you "
          "from existence!",
          FALSE, att->ch, 0, def->ch, TO_VICT);
      die(def->ch);
      if (def->ch->desc) {
  ...
  // Precondition: If you're in melee combat and your foe isn't present, stop
  // fighting.
  if ((melee = !(att->weapon_is_gun))) {
    if (att->ch->in_room != def->ch->in_room) {
      send_to_char(att->ch, "You relax with the knowledge that your opponent "
  ...
  att->modifiers[COMBAT_MOD_VISIBILITY] +=
      calculate_vision_penalty(att->ch, def->ch);
  def->modifiers[COMBAT_MOD_VISIBILITY] +=
      calculate_vision_penalty(def->ch, att->ch);

  // Early execution: Nerve strike doesn't require as much setup, so perform it
  // here to save on resources.
  if (melee && IS_NERVE(att->ch) && !(att->weapon) &&
      !(IS_SPIRIT(def->ch) || IS_ELEMENTAL(def->ch))) {
    ...
    rbuf[7] = 0;
    sprintf(rbuf + strlen(rbuf), ">Nerve Targ: Base 4 + impact %d ",
            GET_IMPACT(def->ch));

    att->tn += GET_IMPACT(def->ch) +
               modify_target_rbuf_raw(att->ch, rbuf,
                                      att->modifiers[COMBAT_MOD_VISIBILITY]);
    ...
    if (!att->too_tall)
      att->dice +=
          MIN(GET_SKILL(att->ch, SKILL_UNARMED_COMBAT), GET_OFFENSE(att->ch));
    ...
    if (att->successes > 0) {
      GET_TEMP_QUI_LOSS(def->ch) +=
          (int)(att->successes / 2); // This used to be * 2!
      affect_total(def->ch);
      if (GET_QUI(def->ch) <= 0) {
        act("You hit $N's pressure points successfully, $e is paralyzed!",
            FALSE, att->ch, 0, def->ch, TO_CHAR);
        act("As $n hits you, you feel your body freeze up!", FALSE, att->ch, 0,
            def->ch, TO_VICT);
        act("$N freezes as $n's attack lands successfully!", FALSE, att->ch, 0,
            def->ch, TO_NOTVICT);
      } else {
        act("You hit $N's pressure points successfully, $e seems to slow down!",
            FALSE, att->ch, 0, def->ch, TO_CHAR);
        act("$n's blows seem to bring great pain and you find yourself moving "
            "slower!",
            FALSE, att->ch, 0, def->ch, TO_VICT);
        act("$n's attack hits $N, who seems to move slower afterwards.", FALSE,
            att->ch, 0, def->ch, TO_NOTVICT);
      }
    } else {
      act("You fail to hit any of the needed pressure points on $N.", FALSE,
          att->ch, 0, def->ch, TO_CHAR);
      act("$n fails to land any blows on you.", FALSE, att->ch, 0, def->ch,
          TO_VICT);
      act("$n's unarmed attack misses $N completely.", TRUE, att->ch, 0,
          def->ch, TO_NOTVICT);
    }
    return;
  }
  ...
  // Setup: If the character is rigging a vehicle or is in a vehicle, set veh to
  // that vehicle.
  ...
  if (!melee) {
    ...
    if (!(PLR_FLAGGED(att->ch, PLR_REMOTE) || !AFF_FLAGGED(att->ch, AFF_RIG) ||
          !AFF_FLAGGED(att->ch, AFF_MANNING))) {
      for (int q = 0; q < NUM_WEARS; q++) {
        if (GET_EQ(att->ch, q) &&
            GET_OBJ_TYPE(GET_EQ(att->ch, q)) == ITEM_GYRO) {
          att->gyro = GET_EQ(att->ch, q);
          break;
        }
      }

      // Precondition: If you're using a heavy weapon, you must be strong enough
      // to wield it, or else be using a gyro. CC p99
    ...
    // ans: because it must subtract another round elsewhere

    // Setup: Limit the burst of the weapon to the available ammo, and decrement
    // ammo appropriately.
    if (att->burst_count) {
      if (att->magazine) {
        // When we called has_ammo() earlier, we decremented their ammo by one.
        // Give it back to true up the equation.
        GET_MAGAZINE_AMMO_COUNT(att->magazine)++;

        // Cap their burst to their magazine's ammo.
        att->burst_count =
            MIN(att->burst_count, GET_MAGAZINE_AMMO_COUNT(att->magazine));
        GET_MAGAZINE_AMMO_COUNT(att->magazine) -= att->burst_count;
      }

      // Setup: Compute recoil.
      att->recoil_comp = check_recoil(att->ch, att->weapon);
      att->modifiers[COMBAT_MOD_RECOIL] +=
          MAX(0, att->burst_count - att->recoil_comp);
    ...
    if (att->veh) {
      if (AFF_FLAGGED(att->ch, AFF_MANNING) || AFF_FLAGGED(att->ch, AFF_RIG)) {
        att->modifiers[COMBAT_MOD_RECOIL] +=
            MAX(0, att->modifiers[COMBAT_MOD_RECOIL] - 2);
      } else {
    ...
    // Setup: If you're dual-wielding, take that penalty, otherwise you get your
    // smartlink bonus.
    if (GET_EQ(att->ch, WEAR_WIELD) && GET_EQ(att->ch, WEAR_HOLD))
      att->modifiers[COMBAT_MOD_DUAL_WIELDING] += 2;
    else
      att->modifiers[COMBAT_MOD_SMARTLINK] -=
          check_smartlink(att->ch, att->weapon);

    // Setup: Trying to fire a sniper rifle at close range is tricky. This is
    // non-canon to reduce twinkery.
    ...
    if (att->ch->in_room != def->ch->in_room && !att->veh) {
      ...
      if (att->weapon && IS_RANGED(att->weapon)) {
        weapon_range =
            MIN(find_sight(att->ch), find_weapon_range(att->ch, att->weapon));
        for (int dir = 0; dir < NUM_OF_DIRS && !vict_found; dir++) {
          ...
          if (CAN_GO2(room, dir))
            ...
            nextroom = NULL;
          for (int distance = 1;
               nextroom && (distance <= weapon_range) && !vict_found;
               distance++) {
            for (vict = nextroom->people; vict; vict = vict->next_in_room) {
              if (vict == def->ch) {
      ...
      if (!vict_found) {
        send_to_char(att->ch, "You squint around, but you can't find your "
                              "opponent anywhere.\r\n");
        stop_fighting(att->ch);
        return;
      }
    ...
    // Setup: If your attacker is closing the distance (running), take a penalty
    // per Core p112.
    if (AFF_FLAGGED(def->ch, AFF_APPROACH))
      att->modifiers[COMBAT_MOD_MOVEMENT] += 2;

    // Setup: If you have a gyro mount, it negates recoil and movement penalties
    // up to its rating.
    if (att->gyro &&
        !(AFF_FLAGGED(att->ch, AFF_MANNING) || AFF_FLAGGED(att->ch, AFF_RIG) ||
          AFF_FLAGGED(att->ch, AFF_PILOT)))
      att->modifiers[COMBAT_MOD_GYRO] -=
          MIN(att->modifiers[COMBAT_MOD_MOVEMENT] +
                  att->modifiers[COMBAT_MOD_RECOIL],
              GET_OBJ_VAL(att->gyro, 0));
    else if (IS_NPC(att->ch)) {
      // We assume that NPCs have a gyro equipped, and that the builder just
      // didn't want to be handing out free gyros everywhere for balance
      // reasons.
      att->modifiers[COMBAT_MOD_GYRO] -=
          MIN(att->modifiers[COMBAT_MOD_MOVEMENT] +
                  att->modifiers[COMBAT_MOD_RECOIL],
    ...
    sprintf(rbuf + strlen(rbuf), "|%s", GET_CHAR_NAME(def->ch));
    rbuf[7] = 0;
    sprintf(rbuf + strlen(rbuf), ">Targ: (burst/comp %d-%d, Base 4) ",
            att->burst_count, att->recoil_comp);

    att->tn += modify_target_rbuf_raw(att->ch, rbuf,
                                      att->modifiers[COMBAT_MOD_VISIBILITY]);
    for (int mod_index = 0; mod_index < NUM_COMBAT_MODIFIERS; mod_index++) {
      buf_mod(rbuf, combat_modifiers[mod_index], att->modifiers[mod_index]);
      att->tn += att->modifiers[mod_index];
    ...
    if (!att->too_tall)
      att->dice += MIN(GET_SKILL(att->ch, GET_WEAPON_SKILL(att->weapon)),
                       GET_OFFENSE(att->ch));

    att->successes = success_test(att->dice, att->tn);

    // Display post-test information.
    sprintf(rbuf, "Fight: Ski %d, TN %d, Suc %d", att->dice, att->tn,
            att->successes);
    act(rbuf, 1, att->ch, NULL, NULL, TO_ROLLS);

    // Dodge test.
    if (AWAKE(def->ch) && !AFF_FLAGGED(def->ch, AFF_SURPRISE) &&
        !def->too_tall && !AFF_FLAGGED(def->ch, AFF_PRONE)) {
      // Previous code only allowed you to sidestep if you had also allocated at
      // least one normal dodge die. Why?
      def->dice = GET_DEFENSE(def->ch) + GET_POWER(def->ch, ADEPT_SIDESTEP);
      def->tn += damage_modifier(def->ch, buf) + (int)(att->burst_count / 3) +
                 def->footanchors;
      def->successes = MAX(success_test(def->dice, def->tn), 0);
      att->successes -= def->successes;

      sprintf(rbuf, "Dodge: Dice %d, TN %d, Suc %d. NetSuc %d", def->dice,
              def->tn, def->successes, att->successes);
      act(rbuf, 1, att->ch, NULL, NULL, TO_ROLLS);
    } else
  ...
  // Melee combat.
    ...
    if (IS_SPIRIT(def->ch) || IS_ELEMENTAL(def->ch)) {
      ...
      def->dice = get_skill(def->ch, def->skill, def->tn) +
                  (def->too_tall ? 0
                                 : MIN(GET_SKILL(def->ch, def->skill),
                                       GET_OFFENSE(def->ch)));
      att->dice = get_skill(att->ch, att->skill, att->tn) +
                  (att->too_tall ? 0
                                 : MIN(GET_SKILL(att->ch, att->skill),
                                       GET_OFFENSE(att->ch)));
    }
    ...
    // Bugfix: If you're unconscious or mortally wounded, you don't get to
    // counterattack.
    ...
    // Reach is always used offensively. TODO: Add option to use it defensively
    // instead.
    if (net_reach > 0)
    ...
    sprintf(
        ...
        (GET_PHYSICAL(def->ch) <= 0 || GET_MENTAL(def->ch) <= 0) ? " (incap)"
                                                                 : "");
    rbuf[7] = 0;
    def->tn += modify_target_rbuf_raw(att->ch, NULL,
                                      def->modifiers[COMBAT_MOD_VISIBILITY]);
    for (int mod_index = 0; mod_index < NUM_COMBAT_MODIFIERS; mod_index++) {
      buf_mod(rbuf, combat_modifiers[mod_index], att->modifiers[mod_index]);
    ...
    if (def->weapon ? (GET_OBJ_TYPE(def->weapon) != ITEM_WEAPON ||
                       GET_OBJ_TYPE(def->weapon) != ITEM_FIREWEAPON)
                    : FALSE) {
      // Defender's wielding a non-weapon? Whoops, net successes will never be
      // less than 0.
      net_successes = MAX(0, net_successes);
    }

    act("$n clashes with $N in melee combat.", FALSE, att->ch, 0, def->ch,
        TO_ROOM);
    act("You clash with $N in melee combat.", FALSE, att->ch, 0, def->ch,
        TO_CHAR);
    sprintf(rbuf,
            "%s> sk %d targ %d\r\n"
            "%s> sk %d targ %d\r\n Reach %c%d NetSucc %d (att %d, def %d)",
            GET_CHAR_NAME(att->ch), att->dice, att->tn, GET_CHAR_NAME(def->ch),
            def->dice, def->tn, net_reach > 0 ? 'b' : 't',
            net_reach < 0 ? -net_reach : net_reach, net_successes,
            att->successes, def->successes);
    act(rbuf, 1, att->ch, NULL, NULL, TO_ROLLS);

    // If your enemy got more successes than you, guess what? You're the one who
    // gets their face caved in.
    if (net_successes < 0) {
      // This messaging gets a little annoying.
      act("You successfully counter $N's attack!", FALSE, def->ch, 0, att->ch,
          TO_CHAR);
      act("$n deflects your attack and counterstrikes!", FALSE, def->ch, 0,
          att->ch, TO_VICT);
      act("$n deflects $N's attack and counterstrikes!", FALSE, def->ch, 0,
          att->ch, TO_NOTVICT);
  ...
  if (att->weapon) {
    ...
    if (att->weapon_is_gun) {
      ...
      att->damage_level =
          GET_WEAPON_DAMAGE_CODE(att->weapon) + (int)(att->burst_count / 3);
      ...
      // Weapon fired without a magazine (probably by an NPC)-- we assume its
      // ammo type is normal.
      else {
        att->power -= GET_BALLISTIC(def->ch);
      }
      ...
      if (GET_SKILL(att->ch, GET_WEAPON_SKILL(att->weapon)) >= 8 &&
          !SHOTS_TRIGGERED(att->ch))
        SHOTS_FIRED(att->ch)++;
    }
    // Melee weapon.
    else {
  ...
  // Cyber and unarmed combat.
    ...
    if (att->num_cyberweapons) {
      ...
      if (att->handspurs) {
        ...
        sprintf(buf,
                "SYSERR in hit(): num_cyberweapons %d but no weapons found.",
                att->num_cyberweapons);
        log(buf);
      }
    }
    // No cyberweapons active-- check for bone lacing, then proceed with
    // adept/standard slapping.
    else {
      // TODO: Implement option to use bone lacing to cause physical damage at
      // half power.
      ...
      if (GET_POWER(att->ch, ADEPT_PENETRATINGSTRIKE) &&
        ...
        att->power -= MAX(0, GET_IMPACT(def->ch) -
  ...
  // Handle spirits and elementals being little divas with their special combat
  // rules.
  if (IS_SPIRIT(def->ch) || IS_ELEMENTAL(def->ch)) {
  ...
  int bod_success = 0,
      bod = GET_BOD(def->ch) + (def->too_tall ? 0 : GET_BODY(def->ch));
  if (IS_SPIRIT(att->ch) && GET_MAG(def->ch) > 0 &&
      GET_SKILL(def->ch, SKILL_CONJURING))
    bod_success =
        success_test(MAX(bod, GET_SKILL(def->ch, SKILL_CONJURING)), att->power);
  ...
  sprintf(rbuf, "Bod %d+%d, Pow %d, BodSuc %d, ResSuc %d: Dam %s->%s. %d%c.",
          ...
          wound_name[MIN(DEADLY, MAX(0, damage_total))], damage_total,
          att->is_physical ? 'P' : 'M');
  act(rbuf, 1, att->ch, NULL, NULL, TO_ROLLS);
  if (!melee)
    combat_message(att->ch, def->ch, att->weapon, MAX(0, damage_total),
                   att->burst_count);
  ...
  // If you're firing a heavy weapon without a gyro, you need to test against
  // the damage of the recoil.
  if (att->weapon && !IS_NPC(att->ch) && !att->gyro &&
      GET_WEAPON_SKILL(att->weapon) >= SKILL_MACHINE_GUNS &&
      GET_WEAPON_SKILL(att->weapon) <= SKILL_ASSAULT_CANNON &&
      !(PLR_FLAGGED(att->ch, PLR_REMOTE) || AFF_FLAGGED(att->ch, AFF_RIG) ||
        AFF_FLAGGED(att->ch, AFF_MANNING))) {
    int recoil_successes = success_test(GET_BOD(att->ch) + GET_BODY(att->ch),
                                        GET_WEAPON_POWER(att->weapon) / 2 +
                                            modify_target(att->ch));
    int staged_dam = stage(-recoil_successes, LIGHT);
    sprintf(rbuf, "Heavy Recoil: %d successes, L->%s wound.", recoil_successes,
            staged_dam == LIGHT ? "L" : "no");
    act(rbuf, 1, att->ch, NULL, NULL, TO_ROLLS);
...
int find_sight(struct char_data *ch) {
  ...
  if ((!IS_NPC(ch) && access_level(ch, LVL_VICEPRES)) ||
      AFF_FLAGGED(ch, AFF_VISION_MAG_3))
    sight = 4;
  ...
  if (SECT(get_ch_in_room(ch)) != SPIRIT_HEARTH &&
      (IS_NPC(ch) || !access_level(ch, LVL_VICEPRES)))
...
bool ranged_response(struct char_data *ch, struct char_data *vict) {
  ...
  if (!vict || ch->in_room == vict->in_room || GET_POS(vict) <= POS_STUNNED ||
      !vict->in_room || (IS_NPC(vict) && (MOB_FLAGGED(vict, MOB_INANIMATE))) ||
      CH_IN_COMBAT(vict)) {
    return FALSE;
  }
  ...
  if (!AFF_FLAGGED(vict, AFF_PRONE) &&
      (IS_NPC(vict) && MOB_FLAGGED(vict, MOB_WIMPY) &&
    ...
    for (dir = 0; dir < NUM_OF_DIRS && !is_responding; dir++) {
      ...
      for (distance = 1; !is_responding && (nextroom && (distance <= 4));
           distance++) {
        for (temp = nextroom->people; !is_responding && temp;
             temp = temp->next_in_room) {
          if (temp == ch && (distance > range || distance > sight) &&
              IS_NPC(vict) && !MOB_FLAGGED(vict, MOB_SENTINEL)) {
            is_responding = TRUE;
            act("$n charges towards $s distant foe.", TRUE, vict, 0, 0,
                TO_ROOM);
    ...
    for (dir = 0; dir < NUM_OF_DIRS && !is_responding; dir++) {
      if (CAN_GO(vict, dir) && EXIT2(room, dir)->to_room == ch->in_room) {
        ...
        set_fighting(vict, ch);
        act("$n arrives in a rush of fury, immediately attacking $N!", TRUE,
            vict, 0, ch, TO_NOTVICT);
        act("$n arrives in a rush of fury, rushing straight towards you!", TRUE,
            vict, 0, ch, TO_VICT);
      }
    }
  }
...
void explode(struct char_data *ch, struct obj_data *weapon,
  ...
  for (victim = room->people; victim; victim = next_vict) {
    ...
    damage_total = convert_damage(
        stage((number(1, 3) -
               success_test(GET_BOD(victim) + GET_BODY(victim),
                            MAX(2, (power - (int)(GET_IMPACT(victim) / 2))) +
...
void target_explode(struct char_data *ch, struct obj_data *weapon,
  ...
  for (obj = room->contents; obj; obj = next) {
    next = obj->next_content;
    damage_obj(NULL, obj,
               GET_WEAPON_DAMAGE_CODE(weapon) * 2 +
                   (int)(GET_WEAPON_POWER(weapon) / 6),
               DAMOBJ_EXPLODE);
  }
  ...
  for (victim = room->people; victim; victim = next_vict) {
    ...
    for (i = 0; i < (NUM_WEARS - 1); i++)
      if (GET_EQ(victim, i) && number(1, 100) < 100)
        damage_obj(NULL, GET_EQ(victim, i),
                   GET_WEAPON_DAMAGE_CODE(weapon) * 2 +
                       (int)(GET_WEAPON_POWER(weapon) / 6),
                   DAMOBJ_EXPLODE);
    ...
    if (!mode) {
      if (GET_WEAPON_ATTACK_TYPE(weapon) == TYPE_ROCKET)
        damage_total = convert_damage(
            stage((number(3, 6) -
                   success_test(GET_BOD(victim) + GET_BODY(victim),
                                MAX(2, (GET_WEAPON_POWER(weapon) -
                                        (int)(GET_IMPACT(victim) / 2))) +
                                    modify_target(victim))),
                  GET_WEAPON_DAMAGE_CODE(weapon)));
      else
        damage_total = convert_damage(
            stage((number(2, 5) -
                   success_test(GET_BOD(victim) + GET_BODY(victim),
      ...
      damage_total = convert_damage(stage(
          (number(2, 5) - success_test(GET_BOD(victim) + GET_BODY(victim),
                                       MAX(2, (GET_WEAPON_POWER(weapon) - 4 -
                                               (int)(GET_IMPACT(victim) / 2))) +
                                           modify_target(victim))),
          GET_WEAPON_DAMAGE_CODE(weapon) - 1));
    damage(ch, victim, damage_total, TYPE_EXPLOSION, PHYSICAL);
  }
...
void range_combat(struct char_data *ch, char *target, struct obj_data *weapon,
  ...
  if ((ch->char_specials.rigging ? ch->char_specials.rigging->in_room
                                 : ch->in_room)
          ->peaceful) {
  ...
  if (GET_OBJ_TYPE(weapon) == ITEM_WEAPON &&
    ...
    if (!number(0, 2)) {
      ...
      scatter[1] =
          left >= 0 && CAN_GO(ch, left) ? EXIT(ch, left)->to_room : NULL;
      scatter[2] =
          right >= 0 && CAN_GO(ch, right) ? EXIT(ch, right)->to_room : NULL;
      scatter[3] =
          CAN_GO2(nextroom, dir) ? EXIT2(nextroom, dir)->to_room : NULL;
      ...
      for (temp2 = 0; temp2 < 4; temp2++) {
        if (scatter[temp2] && !number(0, temp - 1)) {
          if (temp2 == 0) {
            ...
            sprintf(buf, "Your realize your aim must've been off-target as $p "
                         "lands at your feet.");
          } else if (temp2 == 3)
  ...
  if (vict) {
    if (vict == FIGHTING(ch)) {
      ...
      act("$N seems to be out of $p's range.", FALSE, ch, weapon, vict,
          TO_CHAR);
      return;
    ...
    if (GET_OBJ_TYPE(weapon) == ITEM_FIREWEAPON) {
      act("$n draws $p and fires into the distance!", TRUE, ch, weapon, 0,
          TO_ROOM);
      act("You draw $p, aim it at $N and fire!", FALSE, ch, weapon, vict,
          TO_CHAR);
      check_killer(ch, vict);
      ...
      if (CH_IN_COMBAT(ch))
        stop_fighting(ch);
      hit(ch, vict,
          (GET_EQ(ch, WEAR_WIELD) ? GET_EQ(ch, WEAR_WIELD)
                                  : GET_EQ(ch, WEAR_HOLD)),
          (GET_EQ(vict, WEAR_WIELD) ? GET_EQ(vict, WEAR_WIELD)
                                    : GET_EQ(vict, WEAR_HOLD)));
      WAIT_STATE(ch, 2 * PULSE_VIOLENCE);
      return;
    }

    act("$n aims $p and fires into the distance!", TRUE, ch, weapon, 0,
        TO_ROOM);
    act("You aim $p at $N and fire!", FALSE, ch, weapon, vict, TO_CHAR);
    if (IS_GUN(GET_WEAPON_ATTACK_TYPE(weapon))) {
      check_killer(ch, vict);
      if (GET_WEAPON_ATTACK_TYPE(weapon) < TYPE_PISTOL ||
          has_ammo_no_deduct(ch, weapon)) {
        if (IS_NPC(vict) && !IS_PROJECT(vict) && !CH_IN_COMBAT(vict)) {
          GET_DEFENSE(vict) = GET_COMBAT(vict);
          GET_OFFENSE(vict) = 0;
        }
      ...
      if (!number(0, 49)) {
        ...
        for (temp2 = 0; temp2 < 4; temp2++)
          if (scatter[temp2] && !number(0, temp - 1)) {
            if (temp2 == 0) {
              act("$p's trajectory is slightly off...", FALSE, ch, weapon, 0,
                  TO_ROOM);
              ...
              sprintf(buf,
                      "Your aim is slightly off, the %s veering to the %s.",
                      (GET_OBJ_VAL(weapon, 3) == TYPE_ROCKET ? "rocket"
                                                             : "grenade"),
                      dirs[temp2 == 1 ? left : right]);
            ...
            if (GET_OBJ_VAL(weapon, 3) == TYPE_ROCKET)
              for (temp = 0; temp < NUM_OF_DIRS; temp++)
                if (CAN_GO2(scatter[temp2], temp))
                  target_explode(ch, weapon,
                                 EXIT2(scatter[temp2], temp)->to_room, 1);
            return;
          }
      }
  ...
  for (distance = 1; nextroom && distance <= sight; distance++) {
    if (EXIT2(nextroom, dir) && EXIT2(nextroom, dir)->keyword &&
        ...
        (PRF_FLAGGED(ch, PRF_HOLYLIGHT) ||
         ...
         ((light_level(nextroom) == LIGHT_MINLIGHT ||
           light_level(nextroom) == LIGHT_PARTLIGHT) &&
...
void roll_individual_initiative(struct char_data *ch) {
  ...
  if (AWAKE(ch)) {
    ...
    if (IS_NPC(ch)) {
      if (GET_MOBALERT(ch) == MALERT_CALM && FIGHTING(ch) &&
          success_test(GET_REA(ch), 4) <
              success_test(GET_REA(FIGHTING(ch)), 4)) {
        GET_INIT_ROLL(ch) = 0;
  ...
  char rbuf[MAX_STRING_LENGTH];
  sprintf(rbuf, "Init: %2d %s", GET_INIT_ROLL(ch), GET_NAME(ch));
...
void roll_initiative(void) {
  ...
  for (ch = combat_list; ch; ch = next_combat_list) {
    ...
    if (ch->in_room && ch->in_room->poltergeist[0]) {
      int dam = convert_damage(
          stage(-success_test(GET_QUI(ch),
                              ch->in_room->poltergeist[1] - GET_IMPACT(ch)),
                LIGHT));
      if (dam > 0) {
...
void perform_violence(void) {
  ...
  for (ch = combat_list; ch; ch = next_combat_list) {
    ...
    if (!CH_IN_COMBAT(ch) || !AWAKE(ch))
      ...
      GET_INIT_ROLL(ch) -= 10;
      for (struct spirit_sustained *ssust = SPIRIT_SUST(ch); ssust;
           ssust = ssust->next)
        if (ssust->type == ENGULF) {
          if (ssust->caster) {
            int dam;
            if (IS_SPIRIT(ch) ||
                (IS_ELEMENTAL(ch) && GET_SPARE1(ch) == ELEM_WATER)) {
              dam = convert_damage(stage(
                  -success_test(GET_BOD(ssust->target),
                                GET_SPARE2(ch) + GET_EXTRA(ssust->target)),
                  MODERATE));
              act("$n can contorts in pain as the water engulfs $m!", TRUE,
                  ssust->target, 0, 0, TO_ROOM);
              send_to_char(
                  "The water crushes you and leaves you unable to breath!\r\n",
                  ssust->target);
              if (dam > 0)
              ...
              case ELEM_FIRE:
                dam = convert_damage(stage(
                    -success_test(GET_BOD(ssust->target),
                                  GET_SPARE2(ch) - GET_IMPACT(ssust->target) +
                                      (MOB_FLAGGED(ch, MOB_FLAMEAURA) ? 2 : 0)),
                    MODERATE));
                act("$n can be heard screaming from inside $s prison of "
                    "flames!",
                    TRUE, ssust->target, 0, 0, TO_ROOM);
                send_to_char("The flames continue to burn you, causing "
                             "horrendous pain!\r\n",
                             ssust->target);
                if (dam > 0)
              ...
              case ELEM_EARTH:
                dam = convert_damage(stage(
                    -success_test(GET_BOD(ssust->target),
                                  GET_SPARE2(ch) - GET_IMPACT(ssust->target)),
                    SERIOUS));
                act("$n can be heard screaming from inside $s earthen prison.",
                    TRUE, ssust->target, 0, 0, TO_ROOM);
                send_to_char("The earth around you compresses against you, "
              ...
              case ELEM_AIR:
                dam = convert_damage(
                    stage(-success_test(MAX(GET_WIL(ssust->target),
                                            GET_BOD(ssust->target)),
                                        GET_SPARE2(ch)),
                          SERIOUS));
                act("$n chokes and coughs.", TRUE, ssust->target, 0, 0,
                    TO_ROOM);
                send_to_char("Noxious fumes fill your throat and lungs, "
                             "causing your vision to swim.\r\n",
                             ssust->target);
                if (dam > 0)
                  damage(ch, ssust->target, dam, TYPE_FUMES, MENTAL);
                break;
            ...
            if (success_test(GET_STR(ch), GET_LEVEL(ssust->target)) -
                    success_test(GET_LEVEL(ssust->target), GET_STR(ch)) >
                0)
              stop_spirit_power(ssust->target, ENGULF);
            else {
              act("$n struggles against the spirit engulfing $m!", FALSE, ch, 0,
                  0, TO_ROOM);
              act("You struggle against $n's engulfment!", FALSE, ssust->target,
                  0, ch, TO_VICT);
            }
          }
          engulfed = TRUE;
      ...
      if ((ch->points.fire[0] > 0 && success_test(GET_WIL(ch), 6) < 0) ||
          engulfed)
        continue;
      if (IS_AFFECTED(ch, AFF_BANISH) && FIGHTING(ch) &&
          (IS_ELEMENTAL(FIGHTING(ch)) || IS_SPIRIT(FIGHTING(ch)))) {
        ...
        for (int i = 0; i < NUM_WEARS; i++)
          if (GET_EQ(mage, i) && GET_OBJ_TYPE(GET_EQ(mage, i)) == ITEM_FOCUS &&
              GET_OBJ_VAL(GET_EQ(mage, i), 0) == FOCI_SPIRIT &&
              GET_OBJ_VAL(GET_EQ(mage, i), 2) == GET_IDNUM(mage) &&
              GET_OBJ_VAL(GET_EQ(mage, i), 3) == GET_SPARE1(spirit) &&
        ...
        if (GET_ACTIVE(spirit) == GET_IDNUM(mage))
          skill += GET_CHA(mage);
        int success = resisted_test(skill, GET_LEVEL(spirit), GET_LEVEL(spirit),
                                    GET_MAG(mage) / 100);
        if (success < 0) {
          GET_EXTRA(mage) = 0;
          GET_EXTRA(spirit) = 1;
          ...
          send_to_char(
              mage,
              "You lock minds with %s but are beaten back by its force!\r\n",
              GET_NAME(spirit));
        } else if (success == 0) {
          send_to_char(
              mage, "You lock minds with %s but fail to gain any ground.\r\n",
              GET_NAME(spirit));
        } else {
          GET_EXTRA(mage) = 1;
          GET_EXTRA(spirit) = 0;
          send_to_char(
              mage, "You lock minds with %s and succeed in weakening it!\r\n",
              GET_NAME(spirit));
        ...
        if (GET_LEVEL(spirit) < 1) {
          send_to_char(mage,
                       "You drain %s off the last of its magical force, "
                       "banishing it to the metaplanes!\r\n",
                       GET_NAME(spirit));
          act("$N is banished to the metaplanes as $n drains the last of its "
              "magical force.",
              FALSE, mage, 0, spirit, TO_ROOM);
          ...
          send_to_char(mage,
                       "Your magic spent from your battle with %s, you fall "
                       "unconscious.\r\n",
                       GET_NAME(spirit));
          act("$n falls unconscious, drained by magical combat.", FALSE, mage,
              0, 0, TO_ROOM);
      ...
      if (FIGHTING(ch)) {
        if (IS_NPC(ch) && !ch->desc && GET_SKILL(ch, SKILL_SORCERY) > 0 &&
            GET_MENTAL(ch) > 400 && ch->in_room == FIGHTING(ch)->in_room &&
            success_test(1, 8 - GET_SKILL(ch, SKILL_SORCERY))) {
          mob_magic(ch);
          continue;
        }
        if (ch->squeue) {
          cast_spell(ch, ch->squeue->spell, ch->squeue->sub, ch->squeue->force,
                     ch->squeue->arg);
      ...
      if (IS_AFFECTED(ch, AFF_APPROACH)) {
        if (IS_AFFECTED(FIGHTING(ch), AFF_APPROACH) ||
          ...
          for (struct obj_data *cyber = ch->cyberware; cyber;
               cyber = cyber->next_content)
            if (GET_OBJ_VAL(cyber, 0) == CYB_HYDRAULICJACK)
              quickness += GET_OBJ_VAL(cyber, 1);
            else if (GET_OBJ_VAL(cyber, 0) == CYB_FOOTANCHOR &&
                     !GET_OBJ_VAL(cyber, 9))
              footanchor = TRUE;
          for (struct spirit_sustained *ssust = SPIRIT_SUST(ch); ssust;
               ssust = ssust->next)
            if (ssust->type == MOVEMENTUP && ssust->caster == FALSE &&
                GET_LEVEL(ssust->target))
              quickness *= GET_LEVEL(ssust->target);
            else if (ssust->type == MOVEMENTDOWN && ssust->caster == FALSE &&
                     GET_LEVEL(ssust->target))
              quickness /= GET_LEVEL(ssust->target);
          ...
          if (quickness > 0 && success_test(quickness, target) > 1) {
            ...
            act("$n closes the distance and strikes.", FALSE, ch, 0, 0,
                TO_ROOM);
            AFF_FLAGS(ch).RemoveBit(AFF_APPROACH);
          } else {
      ...
      if (AFF_FLAGGED(ch, AFF_MANNING) || PLR_FLAGGED(ch, PLR_REMOTE) ||
        ...
        hit(ch, FIGHTING(ch),
            GET_EQ(ch, WEAR_WIELD) ? GET_EQ(ch, WEAR_WIELD)
                                   : GET_EQ(ch, WEAR_HOLD),
            GET_EQ(FIGHTING(ch), WEAR_WIELD) ? GET_EQ(FIGHTING(ch), WEAR_WIELD)
                                             : GET_EQ(FIGHTING(ch), WEAR_HOLD));
        if (GET_EQ(ch, WEAR_WIELD) && GET_EQ(ch, WEAR_HOLD) && FIGHTING(ch))
          hit(ch, FIGHTING(ch), GET_EQ(ch, WEAR_HOLD),
              GET_EQ(FIGHTING(ch), WEAR_WIELD)
                  ? GET_EQ(FIGHTING(ch), WEAR_WIELD)
                  : GET_EQ(FIGHTING(ch), WEAR_HOLD));
      }
...
void order_list(bool first, ...) {
  ...
  for (one = two; one; previous = NULL, one = next) {
    ...
    for (two = combat_list; two && two->next_fighting;
         previous = two, two = two->next_fighting) {
  ...
  if (first)
    for (one = combat_list; one; one = next) {
      next = one->next_fighting;
      two = one;
      if (GET_TRADITION(one) == TRAD_ADEPT &&
          GET_POWER(one, ADEPT_QUICK_STRIKE) && GET_PHYSICAL(one) >= 1000 &&
          GET_MENTAL(one) >= 1000) {
...
void order_list(struct char_data *start) {
  ...
  for (one = start; one; previous = NULL, one = next) {
    next = one->next_fighting;
    for (two = start; two && two->next_fighting;
         previous = two, two = two->next_fighting) {
      if (GET_INIT_ROLL(two->next_fighting) > GET_INIT_ROLL(two)) {
        if (previous)
          previous->next_fighting = two->next_fighting;
...
void chkdmg(struct veh_data *veh) {
  if (veh->damage <= 1) {
    ...
    if (veh->cspeed >= SPEED_IDLE) {
      ...
      sprintf(
          buf,
          "%s careens off the road, its occupants hurled to the street!\r\n",
          GET_VEH_NAME(veh));
      send_to_room(buf, veh->in_room);

      if (veh->rigger) {
        send_to_char(
            "Your mind is blasted with pain as your vehicle is wrecked.\r\n",
            veh->rigger);
        damage(veh->rigger, veh->rigger,
               convert_damage(
                   stage(-success_test(GET_WIL(veh->rigger), 6), SERIOUS)),
               TYPE_CRASH, MENTAL);
    ...
    for (i = veh->people; i; i = next) {
      ...
      // Deal damage.
      damage_rating = convert_damage(
          stage(-success_test(GET_BOD(i), damage_tn), damage_rating));
...
void vram(struct veh_data *veh, struct char_data *ch, struct veh_data *tveh) {
  ...
  if (ch) {
    ...
    if (IS_NPC(ch) && MOB_FLAGGED(ch, MOB_NORAM)) {
      ...
      sprintf(buf1, "%s can't seem to get close enough to $n to run $m down!",
              GET_VEH_NAME(veh));
      sprintf(buf2, "%s can't even get close to you!", GET_VEH_NAME(veh));
      send_to_driver(buf, veh);
    } else if (damage_total < LIGHT) {
      sprintf(buf, "You ram into %s, but %s armor holds!",
              thrdgenders[(int)GET_SEX(ch)], HSHR(ch));
      sprintf(buf1, "%s rams into $n, but $s armor holds!", GET_VEH_NAME(veh));
      sprintf(buf2, "%s rams into you, but your armor holds!",
              GET_VEH_NAME(veh));
      send_to_driver(buf, veh);
    ...
    if (tveh->cspeed > SPEED_IDLE)
      ...
      case SERIOUS:
        ...
        if (tveh->rigger)
          ...
          for (struct char_data *pilot = tveh->people; pilot;
               pilot = pilot->next_in_veh)
    ...
    if (veh->cspeed > SPEED_IDLE)
      ...
      case SERIOUS:
        ...
        if (veh->rigger)
          ...
          for (struct char_data *pilot = veh->people; pilot;
               pilot = pilot->next_in_veh)
            if (AFF_FLAGGED(pilot, AFF_PILOT))
              crash_test(pilot);
        break;
...
void vcombat(struct char_data *ch, struct veh_data *veh) {
  ...
  if (get_speed(veh) > 10 && !AFF_FLAGGED(ch, AFF_COUNTER_ATT) &&
      ((!wielded || !IS_GUN(GET_OBJ_VAL(wielded, 3))))) {
    vram(veh, ch, NULL);
    return;
  }
  if (wielded) {
    ...
    if (!has_ammo(ch, wielded))
      return;
    // If it's gotten here, it's a gun with either a magazine or infinite ammo.
    // Deduct ammo from magazine if it exists.
    if (IS_BURST(wielded) && wielded->contains &&
        GET_OBJ_TYPE(wielded->contains) == ITEM_GUN_MAGAZINE) {
    ...
    // TODO: Wait, so where is the code that has burst increase the power of the
    // shot? -LS
    ...
    for (obj = ch->cyberware;
      ...
      switch (GET_OBJ_VAL(obj, 2)) {
        // TODO: Figure out what these magic numbers are and comment. Until
    then, can't fix the implicit fallthrough at case 29.
        // FOLLOW-UP: cyberware value 2 is its grade (alpha/beta/etc). Whatever
    this code was, it's non-functional. case 19: damage_total = LIGHT; break;
        case 29:
  ...
  if (wielded && IS_OBJ_STAT(wielded, ITEM_SNIPER) &&
  ...
  if (damage_total < LIGHT) {
    ...
    sprintf(buf, "$n's %s causes extensive damage to %s paintwork.", ammo_type,
            GET_VEH_NAME(veh));
    sprintf(buf1, "Your attack causes extensive damage to %s paintwork.",
            GET_VEH_NAME(veh));
    sprintf(buf2, "A %s scratches your paintjob.\r\n", ammo_type);
  } else if (damage_total == MODERATE) {
    sprintf(buf, "$n's %s leaves %s riddled with holes.", ammo_type,
            GET_VEH_NAME(veh));
    sprintf(buf1, "Your attack leave %s riddled with holes.",
            GET_VEH_NAME(veh));
    ...
    sprintf(buf, "$n's %s completely destroys %s.", ammo_type,
            GET_VEH_NAME(veh));
  ...
  if (veh->owner && !IS_NPC(ch)) {
    ...
    sprintf(buf, "%s attacked vehicle (%s) owned by player %s (%ld).",
            GET_CHAR_NAME(ch), GET_VEH_NAME(veh), cname, veh->owner);
    delete[] cname;
    ...
    for (mount = veh->mount; mount; mount = mount->next_content) {
      for (gun = mount->contains; gun; gun = gun->next_content)
        if (GET_OBJ_TYPE(gun) == ITEM_WEAPON)
          break;
      if (!mount->worn_by && gun &&
          ((mount->targ && FIGHTING(ch) == mount->targ) ||
           (mount->tveh && FIGHTING_VEH(ch) == mount->tveh))) {
