...
#define IS_CLOSED(x, y)                                                        \
  (IS_SET(world[(x)].dir_option[(y)]->exit_info, EX_CLOSED))

#define VALID_EDGE(x, y)                                                       \
  (world[(x)].dir_option[(y)] && (TOROOM(x, y) != NOWHERE) &&                  \
   (!IS_CLOSED(x, y)) &&                                                       \
   (ROOM_FLAGGED(&world[x], ROOM_ROAD) ||                                      \
    ROOM_FLAGGED(&world[x], ROOM_GARAGE)) &&                                   \
   (!ROOM_FLAGGED(&world[x], ROOM_NOGRID)) && (!IS_MARKED(TOROOM(x, y))))
...
int find_first_step(vnum_t src, vnum_t target) {
  ...
  if (src < 0 || src > top_of_world || target < 0 || target > top_of_world) {
    sprintf(buf3,
            "Illegal value passed to find_first_step (graph.c). Expected "
            "constraints: 0 <= src %ld <= %ld; 0 <= target %ld <= %ld.",
