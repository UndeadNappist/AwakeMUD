...
void aedit_disp_weapon_menu(struct descriptor_data *d) {
  ...
  for (int counter = 0; counter < WEAP_CANNON - WEAP_HOLDOUT; counter += 2) {
    send_to_char(CH, "  %2d) %-15s %2d) %s\r\n", counter,
                 weapon_type[counter + WEAP_HOLDOUT], counter + 1,
                 counter + 1 < WEAP_CANNON - WEAP_HOLDOUT
...
void aedit_parse(struct descriptor_data *d, const char *arg) {
  ...
  case AEDIT_MENU:
    ...
    case 'q':
      ...
      sprintf(buf, "a box of %s %s ammunition",
              ammo_type[GET_AMMOBOX_TYPE(OBJ)].name,
              weapon_type[GET_AMMOBOX_WEAPON(OBJ)]);
...
bool ammo_test(struct char_data *ch, struct obj_data *obj) {
  if (GET_NUYEN(ch) <= ammo_type[GET_AMMOBOX_TYPE(obj)].cost * 10) {
    send_to_char(
        ch, "You don't have enough nuyen for the materials to create %s.\r\n",
        GET_OBJ_NAME(obj));
    if (IS_WORKING(ch))
      STOP_WORKING(ch);
    return FALSE;
  }
  int skillnum = gunsmith_skill(GET_AMMOBOX_WEAPON(obj)),
      target = ammo_type[GET_AMMOBOX_TYPE(obj)].tn;
  int skill = get_skill(ch, skillnum, target);
  int success = success_test(skill, target);
  int csuccess = MIN((int)(success * ((float)GET_COST_BREAKUP(ch) / 100)), 10);
  if (csuccess == success)
    csuccess--;
  sprintf(buf, "AmmoTest: Skill %d, Target %d, Success %d(c%d/t%d)", skill,
          target, success, csuccess, success - csuccess);
  act(buf, FALSE, ch, NULL, NULL, TO_ROLLS);
  if (success > 0)
    GET_AMMOBOX_TIME_TO_COMPLETION(obj) =
        (int)((ammo_type[GET_AMMOBOX_TYPE(obj)].time /
               MAX(success - csuccess, 1)) *
              60);
  else
    GET_AMMOBOX_TIME_TO_COMPLETION(obj) = -1;
  GET_NUYEN(ch) -= (int)((ammo_type[GET_AMMOBOX_TYPE(obj)].cost * 10) *
                         (1 - (csuccess * .05)));
...
void ammo_build(struct char_data *ch, struct obj_data *obj) {
  if (GET_AMMOBOX_CREATOR(obj) != GET_IDNUM(ch))
    ...
    if (!workshop) {
      ...
      for (workshop = ch->carrying; workshop;
           workshop = workshop->next_content) {
        if (GET_OBJ_TYPE(workshop) == ITEM_WORKSHOP &&
            GET_WORKSHOP_TYPE(workshop) == TYPE_AMMO &&
          ...
          if (GET_AMMOBOX_TYPE(obj) == AMMO_NORMAL ||
              (GET_AMMOBOX_TYPE(obj) == GET_WORKSHOP_AMMOKIT_TYPE(workshop)))
            break;
          else
            kitwarn = TRUE;
        }
    ...
    if (!workshop) {
      if (kitwarn) {
        send_to_char(
            ch,
            "Your ammunition kit doesn't have the right tooling for %s ammo. "
            "You'll need a different kit or an ammunition workshop.",
            ammo_type[GET_AMMOBOX_TYPE(obj)].name);
      } else {
        if (GET_AMMOBOX_TYPE(obj) != AMMO_NORMAL) {
          send_to_char(
              ch,
              "You need an ammunition workshop or %s %s ammunition kit.\r\n",
