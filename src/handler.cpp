...
extern void print_object_location(int, struct obj_data *, struct char_data *,
                                  int);
...
char *fname_allchars(char *namelist) {
  ...
  for (point = holder; *namelist && *namelist != ' ' && *namelist != '\r' &&
                       *namelist != '\n' && *namelist != '\t';
       namelist++, point++)
    *point = *namelist;
...
void affect_modify(struct char_data *ch, byte loc, sbyte mod,
  ...
  case APPLY_MAGIC_POOL:
    GET_MAGIC(ch) +=
        mod; /* GET_MAGIC gets their magic pool, GET_MAG is for attribute*/
    break;
...
void affect_veh(struct veh_data *veh, byte loc, sbyte mod) {
  ...
  case VAFF_PILOT:
    ...
    break;
  default:
    log_vfprintf("SYSLOG: Unknown apply adjust: %s/%d.", veh->short_description,
                 loc);
    break;
  }
}
void spell_modify(struct char_data *ch, struct sustain_data *sust, bool add) {
  ...
  case SPELL_TREAT:
    ...
    // Now that we meet 0 ≤ hp change ≤ max_phys, apply the add/subtract
    // multiplier.
    mod *= tmp;

    // Finally, apply it to character, capping at their max physical. No
    // negative cap is applied.
...
void affect_total(struct char_data *ch) {
  ...
  for (cyber = ch->cyberware; cyber; cyber = cyber->next_content) {
    for (j = 0; j < MAX_OBJ_AFFECT; j++)
      affect_modify(ch, cyber->affected[j].location,
                    cyber->affected[j].modifier, cyber->obj_flags.bitvector,
                    FALSE);
  }
  ...
  for (cyber = ch->bioware; cyber; cyber = cyber->next_content) {
    if ((GET_OBJ_VAL(cyber, 0) != BIO_ADRENALPUMP ||
         (GET_OBJ_VAL(cyber, 0) == BIO_ADRENALPUMP)) &&
  ...
  GET_MAGIC(ch) = 0;
  GET_CONTROL(ch) = 0;
  if ((GET_RACE(ch) == RACE_TROLL || GET_RACE(ch) == RACE_CYCLOPS ||
       GET_RACE(ch) == RACE_FOMORI || GET_RACE(ch) == RACE_GIANT ||
    ...
    for (i = 0; i < (NUM_WEARS - 1); i++) {
      if ((worn_item = GET_EQ(ch, i))) {
        ...
        if (GET_OBJ_TYPE(worn_item) == ITEM_FOCUS)
          ...
          for (j = 0; j < MAX_OBJ_AFFECT; j++)
            affect_modify(ch, worn_item->affected[j].location,
        ...
        if (GET_OBJ_TYPE(worn_item) == ITEM_WORN) {
          if (GET_WORN_MATCHED_SET(worn_item) &&
              (!suittype || suittype == GET_WORN_MATCHED_SET(worn_item))) {
            suitbal += GET_WORN_BALLISTIC(worn_item);
            suitimp += GET_WORN_IMPACT(worn_item);
            suittype = GET_WORN_MATCHED_SET(worn_item);
          } else if (GET_WORN_BALLISTIC(worn_item) ||
                     GET_WORN_IMPACT(worn_item)) {
            int bal = 0, imp = 0;
            if (GET_WORN_MATCHED_SET(worn_item)) {
            ...
            for (j = 0; j < (NUM_WEARS - 1); j++) {
              if ((temp_item = GET_EQ(ch, j)) && j != i &&
                  GET_OBJ_TYPE(temp_item) == ITEM_WORN) {
                if ((highestbal ||
                     GET_WORN_BALLISTIC(worn_item) <
                         (GET_WORN_MATCHED_SET(temp_item)
                              ? GET_WORN_BALLISTIC(temp_item) / 100
                              : GET_WORN_BALLISTIC(temp_item))) &&
                    bal == GET_WORN_BALLISTIC(worn_item))
                  bal /= 2;
                if ((highestimp || GET_WORN_IMPACT(worn_item) <
                                       (GET_WORN_MATCHED_SET(temp_item)
                                            ? GET_WORN_IMPACT(temp_item) / 100
                                            : GET_WORN_IMPACT(temp_item))) &&
                    imp == GET_WORN_IMPACT(worn_item))
                  imp /= 2;
              }
            }
            ...
            if (!IS_OBJ_STAT(worn_item, ITEM_FORMFIT)) {
              GET_TOTALIMP(ch) += GET_WORN_BALLISTIC(worn_item) /
                                  (GET_WORN_MATCHED_SET(worn_item) ? 100 : 1);
              GET_TOTALBAL(ch) += GET_WORN_IMPACT(worn_item) /
                                  (GET_WORN_MATCHED_SET(worn_item) ? 100 : 1);
            }
          }
        }
  ...
  for (cyber = ch->cyberware; cyber; cyber = cyber->next_content) {
    ...
    else if (GET_OBJ_VAL(cyber, 0) == CYB_DERMALSHEATHING ||
             GET_OBJ_VAL(cyber, 0) == CYB_DERMALPLATING) {
      if (GET_OBJ_VAL(cyber, 0) == CYB_DERMALSHEATHING &&
          GET_OBJ_VAL(cyber, 3) == 1 && !wearing)
  ...
  for (cyber = ch->bioware; cyber; cyber = cyber->next_content) {
    if (GET_OBJ_VAL(cyber, 0) != BIO_ADRENALPUMP ||
        (GET_OBJ_VAL(cyber, 0) == BIO_ADRENALPUMP && GET_OBJ_VAL(cyber, 5) > 0))
      for (j = 0; j < MAX_OBJ_AFFECT; j++)
        affect_modify(ch, cyber->affected[j].location,
                      cyber->affected[j].modifier, cyber->obj_flags.bitvector,
                      TRUE);
  ...
  if (!one && !two) /* unarmed, so use cyber-weapons or unarmed combat */
  {
    ...
    if (REAL_SKILL(ch, GET_OBJ_VAL(one, 4)) <=
        REAL_SKILL(ch, GET_OBJ_VAL(two, 4))) {
      if (!REAL_SKILL(ch, GET_OBJ_VAL(one, 4)))
        skill = REAL_SKILL(ch, return_general(GET_OBJ_VAL(one, 4)));
      else
        skill = REAL_SKILL(ch, GET_OBJ_VAL(one, 4));
  ...
  if ((IS_NPC(ch) && GET_MAG(ch) > 0) || (GET_TRADITION(ch) == TRAD_SHAMANIC ||
    ...
    GET_CASTING(ch) = MAX(0, GET_MAGIC(ch) - GET_DRAIN(ch) - GET_REFLECT(ch) -
                                 GET_SDEFENSE(ch));
  }
  if (REAL_SKILL(ch, SKILL_COMPUTER) > 0) {
    ...
    if (PLR_FLAGGED(ch, PLR_MATRIX) && ch->persona) {
      ...
      for (struct obj_data *deck = ch->carrying; deck;
           deck = deck->next_content)
        if (GET_OBJ_TYPE(deck) == ITEM_CYBERDECK ||
            GET_OBJ_TYPE(deck) == ITEM_CUSTOM_DECK) {
  ...
  if (GET_EQ(ch, WEAR_WIELD) &&
    ...
    if (!IS_GUN(GET_OBJ_VAL(GET_EQ(ch, WEAR_WIELD), 3)) &&
        GET_WEAPON_REACH(GET_EQ(ch, WEAR_WIELD)) > 0)
      ...
      if (GET_OBJ_VAL(GET_EQ(ch, WEAR_WIELD), 9) &&
          ...
          (attach = &obj_proto[real_object(
               GET_OBJ_VAL(GET_EQ(ch, WEAR_WIELD), 9))]) &&
...
void veh_from_room(struct veh_data *veh) {
  ...
  if (!veh->in_room && !veh->in_veh) {
    log("SYSERR: veh->in_room and veh->in_veh are both null; did you call "
        "veh_from_room twice?");
    return;
  }
  if (veh->in_veh) {
...
void char_to_veh(struct veh_data *veh, struct char_data *ch) {
  if (!veh || !ch)
    mudlog("SYSLOG: Illegal value(s) passed to char_to_veh", NULL, LOG_SYSLOG,
           TRUE);
  else {
...
void veh_to_room(struct veh_data *veh, struct room_data *room) {
  if (!veh || !room)
    mudlog("SYSLOG: Illegal value(s) passed to veh_to_room", NULL, LOG_SYSLOG,
           TRUE);
  else {
...
void veh_to_veh(struct veh_data *veh, struct veh_data *dest) {
  if (!veh || !dest)
    mudlog("SYSLOG: Illegal value(s) passed to veh_to_veh", NULL, LOG_SYSLOG,
           TRUE);
  else {
    if (veh->in_veh || veh->in_room)
      veh_from_room(veh);
...
void icon_to_host(struct matrix_icon *icon, vnum_t to_host) {
  ...
  if (!icon || to_host < 0 || to_host > top_of_matrix)
    mudlog("SYSLOG: Illegal value(s) passed to icon_to_host", NULL, LOG_SYSLOG,
           TRUE);
  else {
    if (icon->decker)
      for (struct matrix_icon *icon2 = matrix[to_host].icons; icon2;
...
void icon_from_host(struct matrix_icon *icon) {
  ...
  if (icon == NULL || icon->in_host == NOWHERE) {
    log("If thats happens to much im going to make it intentionally "
  ...
  temp = NULL;
  for (struct matrix_icon *icon2 = matrix[icon->in_host].icons; icon2;
       icon2 = icon2->next_in_host) {
...
void char_to_room(struct char_data *ch, struct room_data *room) {
  ...
  if (affected_by_spell(ch, SPELL_SILENCE)) {
    ...
    for (struct sustain_data *sust = GET_SUSTAINED(ch); sust; sust = sust->next)
      if (sust->spell == SPELL_SILENCE &&
          MIN(sust->force, sust->success) > room->silence[1]) {
  ...
  if (affected_by_spell(ch, SPELL_SHADOW)) {
    room->shadow[0]++;
    for (struct sustain_data *sust = GET_SUSTAINED(ch); sust; sust = sust->next)
      if (sust->spell == SPELL_SHADOW &&
          MIN(sust->force, sust->success) > room->shadow[1]) {
  ...
  if (affected_by_spell(ch, SPELL_LIGHT)) {
    room->light[0]++;
    for (struct sustain_data *sust = GET_SUSTAINED(ch); sust; sust = sust->next)
      if (sust->spell == SPELL_LIGHT &&
          MIN(sust->force, sust->success) > room->light[1]) {
...
// Checks obj_to_x preconditions for common errors. Overwrites buf3. Returns
// TRUE for kosher, FALSE otherwise.
bool check_obj_to_x_preconditions(struct obj_data *object,
                                  struct char_data *ch) {
  if (!object) {
    mudlog("ERROR: Null object passed to check_obj_to_x_preconditions().", NULL,
           LOG_SYSLOG, TRUE);
    return FALSE;
  }

  // Pre-compose our message header.
  sprintf(buf3, "ERROR: check_obj_to_x_preconditions() failure for %s (%ld): ",
          GET_OBJ_NAME(object), GET_OBJ_VNUM(object));

  // Fail if the object already has next_content. This implies that it's part of
  // someone else's linked list-- never merge them!
  ...
  if (object->carried_by) {
    sprintf(ENDOF(buf3), "Object already belongs to %s.",
  ...
  if (object->in_room) {
    sprintf(ENDOF(buf3), "Object is already in room %ld.",
  ...
  if (object->in_veh) {
    sprintf(ENDOF(buf3), "Object is already in vehicle %s.",
...
void obj_to_char(struct obj_data *object, struct char_data *ch) {
  ...
  for (i = ch->carrying; i; i = i->next_content) {
    ...
    if (object == i) {
      mudlog("ERROR: i == object in obj_to_char(). How we even got here, I "
             "don't know.",
             ch, LOG_SYSLOG, TRUE);
      return;
    }

    // If their inventory list has turned into an infinite loop due to a bug,
    // warn about it and bail out here instead of hanging the MUD.
    if (i == i->next_content) {
      sprintf(buf3,
              "ERROR: Infinite loop detected in obj_to_char. Looping object is "
              "%s (%ld). Bailing out, %s is not getting %s %s (%ld).",
    ...
    // If they already have a copy of this object, break out of the loop while
    // preserving i as pointing to that copy and op as pointing to the last
    // thing we processed (if anything).
  ...
  // If i exists (e.g. we broke out of the loop early by finding a match to this
  // item we're trying to give the character)...
  if (i) {
    // Set our to-give object's next_content to the matching item i.
    object->next_content = i;
    // If op exists (which points to the object we evaluated immediately before
    // i), point its next_content to object. Essentially this is a linked list
    // insert.
    if (op) {
      op->next_content = object;
    }
    // Otherwise, we know our object that we're giving them is the head of their
    // carrying list-- set it as such.
    else {
      ch->carrying = object;
    }
  }
  ...
  // Set the object as being carried by this character, and increase their carry
  // weight and carry number accordingly.
...
void obj_to_cyberware(struct obj_data *object, struct char_data *ch) {
  ...
  // Precondition: The character in question must exist.
  if (!ch) {
    mudlog("SYSLOG: NULL char passed to obj_to_cyberware", NULL, LOG_SYSLOG,
           TRUE);
    return;
  }
...
void obj_to_bioware(struct obj_data *object, struct char_data *ch) {
  ...
  // Precondition: The character in question must exist.
  if (!ch) {
    mudlog("SYSLOG: NULL char passed to obj_to_bioware", NULL, LOG_SYSLOG,
           TRUE);
    return;
  }
  ...
  if (GET_OBJ_VAL(bio, 0) == BIO_ADRENALPUMP && GET_OBJ_VAL(bio, 5) < 1)
    for (i = 0; i < MAX_OBJ_AFFECT; i++)
      affect_modify(bio->carried_by, bio->affected[i].location,
                    bio->affected[i].modifier, bio->obj_flags.bitvector, FALSE);
...
void obj_from_char(struct obj_data *object) {
  ...
  if (object == NULL) {
    mudlog("ERROR: NULL object passed to obj_from_char", NULL, LOG_SYSLOG,
           TRUE);
    return;
  }
  if (object->carried_by == NULL) {
    mudlog("ERROR: obj_from_char() called on obj that had no carried_by!", NULL,
           LOG_SYSLOG, TRUE);
    return;
  }
  if (object->in_obj) {
    sprintf(
        buf, "DEBUG: %s removed from char (%s), also in obj (%s). Removing.",
        object->text.name,
        GET_CHAR_NAME(object->carried_by) ? GET_CHAR_NAME(object->carried_by)
                                          : GET_NAME(object->carried_by),
        object->in_obj->text.name);
...
void equip_char(struct char_data *ch, struct obj_data *obj, int pos) {
  ...
  if (GET_EQ(ch, pos)) {
    log_vfprintf("SYSLOG: Char is already equipped: %s, %s", GET_NAME(ch),
                 obj->text.name);
    return;
  }
  if (obj->carried_by) {
  ...
  if (IS_OBJ_STAT(obj, ITEM_GODONLY) && !IS_NPC(ch) && !IS_SENATOR(ch)) {
    act("You are zapped by $p and instantly let go of it.", FALSE, ch, obj, 0,
        TO_CHAR);
    act("$n is zapped by $p and instantly lets go of it.", FALSE, ch, obj, 0,
        TO_ROOM);
...
struct obj_data *unequip_char(struct char_data *ch, int pos, bool focus) {
  ...
  if (pos < 0 || pos >= NUM_WEARS)
    log_vfprintf("SYSERR: pos < 0 || pos >= NUM_WEARS, %s - %d", GET_NAME(ch),
                 pos);

  if (!GET_EQ(ch, pos))
    log_vfprintf("SYSERR: Trying to remove non-existent item from %s at %d",
                 GET_NAME(ch), pos);
  ...
  if (GET_OBJ_TYPE(obj) == ITEM_FOCUS && GET_OBJ_VAL(obj, 4) && focus) {
    ...
    if (GET_OBJ_VAL(obj, 0) == FOCI_SUSTAINED) {
      for (struct sustain_data *sust = GET_SUSTAINED(ch); sust;
           sust = sust->next)
        if (sust->focus == obj) {
...
int vnum_from_non_connected_zone(int vnum) {
  ...
  for (counter = 0; counter <= top_of_zone_table; counter++)
    if (!(zone_table[counter].connected) &&
        vnum >= (zone_table[counter].number * 100) &&
...
struct char_data *get_char_room(const char *name, struct room_data *room) {
  ...
  for (i = room->people; i && (j <= number); i = i->next_in_room)
    if (isname(tmp, GET_KEYWORDS(i)) || isname(tmp, GET_NAME(i)) ||
...
void obj_to_veh(struct obj_data *object, struct veh_data *veh) {
  ...
  if (!veh) {
    mudlog("SYSLOG: NULL vehicle passed to obj_to_veh.", NULL, LOG_SYSLOG,
           TRUE);
    return;
  }
...
void obj_to_obj(struct obj_data *obj, struct obj_data *obj_to) {
  ...
  if (obj == obj_to) {
    mudlog(
        "ERROR: Attempting to put an object inside of itself in obj_to_obj().",
        NULL, LOG_SYSLOG, TRUE);
    return;
  }

  // Scan through the contents for something that matches this item. If we find
  // it, that's where we can nest this item.
  for (i = obj_to->contains; i; i = i->next_content) {
    if (i->item_number == obj->item_number &&
      ...
      break;
    // op points to the last thing we saw-- if a match is found, op is the thing
    // immediately before it. Otherwise, op is the last thing in the list.
    op = i;
  }
  ...
  if (i) {
    ...
    // with i existing, op must point to the object immediately before i in the
    // list.
    if (op)
      op->next_content = obj;
  ...
  // Cascade the weight change all the way up to the second-highest containing
  // object (handles bag-in-bag-in-bag situations).
  for (tmp_obj = obj->in_obj; tmp_obj->in_obj; tmp_obj = tmp_obj->in_obj)
    weight_change_object(tmp_obj, GET_OBJ_WEIGHT(obj));

  // Update the highest container's weight as well.
  if (GET_OBJ_TYPE(tmp_obj) != ITEM_CYBERDECK ||
      GET_OBJ_TYPE(tmp_obj) != ITEM_CUSTOM_DECK ||
      GET_OBJ_TYPE(tmp_obj) != ITEM_DECK_ACCESSORY)
    weight_change_object(tmp_obj, GET_OBJ_WEIGHT(obj));

  // If someone's carrying or wearing the highest container, increment their
  // carry weight by the weight of the obj we just put in.
...
void obj_from_obj(struct obj_data *obj) {
  ...
  if (GET_OBJ_TYPE(temp) != ITEM_CYBERDECK ||
      GET_OBJ_TYPE(temp) != ITEM_DECK_ACCESSORY ||
      GET_OBJ_TYPE(temp) != ITEM_CUSTOM_DECK)
    weight_change_object(temp, -GET_OBJ_WEIGHT(obj));
  if (temp->carried_by)
...
void extract_icon(struct matrix_icon *icon) {
  ...
  for (struct phone_data *k = phone_list; k; k = k->next)
    if (k->persona == icon) {
      ...
      if (k->dest) {
        if (k->dest->persona)
          send_to_icon(k->dest->persona,
                       "The call is terminated from the other end.\r\n");
  ...
  if (icon->in_host) {
    if (icon->fighting) {
      for (struct matrix_icon *vict = matrix[icon->in_host].icons; vict;
           vict = vict->next_in_host)
...
void extract_veh(struct veh_data *veh) {
  if (veh->in_room == NULL && veh->in_veh == NULL) {
    if (veh->carriedvehs || veh->people) {
      sprintf(buf, "SYSERR: extract_veh called on vehicle-with-contents "
                   "without containing room or veh! The game will likely now "
                   "shit itself and die; GLHF.");
      mudlog(buf, NULL, LOG_SYSLOG, TRUE);
    }
  }

  // Find the online owner of the vehicle for future modifications and
  // notifications.

  if (veh->prev_sub) {
    // If there is a prior entry in the subscriber doubly-linked list, just
    // strip us out.
    veh->prev_sub->next_sub = veh->next_sub;
  } else {
    // Veh is the list head. Look for an online owner– they need their list's
    // head updated to point to our next_sub value.
  ...
  // If there's a vehicle after us in the list, make sure its prev reflects our
  // prev.
  if (veh->next_sub)
  ...
  while ((temp = veh->carriedvehs)) {
    ...
    sprintf(buf2,
            "You feel the sickening sensation of falling as %s disintegrates "
            "around your vehicle.\r\n",
  ...
  while ((ch = veh->people)) {
    ...
    if (veh->in_room) {
      ...
      sprintf(buf, "As %s disintegrates, $n falls out!",
              veh->short_description);
  ...
  if (veh->towing) {
    ...
    if (ch->in_veh->in_room) {
      ...
      sprintf(buf,
              "SYSERR: Veh %s (%ld) has neither in_room nor in_veh! Dropping "
              "towed veh in Dante's Garage.",
              GET_VEH_NAME(ch->in_veh), ch->in_veh->idnum);
      mudlog(buf, ch, LOG_SYSLOG, TRUE);
      // Can't stop, we're already blowing up the vehicle. Drop it in Dante's
      // garage.
...
void extract_obj(struct obj_data *obj) {
  ...
  if (GET_OBJ_TYPE(obj) == ITEM_PHONE || (GET_OBJ_TYPE(obj) == ITEM_CYBERWARE &&
      ...
      if (phone->phone == obj) {
        if (phone->dest) {
          ...
          if (phone->dest->persona)
            send_to_icon(phone->dest->persona,
                         "The connection is closed from the other side.\r\n");
          else {
            if (phone->dest->phone->carried_by) {
              if (phone->dest->connected) {
                ...
                if (phone->dest->phone->carried_by)
                  send_to_char("Your phone stops ringing.\r\n",
                               phone->dest->phone->carried_by);
                else if (phone->dest->phone->in_obj &&
...
void extract_char(struct char_data *ch) {
  ...
  if (ch->followers) {
    ...
    for (struct follow_type *follow = ch->followers; follow;
         follow = nextfollow) {
  ...
  if (IS_ELEMENTAL(ch) && GET_SUSTAINED_NUM(ch)) {
    for (struct descriptor_data *d = descriptor_list; d; d = d->next)
      if (d->character && GET_IDNUM(d->character) == GET_ACTIVE(ch)) {
        for (struct sustain_data *sust = GET_SUSTAINED(d->character); sust;
             sust = sust->next)
          if (sust->spirit == ch) {
  ...
  /* remove the character from their room (use in_room to reference ch->in_room
   * after this) */
  ...
  if (!IS_NPC(ch)) {
    ...
    PLR_FLAGS(ch).RemoveBits(PLR_MATRIX, PLR_PROJECT, PLR_SWITCHED, PLR_WRITING,
                             PLR_MAILING, PLR_EDITING, PLR_SPELL_CREATE,
                             PLR_PROJECT, PLR_CUSTOMIZE, PLR_REMOTE, ENDBIT);
...
struct char_data *get_player_vis(struct char_data *ch, char *name, int inroom) {
  ...
  for (i = character_list; i; i = i->next)
    if (!IS_NPC(i) && (!inroom || i->in_room == ch->in_room) &&
        (isname(name, GET_KEYWORDS(i)) || isname(name, GET_CHAR_NAME(i)) ||
         recog(ch, i, name)) &&
...
struct char_data *get_char_veh(struct char_data *ch, char *name,
                               struct veh_data *veh) {
  struct char_data *i;

  for (i = veh->people; i; i = i->next_in_veh)
    if ((isname(name, GET_KEYWORDS(i)) || isname(name, GET_CHAR_NAME(i)) ||
         recog(ch, i, name)) &&
        CAN_SEE(ch, i))
      return i;
...
struct char_data *get_char_room_vis(struct char_data *ch, char *name) {
  ...
  for (i = ch->in_veh ? ch->in_veh->people : ch->in_room->people;
       i && j <= number; i = ch->in_veh ? i->next_in_veh : i->next_in_room)
    if ((isname(tmp, GET_KEYWORDS(i)) || isname(tmp, GET_NAME(i)) ||
         recog(ch, i, name)) &&
        CAN_SEE(ch, i))
      if (++j == number)
...
struct char_data *get_char_in_list_vis(struct char_data *ch, char *name,
  ...
  for (; list && j <= number; list = list->next_in_room)
    if ((isname(tmp, GET_KEYWORDS(list)) || isname(tmp, GET_NAME(list)) ||
         recog(ch, list, name)) &&
        CAN_SEE(ch, list))
      if (++j == number)
        return list;
...
struct char_data *get_char_vis(struct char_data *ch, char *name) {
  ...
  // Short circuit: If you're looking for yourself, we don't have to search very
  // hard.
  if (!str_cmp(name, "self") || !str_cmp(name, "me") ||
      !str_cmp(name, "myself"))
    return ch;
  ...
  for (i = character_list; i && (j <= number); i = i->next)
    if ((isname(tmp, GET_KEYWORDS(i)) || recog(ch, i, name)) && CAN_SEE(ch, i))
      if (++j == number)
        return i;
...
struct obj_data *get_obj_in_list_vis(struct char_data *ch, char *name,
  ...
  for (i = list; i && (j <= number); i = i->next_content) {
    if (ch->in_veh && i->in_veh && i->vfront != ch->vfront)
      continue;
    if (isname(tmp, i->text.keywords) || isname(tmp, i->text.name) ||
        (i->restring && isname(tmp, i->restring)))
      if (++j == number)
        return i;
  }
  return NULL;
  ...
  for ((*j) = 0; (*j) < NUM_WEARS && i <= number; (*j)++)
    if (equipment[(*j)]) {
      if (isname(tmp, equipment[(*j)]->text.keywords) ||
          isname(tmp, equipment[(*j)]->text.name) ||
          (equipment[(*j)]->restring && isname(tmp, equipment[(*j)]->restring)))
        if (++i == number)
          return (equipment[(*j)]);
      if (GET_OBJ_TYPE(equipment[(*j)]) == ITEM_WORN &&
          equipment[(*j)]->contains)
        for (struct obj_data *obj = equipment[(*j)]->contains; obj;
    ...
    if (ch->in_veh) {
      if ((*tar_obj = get_obj_in_list_vis(ch, name, ch->in_veh->contents)))
        return (FIND_OBJ_ROOM);
                    ...
                    get_obj_in_list_vis(ch, name, ch->in_room->contents)))
      return (FIND_OBJ_ROOM);
  }
...
int veh_skill(struct char_data *ch, struct veh_data *veh) {
  ...
  // Assume any NPC with a vehicle and no skill has a minimum skill of 4 and
  // that the builder just forgot to set it.
