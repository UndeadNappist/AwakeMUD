...
ACMD(do_helpedit) {
  if (!ch->desc) {
    send_to_char("You don't even have a descriptor, how are you going to edit "
                 "a helpfile?\r\n",
                 ch);
    mudlog("Character without descriptor tried to edit a helpfile, wut?", ch,
           LOG_SYSLOG, TRUE);
    return;
  }
  ...
  if (!*argument) {
    send_to_char("Syntax: HELPEDIT NEW or HELPEDIT <exact and full title of "
                 "the help file to edit>\r\n",
                 ch);
    return;
  }
  ...
  if (str_cmp(argument, "new") == 0) {
    // Create a new helpfile and display menu (which puts them into edit mode),
    // then bail out of this function.
  ...
  sprintf(buf, "SELECT name, body FROM help_topic WHERE name='%s'",
          prepare_quotes(buf2, argument, sizeof(buf2)));
  if (mysql_wrapper(mysql, buf)) {
    send_to_char("Sorry, an error occurred. Try again later.\r\n", ch);
    mudlog("SYSERR: A MySQL error occurred while attempting to edit a "
           "helpfile. See log for details.",
           ch, LOG_SYSLOG, TRUE);
    return;
  }
  ...
  if (!row) {
    ...
    send_to_char(
        "No helpfile by that name currently exists. Please use the *exact and "
        "full* name, or HELPEDIT NEW to make a new file.\r\n",
        ch);
    return;
  }
...
ACMD(do_helpexport) {
  ...
  if (!*argument) {
    send_to_char("Syntax: HELPEXPORT <exact and full title of the help file to "
                 "export>\r\n",
                 ch);
    return;
  }
  ...
  if (mysql_wrapper(mysql, buf)) {
    send_to_char("Sorry, an error occurred. Try again later.\r\n", ch);
    mudlog("SYSERR: A MySQL error occurred while attempting to export a "
           "helpfile. See log for details.",
           ch, LOG_SYSLOG, TRUE);
    return;
  }
  ...
  if (!row) {
    mysql_free_result(res);
    send_to_char("No helpfile by that name currently exists. Please use the "
                 "*exact and full* name of the helpfile.\r\n",
                 ch);
    return;
  }
  mysql_free_result(res);
  ...
  send_to_char(ch,
               "Output:\r\n\r\n^c%s^n\r\n\r\n^RWARNING:^n Make sure your "
               "client's line wrap is turned off!",
               double_up_color_codes(output_ptr));
}

// Display the main menu. Also sets our various states and bits for the editor
// char.
void helpedit_disp_menu(struct descriptor_data *d) {
  ...
  else if (!HELPFILE->title ||
           strcmp(HELPFILE->title, HELPFILE->original_title) ==
               0) // Old file, no new title yet.
...
void helpedit_parse_main_menu(struct descriptor_data *d, const char *arg) {
  ...
  case '2':
    ...
    // Force no formatting mode by making the first entry into the string buf
    // the special code /**/.
    char str_buf[50];
    sprintf(str_buf, "/**/");
  ...
  case 't':
    if ((d->edit_convert_color_codes = !d->edit_convert_color_codes))
      send_to_char("OK, color codes for the body will be displayed as tags for "
                   "easier copy-paste.\r\n",
                   CH);
    else
  ...
  case 'q': // Save the edit.
    ...
    if (HELPFILE->title && HELPFILE->original_title &&
        strcmp(HELPFILE->title, HELPFILE->original_title) != 0) {
      // Updated existing and changed name.
      sprintf(buf, "%s wrote new helpfile '%s' (renamed from '%s').",
              GET_CHAR_NAME(CH), HELPFILE->title, HELPFILE->original_title);
      mudlog(buf, CH, LOG_SYSLOG, TRUE);
    } else if (HELPFILE->original_title && !HELPFILE->title) {
      // Updated existing.
      sprintf(buf, "%s wrote new helpfile '%s'.", GET_CHAR_NAME(CH),
              HELPFILE->original_title);
      mudlog(buf, CH, LOG_SYSLOG, TRUE);
    } else {
      // Created new.
      sprintf(buf, "%s wrote new helpfile '%s'.", GET_CHAR_NAME(CH),
              HELPFILE->title ? HELPFILE->title : HELPFILE->original_title);
      mudlog(buf, CH, LOG_SYSLOG, TRUE);
    }
    ...
    sprintf(query_buf, "DELETE FROM help_topic WHERE `name`='%s'",
            prepare_quotes(buf3, HELPFILE->original_title, sizeof(buf)));
    mysql_wrapper(mysql, query_buf);

    // Compose and execute the insert-on-duplicate-key-update query.
    if (mysql_wrapper(mysql, generate_sql_for_helpfile(
                                 HELPFILE->title ? HELPFILE->title
                                                 : HELPFILE->original_title,
                                 HELPFILE->body)))
...
void helpedit_parse(struct descriptor_data *d, const char *arg) {
  ...
  case HELPEDIT_TITLE:
    ...
    if (strlen(arg) > MAX_HELPFILE_TITLE_LENGTH) {
      send_to_char(CH,
                   "The maximum helpfile title length is %d. Enter title: \r\n",
    ...
    if (HELPFILE->original_title &&
        str_cmp(arg, HELPFILE->original_title) != 0) {
      sprintf(buf, "SELECT name FROM help_topic WHERE `name`='%s'",
      ...
      if (row) {
        mysql_free_result(res);
        send_to_char("A helpfile by that name already exists. Select something "
                     "else: \r\n",
                     CH);
        return;
      }
  ...
  default:
    sprintf(
        buf,
        "SYSERR: Unknown helpedit edit mode %d encountered in helpedit_parse.",
        STATE(d));
...
const char *generate_sql_for_helpfile(const char *name, const char *body) {
  if (name == NULL) {
    mudlog("SYSERR: NULL name or body pointer passed to "
           "generate_sql_for_helpfile!",
  ...
  sprintf(output_buf,
          "INSERT INTO help_topic (name, body) VALUES ('%s', '%s')"
...
const char *generate_sql_for_helpfile_by_name(const char *name) {
  if (!name || !*name) {
    sprintf(buf3,
            "SYSERR: generate_sql_for_helpfile_by_name received name '%s'.",
            name);
  ...
  if (strchr(name, '%')) {
    sprintf(buf3,
            "SYSERR: generate_sql_for_helpfile_by_name received name that "
            "contained %% '%s'.",
            name);
    mudlog(buf3, NULL, LOG_SYSLOG, TRUE);
    return NULL;
  ...
  char quotebuf[500];
  sprintf(buf, "SELECT * FROM help_topic WHERE name='%s'",
          prepare_quotes(quotebuf, name, sizeof(quotebuf)));
  if (mysql_wrapper(mysql, buf))
    return NULL;
  ...
  sprintf(buf3,
          "SYSERR: Received invalid name '%s' to "
          "generate_sql_for_helpfile_by_name, returning no query.",
          name);
  mudlog(buf3, NULL, LOG_SYSLOG, TRUE);
  return NULL;
}
