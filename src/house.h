...
struct house_control_rec {
  ...
  house_control_rec()
      : vnum(NOWHERE), atrium(NOWHERE), key(0), owner(0), name(NULL),
        next(NULL) {}
};
...
struct landlord {
  ...
  landlord() : vnum(NOWHERE), race(0), rooms(NULL), next(NULL) {}
};
...
#define TOROOM(room_rnum, dir)                                                 \
  ((world[(room_rnum)].dir_option[(dir)] &&                                    \
    world[(room_rnum)].dir_option[(dir)]->to_room)                             \
       ? real_room(world[(room_rnum)].dir_option[(dir)]->to_room->number)      \
       : NOWHERE)
