  ...
  for (counter = 0; counter < MAX_OBJ_AFFECT; counter++) {
    if (d->edit_obj->affected[counter].modifier) {
      ...
      send_to_char(CH, " %d) %+d to %s%s%s\r\n", counter + 1,
                   d->edit_obj->affected[counter].modifier, CCCYN(CH, C_CMP),
                   buf2, CCNRM(CH, C_CMP));
    } else
  ...
  if (GET_OBJ_TYPE(d->edit_obj) == ITEM_MOD) {
    for (counter = 0; counter < VAFF_MAX; counter += 2)
      send_to_char(CH, "%2d) %-18s    %2d) %-18s\r\n", counter,
                   veh_aff[counter], counter + 1,
                   counter + 1 < VAFF_MAX ? veh_aff[counter + 1] : "");
  } else {
    for (counter = 0; counter < APPLY_MAX; counter += 2)
      send_to_char(CH, "%2d) %-18s    %2d) %-18s\r\n", counter,
                   apply_types[counter], counter + 1,
                   counter + 1 < APPLY_MAX ? apply_types[counter + 1] : "");
  }
...
void iedit_disp_weapon_menu(struct descriptor_data *d) {
  ...
  for (int counter = 0; counter < MAX_WEAP; counter += 2)
    send_to_char(CH, "%2d) %-18s %2d) %-18s\r\n", counter, weapon_type[counter],
                 counter + 1,
                 counter + 1 < MAX_WEAP ? weapon_type[counter + 1] : "");
...
void iedit_disp_spells_menu(struct descriptor_data *d) {
  ...
  for (int counter = 1; counter < MAX_SPELLS; counter += 3)
    send_to_char(CH, "%2d) %-18s %2d) %-18s %2d) %-18s\r\n", counter,
                 spells[counter].name, counter + 1,
                 counter + 1 < MAX_SPELLS ? spells[counter + 1].name : "",
                 counter + 2,
                 counter + 2 < MAX_SPELLS ? spells[counter + 2].name : "");
...
void iedit_disp_val1_menu(struct descriptor_data *d) {
  ...
  case ITEM_GUN_MAGAZINE:
    send_to_char(
        "Number of bullets magazine contains (must match max load of weapon): ",
        CH);
    break;
  case ITEM_GUN_AMMO:
  ...
  case ITEM_MOD:
    ...
    break;
  case ITEM_HOLSTER:
    send_to_char("Type (0 - Pistol/SMG, 1 - Sword, 2 - Rifle (or larger)): ",
                 d->character);
    break;
  case ITEM_WORN:
  ...
  case ITEM_RCDECK:
    ...
    break;
  case ITEM_PHONE:
    send_to_char("Area Code (0 - Seattle, 1 - Tacoma, 2 - Portland)): ",
                 d->character);
    break;
  case ITEM_CHIP:
    CLS(CH);
...
void iedit_disp_val2_menu(struct descriptor_data *d) {
  ...
  case ITEM_FIREWEAPON:
    send_to_char(
        "Base Damage (1 - Light, 2 - Moderate, 3 - Serious, 4 - Deadly): ",
        d->character);
    break;
  case ITEM_DRINKCON:
  case ITEM_FOUNTAIN:
...
void iedit_disp_val3_menu(struct descriptor_data *d) {
  ...
  case ITEM_BIOWARE:
    if (GET_OBJ_VAL(OBJ, 0) < BIO_CEREBRALBOOSTER)
      send_to_char(CH, "Cultured (%d - Yes, %d - No): ", BIOWARE_CULTURED,
                   BIOWARE_STANDARD);
    else {
...
void iedit_disp_val4_menu(struct descriptor_data *d) {
  ...
  case ITEM_CYBERWARE:
    ...
    case CYB_CRANIALCYBER:
      send_to_char("Vnum of cyberdeck to be included in headware (0 to quit): ",
                   CH);
      break;
    case CYB_EYES:
  ...
  case ITEM_PROGRAM:
    if (GET_OBJ_VAL(d->edit_obj, 0) == SOFT_ATTACK)
      send_to_char("Damage Rating (1-Light 2-Moderate 3-Serious 4-Deadly): ",
                   CH);
    else
      iedit_disp_menu(d);
    break;
...
void iedit_disp_val12_menu(struct descriptor_data *d) {
  ...
  case ITEM_WEAPON:
    send_to_char(
        "Enter the amount of built-in recoil compensation this weapon has: ",
        CH);
    break;
  default:
    iedit_disp_menu(d);
...
void iedit_disp_cybertype_menu(struct descriptor_data *d) {
  ...
  for (int counter = 0; counter < NUM_CYBER; counter += 2) {
    send_to_char(CH, "%2d) %-20s %2d) %-20s\r\n", counter + 1,
                 cyber_types[counter], counter + 2,
                 counter + 1 < NUM_CYBER ? cyber_types[counter + 1] : "");
  }
...
void iedit_disp_biotype_menu(struct descriptor_data *d) {
  ...
  for (int counter = 0; counter < NUM_BIOWARE; counter += 2) {
    send_to_char(CH, "%2d) %-20s %2d) %-20s\r\n", counter + 1,
                 bio_types[counter], counter + 2,
                 counter + 1 < NUM_BIOWARE ? bio_types[counter + 1] : "");
  }
...
void iedit_disp_extra_menu(struct descriptor_data *d) {
  ...
  for (counter = 0; counter < ITEM_EXTRA_MAX; counter += 2)
    send_to_char(CH, "%2d) %-20s %2d) %-20s\r\n", counter + 1,
                 extra_bits[counter], counter + 2,
                 counter + 1 < ITEM_EXTRA_MAX ? extra_bits[counter + 1] : "");

  GET_OBJ_EXTRA(d->edit_obj)
      .PrintBits(buf1, MAX_STRING_LENGTH, extra_bits, ITEM_EXTRA_MAX);
  send_to_char(CH,
               "Object flags: %s%s%s\r\n"
...
void iedit_disp_aff_menu(struct descriptor_data *d) {
  ...
  for (int counter = 0; counter < AFF_MAX; counter += 2)
    send_to_char(CH, "%2d) %-20s %2d) %-20s\r\n", counter + 1,
                 affected_bits[counter], counter + 2,
                 counter + 1 < AFF_MAX ? affected_bits[counter + 1] : "");

  OBJ->obj_flags.bitvector.PrintBits(buf1, MAX_STRING_LENGTH, affected_bits,
                                     AFF_MAX);
  send_to_char(CH,
               "Object flags: %s%s%s\r\n"
...
void iedit_disp_wear_menu(struct descriptor_data *d) {
  ...
  for (counter = 0; counter < ITEM_WEAR_MAX; counter += 2) {
    send_to_char(CH, "%2d) %-20s %2d) %-20s\r\n", counter + 1,
                 wear_bits[counter], counter + 2,
                 counter + 1 < ITEM_WEAR_MAX ? wear_bits[counter + 1] : "");
  }

  GET_OBJ_WEAR(d->edit_obj)
      .PrintBits(buf1, MAX_STRING_LENGTH, wear_bits, ITEM_WEAR_MAX);
  send_to_char(CH,
               "Wear flags: %s%s%s\r\n"
...
void iedit_disp_legality_menu(struct descriptor_data *d) {
  ...
  if (d->edit_mode == IEDIT_LEGAL1) {
    send_to_char("1) Small Blade\r\n"
                 ...
                 "Select Security Category (0 for other): ",
                 CH);
...
void iedit_disp_menu(struct descriptor_data *d) {
  ...
  OBJ->obj_flags.wear_flags.PrintBits(buf1, MAX_STRING_LENGTH, wear_bits,
                                      NUM_WEARS);
  ...
  send_to_char(CH, "f) Item Legality: ^c%d%s^n-^c%s^n\r\n",
               GET_LEGAL_NUM(d->edit_obj),
               GET_LEGAL_PERMIT(d->edit_obj) ? "P" : "",
               legality_codes[GET_LEGAL_CODE(d->edit_obj)][0]);
  if (GET_OBJ_TYPE(d->edit_obj) == ITEM_WEAPON)
    send_to_char(
        CH, "g) Item values: ^c%d %d %d %d %d %d %d %d %d %d %d %d (%d)^n\r\n",
    ...
    send_to_char(CH,
                 "g) Item values: ^c%d %d %d %d %d %d %d %d %d %d %d %d^n\r\n",
                 GET_OBJ_VAL(d->edit_obj, 0), GET_OBJ_VAL(d->edit_obj, 1),
                 GET_OBJ_VAL(d->edit_obj, 2), GET_OBJ_VAL(d->edit_obj, 3),
                 GET_OBJ_VAL(d->edit_obj, 4), GET_OBJ_VAL(d->edit_obj, 5),
                 GET_OBJ_VAL(d->edit_obj, 6), GET_OBJ_VAL(d->edit_obj, 7),
  ...
  send_to_char(CH,
               ...
               GET_OBJ_STREET_INDEX(d->edit_obj));
...
void iedit_parse(struct descriptor_data *d, const char *arg) {
  ...
  case IEDIT_CONFIRM_SAVESTRING:
    ...
    case 'Y': {
      ...
      if (!vnum_from_non_connected_zone(d->edit_number)) {
        sprintf(buf, "%s wrote new obj #%ld", GET_CHAR_NAME(d->character),
                d->edit_number);
      ...
      if (obj_number > 0) {
        ...
        if (obj_proto[obj_number].ex_description) {
          for (This = obj_proto[obj_number].ex_description; This;
               This = next_one) {
            next_one = This->next;
         ...
         This code shamelessly ripped off from db.c */
        {
          ...
          for (zone = 0; zone <= top_of_zone_table; zone++)
            for (cmd_no = 0; cmd_no < zone_table[zone].num_cmds; cmd_no++) {
              ...
              case 'O':
                ZCMD.arg1 =
                    (ZCMD.arg1 >= d->edit_obj->item_number ? ZCMD.arg1 + 1
                                                           : ZCMD.arg1);
                break;
              case 'G':
                ZCMD.arg1 =
              ...
              case 'P':
                ZCMD.arg1 =
                    (ZCMD.arg1 >= d->edit_obj->item_number ? ZCMD.arg1 + 1
                                                           : ZCMD.arg1);
                ZCMD.arg3 =
                    (ZCMD.arg3 >= d->edit_obj->item_number ? ZCMD.arg3 + 1
                                                           : ZCMD.arg3);
                break;
              case 'R': /* rem obj from room */
                ZCMD.arg2 =
                    (ZCMD.arg2 >= d->edit_obj->item_number ? ZCMD.arg2 + 1
                                                           : ZCMD.arg2);
                break;
              }
            }
  ...
  case IEDIT_MAIN_MENU:
      ...
      if (*(arg + 1) && isdigit(*(arg + 1))) {
        ...
        default:
          send_to_char(d->character,
                       "Invalid G-menu bypass specified. Must be in range "
                       "G0-G11, but got G%d instead.\r\n",
                       bypass_num);
    ...
    case 'I':
      ...
      if (!d->edit_obj->ex_description) {
        ...
        memset((char *)d->edit_obj->ex_description, 0,
    ...
    case 'j':
    case 'J':
      send_to_char("Enter sourcebook info (abbreviation and pages, ex: MitS "
                   "p34, 37-48):",
                   d->character);
  ...
  case IEDIT_TYPE:
    ...
    if ((number < 1) || (number > NUM_ITEMS) ||
        (number == ITEM_CYBERWARE && !access_level(CH, LVL_ADMIN)) ||
    ...
    if (number == ITEM_GUN_MAGAZINE) {
      send_to_char(
          "NOTICE: Gun magazines were deprecated with the creation of the new "
          "ammo system. You probably don't need to make one.\r\n",
          d->character);
    }
  ...
  case IEDIT_VALUE_1:
    ...
    case ITEM_SPELL_FORMULA:
      ...
      GET_OBJ_VAL(d->edit_obj, 3) = GET_OBJ_VAL(d->edit_obj, 4) =
          GET_OBJ_VAL(d->edit_obj, 5) = GET_OBJ_VAL(d->edit_obj, 6) =
              GET_OBJ_VAL(d->edit_obj, 7) = GET_OBJ_VAL(d->edit_obj, 8) =
  ...
  case IEDIT_VALUE_2:
    ...
    case ITEM_CHIP:
      if (number < 0 || number > 15) {
        send_to_char(
            CH, "Rating must be between 0 and 15. Enter skillsoft rating: ");
        return;
      }
      break;
    case ITEM_CONTAINER:
  ...
  case IEDIT_VALUE_3:
    ...
    case ITEM_WEAPON:
    case ITEM_FIREWEAPON:
      if (number < -WEAPON_MAXIMUM_STRENGTH_BONUS ||
          number > WEAPON_MAXIMUM_STRENGTH_BONUS) {
    ...
    case ITEM_BIOWARE:
      if (number != BIOWARE_STANDARD && number != BIOWARE_CULTURED) {
        send_to_char(CH, "Invalid choice. %d for yes, %d for no: ",
                     BIOWARE_CULTURED, BIOWARE_STANDARD);
        return;
      }
      break;
    case ITEM_CYBERWARE:
    ...
    case ITEM_DECK_ACCESSORY:
      ...
      case 5:
        if (number < 1 || number > 3) {
          send_to_char(
              "Values must range from 1 to 3.  Reaction increase level: ", CH);
          return;
        }
      }
      break;
    case ITEM_GUN_ACCESSORY:
      if (GET_OBJ_VAL(OBJ, 1) == 3 && (number < -4 || number > -1)) {
        send_to_char("Modifier must be between -4 and -1.  Recoil modifier: ",
                     CH);
        return;
      }
      break;
    case ITEM_FOCUS:
      if (GET_OBJ_VAL(OBJ, 0) == FOCI_SPIRIT &&
          (number < 1 || number > NUM_SPIRITS)) {
        send_to_char("Invalid response.  Focus spirit type: ", CH);
        return;
      } else if (GET_OBJ_VAL(OBJ, 0) == FOCI_SPEC_SPELL &&
                 (number < 1 || number > MAX_SPELLS)) {
        send_to_char("Invalid choice!\r\n", CH);
        iedit_disp_val3_menu(d);
        return;
  ...
  case IEDIT_VALUE_4:
    ...
    case ITEM_WEAPON:
      ...
      switch (number) {
        // Determine what attachment locations are available by default based on
        // weapon type. 0 = available, -1 = not.
    ...
    case ITEM_FIREWEAPON:
      if (number < 1 ||
          (!access_level(CH, LVL_ADMIN) && number == TYPE_BIFURCATE) ||
    ...
    case ITEM_DECK_ACCESSORY:
      if (number < 1 || number > 15) {
        send_to_char(
            "MPCP value must be between 1 and 15!\r\nMPCP to enhance: ", CH);
        return;
      }
      break;
    case ITEM_CYBERWARE:
      ...
      case CYB_CRANIALCYBER:
        if (real_object(number) < 1 ||
            obj_proto[real_object(number)].obj_flags.type_flag !=
                ITEM_CYBERDECK) {
          send_to_char("Object doesn't exist or is not a cyberdeck! Vnum of "
                       "cyberdeck to be included in headware (0 to quit): ",
                       CH);
          return;
        }
        break;
  ...
  case IEDIT_VALUE_7:
    ...
    case ITEM_FIREWEAPON:
      if (number < 1 || number > 10) {
        send_to_char(
            "Strength min. must be between 1 and 10.  Enter strength minimum: ",
            CH);
        return;
      }
      break;
    case ITEM_CYBERDECK:
      if (number < 0 ||
          (number > 1 && number > (int)(GET_OBJ_VAL(d->edit_obj, 0) / 3))) {
        send_to_char(
            CH, "Reaction upgrade must be between 0 and %d. Reaction Upgrade: ",
            (int)(GET_OBJ_VAL(d->edit_obj, 0) / 3));
        return;
      }
      break;
    default:
  ...
  case IEDIT_VALUE_8:
    ...
    case ITEM_WEAPON:
      if (!access_level(CH, LVL_ADMIN)) {
        ...
        if (!(you_know_this_would_leak_memory_right =
                  read_object(number, VIRTUAL))) {
        ...
        for (j = 0; (j < MAX_OBJ_AFFECT && !modified); j++)
          if (!(OBJ->affected[j].modifier)) {
            OBJ->affected[j].location =
                read_object(number, VIRTUAL)->affected[0].location;
            OBJ->affected[j].modifier =
                read_object(number, VIRTUAL)->affected[0].modifier;
            modified = TRUE;
          }
      }
      break;
  ...
  case IEDIT_VALUE_12:
    ...
    case ITEM_WEAPON:
      // NOTE: This does NOT actually modify value '12' (GET_OBJ_VAL(obj, 11)).
      // It changes the attempts field instead. For weapons, value '12' is
      // currently the selected firemode of the weapon.
      if (number < 0 || number > 4) {
        send_to_char("Integral recoil compensation must be between 0 and 4 "
                     "(inclusive). Enter recoil comp: ",
                     CH);
        return;
      }
  ...
  case IEDIT_APPLY:
    ...
    if (number == 0) {
      ...
      iedit_disp_prompt_apply_menu(d);
    } else if ((GET_OBJ_TYPE(d->edit_obj) == ITEM_MOD &&
                (number < 0 || number >= VAFF_MAX)) ||
  ...
  case IEDIT_STREETINDEX:
    ...
    if (fnumber <= 0 || fnumber > 10) {
      send_to_char("Street index (black market cost multiplier) must be "
                   "between 0 and 10. Enter street index: ",
                   CH);
    } else {
...
#define MISCDATA ((struct extra_descr_data *)*d->misc_data)
      ...
      if (!MISCDATA->keyword || !MISCDATA->description) {
        ...
        struct extra_descr_data *temp = d->edit_obj->ex_description,
                                *next = NULL;
        if (temp == MISCDATA) {
  ...
  for (counter = zone_table[zone].number * 100; counter <= zone_table[zone].top;
    ...
    if (realcounter >= 0) {
      ...
      fprintf(fp,
              ...
              "AffFlags:\t%s\n"
              "Material:\t%s\n",
              item_types[(int)GET_OBJ_TYPE(obj)], GET_OBJ_WEAR(obj).ToString(),
      ...
      fprintf(fp,
              ...
              "\tLegalPermit:\t%d\n"
              "\tStreetIndex:\t%.2f\n",
              GET_OBJ_WEIGHT(obj), GET_OBJ_BARRIER(obj), GET_OBJ_COST(obj),
              GET_OBJ_AVAILTN(obj), GET_OBJ_AVAILDAY(obj), GET_LEGAL_NUM(obj),
              GET_LEGAL_CODE(obj), GET_LEGAL_PERMIT(obj),
      ...
      for (counter2 = 0; counter2 < MAX_OBJ_AFFECT; counter2++) {
        ...
        if (af->location != APPLY_NONE && af->modifier != 0) {
          fprintf(fp,
                  ...
                  "\tLocation:\t%s\n"
                  "\tModifier:\t%d\n",
                  counter2, apply_types[(int)af->location], af->modifier);
        }
      }
