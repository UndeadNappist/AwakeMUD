...
struct command_info cmd_info[] = {
    ...
    /* now, the main list -- note that spec-proc commands and socials come after
       this list. */
...
#ifdef SELFADVANCE
    // Allows running an unattended test port where anyone can bump themselves
    // up to level 9.
    ...
    {"pages", POS_DEAD, do_switched_message_history, LVL_ARCHITECT,
     COMM_CHANNEL_PAGES},
    ...
    {"wtells", POS_DEAD, do_switched_message_history, LVL_BUILDER,
     COMM_CHANNEL_WTELLS},
    {"wts", POS_DEAD, do_switched_message_history, LVL_BUILDER,
     COMM_CHANNEL_WTELLS},
    ...
    /* this must be last; do not touch anything below this line unless you know
       what you're doing */
    {"\n", 0, 0, 0, 0}};
/* this must be last; do not touch anything above this line unless you know what
 * you're doing */
...
const char *reserved[] = {
    "self",    "me",        "myself", "all",   "room",
    "someone", "something", "new",    "guest", "deleted",
...
void nonsensical_reply(struct char_data *ch) {
  ...
  if (ch->desc && ++ch->desc->invalid_command_counter >= 5) {
    send_to_char(ch,
                 "^GStuck? Need help? Feel free to ask on the %s channel! "
                 "(%s%s <message>)^n\r\n",
  ...
  /*  Removing the prior 'funny' messages and replacing them with something
  understandable by MUD newbies. switch (number(1, 9))
  {
  case 1:
    send_to_char(ch, "Huh?!?\r\n");
    break;
...
void command_interpreter(struct char_data *ch, char *argument, char *tcname) {
  ...
  // They entered something? KaVir's protocol snippet says to clear their
  // WriteOOB.
  if (ch->desc)
  ...
  if (PLR_FLAGGED(ch, PLR_MATRIX)) {
    ...
    if (*mtx_info[cmd].command == '\n') {
      // If the command exists outside of the Matrix, let them know that it's
      // not an option here.
    ...
    if (IS_PROJECT(ch) && ch->desc && ch->desc->original &&
        AFF_FLAGGED(ch->desc->original, AFF_TRACKING) &&
        cmd != find_command("track")) {
    ...
    if (PLR_FLAGGED(ch, PLR_FROZEN)) {
      if (!access_level(ch, LVL_VICEPRES)) {
        ...
        send_to_char("The ice covering you crackles alarmingly as you slam "
                     "your sovereign will through it.\r\n",
                     ch);
    }
    ...
    if (AFF_FLAGGED(ch, AFF_PETRIFY) &&
      ...
      if (!access_level(ch, LVL_VICEPRES)) {
        ...
        send_to_char("You abuse your administrative powers and force your "
                     "petrified body to respond.\r\n",
                     ch);
    }
    ...
    if (affected_by_power(ch, ENGULF) &&
        cmd_info[cmd].minimum_position != POS_DEAD) {
      if (!access_level(ch, LVL_VICEPRES)) {
        send_to_char("You are currently being engulfed!\r\n", ch);
        return;
      } else
        send_to_char("Administrative power roars through your veins as you "
                     "muscle through your engulfment.\r\n",
                     ch);
    }
    ...
    if (GET_QUI(ch) <= 0 && cmd_info[cmd].minimum_position != POS_DEAD) {
      if (!access_level(ch, LVL_VICEPRES)) {
        ...
        send_to_char("You draw upon your mantle of administrative power and "
                     "push through your paralysis.\r\n",
                     ch);
    }
    ...
    if (GET_POS(ch) < cmd_info[cmd].minimum_position) {
      ...
      case POS_DEAD:
        ...
        break;
      case POS_MORTALLYW:
        send_to_char("You are in a pretty bad shape! You can either wait for "
                     "help, or give up by typing DIE.\r\n",
                     ch);
        break;
      case POS_STUNNED:
...
int is_abbrev(const char *arg1, const char *arg2) {
  if (!arg1 || !arg2) {
    char temp_buf[500];
    sprintf(temp_buf,
            "SYSERR: Received null arg in is_abbrev (arg1 = %s, arg2 = %s)",
            arg1, arg2);
...
int special(struct char_data *ch, int cmd, char *arg) {
  ...
  if (veh && GET_ROOM_SPEC(veh->in_room) != NULL)
    if (GET_ROOM_SPEC(veh->in_room)(ch, &world[real_room(veh->in_room->number)],
                                    cmd, arg))
      return 1;
  if (GET_ROOM_SPEC(ch->in_room) != NULL)
    if (GET_ROOM_SPEC(ch->in_room)(ch, &world[real_room(ch->in_room->number)],
                                   cmd, arg))
      return 1;
  ...
  for (i = ch->in_veh ? ch->in_veh->contents : ch->in_room->contents; i;
       i = i->next_content)
...
int perform_dupe_check(struct descriptor_data *d) {
  ...
  PLR_FLAGS(d->character)
      .RemoveBits(PLR_MAILING, PLR_EDITING, PLR_SPELL_CREATE, PLR_CUSTOMIZE,
  ...
  case RECON:
    ...
    sprintf(buf, "%s [%s] has reconnected.", GET_CHAR_NAME(d->character),
            d->host);
    mudlog(buf, d->character, LOG_CONNLOG, TRUE);
    break;
  case USURP:
    SEND_TO_Q("You take over your own body, already in use!\r\n", d);
    act("$n shakes $s head to clear it.", TRUE, d->character, 0, 0, TO_ROOM);
    ...
    if (d->character->persona) {
      ...
      send_to_host(d->character->persona->in_host, buf, d->character->persona,
                   TRUE);
      ...
      for (struct char_data *temp = d->character->in_room->people; temp;
...
void nanny(struct descriptor_data *d, char *arg) {
    ...
    if (!*arg) {
      ...
      if (does_player_exist(tmp_name)) {
        ...
        if (PRF_FLAGGED(d->character, PRF_HARDCORE) &&
            PLR_FLAGGED(d->character, PLR_JUST_DIED)) {
        ...
        sprintf(buf,
                "Welcome back. Enter your password. Not %s? Enter 'abort' to "
        ...
        for (struct descriptor_data *k = descriptor_list; k; k = k->next)
          if (k->character && k->character->player.char_name &&
              !str_cmp(k->character->player.char_name, tmp_name)) {
            SEND_TO_Q("There is already someone creating a character with that "
                      "name.\r\nName: ",
                      d);
            return;
          }
  ...
  case CON_NAME_CNFRM: /* wait for conf. of new name    */
    if (UPPER(*arg) == 'Y') {
      ...
      if (restrict) {
        if (restrict == LVL_MAX)
          SEND_TO_Q(
              "The mud is being reconfigured.  Try again a bit later.\r\n", d);
        else
      ...
      sprintf(buf, "Give me a password for %s: ", GET_CHAR_NAME(d->character));
      SEND_TO_Q(buf, d);
      echo_off(d);
  ...
  case CON_PASSWORD: /* get pwd for known player      */
    ...
    if (!*arg)
      ...
      if (!validate_and_update_password(arg, GET_PASSWD(d->character))) {
        sprintf(buf, "Bad PW: %s [%s]", GET_CHAR_NAME(d->character), d->host);
        mudlog(buf, d->character, LOG_CONNLOG, TRUE);
      ...
      sprintf(query_buf, "UPDATE pfiles SET password='%s' WHERE idnum=%ld;",
              prepare_quotes(prepare_quotes_buf, GET_PASSWD(d->character),
                             sizeof(prepare_quotes_buf) /
                                 sizeof(prepare_quotes_buf[0])),
              GET_IDNUM(d->character));
#else
      sprintf(query_buf, "UPDATE pfiles SET password='%s' WHERE idnum=%ld;",
              GET_PASSWD(d->character), GET_IDNUM(d->character));
#endif
      ...
      if (isbanned(d->host) == BAN_SELECT &&
        ...
        SEND_TO_Q("Sorry, this char has not been cleared for login from your "
                  "site!\r\n",
                  d);
        STATE(d) = CON_CLOSE;
      ...
      if (!access_level(d->character, restrict)) {
        if (restrict == LVL_MAX)
          SEND_TO_Q("The mud is about to reboot. Please try again in a few "
                    "minutes.\r\n",
                    d);
        else
      ...
      PLR_FLAGS(d->character)
          .RemoveBits(PLR_WRITING, PLR_MAILING, PLR_EDITING, PLR_SPELL_CREATE,
                      PLR_CUSTOMIZE, PLR_PROJECT, PLR_MATRIX, ENDBIT);
      ...
      if (PLR_FLAGGED(d->character, PLR_AUTH))
        ...
        sprintf(buf, "%s [%s] has connected.", GET_CHAR_NAME(d->character),
                d->host);
  ...
  case CON_QVERIFYPW:
    ...
    if (STATE(d) == CON_CNFPASSWD) {
      ...
      if (dirty_password) { // STATE(d) is changed directly above this after
                            // all...
        char query_buf[2048];
#ifdef NOCRYPT
        char prepare_quotes_buf[2048];
  ...
  case CON_MENU: /* get selection from main menu  */
    ...
    case '1':
      ...
      if (PLR_FLAGGED(d->character, PLR_JUST_DIED)) {
        ...
        // TODO: If you died and then hit 1, your old character's data is leaked
        // here.
        ...
        if (PLR_FLAGGED(d->character, PLR_NEWBIE)) {
          ...
          GET_PHYSICAL(d->character) =
              (int)(GET_MAX_PHYSICAL(d->character) * .4);
      ...
      if (GET_LOADROOM(d->character) == RM_NEWBIE_LOADROOM &&
          !PLR_FLAGGED(d->character, PLR_NEWBIE))
      ...
      if (IS_SENATOR(d->character) &&
          (load_room <= 0 || load_room == real_room(mortal_start_room)))
        load_room = real_room(GET_LOADROOM(d->character));

      if (load_room < 0) {
        sprintf(buf,
                "SYSERR: Character %s is loading in with invalid load room %ld "
                "(%ld). Changing to Grog's place (35500).",
                GET_CHAR_NAME(d->character), GET_LOADROOM(d->character),
                load_room);
      ...
      if (ROOM_FLAGGED(&world[load_room], ROOM_HOUSE) &&
          !House_can_enter(d->character, world[load_room].number))
      ...
      if (load_room == NOWHERE) {
        if (PLR_FLAGGED(d->character, PLR_NEWBIE))
          load_room =
              real_room(RM_NEWBIE_LOADROOM); // The Neophyte Hotel (previously
                                             // 8039 which does not exist)
        else
          load_room = real_room(mortal_start_room);
      }
  ...
  case CON_QDELCONF2:
    if (!strcmp(arg, "yes") || !strcmp(arg, "YES")) {
      ...
      sprintf(buf, "Character '%s' deleted!\r\nGoodbye.\r\n",
              GET_CHAR_NAME(d->character));
      SEND_TO_Q(buf, d);
      sprintf(buf, "%s (lev %d) has self-deleted.", GET_CHAR_NAME(d->character),
