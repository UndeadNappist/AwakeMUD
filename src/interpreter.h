...
/*
#define ACMD(name)  \
   void (name)(struct char_data *ch, __attribute__ ((unused)) char *argument,
__attribute__ ((unused)) int cmd, __attribute__ ((unused)) int subcmd)
*/

#define ACMD_DECLARE(name)                                                     \
  void(name)(struct char_data * ch, char *argument, int cmd, int subcmd)
#define ACMD(name)                                                             \
  static void impl_##name##_(struct char_data *ch, char *argument, int cmd,    \
                             int subcmd);                                      \
  void(name)(struct char_data * ch, char *argument, int cmd, int subcmd) {     \
...
struct alias {
  ...
  int type;
  struct alias *next;
  alias() : command(NULL), replacement(NULL), next(NULL) {}
};
#endif
