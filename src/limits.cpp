...
extern void damage_equip(struct char_data *ch, struct char_data *victim,
                         int power, int type);
...
void gain_condition(struct char_data *ch, int condition, int value) {
  ...
  if (GET_COND(ch, condition) || PLR_FLAGGED(ch, PLR_CUSTOMIZE) ||
      ...
      PLR_FLAGGED(ch, PLR_AUTH) || PLR_FLAGGED(ch, PLR_MATRIX))
    return;
...
void remove_patch(struct char_data *ch) {
  ...
  case 1:
    act("The effects of $p wear off, leaving you exhausted!", FALSE, ch, patch,
        0, TO_CHAR);
  ...
  case 2:
    stun = resisted_test(
        GET_OBJ_VAL(patch, 1),
        GET_REAL_BOD(ch) - (GET_BIOOVER(ch) > 0 ? GET_BIOOVER(ch) / 2 : 0),
        GET_REAL_BOD(ch) - (GET_BIOOVER(ch) > 0 ? GET_BIOOVER(ch) / 2 : 0),
        GET_OBJ_VAL(patch, 1));
    if (stun > 0) {
      act("You feel the drugs from $p take effect.", FALSE, ch, patch, 0,
          TO_CHAR);
    ...
    break;
  case 3:
    if (success_test(GET_REAL_BOD(ch) -
                         (GET_BIOOVER(ch) > 0 ? GET_BIOOVER(ch) / 2 : 0),
                     GET_OBJ_VAL(patch, 1)) > 0)
  ...
  for (ch = character_list; ch; ch = next) {
    ...
    if (IS_NPC(ch) && ch->desc && ch->desc->original) {
      ...
      if (!(IS_SENATOR(ch) || IS_WORKING(ch) ||
            PLR_FLAGGED(ch, PLR_NO_IDLE_OUT)) ||
          !ch->desc) {
        ...
        /* Disabled-- I get protecting them by moving them to the void, but why
        DC them? else if (ch->char_specials.timer > 30) { if (ch->in_room)
            char_from_room(ch);
          char_to_room(ch, &world[1]);
          if (GET_QUEST(ch))
            end_quest(ch);
        ...
        */
        ...
        extract_char(ch);
                 ...
                 GET_INVIS_LEV(ch) < 2 && PRF_FLAGGED(ch, PRF_AUTOINVIS))
...
void process_regeneration(int half_hour) {
  ...
  for (ch = character_list; ch; ch = next_char) {
    ...
    if (GET_POS(ch) == POS_MORTALLYW && !AFF_FLAGS(ch).IsSet(AFF_STABILIZE) &&
        half_hour) {
      bool dam = TRUE;
  ...
  for (int i = 0; i <= top_of_world; i++) {
    ...
    if (world[i].background[CURRENT_BACKGROUND_COUNT] &&
        world[i].background[CURRENT_BACKGROUND_TYPE] >= AURA_BLOOD_MAGIC)
  ...
  for (i = character_list; i; i = next_char) {
    ...
    if (!IS_NPC(i)) {
      ...
      if (!GET_POWER(i, ADEPT_SUSTENANCE) || !(time_info.hours % 3)) {
        // Leave this so that people's stomachs empty over time (can eat/drink
      ...
      if (SHOTS_FIRED(i) >= 10000 && !SHOTS_TRIGGERED(i) && !number(0, 3)) {
        SHOTS_TRIGGERED(i)++;
        send_to_char("You feel you could benefit with some time at a shooting "
                     "range.\r\n",
                     i);
      }
      if (GET_MAG(i) > 0) {
        ...
        for (int x = 0; x < NUM_WEARS; x++)
          if (GET_EQ(i, x) && GET_OBJ_TYPE(GET_EQ(i, x)) == ITEM_FOCUS &&
              GET_OBJ_VAL(GET_EQ(i, x), 2) == GET_IDNUM(i) &&
              GET_OBJ_VAL(GET_EQ(i, x), 4)) {
            force += GET_OBJ_VAL(GET_EQ(i, x), 1);
            total++;
          }
        if (force * 100 > GET_REAL_MAG(i) * 2 &&
            success_test(GET_REAL_MAG(i) / 100, force / 2) < 1) {
          int num = number(1, total);
          struct obj_data *foci = NULL;
          for (int x = 0; x < NUM_WEARS && !foci; x++)
            if (GET_EQ(i, x) && GET_OBJ_TYPE(GET_EQ(i, x)) == ITEM_FOCUS &&
                GET_OBJ_VAL(GET_EQ(i, x), 2) == GET_IDNUM(i) &&
                GET_OBJ_VAL(GET_EQ(i, x), 4) && !--num)
              foci = GET_EQ(i, x);
          if (foci) {
            send_to_char(
                i, "You feel some of your magic becoming locked in %s.\r\n",
                GET_OBJ_NAME(foci));
      ...
      if (HUNTING(i) && !AFF_FLAGGED(i, AFF_TRACKING) &&
          ++HOURS_SINCE_TRACK(i) > 8)
        HUNTING(i) = NULL;
      if (i->bioware)
        check_bioware(i);
      for (int x = 0; x < NUM_DRUGS; x++) {
        ...
        if (GET_DRUG_ADDICT(i, x) > 0) {
          ...
          if (!(GET_DRUG_ADDTIME(i, x) % 168) && GET_REAL_BOD(i) == 1) {
            ...
            send_to_char(
                i, "Your health suffers at the hand of your %s addiction.\r\n",
                drug_types[x].name);
          }
          if (!(GET_DRUG_ADDTIME(i, x) % 720) &&
              ((drug_types[x].mental_addiction &&
                success_test(GET_WIL(i), drug_types[x].mental_addiction +
                                             GET_DRUG_EDGE(i, x)) < 1) ||
               (drug_types[x].physical_addiction &&
                success_test(GET_BOD(i), drug_types[x].physical_addiction +
                                             GET_DRUG_EDGE(i, x)) < 1))) {
            if (GET_REAL_BOD(i) > 1)
              GET_REAL_BOD(i)--;
            else if (GET_PERM_BOD_LOSS(i) <
                     racial_limits[(int)GET_RACE(i)][0][0])
              GET_PERM_BOD_LOSS(i)++;
            send_to_char(
                i, "Your health suffers at the hand of your %s addiction.\r\n",
                drug_types[x].name);
          ...
          if (AFF_FLAGGED(i, AFF_WITHDRAWAL)) {
            ...
            send_to_char(i, "Your body cries out for some %s.\r\n",
                         drug_types[x].name);
          } else if (tsl >= drug_types[x].fix + 1 &&
                     !AFF_FLAGGED(i, AFF_WITHDRAWAL_FORCE)) {
            send_to_char(i, "You begin to go into %s withdrawal.\r\n",
                         drug_types[x].name);
            AFF_FLAGS(i).SetBit(AFF_WITHDRAWAL_FORCE);
            affect_total(i);
          } else if (tsl >= drug_types[x].fix) {
            if (drug_types[x].mental_addiction
                    ? success_test(GET_WIL(i), drug_types[x].mental_addiction +
                                                   GET_DRUG_EDGE(i, x))
                    : 1 < 1 || drug_types[x].physical_addiction
                          ? success_test(GET_REAL_BOD(i),
                                         drug_types[x].physical_addiction +
                                             GET_DRUG_EDGE(i, x))
                          : 1 < 1) {
              if (AFF_FLAGGED(i, AFF_WITHDRAWAL_FORCE)) {
                sprintf(buf,
                        "Your lack of %s is causing you great pain and "
                        "discomfort.\r\n",
                        drug_types[x].name);
              ...
              for (struct obj_data *obj = i->carrying; obj;
                   obj = obj->next_content) {
                if (GET_OBJ_TYPE(obj) == ITEM_DRUG &&
                    GET_OBJ_VAL(obj, 0) == x) {
                  ...
                  if (GET_DRUG_DOSE(i) > GET_DRUG_TOLERANT(i, x))
                    ...
                    sprintf(buf,
                            "You attempt to satisfy your craving for %s.\r\n",
                            drug_types[x].name);
                  break;
                }
              }
    ...
    if (IS_PROJECT(i)) {
      if (AFF_FLAGGED(i->desc->original, AFF_TRACKING) &&
          HUNTING(i->desc->original) &&
          !--HOURS_LEFT_TRACK(i->desc->original)) {
        act("The astral signature leads you to $N.", FALSE, i, 0,
            HUNTING(i->desc->original), TO_CHAR);
        char_from_room(i);
      ...
      GET_ESS(i) -= 100;
      if (GET_ESS(i) <= 0) {
        send_to_char("As you feel the attachment to your physical body fade "
                     "you quickly return.\r\n",
                     i);
        PLR_FLAGS(i->desc->original).RemoveBit(PLR_PROJECT);
        i->desc->character = i->desc->original;
        i->desc->original = NULL;
        GET_PHYSICAL(i->desc->character) =
            -(GET_BOD(i->desc->character) - 1) * 100;
        ...
        send_to_char(
            "You feel memories of your physical body slipping away.\r\n", i);
    }
      ...
      if (shop_table[shop_nr].order) {
        ...
        for (struct shop_order_data *order = shop_table[shop_nr].order; order;
          ...
          fprintf(fl,
                  ...
                  "\t\tPrice:\t%d\n"
                  "\t\tSent:\t%d\n",
                  order->item, order->player, order->timeavail, order->number,
                  order->price, order->sent);
        }
        fclose(fl);
      }
    }
...
// Returns TRUE if vehicle is in one of the Junkyard vehicle-depositing rooms,
// FALSE otherwise.
...
void save_vehicles(void) {
  ...
  for (veh = veh_list; veh; veh = veh->next)
    if ((veh->owner > 0 &&
         (veh->damage < 10 || !veh_is_in_junkyard(veh) || veh->in_veh ||
          ROOM_FLAGGED(veh->in_room, ROOM_GARAGE))) &&
        (does_player_exist(veh->owner)))
      num_veh++;

  if (!(fl = fopen("veh/vfile", "w"))) {
    mudlog("SYSERR: Can't Open Vehicle File For Write.", NULL, LOG_SYSLOG,
           FALSE);
    return;
  }
  fprintf(fl, "%d\n", num_veh);
  ...
  for (veh = veh_list, v = 0; veh && v < num_veh; veh = veh->next) {
    ...
    if ((veh->damage >= 10 &&
         !(veh->in_veh || ROOM_FLAGGED(veh->in_room, ROOM_GARAGE)))) {
      // If the vehicle is wrecked and is in neither a containing vehicle nor a
      // garage...
      if (veh_is_in_junkyard(veh)) {
        // If it's already in the junkyard, we don't save it-- they should have
        // come and fixed it.
        continue;
      } else {
    ...
    /* Disabling this code-- we want to save ownerless vehicles so that they can
    disgorge their contents when they load in next. if
    (!does_player_exist(veh->owner)) { veh->owner = 0; continue;
    }
     */
    ...
    if (veh->sub)
      for (i = character_list; i; i = i->next)
        if (GET_IDNUM(i) == veh->owner) {
          found = FALSE;
          for (struct veh_data *f = i->char_specials.subscribe; f;
               f = f->next_sub)
            if (f == veh)
              found = TRUE;
          if (!found) {
    ...
    if (send_veh_to_junkyard) {
      ...
      // If veh is not in a garage (or the owner is not allowed to enter the
      // house anymore), send it to a garage.
  ...
  if (!(fl = fopen("etc/consist", "w"))) {
    mudlog("SYSERR: Can't Open Consistency File For Write.", NULL, LOG_SYSLOG,
           FALSE);
    return;
  }
  for (int m = 0; m < 5; m++) {
...
void misc_update(void) {
  ...
  for (ch = character_list; ch; ch = next_ch) {
        ...
        if (sus && sus->caster && spells[sus->spell].duration == PERMANENT)
          if (++sus->time >=
            ...
            if (next && sus->idnum == next->idnum)
              next = next->next;
            if (sus->other)
              for (struct sustain_data *vsus = GET_SUSTAINED(sus->other); vsus;
                   vsus = nsus) {
                nsus = vsus->next;
                if (vsus->other == ch && vsus->idnum == sus->idnum &&
                    (sus->other == ch ? vsus != sus : 1)) {
    ...
    if (affected_by_spell(ch, SPELL_CONFUSION) ||
        affected_by_spell(ch, SPELL_CHAOS) ||
        affected_by_power(ch, CONFUSION)) {
      if (number(1, 10) >= 5) {
        switch (number(0, 10)) {
        case 0:
        ...
        case 4:
          ...
          break;
        case 5:
          send_to_char("A large troll carrying a panther assault cannon "
    ...
    if (!IS_NPC(ch)) {
      ...
      if (GET_DRUG_DOSE(ch) && --GET_DRUG_DURATION(ch) < 0 &&
          !GET_DRUG_STAGE(ch)) {
        bool physical = TRUE;
        if (GET_DRUG_AFFECT(ch) == DRUG_HYPER ||
            GET_DRUG_AFFECT(ch) == DRUG_BURN)
          physical = FALSE;
        if (drug_types[GET_DRUG_AFFECT(ch)].level > 0) {
          dam = drug_types[GET_DRUG_AFFECT(ch)].level +
                (GET_DRUG_DOSE(ch) > 1 ? 1 : 0);
          power = drug_types[GET_DRUG_AFFECT(ch)].power +
                  (GET_DRUG_DOSE(ch) > 2 ? GET_DRUG_DOSE(ch) - 2 : 0);
        ...
        case DRUG_ACTH:
          ...
          break;
        case DRUG_HYPER:
          sprintf(buf, "The world seems to swirl around you as your mind is "
                       "bombarded with feedback.\r\n");
        ...
        case DRUG_KAMIKAZE:
          sprintf(
              buf,
              "Your body feels alive with energy and the desire to fight.\r\n");
          GET_DRUG_DURATION(ch) = 100 * srdice();
          break;
        case DRUG_PSYCHE:
        ...
        case DRUG_BLISS:
          sprintf(
              buf,
              "The world fades into bliss as your body becomes sluggish.\r\n");
        ...
        case DRUG_CRAM:
          ...
          break;
        case DRUG_NITRO:
          sprintf(buf, "You lose sense of yourself as your entire body comes "
                       "alive with energy.\r\n");
          GET_DRUG_DURATION(ch) = 100 * srdice();
          break;
        case DRUG_NOVACOKE:
          ...
          break;
        case DRUG_ZEN:
          sprintf(buf, "You start to loose your sense of reality as your sight "
                       "fills with hallucinations.\r\n");
          GET_DRUG_DURATION(ch) = 100 * srdice();
          break;
        }
        send_to_char(buf, ch);
        ...
        int toxin = 0;
        for (struct obj_data *obj = ch->bioware; obj && !toxin;
             obj = obj->next_content)
        ...
        case DRUG_JAZZ:
          GET_DRUG_DURATION(ch) = 100 * srdice();
          damage(ch, ch,
                 convert_damage(stage(
                     -success_test(GET_REAL_BOD(ch) - (GET_BIOOVER(ch) > 0
                                                           ? GET_BIOOVER(ch) / 2
                                                           : 0),
                                   8 - toxin),
                     LIGHT)),
        ...
        case DRUG_KAMIKAZE:
          ...
          break;
        case DRUG_CRAM:
          GET_DRUG_DURATION(ch) =
              MAX(1, 12 - GET_REAL_BOD(ch) -
                         (GET_BIOOVER(ch) > 0 ? GET_BIOOVER(ch) / 2 : 0)) *
              600;
          break;
        case DRUG_NITRO:
        ...
        if (drug_types[GET_DRUG_AFFECT(ch)].tolerance) {
          if (GET_DRUG_DOSES(ch, GET_DRUG_AFFECT(ch)) == 1 &&
              success_test(GET_REAL_BOD(ch),
                           drug_types[GET_DRUG_AFFECT(ch)].tolerance) < 1)
            GET_DRUG_TOLERANT(ch, GET_DRUG_AFFECT(ch))++;
          if (((!GET_DRUG_ADDICT(ch, GET_DRUG_AFFECT(ch)) &&
                !(drug_types[GET_DRUG_AFFECT(ch)].edge_preadd %
                  ++GET_DRUG_DOSES(ch, GET_DRUG_AFFECT(ch)))) ||
               (GET_DRUG_ADDICT(ch, GET_DRUG_AFFECT(ch)) &&
                !(drug_types[GET_DRUG_AFFECT(ch)].edge_posadd %
                  ++GET_DRUG_DOSES(ch, GET_DRUG_AFFECT(ch))))) &&
              success_test(GET_REAL_BOD(ch),
                           drug_types[GET_DRUG_AFFECT(ch)].tolerance +
                               GET_DRUG_EDGE(ch, GET_DRUG_AFFECT(ch))))
            GET_DRUG_TOLERANT(ch, GET_DRUG_AFFECT(ch))++;
        }
      ...
      if (!ch->desc && GET_MOB_VNUM(ch) >= 20 && GET_MOB_VNUM(ch) <= 22) {
        act("$n dissolves into the background and is no more.", TRUE, ch, 0, 0,
            TO_ROOM);
        ...
        if (!check_spirit_sector(ch->in_room, GET_SPARE1(ch))) {
          act("Being away from its environment, $n suddenly ceases to exist.",
              TRUE, ch, 0, 0, TO_ROOM);
    ...
    if (ch->points.fire[0] > 0) {
      if (ch->in_room->sector_type != SPIRIT_HEARTH &&
      ...
      int dam = convert_damage(
          stage(-success_test(GET_BOD(ch) +
                                  GET_POWER(ch, ADEPT_TEMPERATURE_TOLERANCE),
                              6 + ch->points.fire[1]++ - GET_IMPACT(ch)),
                MODERATE));
      ch->points.fire[1]++;
