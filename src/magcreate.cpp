...
void spedit_disp_menu(struct descriptor_data *d) {
  ...
  if (GET_OBJ_VAL(SPELL, 1) == SPELL_INCATTR ||
      GET_OBJ_VAL(SPELL, 1) == SPELL_INCCYATTR ||
      GET_OBJ_VAL(SPELL, 1) == SPELL_DECATTR ||
      GET_OBJ_VAL(SPELL, 1) == SPELL_DECCYATTR)
...
void spedit_parse(struct descriptor_data *d, const char *arg) {
  ...
  case SPEDIT_MENU:
    ...
    case '4':
      if (!GET_OBJ_VAL(SPELL, 9))
        send_to_char("You must select a category first. Enter your choice: ",
                     CH);
      else
        spedit_disp_spell_type(d);
      break;
    case '5':
    ...
    case '6':
      ...
      // Explicit fallthrough-- you can only select an attribute for
      // attribute-linked spells. fall through
    default:
  ...
  case SPEDIT_NAME:
    ...
    break;
  case SPEDIT_FORCE:
    x = MIN(d->edit_number2, GET_SKILL(CH, SKILL_SPELLDESIGN)
                                 ? GET_SKILL(CH, SKILL_SPELLDESIGN)
...
void spell_design(struct char_data *ch, struct obj_data *formula) {
  ...
  if (GET_OBJ_VAL(formula, 8) != GET_IDNUM(ch)) {
    send_to_char("You don't understand where the creator of this spell is "
                 "coming from.\r\n",
                 ch);
    return;
  }
  ...
  if (ch->in_veh) {
    for (lib = ch->in_veh->contents; lib; lib = lib->next_content)
      if (GET_OBJ_TYPE(lib) == ITEM_MAGIC_TOOL &&
          GET_OBJ_VAL(lib, 1) >= GET_OBJ_VAL(formula, 0) &&
          ((GET_TRADITION(ch) == TRAD_SHAMANIC &&
            GET_OBJ_VAL(lib, 0) == TYPE_LODGE &&
            GET_OBJ_VAL(lib, 3) == GET_IDNUM(ch)) ||
           (GET_TRADITION(ch) != TRAD_SHAMANIC &&
            GET_OBJ_VAL(lib, 0) == TYPE_LIBRARY_SPELL)))
        break;
  } else {
  ...
  if (!lib) {
    send_to_char(
        "You don't have the right tools here to design that spell.\r\n", ch);
    return;
  }
  ...
  if (GET_OBJ_VAL(formula, 6))
    ...
    if (skill < GET_OBJ_VAL(formula, 0)) {
      send_to_char(
          "You don't have the necessary skills to design this spell.\r\n", ch);
      return;
    }
    ...
    if (spells[GET_OBJ_VAL(formula, 1)].category == DETECTION ||
        spells[GET_OBJ_VAL(formula, 1)].category == ILLUSION)
      target--;
    if (spells[GET_OBJ_VAL(formula, 1)].physical)
      target++;
    if (spells[GET_OBJ_VAL(formula, 1)].duration == SUSTAINED)
      target++;
    if (GET_OBJ_VAL(formula, 1) == SPELL_DETOX ||
        GET_OBJ_VAL(formula, 1) == SPELL_RESISTPAIN)
      target -= 2;
    if (GET_OBJ_VAL(formula, 1) == SPELL_CLOUT ||
        GET_OBJ_VAL(formula, 1) == SPELL_STUNBALL ||
    ...
    if (GET_TRADITION(ch) == TRAD_SHAMANIC)
      ...
      for (struct spirit_data *spir = GET_SPIRIT(ch);
           spir && skill == startskill; spir = spir->next)
        if (spir->called) {
