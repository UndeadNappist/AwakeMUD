...
/* external structs */
extern void resist_drain(struct char_data *ch, int power, int drain_add,
                         int wound);
extern int find_sight(struct char_data *ch);
extern int find_weapon_range(struct char_data *ch, struct obj_data *weapon);

extern void cast_detection_spell(struct char_data *ch, int spell, int force,
                                 char *arg, char_data *mob);
extern void cast_manipulation_spell(struct char_data *ch, int spell, int force,
                                    char *arg, char_data *mob);
extern void cast_illusion_spell(struct char_data *ch, int spell, int force,
                                char *arg, char_data *mob);
extern void cast_health_spell(struct char_data *ch, int spell, int sub,
                              int force, char *arg, char_data *mob);
...
#define MOB_AGGR_TO_RACE                                                       \
  MOB_AGGR_ELF, MOB_AGGR_DWARF, MOB_AGGR_HUMAN, MOB_AGGR_ORK, MOB_AGGR_TROLL
...
void mobact_change_firemode(struct char_data *ch) {
  ...
  // For some reason, NPCs were set to prefer burst fire above all other modes.
  // Preserving this logic for now.
  if (IS_SET(GET_OBJ_VAL(weapon, 10), 1 << MODE_BF)) {
    mode_count += 1;
    GET_OBJ_VAL(weapon, 11) = MODE_BF;
  }

  // Send a message to the room, but only if the weapon has received a new fire
  // selection mode and has more than one available.
  if (prev_value != GET_OBJ_VAL(weapon, 11) && mode_count > 1) {
    act("$n flicks the fire selector switch on $p.", TRUE, ch, weapon, 0,
        TO_ROOM);
  }
}

// Check if an NPC successfully fires a spec-proc. Returns TRUE if so, FALSE
// otherwise.
...
bool mobact_process_in_vehicle_guard(struct char_data *ch) {
  ...
  // Precondition: Vehicle must exist, we must be manning or driving, and we
  // must not be astral.
  if (!ch->in_veh ||
      !(AFF_FLAGGED(ch, AFF_PILOT) || AFF_FLAGGED(ch, AFF_MANNING)) ||
      IS_ASTRAL(ch))
    return FALSE;
  ...
  if (ROOM_FLAGGED(ch->in_veh->in_room, ROOM_PEACEFUL) ||
      !(MOB_FLAGGED(ch, MOB_GUARD)))
    return FALSE;

  /* Guard NPCs. */

  // If we're not in a road or garage, we expect to see no vehicles and will
  // attack any that we see.
  if (!(ROOM_FLAGGED(ch->in_veh->in_room, ROOM_ROAD) ||
        ROOM_FLAGGED(ch->in_veh->in_room, ROOM_GARAGE))) {
    for (tveh = ch->in_veh->in_room->vehicles; tveh; tveh = tveh->next_veh) {
      // No attacking your own vehicle.
      if (tveh == ch->in_veh)
        continue;

      // TODO: Only attack player-owned vehicles and vehicles that have player
      // occupants or drivers.
  ...
  if (!tveh) {
    ...
    for (vict = ch->in_veh->in_room->people; vict; vict = vict->next_in_room) {
      // Skip over invalid targets (NPCs, no-hassle imms, invisibles, and
      // downed).
      if (IS_NPC(vict) || PRF_FLAGGED(vict, PRF_NOHASSLE) ||
          !CAN_SEE_ROOM_SPECIFIED(ch, vict, ch->in_veh->in_room) ||
          GET_PHYSICAL(vict) <= 0)
        continue;
      ...
      for (int i = 0; i < NUM_WEARS; i++) {
        ...
        if (GET_EQ(vict, i) &&
            violates_zsp(GET_SECURITY_LEVEL(ch->in_veh->in_room), vict, i,
                         ch)) {
  ...
  sprintf(buf, "guard %s: ch = %s, tveh = %s, vict = %s",
          AFF_FLAGGED(ch, AFF_PILOT) ? "driver" : "gunner", GET_NAME(ch),
          tveh ? tveh->name : "none", vict ? GET_NAME(vict) : "none");
  ...
  // Dakka o'clock.
    ...
    mudlog("SYSERR: Could not find mount manned by NPC for "
           "mobact_process_in_vehicle_aggro().",
...
bool mobact_process_in_vehicle_aggro(struct char_data *ch) {
  ...
  // Target selection. We disallow targeting of unowned vehicles so our guards
  // don't Thunderdome each other before players even show up.
  for (tveh = ch->in_veh->in_room->vehicles; tveh; tveh = tveh->next_veh) {
  ...
  if (!tveh) {
    // If we've gotten here, character is either astral or is not willing to /
    // failed to attack a vehicle.
    for (vict = ch->in_veh->in_room->people; vict; vict = vict->next_in_room) {
      // Skip conditions: Invisible, no-hassle, already downed, or is an NPC who
      // is neither a player's astral body nor a player's escortee.
      if ((IS_NPC(vict) && !IS_PROJECT(vict) && !is_escortee(vict)) ||
          !CAN_SEE_ROOM_SPECIFIED(ch, vict, ch->in_veh->in_room) ||
          PRF_FLAGGED(vict, PRF_NOHASSLE) || GET_PHYSICAL(vict) <= 0)
        continue;
      ...
      if (!MOB_FLAGS(ch).AreAnySet(MOB_AGGR_TO_RACE, ENDBIT) ||
          ...
          (MOB_FLAGGED(ch, MOB_AGGR_ELF) &&
           (GET_RACE(vict) == RACE_ELF || GET_RACE(vict) == RACE_WAKYAMBI ||
            GET_RACE(vict) == RACE_NIGHTONE || GET_RACE(vict) == RACE_DRYAD)) ||
          // If NPC is aggro towards dwarves, and victim is a dwarf subrace,
          // or...
          (MOB_FLAGGED(ch, MOB_AGGR_DWARF) &&
           (GET_RACE(vict) == RACE_DWARF || GET_RACE(vict) == RACE_GNOME ||
            GET_RACE(vict) == RACE_MENEHUNE ||
            GET_RACE(vict) == RACE_KOBOROKURU)) ||
          ...
          (MOB_FLAGGED(ch, MOB_AGGR_ORK) &&
           (GET_RACE(vict) == RACE_ORK || GET_RACE(vict) == RACE_HOBGOBLIN ||
            GET_RACE(vict) == RACE_OGRE || GET_RACE(vict) == RACE_SATYR ||
            GET_RACE(vict) == RACE_ONI)) ||
          // If NPC is aggro towards trolls, and victim is a troll subrace:
          (MOB_FLAGGED(ch, MOB_AGGR_TROLL) &&
           (GET_RACE(vict) == RACE_TROLL || GET_RACE(vict) == RACE_CYCLOPS ||
            GET_RACE(vict) == RACE_FOMORI || GET_RACE(vict) == RACE_GIANT ||
            GET_RACE(vict) == RACE_MINOTAUR)))
      // Kick their ass.
      {
        break;
      }
  ...
  sprintf(buf, "aggro %s: ch = %s, tveh = %s, vict = %s",
          AFF_FLAGGED(ch, AFF_PILOT) ? "driver" : "gunner", GET_NAME(ch),
          tveh ? tveh->name : "none", vict ? GET_NAME(vict) : "none");
  mudlog(buf, ch, LOG_SYSLOG, TRUE);
...
bool mobact_process_aggro(struct char_data *ch, struct room_data *room) {
  ...
  if (!ROOM_FLAGGED(room, ROOM_PEACEFUL) &&
    ...
    // If I am not astral, am in the same room, and am willing to attack a
    // vehicle this round (coin flip), pick a fight with a vehicle.
...
bool mobact_process_memory(struct char_data *ch, struct room_data *room) {
  ...
  if (MOB_FLAGGED(ch, MOB_MEMORY) && MEMORY(ch)) {
    for (vict = room->people; vict; vict = vict->next_in_room) {
      ...
      if (memory(ch, vict)) {
        act("'Hey! You're the fragger that attacked me!!!', exclaims $n.",
...
bool mobact_process_helper(struct char_data *ch) {
  ...
  if (MOB_FLAGGED(ch, MOB_HELPER)) {
    for (vict = ch->in_room->people; vict; vict = vict->next_in_room) {
      // Ensure we're neither of the fighting parties. This check should be
      // redundant since no fighting NPC can proceed through mobile_activity().
      if (ch == vict || !FIGHTING(vict) || ch == FIGHTING(vict))
        continue;

      // If victim is an NPC who is fighting a player, and I can see the player,
      // assist the NPC.
      if (IS_NPC(vict) && !IS_NPC(FIGHTING(vict)) &&
        ...
        // The player is not in my room, so I have to do a ranged response.
          ...
          if (ranged_response(FIGHTING(vict), ch)) {
            // TODO: This doesn't fire a message if the NPC is wielding a ranged
            // weapon.
            act("$n jumps to $N's aid against $S distant attacker!", FALSE, ch,
                0, vict, TO_ROOM);
          }
        }
      ...
      // If the victim is a player who is fighting an NPC, and I can see the
      // player, assist the NPC.
      if (!IS_NPC(vict) && IS_NPC(FIGHTING(vict)) && CAN_SEE(ch, vict)) {
        // A player in my area is attacking an NPC.
        act("$n jumps to the aid of $N!", FALSE, ch, 0, FIGHTING(vict),
            TO_ROOM);
        stop_fighting(ch);
...
bool mobact_process_guard(struct char_data *ch, struct room_data *room) {
  ...
  if (MOB_FLAGGED(ch, MOB_GUARD)) {
    ...
    if (ch->in_room == room) {
      for (veh = room->vehicles; veh; veh = veh->next_veh) {
        // If the room we're in is neither a road nor a garage, attack any
        // vehicles we see. NOTE: Previous logic required that the vehicle be
        // damaged to be a valid attack target.
        if (!(ROOM_FLAGGED(room, ROOM_ROAD) ||
              ROOM_FLAGGED(room, ROOM_GARAGE))) {
          // TODO: Only attack player-owned vehicles and vehicles that have
          // player occupants or drivers.
    ...
    for (vict = room->people; vict; vict = vict->next_in_room) {
      ...
      for (i = 0; i < NUM_WEARS; i++) {
        // If victim's equipment is illegal here, blast them.
        if (GET_EQ(vict, i) &&
            violates_zsp(GET_SECURITY_LEVEL(room), vict, i, ch)) {
          stop_fighting(ch);
          set_fighting(ch, vict);
          return true;
        }
...
bool mobact_process_self_buff(struct char_data *ch) {
  if (GET_SKILL(ch, SKILL_SORCERY) && !MOB_FLAGGED(ch, MOB_SPEC)) {
    ...
    if (GET_PHYSICAL(ch) < GET_MAX_PHYSICAL(ch) && !AFF_FLAGGED(ch, AFF_HEALED))
      cast_health_spell(ch, SPELL_HEAL, 0, number(1, GET_MAG(ch) / 100), NULL,
                        ch);
    ...
    if (number(0, 15) == 0) {
      ...
      if (!affected_by_spell(ch, SPELL_ARMOUR)) {
        cast_manipulation_spell(ch, SPELL_ARMOUR, number(1, GET_MAG(ch) / 100),
                                NULL, ch);
      }

      // If not invisible already, apply an invisibility spell based on my magic
      // rating and sorcery skill.
      else if (!affected_by_spell(ch, SPELL_INVIS) &&
               !affected_by_spell(ch, SPELL_IMP_INVIS)) {
        if (MIN(GET_SKILL(ch, SKILL_SORCERY), GET_MAG(ch) / 100) <= 5) {
          // Lower skill means standard invisibility. Gotta make thermographic
          // vision useful somehow.
          cast_illusion_spell(ch, SPELL_INVIS, number(1, GET_MAG(ch) / 100),
                              NULL, ch);
        } else {
          // Look out, we've got a badass over here.
          cast_illusion_spell(ch, SPELL_IMP_INVIS, number(1, GET_MAG(ch) / 100),
                              NULL, ch);
        }
      }
      ...
      // Apply combat sense to self.
        ...
        cast_detection_spell(ch, SPELL_COMBATSENSE,
                             number(1, GET_MAG(ch) / 100), NULL, ch);
      }
      ...
      // We're dead-set on casting a spell, so try to boost attributes.
        ...
        switch (number(1, 3)) {
        case 1:
          cast_health_spell(ch, SPELL_INCATTR, STR,
                            number(1, GET_MAG(ch) / 100), NULL, ch);
          break;
        case 2:
          cast_health_spell(ch, SPELL_INCATTR, QUI,
                            number(1, GET_MAG(ch) / 100), NULL, ch);
          break;
        case 3:
          cast_health_spell(ch, SPELL_INCATTR, BOD,
                            number(1, GET_MAG(ch) / 100), NULL, ch);
          break;
        }
      }
...
bool mobact_process_movement(struct char_data *ch) {
  ...
  // NPC in a vehicle that's not in another vehicle? Have them drive and obey
  ...
  if (ch->in_veh) {
    ...
    for (int tries = 0; tries < 5; tries++) {
      ...
      if (EXIT(ch->in_veh, door) &&
          ROOM_FLAGS(EXIT(ch->in_veh, door)->to_room)
              .AreAnySet(ROOM_ROAD, ROOM_GARAGE, ENDBIT) &&
          !ROOM_FLAGS(EXIT(ch->in_veh, door)->to_room)
               .AreAnySet(ROOM_NOMOB, ROOM_DEATH, ENDBIT)) {
        // sprintf(buf3, "Driver %s attempted to move %s from %s [%ld].",
        // GET_NAME(ch), fulldirs[door], GET_ROOM_NAME(ch->in_veh->in_room),
        // GET_ROOM_VNUM(ch->in_veh->in_room));
        perform_move(ch, door, LEADER, NULL);
        // sprintf(ENDOF(buf3), "  After move, driver now in %s [%ld].",
        // GET_ROOM_NAME(ch->in_veh->in_room),
        // GET_ROOM_VNUM(ch->in_veh->in_room)); mudlog(buf3, ch, LOG_SYSLOG,
        // TRUE);
        return TRUE;
      }
    }
  ...
  // NPC not in a vehicle (walking).
    ...
    for (int tries = 0; tries < 5; tries++) {
      ...
      door = number(NORTH, DOWN);
      if (!CAN_GO(ch, door) || ROOM_FLAGS(EXIT(ch, door)->to_room)
                                   .AreAnySet(ROOM_NOMOB, ROOM_DEATH, ENDBIT))
        continue;

      // If their exit leads to a different zone, check if they're allowed to
      // wander.
...
void mobile_activity(void) {
  ...
  for (ch = character_list; ch; ch = next_ch) {
    ...
    // Skip them if they're a player character, are being possessed, are
    // sleeping, or have no current room.
    if (!IS_MOB(ch) || !AWAKE(ch) || ch->desc || !get_ch_in_room(ch))
      continue;

    // Skip NPCs that are currently fighting someone in their room, or are
    // fighting a vehicle.
    if ((FIGHTING(ch) &&
    ...
    // Look for special procedures. If one fires successfully, do nothing else
    // for this NPC.
    if (!no_specials && MOB_FLAGGED(ch, MOB_SPEC) &&
        mobact_evaluate_spec_proc(ch)) {
      continue;
    }
    ...
    // All these aggressive checks require the character to not be in a peaceful
    // room.
    if (!ROOM_FLAGGED(get_ch_in_room(ch), ROOM_PEACEFUL)) {
      ...
      if (!ch->in_veh) {
        ...
        // Sniper? Check surrounding players and re-apply above for each
        // applicable room.
        if (MOB_FLAGGED(ch, MOB_SNIPER) && GET_EQ(ch, WEAR_WIELD)) {
          // When this is true, we'll stop evaluating sniper and continue to
          // next NPC.
          bool has_acted = FALSE;

          // Calculate their maximum firing range (lesser of vision range and
          // weapon range).
          int max_distance = MIN(find_sight(ch),
                                 find_weapon_range(ch, GET_EQ(ch, WEAR_WIELD)));

          for (dir = 0; !has_acted && !FIGHTING(ch) && dir < NUM_OF_DIRS;
               dir++) {
            current_room = ch->in_room;

            // Check each room in a straight line until we are either out of
            // range or cannot go further.
            for (distance = 1; !has_acted && distance <= max_distance;
                 distance++) {
              // Exit must be valid, and room must belong to same zone as
              // character's room.
              if (CAN_GO2(current_room, dir) &&
                  EXIT2(current_room, dir)->to_room->zone ==
                      ch->in_room->zone) {
                current_room = EXIT2(current_room, dir)->to_room;
              } else {
                // If we can't get to a further room, stop and move to next
                // direction in for loop.
                break;
              }
    ...
    // Cast spells to buff self. NPC must have sorcery skill and must not have
    // any special procedures attached to it.
...
int violates_zsp(int security, struct char_data *ch, int pos,
  ...
  case WEAR_UNDER:
    if (GET_EQ(ch, WEAR_ABOUT) || GET_EQ(ch, WEAR_BODY))
      if (success_test(GET_INT(mob),
                       6 +
                           (GET_EQ(ch, WEAR_ABOUT)
                                ? GET_OBJ_VAL(GET_EQ(ch, WEAR_ABOUT), 7)
                                : 0) +
                           (GET_EQ(ch, WEAR_BODY)
                                ? GET_OBJ_VAL(GET_EQ(ch, WEAR_BODY), 7)
                                : 0)) < 2)
        return 0;
    break;
  case WEAR_LARM:
  ...
  case WEAR_BODY:
    if (GET_EQ(ch, WEAR_ABOUT))
      if (success_test(GET_INT(mob),
                       4 + GET_OBJ_VAL(GET_EQ(ch, WEAR_ABOUT), 7)) < 2)
        return 0;
    break;
  case WEAR_LEGS:
    if (GET_EQ(ch, WEAR_ABOUT))
      if (success_test(GET_INT(mob),
                       2 + GET_OBJ_VAL(GET_EQ(ch, WEAR_ABOUT), 7)) < 2)
        return 0;
    break;
  case WEAR_LANKLE:
  case WEAR_RANKLE:
      ...
      if (success_test(GET_INT(mob),
                       5 +
                           (GET_EQ(ch, WEAR_ABOUT)
                                ...
                                : 0) +
                           (GET_EQ(ch, WEAR_LEGS)
                                ? GET_OBJ_VAL(GET_EQ(ch, WEAR_LEGS), 7)
                                : 0)) < 2)
        return 0;
    break;
  }
...
bool attempt_reload(struct char_data *mob, int pos) {
  ...
  if (!(gun = GET_EQ(mob, pos)) ||
      GET_OBJ_TYPE(GET_EQ(mob, pos)) != ITEM_WEAPON)
    return FALSE;
