  ...
  for (k = 0; strlen(format) > (u_int)k && q < 1023; q++) {
    ...
    for (i = k, j = 0; format[i] && j < 79; i++)
      if (format[i] == '^' && (i < 1 || format[i - 1] != '^') &&
          format[i + 1]) {
        // Look for color code initializers; if they exist, bump the line count
        // longer.
        ...
        j++;
    for (i = MIN((unsigned)strlen(format) - 1, (unsigned)k + line);
         i > k && i > 0; i--)
      if (format[i] == '\n' && format[i - 1] == '\r') {
        k = i + 1;
        break;
    ...
    for (i = MIN((unsigned)strlen(format) - 1, (unsigned)k + line);
         i > k && i > 0; i--)
      if (isspace(format[i]) && isprint(format[i - 1]) &&
          !isspace(format[i - 1])) {
        format[i] = '\r';
        if (format[i + 1] != ' ')
...
// REPLACE_STRING swaps target with d->str if d->str exists (otherwise leaves
// target alone).
...
#define REPLACE_STRING(target)                                                 \
  (REPLACE_STRING_FORMAT_SPECIFIED(target, DONT_FORMAT_INDENT))
#define REPLACE_STRING_WITH_INDENTED_FORMATTING(target)                        \
  (REPLACE_STRING_FORMAT_SPECIFIED(target, DO_FORMAT_INDENT))
    ...
    if (STATE(d) == CON_DECK_CREATE && d->edit_mode == 1) {
      ...
      sprintf(
          buf,
          "INSERT INTO trideo_broadcast (author, message) VALUES (%ld, '%s')",
          GET_IDNUM(d->character),
          prepare_quotes(buf2, *d->str, sizeof(buf2) / sizeof(buf2[0])));
    ...
    if (d->mail_to >= BOARD_MAGIC) {
      ...
      if (IS_NPC(d->character))
        sprintf(tmp, "\r\n--%s (%s/%s%d-%s%d-%s%d)\r\n", GET_NAME(d->character),
                time, month < 10 ? "0" : "", month, day < 10 ? "0" : "", day,
                year < 10 ? "0" : "", year);
      else
        sprintf(tmp, "\r\n--%s (%s/%s%d-%s%d-%s%d)\r\n",
...
ACMD(do_spellset) {
  ...
  // Moved this to the top for quicker precondition failure (we don't care about
  // args if they can't set the person's values in the first place).
  if (IS_NPC(vict)) {
  ...
  if (GET_LEVEL(vict) > GET_LEVEL(ch)) {
    send_to_char(
        "You can't modify the spells of someone more powerful than you.\r\n",
        ch);
    return;
  }
  ...
  // Require that the attribute spells have an attribute specified (see
  // spell->subtype comment).
  if (spelltoset == SPELL_INCATTR || spelltoset == SPELL_INCCYATTR ||
      spelltoset == SPELL_DECATTR || spelltoset == SPELL_DECCYATTR) {
    // Check for the argument.
    if (!*argument) {
      send_to_char("You must supply one of 'bod', 'qui', 'str', 'int', 'wil', "
                   "'cha', 'rea'.\r\n",
                   ch);
      return;
    }

    // Compare against the new short_attributes consts in constants.cpp and find
    // the applicable one.
  ...
  // TODO: Checks for validity (if the character is unable to use the spell,
  // don't set it).
  // TODO: What if you want to remove a spell from someone?

  struct spell_data *spell = new spell_data;

  // Three lines away from working spellset command. How do I get the damn
  // values?

  /* spell->name is a pointer to a char, and other code (spell release etc)
     deletes what it points to on cleanup. Thus, you need to clone the
     spell_type's name into the spell_data to assign your new spell a name. */
  spell->name = str_dup(buf);

  /* The index of the spells[] table is a 1:1 mapping of the SPELL_ type, so you
     can just set the spell->type field to your spelltoset value. */
  spell->type = spelltoset;

  /* This one's the tricky one. Via newmagic.cpp's do_learn, we know that
     subtype is used for the attribute spells to decide which attribute is being
     increased/decreased. To find this, I've included an optional parameter in
     the command after force that specifies 'str', 'bod' etc. */
  ...
  send_to_char("OK.\r\n", ch);
  sprintf(buf, "$n has set your '%s' spell to Force %d.",
          spells[spelltoset].name, force);
...
ACMD(do_skillset) {
  ...
  sprintf(buf2, "%s changed %s's %s from %d to %d.", GET_CHAR_NAME(ch),
          GET_NAME(vict), skills[skill].name, GET_SKILL(vict, skill), value);
  mudlog(buf2, ch, LOG_WIZLOG, TRUE);

  send_to_char(
      vict,
      "Your skill in %s has been altered by the game's administration.\r\n",
      skills[skill].name);
...
void format_tabs(struct descriptor_data *d) {
  ...
  // If there were an even number of tabs, we assume they didn't fuck up and go
  // ahead and do a replacement.
  if (tabcount % 2 == 0) {
    for (unsigned long i = 0; i < strlen(*d->str) - 2; i++) {
      if (*(*d->str + i) == '\\' && *(*d->str + i + 1) == 't') {
        *(*d->str + i) = '\t';
