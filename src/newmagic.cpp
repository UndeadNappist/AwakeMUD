...
#define POWER(name)                                                            \
  void(name)(struct char_data * ch, struct char_data * spirit,                 \
             struct spirit_data * spiritdata, char *arg)
#define SPELLCASTING 0
#define CONJURING 1
#define FAILED_CAST "You fail to bind the mana to your will.\r\n"

extern void die(struct char_data *ch);
extern void damage_equip(struct char_data *ch, struct char_data *vict,
                         int power, int type);
extern void damage_obj(struct char_data *ch, struct obj_data *obj, int power,
                       int type);
...
void adept_release_spell(struct char_data *ch) {
  ...
  spell->spirit = NULL;
  for (struct sustain_data *ospell = GET_SUSTAINED(spell->other); ospell;
       ospell = ospell->next)
  ...
  send_to_char(
      spell->other,
      "You suddenly feel the burden of your %s spell fall back on to you.\r\n",
      buf);
}
...
void end_sustained_spell(struct char_data *ch, struct sustain_data *sust) {
  ...
  if (sust->other) {
    for (vsust = GET_SUSTAINED(sust->other); vsust; vsust = vsust->next)
      if (sust->caster != vsust->caster && vsust->other == ch &&
          vsust->idnum == sust->idnum) {
        if (vsust->spirit) {
    ...
    if (sust->spell == SPELL_INVIS || sust->spell == SPELL_IMP_INVIS)
      act("You blink and suddenly $n appears!", TRUE,
          sust->caster ? sust->other : ch, 0, 0, TO_ROOM);
  }
...
void totem_bonus(struct char_data *ch, int action, int type, int &target,
                 int &skill) {
  ...
  if (GET_TOTEM(ch) == TOTEM_OWL) {
    ...
    if (ROOM_FLAGGED(get_ch_in_room(ch), ROOM_INDOORS) ||
        SECT(get_ch_in_room(ch)) == SPIRIT_HEARTH)
      target += 1;
    ...
    target += 2;
  else if (GET_TOTEM(ch) == TOTEM_SCORPION &&
  ...
  if (action == SPELLCASTING) {
    ...
    case TOTEM_ELK:
    case TOTEM_POLECAT:
      if (type == SPIRIT_FOREST || type == SPIRIT_DESERT ||
          type == SPIRIT_MOUNTAIN || type == SPIRIT_PRAIRIE)
        skill += 2;
      break;
    case TOTEM_STAG:
    case TOTEM_PYTHON:
...
void end_spirit_existance(struct char_data *ch, bool message) {
  ...
  for (struct descriptor_data *d = descriptor_list; d; d = d->next) {
    ...
    if (tempc && GET_IDNUM(tempc) == GET_ACTIVE(ch)) {
      for (struct spirit_data *spirit = GET_SPIRIT(tempc); spirit;
           spirit = spirit->next)
      ...
      if (message && d->character)
        send_to_char(d->character,
                     "%s returns to the metaplanes, its existance in this "
                     "world finished.\r\n",
                     CAP(GET_NAME(ch)));
      break;
    }
...
void elemental_fulfilled_services(struct char_data *ch, struct char_data *mob,
                                  struct spirit_data *spirit) {
  if (spirit->services < 1 &&
      !(MOB_FLAGGED(mob, MOB_SPIRITGUARD) || MOB_FLAGGED(mob, MOB_STUDY) ||
        GET_SUSTAINED(mob) || GET_SUSTAINED_NUM(mob) || CH_IN_COMBAT(mob))) {
    send_to_char(ch,
                 "Its services fulfilled, %s departs to the metaplanes.\r\n",
                 CAP(GET_NAME(mob)));
...
bool conjuring_drain(struct char_data *ch, int force) {
  ...
  if ((GET_POS(ch) <= POS_STUNNED) && (GET_POS(ch) > POS_DEAD)) {
    if (CH_IN_COMBAT(ch))
      stop_fighting(ch);
    send_to_char("You are unable to resist the drain from conjuring and fall "
                 "unconscious!\r\n",
                 ch);
    ...
    if (CH_IN_COMBAT(ch))
      stop_fighting(ch);
    send_to_char("The energy from the conjuring ritual overloads your body "
                 "with energy, killing you...\r\n",
                 ch);
...
void magic_perception(struct char_data *ch, int force, int spell) {
  int base = 4 + (int)(GET_MAG(ch) / 100) - force, target = 0, skill = 0,
      starg = 0;
  ...
  for (struct char_data *vict = ch->in_veh ? ch->in_veh->people
                                           : ch->in_room->people;
       vict; vict = ch->in_veh ? vict->next_in_veh : vict->next_in_room) {
    if (ch == vict)
      continue;
    target = base;
    if (GET_MAG(vict) > 0)
    ...
    if (IS_DUAL(vict) || IS_ASTRAL(vict))
      target -= 2;
    if (success_test(GET_INT(ch) + (GET_TRADITION(ch) == TRAD_ADEPT
                                        ? GET_POWER(ch, ADEPT_IMPROVED_PERCEPT)
                                        : 0),
                     target)) {
      if (IS_DUAL(vict) || IS_ASTRAL(vict))
        act("You notice $n manipulating the astral plane.", FALSE, ch, 0, vict,
            TO_VICT);
      else
        act("You notice $n performing magic.", TRUE, ch, 0, vict, TO_VICT);
    }
  }
}
...
bool spell_drain(struct char_data *ch, int type, int force, int damage) {
  ...
  sprintf(buf, "Drain Test: F:%d%s T:%d Sk: %d", force, wound_name[damage],
          target, GET_WIL(ch) + GET_DRAIN(ch));
  ...
  if ((GET_POS(ch) <= POS_STUNNED) && (GET_POS(ch) > POS_DEAD)) {
    if (CH_IN_COMBAT(ch))
      stop_fighting(ch);
    send_to_char("You are unable to resist the drain from spell casting and "
                 "fall unconscious!\r\n",
                 ch);
    act("$n collapses unconscious!", FALSE, ch, 0, 0, TO_ROOM);
    ...
    if (CH_IN_COMBAT(ch))
      stop_fighting(ch);
    send_to_char(
        "The feedback from spell casting floods your body, killing you...\r\n",
        ch);
    act("$n suddenly collapses, dead!", FALSE, ch, 0, 0, TO_ROOM);
    die(ch);
    return TRUE;
...
struct char_data *find_target_at_range(struct char_data *ch, char *name,
                                       char *direction) {
  return NULL;
}
void create_sustained(struct char_data *ch, struct char_data *vict, int spell,
                      int force, int sub, int success, int drain) {
  struct obj_data *focus = NULL;
  for (int i = 0; !focus && i < NUM_WEARS; i++)
    if (GET_EQ(vict, i) && GET_OBJ_TYPE(GET_EQ(vict, i)) == ITEM_FOCUS &&
        !GET_OBJ_VAL(GET_EQ(vict, i), 4) &&
        GET_OBJ_VAL(GET_EQ(vict, i), 3) == spell &&
        GET_OBJ_VAL(GET_EQ(vict, i), 1) >= force &&
        !GET_OBJ_VAL(GET_EQ(vict, i), 9))
      focus = GET_EQ(vict, i);
  GET_SUSTAINED_NUM(ch)++;
  if (focus) {
    GET_SUSTAINED_FOCI(ch)++;
...
void spell_bonus(struct char_data *ch, int spell, int &skill, int &target) {
  ...
  if (GET_TRADITION(ch) == TRAD_SHAMANIC)
    totem_bonus(ch, SPELLCASTING, spell, target, skill);
           ...
           spells[spell].category != HEALTH) {
    for (struct spirit_data *spirit = GET_SPIRIT(ch); spirit;
         spirit = spirit->next)
      if (((spells[spell].category == MANIPULATION &&
            spirit->type == ELEM_EARTH) ||
...
bool find_duplicate_spell(struct char_data *ch, struct char_data *vict,
                          int spell, int sub) {
...
bool check_spell_victim(struct char_data *ch, struct char_data *vict,
                        int spell) {
  if (!vict)
    send_to_char(NOPERSON, ch);
  else if (((IS_PROJECT(ch) || IS_ASTRAL(ch)) &&
            !(IS_DUAL(vict) || IS_ASTRAL(vict) || IS_PROJECT(vict))) ||
           ((IS_PROJECT(vict) || IS_ASTRAL(vict)) &&
            !(IS_DUAL(ch) || IS_ASTRAL(ch) || IS_PROJECT(ch))))
...
int reflect_spell(struct char_data *ch, struct char_data *vict, int spell,
                  int force, int sub, int target, int &success) {
...
int resist_spell(struct char_data *ch, int spell, int force, int sub) {
  ...
  if (sub)
    ...
    skill = GET_WIL(ch);
  if (GET_TRADITION(ch) == TRAD_SHAMANIC) {
    if (GET_TOTEM(ch) == TOTEM_HORSE &&
        spells[spell].category == GET_TOTEMSPIRIT(ch))
      skill--;
    else if (GET_TOTEM(ch) == TOTEM_LEOPARD &&
             spells[spell].category == ILLUSION)
      skill--;
  }
  if (GET_TRADITION(ch) == TRAD_ADEPT) {
    if (GET_POWER(ch, ADEPT_MAGIC_RESISTANCE))
      skill += GET_POWER(ch, ADEPT_MAGIC_RESISTANCE);
    if (GET_POWER(ch, ADEPT_SPELL_SHROUD) &&
        spells[spell].category == DETECTION)
...
void cast_combat_spell(struct char_data *ch, int spell, int force, char *arg) {
  ...
  if (!*buf) {
    ...
    if (basedamage > 4 || basedamage == 0) {
      send_to_char("That is not a valid damage level, please choose between "
  ...
  int target = modify_target(ch),
      skill = GET_SKILL(ch, SKILL_SORCERY) +
              MIN(GET_SKILL(ch, SKILL_SORCERY), GET_CASTING(ch)),
      success = 0;
  ...
  case SPELL_MANABOLT:
    target += GET_WIL(vict);
    if ((!IS_NPC(ch) && (IS_NPC(vict) && MOB_FLAGGED(vict, MOB_INANIMATE))) ||
        (PLR_FLAGGED(ch, PLR_KILLER) && !IS_NPC(vict)))
      success = -1;
    else
      success = success_test(skill, target);
    if (success > 0 && GET_REFLECT(vict) &&
        (reflected =
             reflect_spell(ch, vict, spell, force, 0, target, success))) {
      vict = ch;
      ch = temp;
    }
    ...
    if (success > 0) {
      ...
      if (GET_MENTAL(vict) - (dam * 100) <= 0) {
        act("$n falls to the floor as ^Rblood^n erupts from every pore on $s "
            "head.",
            TRUE, vict, 0, 0, TO_ROOM);
        send_to_char("Your world turns to red as you feel blood flow from "
                     "every pore on your head.\r\n",
                     vict);
      } else if (GET_MENTAL(vict) - (dam * 100) <= 300) {
        act("$n grabs $s head in pain as blood flows from $s ears.", TRUE, vict,
            0, 0, TO_ROOM);
        send_to_char("Your head is filled with immense pain as your ears begin "
                     "to bleed.\r\n",
                     vict);
      } else if (GET_MENTAL(vict) - (dam * 100) <= 700) {
        act("$n grabs ahold of $s head in pain.", TRUE, vict, 0, 0, TO_ROOM);
        send_to_char("A sudden flash of pain in your head causes you to "
                     "reflexively grab it.\r\n",
                     vict);
      } else {
        act("$n seems to flinch slightly as blood trickles from $s nose.", TRUE,
            vict, 0, 0, TO_ROOM);
  ...
  case SPELL_STUNBOLT:
    ...
    if (success > 0) {
      ...
      if (GET_MENTAL(vict) - (dam * 100) <= 0) {
        ...
        send_to_char("The world turns black around you as you suddenly fall "
                     "unconscious.\r\n",
                     vict);
      } else if (GET_MENTAL(vict) - (dam * 100) <= 300) {
        act("$n grabs $s head and cringes.", TRUE, vict, 0, 0, TO_ROOM);
        send_to_char("Your head is filled with an unbearable pressure as your "
                     "vision begins to fade.\r\n",
                     vict);
      } else if (GET_MENTAL(vict) - (dam * 100) <= 700) {
        act("$n shakes $s head forcibly as though trying to clear it.", TRUE,
            vict, 0, 0, TO_ROOM);
        send_to_char("A wave of tiredness comes over you, but seems to clear "
                     "slightly as you shake your head.\r\n",
                     vict);
      } else {
        act("$n recoils slightly as though hit by an invisible force.", TRUE,
            vict, 0, 0, TO_ROOM);
        send_to_char("You mind goes slightly hazy as though you had just been "
                     "punched.\r\n",
                     vict);
      }
  ...
  case SPELL_POWERBOLT:
    ...
    if (success > 0) {
      int dam = convert_damage(stage(success, basedamage));
      if (GET_MENTAL(vict) - (dam * 100) <= 0) {
        act("$n screams loudly as $s body is torn asunder by magic.", FALSE,
            vict, 0, 0, TO_ROOM);
        send_to_char("You begin to feel yourself tearing apart.\r\n", vict);
      } else if (GET_MENTAL(vict) - (dam * 100) <= 300) {
        act("$n cries out in pain as $s begins to bleed from nearly every pore "
            "on $s body.",
            TRUE, vict, 0, 0, TO_ROOM);
        send_to_char("You uncontrollably scream as an immense force tears at "
                     "your body.\r\n",
                     vict);
      } else if (GET_MENTAL(vict) - (dam * 100) <= 700) {
        act("$n grabs $s stomach as a trickle of blood comes from $s mouth.",
            TRUE, vict, 0, 0, TO_ROOM);
        send_to_char("Your torso is filled with a sharp stabbing pain as you "
                     "cough up some blood.\r\n",
                     vict);
      } else {
        act("$n grimaces, obviously afflicted with mild pain.", TRUE, vict, 0,
            0, TO_ROOM);
        send_to_char(
            "You feel a dull throb of pain flow through your body.\r\n", vict);
      }
      damage(ch, vict, dam, TYPE_COMBAT_SPELL, PHYSICAL);
      if (IS_NPC(vict) && !IS_NPC(ch))
        GET_LASTHIT(vict) = GET_IDNUM(ch);
...
void cast_detection_spell(struct char_data *ch, int spell, int force, char *arg,
  ...
  case SPELL_MINDLINK:
    ...
    for (struct sustain_data *sust = GET_SUSTAINED(vict); sust;
         sust = sust->next)
      if (sust->spell == SPELL_MINDLINK) {
    ...
    if (success > 0) {
      create_sustained(ch, vict, spell, force, 0, success,
                       spells[spell].draindamage);
      act("You successfully sustain that spell on $N.", FALSE, ch, 0, vict,
          TO_CHAR);
  ...
  case SPELL_COMBATSENSE:
    ...
    if (success > 0) {
      create_sustained(ch, vict, spell, force, 0, success,
                       spells[spell].draindamage);
      send_to_char("The world seems to slow down around you as your sense of "
                   "your surroundings becomes clearer.\r\n",
                   vict);
      act("You successfully sustain that spell on $N.", FALSE, ch, 0, vict,
          TO_CHAR);
    } else
...
void cast_health_spell(struct char_data *ch, int spell, int sub, int force,
  ...
  switch (spell) {
  case SPELL_DETOX:
    if ((GET_DRUG_STAGE(vict) == 1 || GET_DRUG_STAGE(vict) == 2) &&
        GET_DRUG_AFFECT(vict))
    ...
    if (success > 0 && !AFF_FLAGGED(vict, AFF_DETOX)) {
      create_sustained(ch, vict, spell, force, 0, success,
                       spells[SPELL_STABILIZE].draindamage);
  ...
  case SPELL_STABILIZE:
    ...
    success = success_test(skill, target);
    if (success > 0 &&
        force >= (GET_PHYSICAL(vict) <= 0 ? -(GET_PHYSICAL(vict) / 100) : 50)) {
      create_sustained(ch, vict, spell, force, 0, success,
                       spells[SPELL_STABILIZE].draindamage);
      send_to_char("Your condition stabilizes, you manage to grab a thin hold "
                   "on life.\r\n",
                   vict);
      act("You successfully sustain that spell on $N.", FALSE, ch, 0, vict,
          TO_CHAR);
  ...
  case SPELL_RESISTPAIN:
    ...
    if (success > 0 && !AFF_FLAGGED(ch, AFF_RESISTPAIN)) {
      create_sustained(ch, vict, spell, force, 0, success,
                       spells[SPELL_RESISTPAIN].draindamage);
  ...
  case SPELL_HEALTHYGLOW:
    ...
    if (success > 0) {
      create_sustained(ch, vict, spell, force, 0, success,
                       spells[SPELL_HEALTHYGLOW].draindamage);
  ...
  case SPELL_HEAL:
    success = MIN(force,
                  success_test(skill, 10 - (int)(GET_ESS(vict) / 100) + target +
                                          (int)(GET_INDEX(ch) / 200)));
    if (GET_PHYSICAL(vict) <= 0)
      drain = DEADLY;
    else if (GET_PHYSICAL(vict) <= 300)
      drain = SERIOUS;
      ...
      drain = MODERATE;
    if (success < 1 || AFF_FLAGGED(vict, AFF_HEALED) ||
  ...
  case SPELL_INCCYATTR:
    if (GET_ATT(vict, sub) != GET_REAL_ATT(vict, sub)) {
      if (GET_TRADITION(vict) == TRAD_ADEPT && sub < CHA) {
        ...
        for (struct sustain_data *sus = GET_SUSTAINED(vict); sus;
             sus = sus->next)
          if (sus->caster == FALSE &&
              (sus->spell == SPELL_INCATTR || sus->spell == SPELL_DECATTR) &&
              sus->subtype == sub) {
            cyber = false;
            break;
          }
      if (cyber && (spell == SPELL_DECATTR || spell == SPELL_INCATTR ||
                    spell == SPELL_INCREA)) {
        sprintf(buf,
                "$N's %s has been modified by technological means and is "
    ...
    if ((spell == SPELL_DECCYATTR || spell == SPELL_INCCYATTR) &&
        (!cyber || GET_ATT(vict, sub) == GET_REAL_ATT(vict, sub))) {
      sprintf(buf,
              "$N's %s has not been modified by technological means and is "
              "immune to this spell.\r\n",
              attributes[sub]);
      act(buf, TRUE, ch, 0, vict, TO_CHAR);
      return;
    ...
    success = (int)(success_test(skill, target) -
                    ((spell == SPELL_DECATTR || spell == SPELL_DECCYATTR)
                         ? resist_spell(vict, spell, force, sub)
                         : 0));
    if (success > 0) {
      create_sustained(ch, vict, spell, force, sub, success,
                       spells[spell].draindamage);
      act("You successfully sustain that spell on $N.", FALSE, ch, 0, vict,
          TO_CHAR);
...
void cast_illusion_spell(struct char_data *ch, int spell, int force, char *arg,
  ...
  case SPELL_STEALTH:
    ...
    success = success_test(skill, target + 4);
    if (success > 0) {
      act("You successfully sustain that spell on $n.", FALSE, vict, 0, ch,
          TO_VICT);
  ...
  case SPELL_SILENCE:
    ...
    if (success > 0) {
      ...
      create_sustained(ch, ch, spell, force, 0, success,
                       spells[spell].draindamage);
    } else
      send_to_char(FAILED_CAST, ch);
    spell_drain(ch, spell, force, 0);
...
void cast_manipulation_spell(struct char_data *ch, int spell, int force,
                             char *arg, struct char_data *mob) {
  struct char_data *vict = NULL;
  bool reflected = FALSE;
  int basedamage = 0;
  ...
  if (find_duplicate_spell(ch, vict, spell, 0))
    return;
  int target = modify_target(ch),
      skill = IS_ELEMENTAL(ch) || IS_SPIRIT(ch)
                  ? GET_LEVEL(ch)
  ...
  case SPELL_POLTERGEIST:
    ...
    if (success > 0) {
      create_sustained(ch, ch, spell, force, 0, success,
                       spells[spell].draindamage);
      act("An invisible wind begins to spin small objects around the area!",
  ...
  case SPELL_LIGHT:
    ...
    if (success > 0) {
      create_sustained(ch, ch, spell, force, 0, success,
                       spells[spell].draindamage);
      act("Light radiates from $n, illuminating the area!", FALSE, ch, 0, 0,
          TO_ROOM);
      act("The area brightens as your light spell takes affect!", FALSE, ch, 0,
          0, TO_CHAR);
    } else
      send_to_char(FAILED_CAST, ch);
    spell_drain(ch, spell, force, 0);
  ...
  case SPELL_ICESHEET:
    ...
    success = success_test(skill, target + 4);
    if (success > 0) {
      ch->in_room->icesheet[0] =
          (int)(3.14 * ((GET_MAG(ch) * GET_MAG(ch)) / 10000));
  ...
  case SPELL_IGNITE:
    ...
    if (success > 0 && GET_REFLECT(vict) &&
        (reflected =
             reflect_spell(ch, vict, spell, force, 0, target + 4, success))) {
      vict = ch;
      ch = temp;
    }
    success -= resist_spell(vict, spell, force, 0);
    ...
    if (success > 0) {
      ...
      act("You succeed in raising the body temperature of $N to dangerous "
          "levels.",
          FALSE, ch, 0, vict, TO_CHAR);
      create_sustained(ch, vict, spell, force, 0, success,
                       MAX(1, 10 / MIN(force, success)));
      ...
      send_to_char(
          "You feel your body heat slightly then return to normal.\r\n", vict);
  ...
  case SPELL_SHADOW:
    ...
    if (success > 0) {
      ...
      act("Dark shadows fall over the area as your spell takes affect.", FALSE,
          ch, 0, 0, TO_CHAR);
    } else
      send_to_char(FAILED_CAST, ch);
    spell_drain(ch, spell, force, 0);
  ...
  case SPELL_CLOUT:
    ...
    if (!AWAKE(vict))
      ...
      success -= success_test(GET_DEFENSE(vict) + GET_DEFENSE(vict)
                                  ? GET_POWER(vict, ADEPT_SIDESTEP)
                                  : 0,
                              4 + damage_modifier(vict, buf));
    }
    success += success_test(skill, 4 + target);
    ...
    if (success <= 0) {
      act("Your clout spell harmlessly disperses as $n dodges it.", FALSE, vict,
          0, ch, TO_VICT);
      send_to_char("You easily dodge it!\r\n", vict);
      act("$n dodges out of the way of unseen force.", TRUE, vict, 0, ch,
          TO_NOTVICT);
    } else {
      success -= success_test(GET_BOD(vict) + GET_BODY(vict),
                              force - GET_IMPACT(vict));
      int dam = convert_damage(stage(success, basedamage));
      if (!AWAKE(vict)) {
        ...
        act("$n is thrown to the ground, unconscious, from an immense "
            "invisible force.",
            TRUE, vict, 0, 0, TO_ROOM);
        send_to_char("It slams into you with the force of a freight train, "
                     "knocking you to the ground, unconscious.\r\n",
                     vict);
      } else if (GET_MENTAL(vict) - (dam * 100) <= 300) {
        act("$n stumbles backwards, almost losing $s footing, as $e is hit by "
            "an invisible force.",
            TRUE, vict, 0, 0, TO_ROOM);
        send_to_char("You stumble backwards, feeling groggy as the air slams "
                     "into you at full force.\r\n",
                     vict);
      } else if (GET_MENTAL(vict) - (dam * 100) <= 700) {
        act("$n steps back, and shakes $s head to clear it.", TRUE, vict, 0, 0,
            TO_ROOM);
        send_to_char("You step back as the force hits you, feeling a little "
                     "worse for wear.\r\n",
                     vict);
      } else if (dam > 0) {
        act("$n recoils, as if from a light punch.", TRUE, vict, 0, 0, TO_ROOM);
        send_to_char(
            "You feel as though a fist strikes you across the face.\r\n", vict);
      } else {
        send_to_char("It rushes past you, not causing any damage.\r\n", vict);
        act("$n doesn't seem hurt by your clout spell.", FALSE, vict, 0, ch,
            TO_VICT);
      }
  ...
  case SPELL_FLAMETHROWER:
    ...
    if (!AWAKE(vict))
      ...
      success -= success_test(GET_DEFENSE(vict) + GET_DEFENSE(vict)
                                  ...
                                  : 0,
                              4 + damage_modifier(vict, buf));
    act("$n's hands seem to spontaneously combust as $e directs a stream of "
        "flame at $N!",
    ...
    if (success <= 0) {
      act("$n dodges the flames, which disperse as they past $m.", FALSE, vict,
          0, 0, TO_ROOM);
      send_to_char("You easily dodge the flames!\r\n", vict);
    } else {
      success -= success_test(GET_BOD(vict) + GET_BODY(vict) +
                                  GET_POWER(ch, ADEPT_TEMPERATURE_TOLERANCE),
                              force - (GET_IMPACT(vict) / 2));
      int dam = convert_damage(stage(success, basedamage));
      if (!AWAKE(vict)) {
        act("$n spasms as the flames hit $m.", TRUE, vict, 0, 0, TO_ROOM);
        send_to_char(
            "You feel a slight burning sensation in the back of your mind.\r\n",
            vict);
      } else if (GET_MENTAL(vict) - (dam * 100) <= 0) {
        act("$n is hit full force by the intense flames causing $m to fall to "
            "the ground, gurgling.",
            TRUE, vict, 0, 0, TO_ROOM);
        send_to_char("The flames burn intensely around you, your last memory "
                     "before falling unconscious is the hideous pain.\r\n",
                     vict);
      } else if (GET_MENTAL(vict) - (dam * 100) <= 300) {
        act("Screams as the flames impact $s body, horribly burning $m.", TRUE,
            vict, 0, 0, TO_ROOM);
        send_to_char("The flames crash into you, causing you great pain as "
                     "they horribly burn you.\r\n",
                     vict);
      } else if (GET_MENTAL(vict) - (dam * 100) <= 700) {
        act("$n cringes as the flames hit, patting at the spots where the "
            "flame continues to burn.",
            TRUE, vict, 0, 0, TO_ROOM);
        send_to_char("As the flames hit you quickly pat at the spots that "
                     "continue to burn, causing searing pain.\r\n",
                     vict);
      } else if (dam > 0) {
        act("The flames burst around $m, causing seemingly little damage.",
            TRUE, vict, 0, 0, TO_ROOM);
        send_to_char("The flames burst around you causing you slight pain as "
                     "it burns some of your hair.\r\n",
                     vict);
      } else {
        act("The flames impact $n, but disperse on impact.", FALSE, vict, 0, ch,
            TO_ROOM);
        send_to_char("The flames rapidly disperse around you, causing only "
                     "mild discomfort.\r\n",
                     vict);
      }
      damage_equip(ch, vict, force, TYPE_FIRE);
      if (!damage(ch, vict, dam, TYPE_MANIPULATION_SPELL, PHYSICAL) &&
          number(0, 6) <= basedamage + 1) {
        act("^RThe flames continue to burn around $m!^N", TRUE, vict, 0, 0,
            TO_ROOM);
  ...
  case SPELL_ACIDSTREAM:
    ...
    if (!AWAKE(vict))
      ...
      success -= success_test(GET_DEFENSE(vict) + GET_DEFENSE(vict)
                                  ...
                                  : 0,
                              4 + damage_modifier(vict, buf));
    act("Dark clouds form around $n moments before it condenses into a dark "
        "sludge and flies towards $N!",
        TRUE, ch, 0, vict, TO_ROOM);
    if (!IS_NPC(ch) && PLR_FLAGGED(ch, PLR_KILLER) && !IS_NPC(vict))
      success = -1;
    else
    ...
    if (success <= 0) {
      act("$n dodges the acid, which evaporates as it passes $m.", FALSE, vict,
          0, 0, TO_ROOM);
      ...
      if (!AWAKE(vict)) {
        ...
        act("As the acid hits $n $e falls to the ground twitching and "
            "screaming as $s body smokes.",
            TRUE, vict, 0, 0, TO_ROOM);
        send_to_char("The fumes from the acid burning through your body fill "
                     "your lungs, burning you from the inside out as you fade "
                     "into unconsciousness.\r\n",
                     vict);
      } else if (GET_MENTAL(vict) - (dam * 100) <= 300) {
        act("The acid hits $n, $e cries out in pain as trails of smoke come "
            "from $s body.",
            TRUE, vict, 0, 0, TO_ROOM);
        send_to_char("The acid impacts you with a great force, causing you to "
                     "step back as it burns through your skin.\r\n",
                     vict);
      } else if (GET_MENTAL(vict) - (dam * 100) <= 700) {
        act("$n cringes as the acid hits.", TRUE, vict, 0, 0, TO_ROOM);
        send_to_char("The acid begins to burn your skin as it hits you, "
                     "causing a bit of pain.\r\n",
                     vict);
      } else if (dam > 0) {
        act("$n is splashed by the acid, causing nothing but mild irritation.",
            TRUE, vict, 0, 0, TO_ROOM);
        send_to_char("The acid splashes against you causing a mild burning "
                     "sensation.\r\n",
                     vict);
      } else {
        act("The acid splashes on $n, but $e doesn't seem to flinch.", FALSE,
            vict, 0, ch, TO_ROOM);
        send_to_char("You are splashed by the acid, but it causes nothing more "
                     "than a moment's irritation.\r\n",
                     vict);
      }
  ...
  case SPELL_LIGHTNINGBOLT:
    ...
    if (!AWAKE(vict))
      ...
      // NOTE: Added sidestep here. Not sure if you should be able to sidestep
      // lightning, but if you can dodge it in the first place...
      success -= success_test(GET_DEFENSE(vict) + GET_DEFENSE(vict)
                                  ? GET_POWER(vict, ADEPT_SIDESTEP)
                                  : 0,
                              4 + damage_modifier(vict, buf));
    }
    act("Lightning bursts forth from $n and heads directly towards $N!", TRUE,
        ch, 0, vict, TO_ROOM);
    if (!IS_NPC(ch) && PLR_FLAGGED(ch, PLR_KILLER) && !IS_NPC(vict))
      success = -1;
    else
    ...
    if (success <= 0) {
      act("$n easily dodges it, and it vanishes to nothing.", FALSE, vict, 0, 0,
          TO_ROOM);
      send_to_char("You easily dodge it!\r\n", vict);
    } else {
      success -= success_test(GET_BOD(vict) + GET_BODY(vict) +
      ...
      int dam = convert_damage(stage(success, basedamage));
      if (!AWAKE(vict)) {
        act("$n's body goes into convulsions as the lightning flows through "
            "it.",
            TRUE, vict, 0, 0, TO_ROOM);
        send_to_char(
            "You feel a slight burning sensation in the back of your mind.\r\n",
        ...
        act("$n is propelled backwards by the force of the lightning bolt, $s "
            "body smoking as it lands, not a sign of life from it.",
            TRUE, vict, 0, 0, TO_ROOM);
        send_to_char("You feel your body begin to spasm as the huge charge of "
                     "electricity fries your nervous system.\r\n",
                     vict);
      } else if (GET_MENTAL(vict) - (dam * 100) <= 300) {
        act("$n is almost lifted in the air by the lightning, spasms filling "
            "$s body, as a thin trail of smoke rise from $m.",
            TRUE, vict, 0, 0, TO_ROOM);
        send_to_char("Your body is filled with pain as the lightning hits you, "
                     "your limbs going into an uncontrollable seizure.\r\n",
                     vict);
      } else if (GET_MENTAL(vict) - (dam * 100) <= 700) {
        act("$n spasms as the lightning hits $m, $s body wracked with spasms "
            "as the lightning dissipates.",
            TRUE, vict, 0, 0, TO_ROOM);
        send_to_char("Pain flashes through your body as the lightning hits, "
                     "your body wracked with several serious spasms.\r\n",
                     vict);
      } else if (dam > 0) {
        act("$n visibly recoils as the lightning hits, but otherwise seems "
            "fine.",
            TRUE, vict, 0, 0, TO_ROOM);
        send_to_char("You recoil as the lightning hits you, your mind going "
                     "fuzzy for a moment.\r\n",
                     vict);
      } else {
        act("The lightning hits $n, but seems to be easily absorbed.", FALSE,
            vict, 0, ch, TO_ROOM);
        send_to_char("Your body absorbs the lightning without harm.\r\n", vict);
      }
      for (int i = 0; i < NUM_WEARS; i++)
        if (GET_EQ(vict, i) && (GET_OBJ_MATERIAL(GET_EQ(vict, i)) == 10 ||
...
void cast_spell(struct char_data *ch, int spell, int sub, int force,
                char *arg) {
...
void mob_magic(struct char_data *ch) {
  ...
  case SPELL_LIGHTNINGBOLT:
  case SPELL_CLOUT:
    sprintf(buf, "%s %s", wound_name[number(1, 4)],
            GET_CHAR_NAME(FIGHTING(ch)));
    break;
  default:
...
bool check_spirit_sector(struct room_data *room, int spirit) {
  if ((spirit == SPIRIT_WIND || spirit == SPIRIT_MIST ||
       spirit == SPIRIT_STORM)) {
    if (SECT(room) == SPIRIT_HEARTH || SECT(room) == SPIRIT_FOREST ||
...
void circle_build(struct char_data *ch, char *type, int force) {
  ...
  if (GET_NUYEN(ch) < force * force) {
    send_to_char(ch,
                 "You need %d nuyen for the materials needed to construct that "
                 "circle.\r\n",
                 force * force);
    return;
  }
  ...
  if (GET_LEVEL(ch) > 1) {
    send_to_char("You use your staff powers to greatly accelerate the artistic "
                 "process.\r\n",
                 ch);
    GET_OBJ_VAL(obj, 9) = 1;
  } else
...
void lodge_build(struct char_data *ch, int force) {
  ...
  if (GET_NUYEN(ch) < force * 500) {
    send_to_char(
        ch, "You need %d nuyen worth of materials to construct that lodge.\r\n",
        force * 500);
    return;
  }
  ...
  GET_OBJ_VAL(obj, 3) = GET_IDNUM(ch);
  if (GET_LEVEL(ch) > 1) {
    send_to_char("You use your staff powers to greatly accelerate the "
...
struct char_data *create_elemental(struct char_data *ch, int type, int force,
                                   int idnum, int trad) {
  struct char_data *mob;
...
ACMD(do_contest) {
  if (GET_ASPECT(ch) == ASPECT_SORCERER || GET_TRADITION(ch) == TRAD_ADEPT ||
      GET_TRADITION(ch) == TRAD_MUNDANE || !GET_SKILL(ch, SKILL_CONJURING)) {
  ...
  int chskill = GET_SKILL(ch, SKILL_CONJURING),
      caskill = GET_SKILL(ch, SKILL_CONJURING) + GET_CHA(ch);
  for (int i = 0; i < NUM_WEARS; i++)
    if (GET_EQ(ch, i) && GET_OBJ_TYPE(GET_EQ(ch, i)) == ITEM_FOCUS &&
  ...
  for (int i = 0; i < NUM_WEARS; i++)
    if (GET_EQ(caster, i) && GET_OBJ_TYPE(GET_EQ(caster, i)) == ITEM_FOCUS &&
        GET_OBJ_VAL(GET_EQ(caster, i), 0) == FOCI_SPIRIT &&
        GET_OBJ_VAL(GET_EQ(caster, i), 2) == GET_IDNUM(ch) &&
  ...
  int chsuc = success_test(chskill, GET_LEVEL(mob)),
      casuc = success_test(caskill, GET_LEVEL(mob));
  struct spirit_data *temp;
  if (chsuc < 1 && casuc < 1) {
    for (struct spirit_data *sdata = GET_SPIRIT(caster); sdata;
         sdata = sdata->next)
...
ACMD(do_unbond) {
  if (GET_TRADITION(ch) == TRAD_MUNDANE) {
    send_to_char(
        "You can't unbond something with no sense of the astral plane.\r\n",
        ch);
    return;
  }
...
ACMD(do_bond) {
  ...
  if (!obj)
    for (int i = 0; i < NUM_WEARS && !obj; i++)
      if (GET_EQ(ch, i) && (isname(buf1, GET_EQ(ch, i)->text.keywords) ||
                            isname(buf1, GET_OBJ_NAME(GET_EQ(ch, i)))))
        obj = GET_EQ(ch, i);
  if (!obj) {
    send_to_char("You don't have that item.\r\n", ch);
    return;
  ...
  if (GET_OBJ_TYPE(obj) == ITEM_DOCWAGON) {
    if (GET_DOCWAGON_BONDED_IDNUM(obj)) {
      if (GET_DOCWAGON_BONDED_IDNUM(obj) == GET_IDNUM(ch))
        ...
        act("$p has already been activated by someone else.", FALSE, ch, obj, 0,
            TO_CHAR);
      return;
    }
    GET_DOCWAGON_BONDED_IDNUM(obj) = GET_IDNUM(ch);
    act("$p's lights begin to subtly flash in a rhythmic sequence.", FALSE, ch,
        obj, 0, TO_CHAR);
    return;
  }
  // If it's an unbonded magazine, we need to search for the weapon they
  // selected.
  if (GET_OBJ_TYPE(obj) == ITEM_GUN_MAGAZINE) {
    // Define aliases used for code readability; nobody likes juggling 'i' and
    // 'obj'.
    ...
    for (weapon = ch->carrying; weapon; weapon = weapon->next_content) {
      if (GET_OBJ_TYPE(weapon) == ITEM_WEAPON &&
          (isname(buf1, weapon->text.keywords) ||
           isname(buf2, GET_OBJ_NAME(weapon))))
        break;
    }
    ...
    // Assign the magazine's values to the correct values supplied by the
    // weapon.
    GET_OBJ_VAL(magazine, 0) = GET_OBJ_VAL(weapon, 5);
    GET_OBJ_VAL(magazine, 1) = GET_OBJ_VAL(weapon, 3);
    sprintf(buf, "a %d-round %s magazine", GET_OBJ_VAL(magazine, 0),
            weapon_type[GET_OBJ_VAL(magazine, 1)]);
    ...
    if (IS_GUN(GET_OBJ_VAL(obj, 3))) {
      for (struct obj_data *i = ch->carrying; i; i = i->next_content) {
        if (GET_OBJ_TYPE(i) == ITEM_GUN_MAGAZINE && !GET_OBJ_VAL(i, 0)) {
          ...
          sprintf(buf, "a %d-round %s magazine", GET_OBJ_VAL(i, 0),
                  weapon_type[GET_OBJ_VAL(i, 1)]);
          if (i->restring)
            delete[] i->restring;
      ...
      send_to_char("There is no need to bond a magazine to that weapon.\r\n",
                   ch);
    return;
  }
  if (GET_OBJ_TYPE(obj) == ITEM_FOCUS) {
    if (GET_TRADITION(ch) == TRAD_MUNDANE)
      ...
      case FOCI_SPIRIT:
        if (!*buf2) {
          send_to_char(ch, "Bond which %s type?\r\n",
                       GET_TRADITION(ch) == TRAD_HERMETIC ? "elemental"
                                                          : "spirit");
          return;
        }
        if (GET_TRADITION(ch) == TRAD_HERMETIC) {
          ...
          for (; spirit < NUM_SPIRITS; spirit++)
            if (is_abbrev(buf2, spirits[spirit].name))
              break;
        if (GET_TRADITION(ch) == TRAD_HERMETIC ? spirit == NUM_ELEMENTS
                                               : spirit == NUM_SPIRITS) {
      ...
      if (GET_OBJ_VAL(obj, 0) == FOCI_SUSTAINED ||
          GET_OBJ_VAL(obj, 0) == FOCI_SPEC_SPELL) {
        for (; spell; spell = spell->next)
        ...
        if (GET_OBJ_VAL(obj, 0) == FOCI_SUSTAINED &&
            spells[spirit].duration != SUSTAINED) {
          send_to_char(
              "You don't need to bond a sustaining foci for this spell.\r\n",
              ch);
          return;
        }
      }
      if (PLR_FLAGGED(ch, PLR_AUTH)) {
        if (GET_FORCE_POINTS(ch) < karma) {
          send_to_char(
              ch,
              "You don't have enough force points to bond that (Need %d).\r\n",
              karma);
          return;
        }
        ...
        if (GET_KARMA(ch) < karma * 100) {
          send_to_char(
              ch, "You don't have enough karma to bond that (Need %d).\r\n",
              karma);
          return;
        }
  ...
  if (is_abbrev(buf, "spirit") || is_abbrev(buf, "elemental")) {
    ...
    if (!GET_SPIRIT(ch)) {
      send_to_char(ch, "You don't have any %s bound to you.\r\n",
                   GET_TRADITION(ch) == TRAD_HERMETIC ? "elementals"
                                                      : "spirits");
      return;
    }
    int i;
    if (!(i = atoi(buf1)) || i > GET_NUM_SPIRITS(ch)) {
      send_to_char(ch,
                   "Which %s do you wish to release from your services?\r\n",
    ...
    for (struct spirit_data *spirit = GET_SPIRIT(ch); spirit;
      ...
      if (--i == 0) {
        ...
        if (GET_TRADITION(ch) == TRAD_HERMETIC)
          send_to_char(
              ch,
              "You release %s from its obligations and it departs to the "
              "metaplanes.\r\n",
        ...
        if (spirit->called)
          for (struct char_data *mob = character_list; mob; mob = mob->next)
            if (IS_NPC(mob) && GET_ACTIVE(mob) == GET_IDNUM(ch) &&
                GET_GRADE(mob) == spirit->id) {
              act("Freed from its services, $n returns to the metaplanes", TRUE,
                  mob, 0, ch, TO_NOTVICT);
              extract_char(mob);
              break;
            }
...
ACMD(do_cast) {
  if (GET_ASPECT(ch) == ASPECT_CONJURER || GET_TRADITION(ch) == TRAD_ADEPT ||
      GET_TRADITION(ch) == TRAD_MUNDANE || !GET_SKILL(ch, SKILL_SORCERY)) {
    send_to_char("You don't have the ability to do that.\r\n", ch);
    return;
  }
  ...
  for (; spell; spell = spell->next)
    if (is_abbrev(spell_name, spell->name) &&
        !(!str_cmp(spell_name, "heal") && spell->type == SPELL_HEALTHYGLOW))
      break;
  if (!spell) {
...
ACMD(do_conjure) {
  ...
  if (force > (GET_MAG(ch) / 100) * 2) {
    send_to_char(
        ch,
        "You can't conjure %s of force more than twice your magic rating.\r\n",
  ...
  if (GET_TRADITION(ch) == TRAD_HERMETIC) {
    ...
    struct obj_data *obj;
    for (struct obj_data *obj = ch->in_room->contents; obj;
         obj = obj->next_content)
      if (GET_OBJ_TYPE(obj) == ITEM_MAGIC_TOOL) {
        if (GET_OBJ_VAL(obj, 0) == TYPE_LIBRARY_CONJURE) {
          if (GET_OBJ_VAL(obj, 1) < force) {
            send_to_char("Your library isn't of a high enough rating to "
                         "conjure that elemental.\r\n",
                         ch);
            return;
          }
          library = TRUE;
          ...
          if (GET_OBJ_VAL(obj, 1) < force) {
            send_to_char("Your hermetic circle isn't of a high enough rating "
                         "to conjure that elemental.\r\n",
                         ch);
            return;
          } else if (GET_OBJ_VAL(obj, 2) != spirit) {
            send_to_char(
                "That circle is for a different type of elemental.\r\n", ch);
            return;
          }
          circle = TRUE;
        }
    ...
    if (!circle || !library) {
      send_to_char("You need a conjuring library and a hermetic circle to "
                   "conjure spirits.\r\n",
                   ch);
      return;
    }
    for (obj = ch->carrying; obj; obj = obj->next_content)
      if (GET_OBJ_TYPE(obj) == ITEM_MAGIC_TOOL &&
          GET_OBJ_VAL(obj, 0) == TYPE_SUMMONING) {
        if (GET_OBJ_COST(obj) < force * 1000) {
          send_to_char("You don't have enough materials to summon at that high "
                       "a force.\r\n",
                       ch);
          return;
        } else
    ...
    if (!obj) {
      send_to_char("You need to be carrying conjuring materials to conjure an "
                   "elemental.\r\n",
                   ch);
      return;
    }
    ...
    if (GET_LEVEL(ch) > 1) {
      send_to_char(ch, "You use your staff powers to greatly accelerate the "
                       "conjuring process.\r\n");
    ...
    if (GET_DOMAIN(ch) != ((spirit == SPIRIT_MIST || spirit == SPIRIT_STORM ||
                            spirit == SPIRIT_WIND)
                               ? SPIRIT_SKY
                               : spirit)) {
      send_to_char(
          "You aren't in the correct domain to conjure that spirit.\r\n", ch);
      return;
    }
    ...
    if (GET_ASPECT(ch) == ASPECT_SHAMANIST &&
        skill == GET_SKILL(ch, SKILL_CONJURING)) {
    ...
    if (!conjuring_drain(ch, force) && AWAKE(ch)) {
      if (success < 1)
        ...
        GET_NUM_SPIRITS(ch)++;
        struct char_data *mob =
            create_elemental(ch, spirit, force, spirdata->id, TRAD_SHAMANIC);
        send_to_char("The spirit hears your call and comes forth from the "
                     "metaplanes.\r\n",
                     ch);
        act("$n fades into existance on the astral plane.", TRUE, mob, 0, 0,
            TO_ROOM);
      }
    }
  }
...
ACMD(do_spells) {
  ...
  for (struct spell_data *spell = GET_SPELLS(ch); spell; spell = spell->next)
    send_to_char(ch, "%-50s Category: %12s Force: %d\r\n", spell->name,
                 spell_category[spells[spell->type].category], spell->force);
}
...
ACMD(do_learn) {
  ...
  if ((GET_TRADITION(ch) == TRAD_HERMETIC && GET_OBJ_VAL(obj, 2)) ||
      (GET_TRADITION(ch) == TRAD_SHAMANIC && !GET_OBJ_VAL(obj, 2))) {
  ...
  for (spell = GET_SPELLS(ch); spell; spell = spell->next)
    if (spell->type == GET_OBJ_VAL(obj, 1) &&
        spell->subtype == GET_OBJ_VAL(obj, 3)) {
      if (spell->force >= force) {
        send_to_char(
            "You already know this spell at an equal or higher force.\r\n", ch);
        return;
      } else {
  ...
  if (GET_KARMA(ch) < (force - oldforce) * 100) {
    send_to_char(ch,
                 "You don't have enough karma to learn this spell at that "
                 "force! (You need %d)\r\n",
                 force);
    return;
  }
  if ((GET_ASPECT(ch) == ASPECT_ELEMFIRE &&
       spells[GET_OBJ_VAL(obj, 1)].category != COMBAT) ||
      (GET_ASPECT(ch) == ASPECT_ELEMEARTH &&
       spells[GET_OBJ_VAL(obj, 1)].category != MANIPULATION) ||
      (GET_ASPECT(ch) == ASPECT_ELEMWATER &&
       spells[GET_OBJ_VAL(obj, 1)].category != ILLUSION) ||
      (GET_ASPECT(ch) == ASPECT_ELEMAIR &&
       spells[GET_OBJ_VAL(obj, 1)].category != DETECTION)) {
    send_to_char("Glancing over the formula you realise you can't bind mana in "
                 "that fashion.\r\n",
                 ch);
  }
  if (GET_ASPECT(ch) == ASPECT_SHAMANIST) {
  ...
  struct obj_data *library =
      ch->in_veh ? ch->in_veh->contents : ch->in_room->contents;
  for (; library; library = library->next_content)
    if (GET_OBJ_TYPE(library) == ITEM_MAGIC_TOOL &&
        GET_OBJ_VAL(library, 1) >= force &&
        ((GET_TRADITION(ch) == TRAD_SHAMANIC &&
          GET_OBJ_VAL(library, 0) == TYPE_LODGE &&
          GET_OBJ_VAL(library, 3) == GET_IDNUM(ch)) ||
         (GET_TRADITION(ch) == TRAD_HERMETIC &&
          GET_OBJ_VAL(library, 0) == TYPE_LIBRARY_SPELL)))
      break;
  if (!library) {
  ...
  if (GET_TRADITION(ch) == TRAD_SHAMANIC && GET_OBJ_VAL(library, 9)) {
    send_to_char(
        "You need to finish building that lodge before you can use it.\r\n",
        ch);
    return;
  }
  ...
  if (GET_TRADITION(ch) == TRAD_SHAMANIC) {
    ...
    for (struct spirit_data *spir = GET_SPIRIT(ch);
         spir && skill == GET_SKILL(ch, SKILL_SORCERY); spir = spir->next)
      if (spir->called) {
  ...
  if (GET_OBJ_VAL(obj, 1) == SPELL_INCATTR ||
      GET_OBJ_VAL(obj, 1) == SPELL_INCCYATTR ||
      GET_OBJ_VAL(obj, 1) == SPELL_DECATTR ||
      GET_OBJ_VAL(obj, 1) == SPELL_DECCYATTR) {
...
ACMD(do_elemental) {
  if (GET_ASPECT(ch) == ASPECT_SORCERER || GET_TRADITION(ch) == TRAD_MUNDANE ||
      GET_TRADITION(ch) == TRAD_ADEPT || !GET_SKILL(ch, SKILL_CONJURING)) {
    send_to_char("You don't have the ability to do that.\r\n", ch);
    return;
  }
  if (!GET_NUM_SPIRITS(ch)) {
  ...
  sprintf(buf, "You currently have the following %s bound:\r\n",
          GET_TRADITION(ch) == TRAD_HERMETIC ? "elementals" : "spirits");
  for (struct spirit_data *elem = GET_SPIRIT(ch); elem;
       elem = elem->next, i++) {
    if (GET_TRADITION(ch) == TRAD_SHAMANIC)
      sprintf(ENDOF(buf), "%d) %-30s (Force %d) Services %d\r\n", i,
              GET_NAME(&mob_proto[real_mobile(spirits[elem->type].vnum)]),
              elem->force, elem->services);
    else
      sprintf(ENDOF(buf), "%d) %-30s (Force %d) Services: %d%10s\r\n", i,
              GET_NAME(&mob_proto[real_mobile(elements[elem->type].vnum)]),
...
ACMD(do_banish) {
  ...
  if (AFF_FLAGGED(ch, AFF_BANISH)) {
    if (GET_EXTRA(ch)) {
      if (FIGHTING(ch)) {
        act("You give up trying to banish $n.", FALSE, FIGHTING(ch), 0, ch,
            TO_VICT);
...
bool spirit_can_perform(int type, int order, int tradition) {
  if (order == SERV_MATERIALIZE || order == SERV_DEMATERIAL ||
      order == SERV_ATTACK)
    return TRUE;
  if (tradition == TRAD_HERMETIC) {
    if (order == SERV_ENGULF || order == SERV_APPEAR || order == SERV_SORCERY ||
        order == SERV_STUDY || order == SERV_SUSTAIN || order == SERV_LEAVE)
      return TRUE;
    switch (type) {
    case ELEM_FIRE:
      if (order == SERV_FLAMEAURA || order == SERV_GUARD ||
          order == SERV_FLAMETHROWER)
        return TRUE;
      break;
    case ELEM_AIR:
      if (order == SERV_MOVEMENT || order == SERV_BREATH ||
          order == SERV_PSYCHOKINESIS)
        return TRUE;
      break;
    case ELEM_WATER:
    case ELEM_EARTH:
    ...
    if ((type != SPIRIT_STORM && type != SPIRIT_DESERT) &&
        order == SERV_ACCIDENT)
      return TRUE;
    ...
    if (type != SPIRIT_STORM && order == SERV_GUARD)
      return TRUE;
    if ((type != SPIRIT_FOREST && type != SPIRIT_STORM &&
         type != SPIRIT_MIST) &&
        order == SERV_SEARCH)
      return TRUE;
    switch (type) {
    ...
    case SPIRIT_RIVER:
      ...
      break;
    case SPIRIT_SEA:
      if (order == SERV_CONFUSION || order == SERV_ENGULF ||
          order == SERV_MOVEMENT)
        return TRUE;
      break;
    case SPIRIT_SWAMP:
      if (order == SERV_BINDING || order == SERV_CONFUSION ||
          order == SERV_ENGULF || order == SERV_FEAR || order == SERV_MOVEMENT)
        return TRUE;
      break;
    }
  }
...
void make_spirit_power(struct char_data *spirit, struct char_data *tch,
                       int type) {
...
void stop_spirit_power(struct char_data *spirit, int type) {
  ...
  for (struct spirit_sustained *ssust = SPIRIT_SUST(spirit); ssust;
       ssust = ssust->next)
    if (ssust->type == type && ssust->caster == TRUE) {
      for (struct spirit_sustained *tsust = SPIRIT_SUST(ssust->target); tsust;
           tsust = tsust->next)
        if (tsust->type == type && tsust->target == spirit) {
          if (type == ENGULF) {
            if (IS_SPIRIT(spirit) ||
                (IS_ELEMENTAL(spirit) && GET_SPARE1(spirit) == ELEM_WATER)) {
              act("The water surrounding $n falls away and soaks into the "
                  "ground almost instantly.",
                  TRUE, ssust->target, 0, 0, TO_ROOM);
              send_to_char("The water surrounding you suddenly vanishes "
                           "allowing you to gasp for breath!\r\n",
                           ssust->target);
            } else {
              switch (GET_SPARE1(spirit)) {
              case ELEM_FIRE:
                act("The fire engulfing $n suddenly goes out.", TRUE,
                    ssust->target, 0, 0, TO_ROOM);
                send_to_char(
                    "The water fire surrounding you suddenly vanishes!\r\n",
                    ssust->target);
                break;
              case ELEM_EARTH:
                act("The dirt and rock engulfing $n suddenly bursts apart, "
                    "covering everyone close by.",
                    TRUE, ssust->target, 0, 0, TO_ROOM);
                send_to_char("The earth surrounding you suddenly bursts open, "
                             "allowing you to gasp for air!\r\n",
                             ssust->target);
                break;
              case ELEM_AIR:
                act("$n begins to gasp for breath as though $s was just "
                    "choked.",
                    TRUE, ssust->target, 0, 0, TO_ROOM);
                send_to_char("The mysterious force oppressing your lungs is "
                             "suddenly gone!\r\n",
                             ssust->target);
                break;
              }
...
POWER(spirit_appear) {
  ...
  spirit = create_elemental(ch, spiritdata->type, spiritdata->force,
                            spiritdata->id, GET_TRADITION(ch));
  act("$n appears in the astral plane.", TRUE, spirit, 0, ch, TO_NOTVICT);
  send_to_char(ch,
               "%s heeds your call and arrives from the metaplanes ready to do "
               "your bidding.\r\n",
               CAP(GET_NAME(spirit)));
}
...
POWER(spirit_sorcery) {
  if (!MOB_FLAGS(spirit).IsSet(MOB_AIDSORCERY)) {
    ...
    send_to_char(
        ch,
        "%s will now assist you in casting a spell next time it is able.\r\n",
        CAP(GET_NAME(spirit)));
  } else
    send_to_char(
        ch, "%s will already assist in spell casting next time it is able.\r\n",
        CAP(GET_NAME(spirit)));
}
...
POWER(spirit_study) {
  if (!MOB_FLAGS(spirit).IsSet(MOB_STUDY)) {
    ...
    send_to_char(
        ch,
        "%s will now assist you in studying a spell next time it is able.\r\n",
        CAP(GET_NAME(spirit)));
  } else
...
POWER(spirit_sustain) {
  ...
  if (GET_SUSTAINED_NUM(spirit))
    ...
    if ((spells[sust->spell].category == MANIPULATION &&
         spiritdata->type == ELEM_EARTH) ||
        (spells[sust->spell].category == COMBAT &&
         spiritdata->type == ELEM_FIRE) ||
        (spells[sust->spell].category == ILLUSION &&
         spiritdata->type == ELEM_WATER) ||
        (spells[sust->spell].category == DETECTION &&
         spiritdata->type == ELEM_AIR)) {
      sust->spirit = spirit;
      GET_SUSTAINED_FOCI(ch)++;
...
POWER(spirit_accident) {
  ...
  if (!tch)
    ...
    int success =
        success_test(MAX(GET_INT(tch), GET_QUI(tch)), GET_SPARE2(spirit));
    for (struct char_data *mob = spirit->in_room->people; mob; mob = mob->next)
      if (IS_NPC(mob) &&
          (GET_RACE(mob) == RACE_SPIRIT || GET_RACE(mob) == RACE_ELEMENTAL) &&
    ...
    if (success < 1) {
      ...
      sprintf(buf, "%s fails to cause $N to have an accident.",
              CAP(GET_NAME(spirit)));
...
POWER(spirit_binding) {
  ...
  if (!tch)
    ...
    act("$N suddenly becomes incapable of movement!", FALSE, spirit, 0, ch,
        TO_VICT);
...
POWER(spirit_engulf) {
  ...
  if (!tch)
    ...
    act("$n rushes towards $N and attempts to engulf them!", FALSE, spirit, 0,
        tch, TO_ROOM);
    int target = GET_QUI(spirit),
        targskill = get_skill(tch, SKILL_UNARMED_COMBAT, target);
    int success = success_test(GET_QUI(spirit), targskill) -
                  success_test(targskill, GET_QUI(spirit));
    if (success < 1) {
    ...
    if (IS_SPIRIT(spirit) ||
        (IS_ELEMENTAL(spirit) && GET_SPARE1(spirit) == ELEM_WATER)) {
      act("The water in the air surrounding $n seems to quickly condense and "
          "engulf $s!",
          TRUE, tch, 0, 0, TO_ROOM);
      send_to_char("The water in the air around you seems to condense and "
                   "swallow you up!\r\n",
                   tch);
    } else {
      switch (GET_SPARE1(spirit)) {
      case ELEM_FIRE:
        act("A strange, unnatural fire suddenly engulfs $n!", TRUE, tch, 0, 0,
            TO_ROOM);
        send_to_char("Fire suddenly engulfs your entire body!\r\n", tch);
        break;
      case ELEM_EARTH:
        act("The ground seems to rise up from below and encase $n!", TRUE, tch,
            0, 0, TO_ROOM);
        send_to_char("Clods of mud and earth slam into you, forming a "
                     "suffocating coccoon!\r\n",
                     tch);
        break;
      case ELEM_AIR:
        act("$n suddenly falls to $s knees and begins to retch!", TRUE, tch, 0,
            0, TO_ROOM);
        send_to_char("A huge pressure falls on your lungs and you find it "
...
POWER(spirit_fear) {
  ...
  if (!tch)
    ...
    int success = success_test(GET_SPARE2(spirit), GET_WIL(tch)) -
                  success_test(GET_WIL(tch), GET_SPARE2(spirit));
    if (success < 1) {
      send_to_char(
          "A dark shadow passes over your mind, but is quickly gone.\r\n", tch);
      ...
      sprintf(buf, "%s succeeds in instilling fear in $N.",
              CAP(GET_NAME(spirit)));
      act(buf, FALSE, ch, 0, tch, TO_CHAR);
...
POWER(spirit_guard) {
  if (!MOB_FLAGGED(spirit, MOB_SPIRITGUARD))
    act("$n begins to guard the area from accidents.", FALSE, spirit, 0, ch,
        TO_VICT);
  else
    act("$n stops guarding the area from accidents.", FALSE, spirit, 0, ch,
        TO_VICT);
...
POWER(spirit_movement) {
  ...
  if (!(*buf1 || *buf)) {
    send_to_char(
        "Do you want to increase or decrease the movement of the target?\r\n",
        ch);
    return;
  }
  ...
  if (!tch)
    send_to_char("Use movement against which target?\r\n", ch);
           ...
           affected_by_power(tch, increase > 0 ? MOVEMENTUP : MOVEMENTDOWN))
    send_to_char(ch, "The %s refuses to perform that service.\r\n",
                 GET_TRADITION(ch) == TRAD_HERMETIC ? "elemental" : "spirit");
  else {
...
POWER(spirit_breath) {
  ...
  if (!tch)
    ...
    act("$n turns towards $N as a cloud of noxious fumes forms around $S.",
        TRUE, spirit, 0, tch, TO_NOTVICT);
    act("$n lets forth a stream of noxious fumes in your direction.", FALSE,
        spirit, 0, tch, TO_VICT);
    int dam = convert_damage(stage(
        -success_test(MAX(GET_WIL(tch), GET_BOD(tch)), GET_SPARE2(spirit)),
        SERIOUS));
    if (dam > 0) {
      act("$n chokes and coughs.", TRUE, tch, 0, 0, TO_ROOM);
      send_to_char("The noxious fumes fill your throat and lungs, causing your "
                   "vision to swim.\r\n",
                   tch);
      damage(spirit, tch, dam, TYPE_FUMES, MENTAL);
    } else {
      act("$n waves $s hand in front of $s face, dispersing the fumes.", TRUE,
          tch, 0, 0, TO_ROOM);
...
POWER(spirit_attack) {
  ...
  if (!tch)
    ...
    send_to_char(ch,
                 "Ordering your own %s to attack you is not a good idea.\r\n",
                 GET_TRADITION(ch) == TRAD_HERMETIC ? "elemental" : "spirit");
  else if (tch == spirit)
    send_to_char(ch, "The %s refuses to perform that service.\r\n",
                 GET_TRADITION(ch) == TRAD_HERMETIC ? "elemental" : "spirit");
...
struct order_data services[] = {{"Appear", spirit_appear, 0},
...
ACMD(do_order) {
  ...
  if (!*buf1) {
    ...
    for (; order < NUM_SERVICES; order++)
      if (is_abbrev(buf, services[order].name) &&
          spirit_can_perform(spirit->type, order, GET_TRADITION(ch)))
        break;
    if (order == NUM_SERVICES) {
      send_to_char(ch,
                   "Which service do you wish to order the %s to perform?\r\n",
                   GET_TRADITION(ch) == TRAD_HERMETIC ? "elemental" : "spirit");
      return;
    }
    if (GET_TRADITION(ch) == TRAD_HERMETIC) {
    ...
    if (!mob) {
      send_to_char(
          ch, "That %s has been ensnared by forces you cannot control.\r\n",
          GET_TRADITION(ch) == TRAD_HERMETIC ? "elemental" : "spirit");
      sprintf(buf,
              "SYSERR: %s belonging to %s (%ld) has disappeared unexpectedly-- "
              "did someone purge it?",
              GET_TRADITION(ch) == TRAD_HERMETIC ? "Elemental" : "Spirit",
    ...
    if (services[order].type == 1 && MOB_FLAGGED(mob, MOB_ASTRAL)) {
      send_to_char(ch,
                   "That %s must materialize before it can use that power.\r\n",
                   GET_TRADITION(ch) == TRAD_HERMETIC ? "elemental" : "spirit");
      return;
    }
...
ACMD(do_domain) {
  ...
  if (!*argument) {
    ...
    if (SECT(temp_room) != SPIRIT_FOREST && SECT(temp_room) != SPIRIT_HEARTH &&
        !ROOM_FLAGGED(temp_room, ROOM_INDOORS)) {
      send_to_char(ch,
                   "You can switch to the following domains:\r\n  %s\r\n  Sky "
                   "Spirit\r\n",
    ...
    int newdomain = -1;
    if (SECT(temp_room) != SPIRIT_FOREST && SECT(temp_room) != SPIRIT_HEARTH &&
        !ROOM_FLAGGED(get_ch_in_room(ch), ROOM_INDOORS)) {
    ...
    if (newdomain == GET_DOMAIN(ch))
      ...
      send_to_char(ch, "You switch your focus to the %s domain.\r\n",
                   GET_DOMAIN(ch) == SPIRIT_SKY ? "Sky spirit"
                                                : spirits[GET_DOMAIN(ch)].name);
      for (struct spirit_data *spirit = GET_SPIRIT(ch); spirit; spirit = next) {
        ...
        if (GET_DOMAIN(ch) != spirit->type) {
          ...
          if (tch) {
            send_to_char(ch,
                         "You release %s from the rest of its services.\r\n",
                         GET_NAME(tch));
...
ACMD(do_deactivate) {
    ...
    for (obj = ch->bioware; obj; obj = obj->next_content)
      if (GET_OBJ_VAL(obj, 0) == BIO_PAINEDITOR) {
        if (!GET_OBJ_VAL(obj, 3))
          ...
          GET_OBJ_VAL(obj, 3) = 0;
          send_to_char("Your body starts to tingle as your perception of touch "
                       "returns.\r\n",
                       ch);
          update_pos(ch);
        }
  ...
  if (GET_OBJ_TYPE(obj) == ITEM_FOCUS) {
    ...
    send_to_char("You can't deactivate that.\r\n", ch);
}
...
ACMD(do_destroy) {
  ...
  if (GET_OBJ_TYPE(obj) == ITEM_MAGIC_TOOL &&
      (GET_OBJ_VAL(obj, 0) == TYPE_LODGE ||
       GET_OBJ_VAL(obj, 0) == TYPE_CIRCLE)) {
    if (GET_OBJ_VAL(obj, 0) == TYPE_LODGE) {
      ...
      send_to_char(ch, "You rub your feet along the lines making up the "
                       "hermetic circle, erasing them.\r\n");
...
ACMD(do_track) {
  ...
  if (!*argument) {
    if (HUNTING(ch->desc->original)) {
      success = success_test(MAX(GET_INT(ch), GET_MAG(ch) / 100),
                             HOURS_SINCE_TRACK(ch->desc->original));
      if (success > 0) {
        AFF_FLAGS(ch->desc->original).SetBit(AFF_TRACKING);
        send_to_char("You pick up the trail and continue tracking the astral "
                     "signature.\r\n",
                     ch);
      } else {
  ...
  two_arguments(argument, buf, buf1);
  if (!generic_find(
          buf, FIND_OBJ_INV | FIND_OBJ_ROOM | FIND_OBJ_EQUIP | FIND_CHAR_ROOM,
          ch, &vict, &obj)) {
  ...
  if (vict) {
    if (*buf1) {
      ...
      } else if (!(obj = get_object_in_equip_vis(vict, buf1, vict->equipment,
                                                 &i))) {
        send_to_char("They aren't wearing that.\r\n", ch);
        return;
      }
    } else if (IS_NPC(vict) &&
               (GET_RACE(vict) == RACE_SPIRIT ||
                GET_RACE(vict) == RACE_ELEMENTAL) &&
               GET_ACTIVE(vict)) {
      for (struct descriptor_data *desc = descriptor_list; desc;
           desc = desc->next)
        if (desc->original && GET_IDNUM(desc->original) == GET_ACTIVE(vict)) {
          vict = desc->original;
          break;
        } else if (desc->character &&
                   GET_IDNUM(desc->character) == GET_ACTIVE(vict)) {
          vict = desc->character;
          break;
        }
    } else {
  ...
  if (obj) {
    ...
    for (struct descriptor_data *desc = descriptor_list; desc;
         desc = desc->next)
      if (desc->original &&
          GET_IDNUM(desc->original) == GET_OBJ_VAL(obj, num)) {
        vict = desc->original;
        break;
      } else if (desc->character &&
                 GET_IDNUM(desc->character) == GET_OBJ_VAL(obj, num)) {
        vict = desc->character;
        break;
      }
  }
  ...
  if (vict && success > 0) {
    ...
    send_to_char(
        "You begin to trace that astral signature back to its owner.\r\n", ch);
  } else {
...
ACMD(do_dispell) {
  ...
  int success =
      success_test(GET_SKILL(ch, SKILL_SORCERY) +
                       MIN(GET_SKILL(ch, SKILL_SORCERY), GET_CASTING(ch)),
                   sust->force);
  int type = sust->spell, force = sust->force;
  sprintf(buf,
          "Dispell $N's %s (force %d) using skill %d vs TN %d: %d successes.",
          spells[sust->spell].name, sust->force,
          GET_SKILL(ch, SKILL_SORCERY) +
              MIN(GET_SKILL(ch, SKILL_SORCERY), GET_CASTING(ch)),
          sust->force, success);
  ...
  if (success > 0) {
    ...
    sust->success =
        MAX(sust->success - success,
            0); // Since it's unsigned, we have to do shit like this.
...
ACMD(do_heal) {
  ...
  if (!*buf)
    ...
    success = MIN(damage_array[basedamage],
                  MIN((GET_PHYSICAL(ch) / 100) - 1, success)) *
              100;
    success = MIN(GET_MAX_PHYSICAL(vict) - GET_PHYSICAL(vict), success);
    GET_PHYSICAL(vict) += success;
    GET_PHYSICAL(ch) -= success;
    WAIT_STATE(ch, 3 RL_SEC);
    act("You feel $n place $s hands on you, $s minstrations seem to cause your "
        "wounds to fade!",
        TRUE, ch, 0, vict, TO_VICT);
    act("You place your hands on $N, you feel $S pain and suffering "
        "transferred to your body!",
        TRUE, ch, 0, vict, TO_CHAR);
    act("$n places $s hands on $N seemingly transferring the wound to $mself!",
...
ACMD(do_relieve) {
  ...
  if (GET_POS(ch) == POS_FIGHTING)
    ...
    act("You feel $n place $s hands on you, a tingling feeling fills your body "
        "as your pain begins to subside",
        TRUE, ch, 0, vict, TO_VICT);
    act("Your hands warm slightly at the energy being channeled through them "
        "into $N's chakra points.",
        TRUE, ch, 0, vict, TO_CHAR);
    act("$n places $s hands on $N. $N appears to feel better.", TRUE, ch, 0,
        vict, TO_NOTVICT);
...
bool can_metamagic(struct char_data *ch, int i) {
  ...
  if (GET_ASPECT(ch) == ASPECT_CONJURER &&
      (i == META_QUICKENING || i == META_REFLECTING || i == META_ANCHORING ||
       i == META_SHIELDING))
    return FALSE;
...
ACMD(do_initiate) {
  if (GET_TRADITION(ch) == TRAD_MUNDANE)
    ...
    if (GET_TRADITION(ch) != TRAD_ADEPT)
      nonsensical_reply(ch);
    else if (GET_KARMA(ch) < 2000 ||
             ch->points.extrapp > (int)(GET_REP(ch) / 50))
...
void init_parse(struct descriptor_data *d, char *arg) {
  ...
  case INIT_MAIN:
    ...
    case '2':
      ...
      if (GET_TRADITION(CH) == TRAD_ADEPT)
        GET_PP(CH) += 100;
      send_to_char("You feel your astral reflection shift and mold itself "
                   "closer to the astral plane.\r\n",
                   CH);
      STATE(d) = CON_PLAYING;
  ...
  case INIT_GEAS:
    ...
    if (*arg == 'q') {
      ...
      for (i = 0; i < NUM_WEARS; i++)
        if (GET_EQ(CH, i) && GET_OBJ_TYPE(GET_EQ(CH, i)) == ITEM_FOCUS &&
            GET_OBJ_VAL(GET_EQ(CH, i), 9) == GET_IDNUM(CH) && --number < 0) {
          obj = GET_EQ(CH, i);
          break;
        }
      if (!obj) {
        send_to_char("You don't have that many geasa. Select geas to shed: ",
                     CH);
        return;
      }
      init_cost(CH, TRUE);
      ...
      if (GET_TRADITION(CH) == TRAD_ADEPT)
        GET_PP(CH) += 100;
      send_to_char(CH,
                   "You feel your magic return from that object, once again "
                   "binding to your spirit.\r\n",
                   CH);
      STATE(d) = CON_PLAYING;
      PLR_FLAGS(CH).RemoveBit(PLR_INITIATE);
    }
  ...
  case INIT_META:
    ...
    if (!number) {
      ...
      send_to_char(
          "You can't learn that metamagic technique. Select ability to learn: ",
          CH);
    } else {
      init_cost(CH, TRUE);
      ...
      if (GET_TRADITION(CH) == TRAD_ADEPT)
        GET_PP(CH) += 100;
      send_to_char(CH,
                   "You feel yourself grow closer to the astral plane as you "
                   "become ready to learn %s.\r\n",
                   metamagic[number]);
      STATE(d) = CON_PLAYING;
      PLR_FLAGS(CH).RemoveBit(PLR_INITIATE);
    }
...
ACMD(do_masking) {
  ...
  if (!*argument) {
    ...
    send_to_char("What do you wish to mask?\r\n", ch);
}
...
ACMD(do_focus) {
  ...
  if (!*argument) {
    ...
    struct sustain_data *spell = GET_SUSTAINED(ch);
    for (; spell; spell = spell->next)
      if (!spell->caster && !spell->spirit && !spell->focus &&
          spells[spell->spell].duration == SUSTAINED)
        break;
    if (!spell) {
      send_to_char(
          "You are not affected by any spells that need sustaining.\r\n", ch);
      return;
    }
    spell->spirit = ch;
    GET_SUSTAINED_NUM(ch)++;
    ...
    send_to_char(spell->other,
                 "You feel a weight lifted from you as someone takes over "
                 "sustaining your %s spell.\r\n",
                 buf);
    ...
    send_to_char("You can only release sustained spells.\r\n", ch);
}

ACMD(do_metamagic) {
  if (GET_GRADE(ch) == 0) {
    send_to_char("You are not close enough to the astral plane to have any "
                 "metamagic powers.\r\n",
                 ch);
    return;
  }
...
ACMD(do_cleanse) {
  ...
  if (!(IS_ASTRAL(ch) || IS_DUAL(ch)))
    ...
    send_to_char("You need practical knowledge of sorcery to cleanse the "
                 "astral plane.\r\n",
                 ch);
  else if (ch->in_veh)
    send_to_char("You'll need to leave your vehicle to attempt to manipulate "
                 "this area's magical background.\r\n",
                 ch);
  else if (!GET_BACKGROUND_COUNT(ch->in_room))
    send_to_char("The astral plane in this area is calm.\r\n", ch);
  else if (GET_BACKGROUND_COUNT(ch->in_room) > GET_GRADE(ch) ||
           GET_BACKGROUND_COUNT(ch->in_room) > 5)
    send_to_char("Cleansing a disturbance of this magnitude is beyond your "
                 "abilities.\r\n",
                 ch);
  else if (GET_BACKGROUND_AURA(ch->in_room) == AURA_POWERSITE)
    send_to_char("You cannot cleanse a power site.\r\n", ch);
  else {
    int success = success_test(GET_SKILL(ch, SKILL_SORCERY),
                               GET_BACKGROUND_COUNT(ch->in_room) * 2),
        background = GET_BACKGROUND_COUNT(ch->in_room);
    success /= 2;
    if (success <= 0)
      send_to_char(
          "You fail to reduce the astral disturbances in this area.\r\n", ch);
    else {
      GET_SETTABLE_BACKGROUND_COUNT(ch->in_room) =
          MAX(0, GET_BACKGROUND_COUNT(ch->in_room) - success);
      if (!GET_BACKGROUND_COUNT(ch->in_room)) {
        send_to_char("You successfully remove all astral disturbances from the "
                     "area.\r\n",
                     ch);
        sprintf(buf,
                "The astral disturbances in this area completely vanish.\r\n");
      } else {
        send_to_char(
            "You succeed in lowering the astral disturbances in the area.\r\n",
            ch);
        sprintf(buf, "The astral disturbances in the are seem to diminish "
                     "slightly.\r\n");
      }
      for (struct char_data *targ = ch->in_room->people; targ;
