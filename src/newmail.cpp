...
void store_mail(long to, struct char_data *from, const char *message_pointer) {
  if (to < 1) {
    log_vfprintf(
        "SYSERR: Invalid 'to' value passed to store_mail: %ld (must be >= 1).",
        to);
    return;
  }
  ...
  if (IS_NPC(from)) {
    log_vfprintf(
        "SYSERR: NPC '%s' attempted to use mail system. NPCs can't send mail.",
        GET_CHAR_NAME(from));
...
void raw_store_mail(long to, long from_id, const char *from_name,
                    const char *message_pointer) {
  // Note: We trust that whoever is calling this has already validated that the
  // message fits in the query buffer.

  char prepared_quote_buf[2 * strlen(message_pointer) + 1];
  prepare_quotes(prepared_quote_buf, message_pointer,
                 sizeof(prepared_quote_buf) / sizeof(prepared_quote_buf[0]));

  sprintf(mail_query_buf,
          "INSERT INTO pfiles_mail (sender_id, sender_name, recipient, text) "
          "VALUES (%ld, '%s', %ld, '%s');",
          from_id, from_name, to, prepared_quote_buf);
  ...
  for (struct descriptor_data *desc = descriptor_list; desc; desc = desc->next)
    if (desc->character && GET_IDNUM(desc->character) == to)
      for (struct obj_data *obj = desc->character->carrying; obj;
           obj = obj->next_content)
        if (GET_OBJ_SPEC(obj) && GET_OBJ_SPEC(obj) == pocket_sec &&
            obj->contains)
...
int amount_of_mail_waiting(struct char_data *ch) {
  ...
  sprintf(mail_query_buf,
          "SELECT COUNT(*) FROM pfiles_mail WHERE recipient = %ld;",
          GET_IDNUM(ch));
  mysql_wrapper(mysql, mail_query_buf);
...
char *get_and_delete_one_message(struct char_data *ch, char *sender) {
  ...
  sprintf(mail_query_buf,
          "SELECT * FROM pfiles_mail WHERE recipient = %ld LIMIT 1;",
          GET_IDNUM(ch));
  mysql_wrapper(mysql, mail_query_buf);
  ...
  sprintf(buf,
          ...
          "\r\n"
          "%s",
          GET_CHAR_NAME(ch), row[MAIL_SQL_SENDER_NAME], row[MAIL_SQL_TIMESTAMP],
          row[MAIL_SQL_TEXT]);

  // Fill out the sender field for the caller.
  strcpy(sender, row[MAIL_SQL_SENDER_NAME]);

  // Prepare the query for deleting the mail. Could have passed as a string, but
  // atoi() guarantees no SQL shenanigans.
  sprintf(mail_query_buf, "DELETE FROM pfiles_mail WHERE idnum = %d;",
          atoi(row[MAIL_SQL_IDNUM]));
...
void postmaster_send_mail(struct char_data *ch, struct char_data *mailman,
  ...
  if (IS_ASTRAL(ch)) {
    send_to_char("If the postal employee could detect the astral plane you "
                 "still would be ignored!\n\r",
                 ch);
    return;
  }
  ...
  /* Ensure the setup completed properly. This will only fail if we're out of
   * memory. */
  if (!ch->desc->str) {
...
void postmaster_check_mail(struct char_data *ch, struct char_data *mailman,
  ...
  if ((amount = amount_of_mail_waiting(ch)) > 0)
    sprintf(buf, "$n tells you, 'You have %d piece%s of mail waiting.'", amount,
            amount > 1 ? "s" : "");
  else
...
void postmaster_receive_mail(struct char_data *ch, struct char_data *mailman,
  ...
  // Consolidate messaging so people who haven't been around for a while don't
  // get walls of mail spam.
  if (mail_given == 1) {
