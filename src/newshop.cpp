...
const char *shop_flags[] = {"Nothing", "Doctor", "!NEGOTIATE", "!RESELL"};

const char *shop_type[3] = {"Grey", "Legal", "Black"};

const char *selling_type[] = {"Always", "Avail", "Stock", "Bought"};
...
bool is_open(struct char_data *keeper, int shop_nr) {
  ...
  if (shop_table[shop_nr].open > shop_table[shop_nr].close) {
    if (time_info.hours < shop_table[shop_nr].open &&
        time_info.hours > shop_table[shop_nr].close)
...
bool is_ok_char(struct char_data *keeper, struct char_data *ch,
                vnum_t shop_nr) {
  char buf[400];
  ...
  if (IS_NPC(ch) || access_level(ch, LVL_BUILDER))
    return TRUE;
  if ((shop_table[shop_nr].races.IsSet(RACE_HUMAN) &&
       GET_RACE(ch) == RACE_HUMAN) ||
      (shop_table[shop_nr].races.IsSet(RACE_ELF) &&
       (GET_RACE(ch) == RACE_ELF || GET_RACE(ch) == RACE_WAKYAMBI ||
        GET_RACE(ch) == RACE_NIGHTONE || GET_RACE(ch) == RACE_DRYAD)) ||
      (shop_table[shop_nr].races.IsSet(RACE_DWARF) &&
       (GET_RACE(ch) == RACE_DWARF || GET_RACE(ch) == RACE_KOBOROKURU ||
        GET_RACE(ch) == RACE_MENEHUNE || GET_RACE(ch) == RACE_GNOME)) ||
      (shop_table[shop_nr].races.IsSet(RACE_ORK) &&
       (GET_RACE(ch) == RACE_ORK || GET_RACE(ch) == RACE_ONI ||
        GET_RACE(ch) == RACE_SATYR || GET_RACE(ch) == RACE_HOBGOBLIN ||
        GET_RACE(ch) == RACE_OGRE)) ||
      (shop_table[shop_nr].races.IsSet(RACE_TROLL) &&
       (GET_RACE(ch) == RACE_TROLL || GET_RACE(ch) == RACE_CYCLOPS ||
        GET_RACE(ch) == RACE_GIANT || GET_RACE(ch) == RACE_MINOTAUR ||
        GET_RACE(ch) == RACE_FOMORI))) {
...
int buy_price(struct obj_data *obj, vnum_t shop_nr) {
  ...
  // If the shop is black or grey market, multiply base cost by the item's
  // street index.
...
int sell_price(struct obj_data *obj, vnum_t shop_nr) {
  ...
  // If the street index is set but is less than 1, multiply by this index
  // regardless of shop legality. This fixes an exploit where someone could buy
  // a discounted thing at a black/grey shop and sell to a legal one for a
  // profit.
...
struct shop_sell_data *find_obj_shop(char *arg, vnum_t shop_nr,
                                     struct obj_data **obj) {
  *obj = NULL;
...
bool shop_receive(struct char_data *ch, struct char_data *keeper, char *arg,
                  int buynum, bool cash, struct shop_sell_data *sell,
                  struct obj_data *obj, struct obj_data *cred, int price,
                  vnum_t shop_nr) {
  ...
  if ((cred && GET_OBJ_VAL(cred, 0) < price) ||
      (!cred && GET_NUYEN(ch) < price)) {
  ...
  if (shop_table[shop_nr].flags.IsSet(SHOP_DOCTOR)) {
    ...
    if (GET_OBJ_TYPE(obj) == ITEM_CYBERWARE) {
      if (ch->real_abils.ess + ch->real_abils.esshole <
          (GET_TOTEM(ch) == TOTEM_EAGLE ? GET_OBJ_VAL(obj, 4) << 1
                                        : GET_OBJ_VAL(obj, 4))) {
      ...
      for (check = ch->cyberware; check != NULL; check = check->next_content) {
        if ((GET_OBJ_VNUM(check) == GET_OBJ_VNUM(obj))) {
          if (GET_OBJ_VAL(check, 0) == CYB_REACTIONENHANCE) {
            if (++enhancers == 6) {
              sprintf(ENDOF(buf), " You already have the maximum number of "
                                  "reaction enhancers installed.");
              do_say(keeper, buf, cmd_say, SCMD_SAYTO);
              return FALSE;
            }
          } else {
        ...
        if (GET_OBJ_VAL(obj, 0) != CYB_EYES &&
            GET_OBJ_VAL(obj, 0) != CYB_FILTRATION &&
            GET_OBJ_VAL(obj, 0) != CYB_REACTIONENHANCE &&
            GET_OBJ_VAL(check, 0) == GET_OBJ_VAL(obj, 0)) {
          sprintf(ENDOF(buf),
                  " You already have a similar piece of cyberware.");
          do_say(keeper, buf, cmd_say, SCMD_SAYTO);
          return FALSE;
        }
      }
      ...
      if ((GET_OBJ_VAL(obj, 0) == BIO_PATHOGENICDEFENSE ||
           GET_OBJ_VAL(obj, 0) == BIO_TOXINEXTRACTOR) &&
          GET_OBJ_VAL(obj, 1) > GET_REAL_BOD(ch) / 2) {
        send_to_char(
            "Your body cannot support pathogenic defense of that level.\r\n",
            ch);
        return FALSE;
      }
    ...
    act("$n takes out a sharpened scalpel and lies $N down on the operating "
        "table.",
    ...
    GET_PHYSICAL(ch) = 100;
    GET_MENTAL(ch) = 100;
    act("You delicately install $p into $N's body.", FALSE, keeper, obj, ch,
        TO_CHAR);
    act("$n performs a delicate procedure on $N.", FALSE, keeper, 0, ch,
        TO_NOTVICT);
    act("$n delicately installs $p into your body.", FALSE, keeper, obj, ch,
        TO_VICT);
    bought = 1;
    if (cred)
    ...
    if ((GET_OBJ_TYPE(obj) == ITEM_DECK_ACCESSORY &&
      ...
      while (obj &&
             (bought < buynum && IS_CARRYING_N(ch) < CAN_CARRY_N(ch) &&
    ...
    if (bought < buynum) {
      ...
      else if (GET_OBJ_WEIGHT(ch->carrying) + IS_CARRYING_W(ch) >
               CAN_CARRY_W(ch))
        sprintf(ENDOF(buf), " You can only carry %d.", bought);
      do_say(keeper, buf, cmd_say, SCMD_SAYTO);
    }
  ...
  // Use our new replace_substring() function to swap out all %d's in arg with
  // the nuyen string.
...
void shop_buy(char *arg, struct char_data *ch, struct char_data *keeper,
              vnum_t shop_nr) {
  char buf[MAX_STRING_LENGTH];
  ...
  if (!(sell = find_obj_shop(arg, shop_nr, &obj))) {
    if (atoi(arg) > 0) {
      // Adapt for the player probably meaning an item number instead of an item
      // with a numeric keyword.
      char oopsbuf[strlen(arg) + 2];
      sprintf(oopsbuf, "#%s", arg);
      sell = find_obj_shop(oopsbuf, shop_nr, &obj);
    }
    if (!sell) {
      sprintf(buf, "%s %s", GET_CHAR_NAME(ch),
              shop_table[shop_nr].no_such_itemk);
      do_say(keeper, buf, cmd_say, SCMD_SAYTO);
      return;
    }
  }
  ...
  if (!str_cmp(buf, "cash")) {
    ...
    send_to_char(
        "Lacking an activated credstick, you choose to deal in cash.\r\n", ch);
    cash = TRUE;
  ...
  if (sell->type == SELL_AVAIL && GET_OBJ_AVAILTN(obj) > 0) {
    for (int q = 0; q <= SHOP_LAST_IDNUM_LIST_SIZE; q++)
      if (sell->lastidnum[q] == GET_IDNUM(ch)) {
        sprintf(buf, "%s Sorry, I couldn't get that in for you.",
                GET_CHAR_NAME(ch));
        do_say(keeper, buf, cmd_say, SCMD_SAYTO);
        extract_obj(obj);
        return;
    ...
    if (success < 1) {
      sprintf(buf, "%s I can't get ahold of that one for a while.",
              GET_CHAR_NAME(ch));
      do_say(keeper, buf, cmd_say, SCMD_SAYTO);
      ...
      float totaltime = ((GET_OBJ_AVAILDAY(obj) * buynum) / success) +
                        (2 * GET_AVAIL_OFFSET(ch));
      if (totaltime < 1) {
        int hours = (int)(24 * totaltime);
        sprintf(buf, "%s That will take about %d %s to come in.",
                GET_CHAR_NAME(ch), hours, hours == 1 ? "hour" : "hours");
      } else {
        sprintf(buf, "%s That will take about %d %s to come in.",
                GET_CHAR_NAME(ch), (int)totaltime,
                totaltime == 1 ? "day" : "days");
      }
      do_say(keeper, buf, cmd_say, SCMD_SAYTO);
      ...
      if (order) {
        ...
        order->number += buynum;
        order->timeavail =
            order->timeavail + (int)(SECS_PER_MUD_DAY * totaltime);
      } else {
    ...
    shop_receive(ch, keeper, arg, buynum, cash, sell, obj,
                 shop_table[shop_nr].type == SHOP_BLACK ? NULL : cred, price,
                 shop_nr);
  }
}

void shop_sell(char *arg, struct char_data *ch, struct char_data *keeper,
               vnum_t shop_nr) {
  ...
  if (!shop_table[shop_nr].buytypes.IsSet(GET_OBJ_TYPE(obj)) ||
      IS_OBJ_STAT(obj, ITEM_NOSELL) || GET_OBJ_COST(obj) < 1) {
  ...
  if (shop_table[shop_nr].flags.IsSet(SHOP_DOCTOR)) {
    ...
    sprintf(buf, "%s Relax...this won't hurt a bit.", GET_CHAR_NAME(ch));
    do_say(keeper, buf, cmd_say, SCMD_SAYTO);
    act("You delicately remove $p from $N's body.", FALSE, keeper, obj, ch,
        TO_CHAR);
    act("$n performs a delicate procedure on $N.", FALSE, keeper, 0, ch,
        TO_NOTVICT);
    act("$n delicately removes $p from your body.", FALSE, keeper, obj, ch,
        TO_VICT);
    GET_MENTAL(ch) = 100;
    GET_PHYSICAL(ch) = 100;
    affect_total(ch);
  ...
  sprintf(buf3, "%s sold %s at %s (%ld) for %d.", GET_CHAR_NAME(ch),
          GET_OBJ_NAME(obj), GET_CHAR_NAME(keeper), shop_table[shop_nr].vnum,
          sellprice);
  ...
  if (!sell) {
    if (!shop_table[shop_nr].flags.IsSet(SHOP_NORESELL) &&
        GET_OBJ_TYPE(obj) != ITEM_GUN_MAGAZINE &&
        GET_OBJ_TYPE(obj) != ITEM_CYBERWARE &&
        GET_OBJ_TYPE(obj) != ITEM_BIOWARE) {
...
void shop_list(char *arg, struct char_data *ch, struct char_data *keeper,
               vnum_t shop_nr) {
  char buf[MAX_STRING_LENGTH];
  if (!is_open(keeper, shop_nr))
    return;
  ...
  if (PRF_FLAGGED(ch, PRF_SCREENREADER)) {
    ...
    for (struct shop_sell_data *sell = shop_table[shop_nr].selling; sell;
         sell = sell->next, i++) {
      // Read the object; however, if it's an invalid vnum or has no sale cost,
      // skip it.
      if (!(obj = read_object(sell->vnum, VIRTUAL)) || GET_OBJ_COST(obj) < 1) {
        ...
        if (obj) {
          sprintf(
              buf2,
              "Shop %ld ('%s'): Removing %s (%ld) from sale due to cost of %d.",
              shop_nr, GET_NAME(keeper), GET_OBJ_NAME(obj), GET_OBJ_VNUM(obj),
              GET_OBJ_COST(obj));
          mudlog(buf2, ch, LOG_SYSLOG, TRUE);
          extract_obj(obj);
        } else {
          sprintf(buf2,
                  "Shop %ld ('%s'): Removing nonexistant item (%ld) from sale.",
                  shop_nr, GET_NAME(keeper), GET_OBJ_VNUM(obj));
          mudlog(buf2, ch, LOG_SYSLOG, TRUE);
        }
        continue;
      }
      ...
      sprintf(ENDOF(buf), "Item #%d: %s for %d nuyen", i, GET_OBJ_NAME(obj),
      ...
      if (!(sell->type == SELL_ALWAYS) &&
          !(sell->type == SELL_AVAIL && GET_OBJ_AVAILDAY(obj) == 0)) {
        if (sell->type == SELL_AVAIL) {
          if (GET_OBJ_AVAILDAY(obj) < 1) {
            int hours = 24 * GET_OBJ_AVAILDAY(obj);
            sprintf(ENDOF(buf), ". It will take %d hour%s to obtain", hours,
                    hours > 1 ? "s" : "");
          } else {
            sprintf(ENDOF(buf), ". It will take %d day%s to obtain",
                    (int)GET_OBJ_AVAILDAY(obj),
                    GET_OBJ_AVAILDAY(obj) > 1 ? "s" : "");
          }
        } else if (sell->stock <= 0) {
          strcat(buf, ". It is currently out of stock");
        } else {
          sprintf(ENDOF(buf), ". Only %d %s in stock", sell->stock,
                  sell->stock > 1 ? "are" : "is");
        }
      }
  ...
  if (shop_table[shop_nr].flags.IsSet(SHOP_DOCTOR)) {
    strcpy(buf, " ##   Available   Item                                Rating "
                "Ess/Index    Price\r\n"
                "--------------------------------------------------------------"
                "-----------------\r\n");
    ...
    for (struct shop_sell_data *sell = shop_table[shop_nr].selling; sell;
      ...
      if (!obj || GET_OBJ_COST(obj) < 1) {
        ...
        if (obj) {
          sprintf(
              buf2, "Shop %ld ('%s'): Removing %s from sale due to cost of %d.",
              shop_nr, GET_NAME(keeper), GET_OBJ_NAME(obj), GET_OBJ_COST(obj));
          mudlog(buf2, ch, LOG_SYSLOG, TRUE);
          extract_obj(obj);
        } else {
          sprintf(buf2, "Shop %ld ('%s'): Removing nonexistant item from sale.",
                  shop_nr, GET_NAME(keeper));
          mudlog(buf2, ch, LOG_SYSLOG, TRUE);
        }
        continue;
      }
      ...
      if (sell->type == SELL_ALWAYS ||
        ...
        if (GET_OBJ_AVAILDAY(obj) < 1)
          sprintf(ENDOF(buf), "~%-2d Hours   ",
                  (int)(24 * GET_OBJ_AVAILDAY(obj)));
        else
          sprintf(ENDOF(buf), "~%-2d Day%c    ", (int)GET_OBJ_AVAILDAY(obj),
                  GET_OBJ_AVAILDAY(obj) > 1 ? 's' : ' ');
      } else {
        if (sell->stock <= 0)
      ...
      if (IS_OBJ_STAT(obj, ITEM_NERPS)) {
        sprintf(ENDOF(buf), "^Y(N)^n %-29s^n %-6s%2s   %0.2f%c  %9d\r\n",
                GET_OBJ_NAME(obj),
                GET_OBJ_TYPE(obj) == ITEM_CYBERWARE ? "Cyber" : "Bio", buf2,
                ((float)GET_OBJ_VAL(obj, 4) / 100),
                GET_OBJ_TYPE(obj) == ITEM_CYBERWARE ? 'E' : 'I',
                buy_price(obj, shop_nr));
      } else {
...
void shop_value(char *arg, struct char_data *ch, struct char_data *keeper,
                vnum_t shop_nr) {
  char buf[MAX_STRING_LENGTH];
  if (!is_open(keeper, shop_nr))
    return;
  ...
  if (!shop_table[shop_nr].buytypes.IsSet(GET_OBJ_TYPE(obj)) ||
    ...
    sprintf(ENDOF(buf),
            " I would be able to give you around %d nuyen for that.",
            sell_price(obj, shop_nr));

  do_say(keeper, buf, cmd_say, SCMD_SAYTO);
}

bool shop_probe(char *arg, struct char_data *ch, struct char_data *keeper,
                vnum_t shop_nr) {
  if (!is_open(keeper, shop_nr))
    return FALSE;
  if (!is_ok_char(keeper, ch, shop_nr))
...
void shop_info(char *arg, struct char_data *ch, struct char_data *keeper,
               vnum_t shop_nr) {
  char buf[MAX_STRING_LENGTH];
  int num = 0, num2 = 0;
  if (!is_open(keeper, shop_nr))
  ...
  case ITEM_WEAPON:
    if (IS_GUN(GET_OBJ_VAL(obj, 3))) {
      ...
      if (GET_WEAPON_INTEGRAL_RECOIL_COMP(obj))
        sprintf(ENDOF(buf),
                ". It has %d round%s of built-in recoil compensation",
                GET_WEAPON_INTEGRAL_RECOIL_COMP(obj),
                GET_WEAPON_INTEGRAL_RECOIL_COMP(obj) > 1 ? "s" : "");
      if (GET_OBJ_VAL(obj, 7) > 0 || GET_OBJ_VAL(obj, 8) > 0 ||
          GET_OBJ_VAL(obj, 9) > 0)
      ...
      if (real_object(GET_OBJ_VAL(obj, 7)) > 0) {
        ...
        if ((GET_OBJ_VAL(obj, 8) > 0 && GET_OBJ_VAL(obj, 9) < 1) ||
            (GET_OBJ_VAL(obj, 8) < 1 && GET_OBJ_VAL(obj, 9) > 0))
          strcat(buf, " and ");
      ...
      sprintf(ENDOF(buf), ". It can hold a maximum of %d rounds.",
              GET_OBJ_VAL(obj, 5));
    } else {
      ...
      if (GET_WEAPON_DAMAGE_CODE(obj) == LIGHT) {
        ...
        sprintf(buf1,
                "SYSERR: Unable to map damage value %d for weapon '%s' (%ld) "
                "to a damage phrase.",
                GET_WEAPON_DAMAGE_CODE(obj), GET_OBJ_NAME(obj),
                GET_OBJ_VNUM(obj));
      ...
      if (GET_WEAPON_REACH(obj)) {
        sprintf(ENDOF(buf),
                " As a long weapon, it gives you %d meter%s of extended reach.",
      ...
      if (GET_WEAPON_STR_BONUS(obj) != 0) {
        if (GET_WEAPON_STR_BONUS(obj) == 1) {
          strcat(buf, " It is somewhat well-constructed and will let you hit a "
                      "little harder in combat.");
          ...
          strcat(buf, " It is extremely well-constructed, letting you hit with "
                      "great strength.");
        } else if (GET_WEAPON_STR_BONUS(obj) == 4) {
          strcat(buf, " It is masterfully constructed, letting you hit as hard "
                      "as possible.");
        } else {
          strcat(buf, " It has an indeterminate strength modifier.");
          sprintf(buf1,
                  "SYSERR: Unable to map strength modifier %d for weapon '%s' "
                  "(%ld) to a feature phrase.",
                  GET_WEAPON_STR_BONUS(obj), GET_OBJ_NAME(obj),
                  GET_OBJ_VNUM(obj));
          mudlog(buf1, NULL, LOG_SYSLOG, TRUE);
        }
      }
    ...
    break;
  case ITEM_WORN:
    num = (GET_OBJ_VAL(obj, 5) > 20 ? (int)(GET_OBJ_VAL(obj, 5) / 100)
                                    : GET_OBJ_VAL(obj, 5)) +
          (GET_OBJ_VAL(obj, 6) > 20 ? (int)(GET_OBJ_VAL(obj, 6) / 100)
                                    : GET_OBJ_VAL(obj, 6));
    if (num < 1)
    ...
    break;
  case ITEM_PROGRAM:
    sprintf(ENDOF(buf), " a rating %d %s program, that is %dMp in size.",
            GET_OBJ_VAL(obj, 1), programs[GET_OBJ_VAL(obj, 0)].name,
            GET_OBJ_VAL(obj, 2));
    break;
  case ITEM_CYBERDECK:
    ...
    if (GET_OBJ_VAL(obj, 1) < 2)
      strcat(
          buf,
          " You better hope you don't run into anything nasty while using it");
  ...
  case ITEM_FOOD:
    ...
    break;
  case ITEM_DOCWAGON:
    strcat(buf, " a DocWagon contract, it will call them out when your vital "
                "signs drop.");
    break;
  case ITEM_CONTAINER:
    ...
    if (obj->obj_flags.wear_flags.AreAnySet(ITEM_WEAR_BACK, ITEM_WEAR_ABOUT,
                                            ENDBIT))
      strcat(buf, " backpack.");
    else
  ...
  case ITEM_SPELL_FORMULA:
    sprintf(ENDOF(buf),
            " a rating %d spell formula, describing %s. It is designed for use "
            "by a %s mage.",
            GET_OBJ_VAL(obj, 0), spells[GET_OBJ_VAL(obj, 1)].name,
            GET_OBJ_VAL(obj, 2) == 1 ? "shamanic" : "hermetic");
    break;
  case ITEM_GUN_ACCESSORY:
    sprintf(ENDOF(buf),
            " a firearm accessory that attaches to the %s of a weapon.",
            (GET_OBJ_VAL(obj, 0) == 0
                 ? "top"
  ...
  case ITEM_GUN_AMMO:
    sprintf(ENDOF(buf),
            " a box of ammunition for reloading %s magazines. It contains %d "
            "rounds of %s ammo.",
            weapon_type[GET_AMMOBOX_WEAPON(obj)], GET_AMMOBOX_QUANTITY(obj),
            ammo_type[GET_AMMOBOX_TYPE(obj)].name);
    break;
  case ITEM_FOCUS:
    sprintf(ENDOF(buf), " a rating %d %s focus.", GET_OBJ_VAL(obj, 1),
            foci_type[GET_OBJ_VAL(obj, 0)]);
    break;
  case ITEM_MAGIC_TOOL:
    if (GET_OBJ_VAL(obj, 0) == TYPE_LIBRARY_CONJURE ||
        GET_OBJ_VAL(obj, 0) == TYPE_LIBRARY_SPELL) {
  ...
  case ITEM_MOD:
    ...
    if (GET_OBJ_VAL(obj, 6) >= MOD_INTAKE_FRONT &&
        GET_OBJ_VAL(obj, 6) <= MOD_INTAKE_REAR)
      strcat(buf, "intake");
    else if (GET_OBJ_VAL(obj, 6) >= MOD_BODY_FRONT &&
             GET_OBJ_VAL(obj, 6) <= MOD_BODY_WINDOWS)
      strcat(buf, "body");
    else if (GET_OBJ_VAL(obj, 6) >= MOD_COMPUTER1 &&
             GET_OBJ_VAL(obj, 6) <= MOD_COMPUTER3)
      strcat(buf, "computer");
    else
      strcat(buf, mod_name[GET_OBJ_VAL(obj, 6)]);
  ...
  if (GET_OBJ_WEIGHT(obj) < 1) {
    ...
    sprintf(ENDOF(buf), "%.0f kilogram%s", GET_OBJ_WEIGHT(obj),
            (GET_OBJ_WEIGHT(obj) >= 2 ? "s" : ""));
  sprintf(ENDOF(buf), " and I couldn't let it go for less than %d nuyen.",
          buy_price(obj, shop_nr));
...
void shop_check(char *arg, struct char_data *ch, struct char_data *keeper,
                vnum_t shop_nr) {
  char buf[MAX_STRING_LENGTH];
  if (!is_open(keeper, shop_nr))
    return;
  ...
  for (struct shop_order_data *order = shop_table[shop_nr].order; order;
       order = order->next)
    if (order->player == GET_IDNUM(ch)) {
      i++;
      float totaltime = order->timeavail - time(0);
      totaltime = totaltime / SECS_PER_MUD_DAY;
      sprintf(ENDOF(buf), " %d) %-30s (%d) - ", i,
              GET_OBJ_NAME(&obj_proto[real_object(order->item)]),
              order->number);
      if (totaltime < 0)
  ...
  if (i == 0) {
    sprintf(buf, "%s You don't have anything on order here.",
            GET_CHAR_NAME(ch));
    do_say(keeper, buf, cmd_say, SCMD_SAYTO);
  } else
    send_to_char(buf, ch);
...
void shop_rec(char *arg, struct char_data *ch, struct char_data *keeper,
              vnum_t shop_nr) {
  if (!is_open(keeper, shop_nr))
    return;
  if (!is_ok_char(keeper, ch, shop_nr))
  ...
  for (struct shop_order_data *order = shop_table[shop_nr].order; order;
       order = order->next)
    if (order->player == GET_IDNUM(ch) && order->timeavail < time(0) &&
        !--number) {
      struct obj_data *obj = read_object(order->item, VIRTUAL),
                      *cred = get_first_credstick(ch, "credstick");
      if (!cred && shop_table[shop_nr].type == SHOP_LEGAL) {
        sprintf(buf, "%s No Credstick, No Sale.", GET_CHAR_NAME(ch));
        do_say(keeper, buf, cmd_say, SCMD_SAYTO);
        return;
      }
      if (shop_receive(ch, keeper, arg, order->number,
                       cred && shop_table[shop_nr].type != SHOP_BLACK ? 0 : 1,
                       NULL, obj,
                       shop_table[shop_nr].type == SHOP_BLACK ? NULL : cred,
                       order->price, shop_nr)) {
...
void shop_cancel(char *arg, struct char_data *ch, struct char_data *keeper,
                 vnum_t shop_nr) {
  char buf[MAX_STRING_LENGTH];
  if (!is_open(keeper, shop_nr))
    return;
  ...
  if (!(number = atoi(arg)))
    ...
    for (struct shop_order_data *order = shop_table[shop_nr].order; order;
      ...
      if (order->player == GET_IDNUM(ch) && !--number) {
        struct shop_order_data *temp;
        sprintf(ENDOF(buf), " I'll let my contacts know you no longer want %s.",
                GET_OBJ_NAME(&obj_proto[real_object(order->item)]));
        REMOVE_FROM_LIST(order, shop_table[shop_nr].order, next);
        delete order;
        do_say(keeper, buf, cmd_say, SCMD_SAYTO);
...
void assign_shopkeepers(void) {
  ...
  for (index = 0; index <= top_of_shopt; index++) {
    ...
    else if (mob_index[rnum].func != shop_keeper &&
             shop_table[index].keeper != 1151) {
...
void randomize_shop_prices(void) {
  ...
  for (int i = 0; i <= top_of_shopt; i++) {
    if (shop_table[i].random_amount)
      shop_table[i].random_current =
          number(-shop_table[i].random_amount, shop_table[i].random_amount);
    else
      shop_table[i].random_current = 0;
    for (struct shop_sell_data *sell = shop_table[i].selling; sell;
         sell = sell->next)
      for (int q = 0; q < SHOP_LAST_IDNUM_LIST_SIZE; q++)
        sell->lastidnum[q] = 0;
  }
...
void list_detailed_shop(struct char_data *ch, vnum_t shop_nr) {
  ...
  sprintf(ENDOF(buf), "Name: %30s Shopkeeper: %s [%5ld]\r\n",
          ...
          mob_proto[real_mobile(shop_table[shop_nr].keeper)]
              .player.physical_text.name,
          shop_table[shop_nr].keeper);
  sprintf(ENDOF(buf),
          "Buy at:     [%1.2f], Sell at: [%1.2f], +/- %%: [%d], Current %%: "
          "[%d], Hours [%d-%d]\r\n",
          shop_table[shop_nr].profit_buy, shop_table[shop_nr].profit_sell,
          shop_table[shop_nr].random_amount, shop_table[shop_nr].random_current,
          shop_table[shop_nr].open, shop_table[shop_nr].close);
  sprintf(ENDOF(buf), "Type:       %s, Etiquette: %s\r\n",
          shop_type[shop_table[shop_nr].type],
          skills[shop_table[shop_nr].ettiquete].name);
  shop_table[shop_nr].races.PrintBits(buf2, MAX_STRING_LENGTH, pc_race_types,
                                      NUM_RACES);
  sprintf(ENDOF(buf), "!Serves:     %s\r\n", buf2);
  shop_table[shop_nr].flags.PrintBits(buf2, MAX_STRING_LENGTH, shop_flags,
                                      SHOP_FLAGS);
  sprintf(ENDOF(buf), "Flags:      %s\r\n", buf2);
  shop_table[shop_nr].buytypes.PrintBits(buf2, MAX_STRING_LENGTH, item_types,
                                         NUM_ITEMS);
  sprintf(ENDOF(buf), "Buytypes:   %s\r\n", buf2);
  strcat(buf, "Selling: \r\n");
  for (struct shop_sell_data *selling = shop_table[shop_nr].selling; selling;
       selling = selling->next)
    sprintf(ENDOF(buf), "%-50s (%5ld) Type: %s Amount: %d\r\n",
            obj_proto[real_object(selling->vnum)].text.name, selling->vnum,
            selling_type[selling->type], selling->stock);
  ...
  for (counter = zone_table[zone].number * 100; counter <= zone_table[zone].top;
    ...
    if (realcounter >= 0) {
      ...
      fprintf(fp,
              ...
              shop->random_amount, shop->open, shop->close,
              shop_type[shop->type]);
      fprintf(fp,
              "NoSuchItemKeeper:\t%s\n"
              "NoSuchItemPlayer:\t%s\n"
              ...
              "Sell:\t%s\n"
              "Name:\t%s\n",
              shop->no_such_itemk, shop->no_such_itemp, shop->not_enough_nuyen,
              shop->doesnt_buy, shop->buy, shop->sell, shop->shopname);
      fprintf(fp,
              "Flags:\t%s\n"
              "Races:\t%s\n"
              "Buytypes:\t%s\n"
              "Etiquette:\t%d\n",
              shop->flags.ToString(), shop->races.ToString(),
              shop->buytypes.ToString(), shop->ettiquete);
      fprintf(fp, "[SELLING]\n");
      int i = 0;
      for (struct shop_sell_data *sell = shop->selling; sell;
           sell = sell->next, i++) {
        fprintf(fp,
                "\t[SELL %d]\n"
                "\t\tVnum:\t%ld\n"
...
void shedit_disp_buytypes_menu(struct descriptor_data *d) {
  ...
  for (int counter = 1; counter < NUM_ITEMS; counter += 2) {
    send_to_char(CH, "%2d) %-20s %2d) %-20s\r\n", counter, item_types[counter],
                 counter + 1,
                 counter + 1 <= NUM_ITEMS ? item_types[counter + 1] : "");
  }
...
void shedit_disp_selling_menu(struct descriptor_data *d) {
  ...
  for (struct shop_sell_data *sell = SHOP->selling; sell;
    ...
    sprintf(buf, "%d) ^c%-50s^n (^c%5ld^n) Type: ^c%6s^n", i,
            GET_OBJ_NAME(&obj_proto[real_object(sell->vnum)]), sell->vnum,
            selling_type[sell->type]);
...
void shedit_parse(struct descriptor_data *d, const char *arg) {
  ...
  case SHEDIT_CONFIRM_SAVESTRING:
    ...
    case 'Y':
      if (!vnum_from_non_connected_zone(d->edit_number)) {
        sprintf(buf, "%s wrote new shop #%ld", GET_CHAR_NAME(d->character),
                d->edit_number);
      ...
      if (shop_nr > 0) {
        rnum_t okn, nkn;
        if (shop_table[shop_nr].keeper != SHOP->keeper &&
            shop_table[shop_nr].keeper != 1151) {
  ...
  case SHEDIT_PROFIT_BUY:
    ...
    if (profit < 1) {
      send_to_char(
          "Buy price multiplier must be at least 1! Enter multiplier: ", CH);
      return;
    }
  ...
  case SHEDIT_PROFIT_SELL:
    ...
    if (profit > 1 || profit <= 0) {
      send_to_char("Sell price multiplier must be greater than 0 and no more "
                   "than 1! Enter multiplier: ",
                   CH);
      return;
    }
  ...
  case SHEDIT_SELL_DELETE:
    if (number > 0) {
      for (struct shop_sell_data *sell = SHOP->selling; number && sell;
           sell = sell->next) {
        number--;
        if (!number) {
          struct shop_sell_data *temp;
