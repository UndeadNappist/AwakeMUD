...
public:
  explicit PerfIntvlData(size_t size, PerfIntvlData *next)
      : mSize(size), mInd(0), mCount(0), mAvgs(size), mMins(size), mMaxes(size),
        mpNextIntvl(next) {}
...
void PerfIntvlData::AddData(double avg, double min, double max) {
  mAvgs[mInd] = avg;
  mMins[mInd] = min;
  mMaxes[mInd] = max;
  ...
  if (mInd < (mSize - 1)) {
    ...
    if (mpNextIntvl) {
      mpNextIntvl->AddData(this->GetAvgAvg(), this->GetMinMin(),
                           this->GetMaxMax());
    }
  }
}

double PerfIntvlData::GetAvgAvg() const {
  double sum = 0;
...
double PerfIntvlData::GetMinMin() const {
  double min = INFINITY;
...
double PerfIntvlData::GetMaxMax() const {
  double max = 0;
...
static void check_thresholds(double val) {
  const unsigned int thresh_count =
      sizeof(threshold_info) / sizeof(threshold_info[0]);
...
size_t PERF_repr(char *out_buf, size_t n) {
  ...
  os << std::fixed << std::setprecision(2)
     << "                     Avg         Min         Max\n\r" << std::setw(3)
     << 1 << " Pulse:   " << std::setw(10) << last_pulse << "% "
     << std::setw(10) << last_pulse << "% " << std::setw(10) << last_pulse
     << "%\n\r"

     << std::setw(3) << sPulseData.GetCount() << " Pulses:  " << std::setw(10)
     << sPulseData.GetAvgAvg() << "% " << std::setw(10) << pulse_min << "% "
     << std::setw(10) << sPulseData.GetMaxMax() << "%\n\r"

     << std::setw(3) << sSecData.GetCount() << " Seconds: " << std::setw(10)
     << std::setw(10) << sSecData.GetAvgAvg() << "% " << std::setw(10)
     << sec_min << "% " << std::setw(10) << sSecData.GetMaxMax() << "%\n\r"

     << std::setw(3) << sMinuteData.GetCount() << " Minutes: " << std::setw(10)
     << std::setw(10) << sMinuteData.GetAvgAvg() << "% " << std::setw(10)
     << min_min << "% " << std::setw(10) << sMinuteData.GetMaxMax() << "%\n\r"

     << std::setw(3) << sHourData.GetCount() << " Hours:   " << std::setw(10)
     << std::setw(10) << sHourData.GetAvgAvg() << "% " << std::setw(10)
     << hour_min << "% " << std::setw(10) << sHourData.GetMaxMax() << "%\n\r"

     << "\n\rMax pulse:      " << max_pulse << "\n\r\n\r";

  for (i = 0; i < thresh_count; ++i) {
    os << "Over " << std::setw(5) << threshold_info[i].threshold
...
public:
  explicit PERF_prof_sect(const char *id)
      : mId(id), mLastEnterTime(), mPulseTotal(), mPulseMax(), mTotal(), mMax(),
        mPulseEnterCount(0), mPulseExitCount(0), mTotalEnterCount(0) {
  ...
  struct timeval const &GetPulseTotal() {
    return mPulseTotal;
  }
  struct timeval const &GetPulseMax() {
    return mPulseMax;
  }
  struct timeval const &GetTotal() {
    return mTotal;
  }
  struct timeval const &GetMax() {
    return mMax;
  }
...
public:
  ...
  void ResetAll();
  size_t ReprPulse(char *out_buf, size_t n) const {
    return ReprBase(out_buf, n, false, nullptr);
  }
  size_t ReprTotal(char *out_buf, size_t n) const {
    return ReprBase(out_buf, n, true, nullptr);
  }
  size_t ReprSect(char *out_buf, size_t n, const char *id) const;

private:
  size_t ReprBase(char *out_buf, size_t n, bool isTotal,
                  PERF_prof_sect *sect) const;
  void SectOutput(std::ostream &os, PERF_prof_sect *sect, bool isTotal) const;

  std::map<std::string, PERF_prof_sect *> mSections;
};

size_t PerfProfMgr::ReprSect(char *out_buf, size_t n, const char *id) const {
  if (!out_buf) {
    return 0;
  }
  if (n < 1) {
...
void PerfProfMgr::SectOutput(std::ostream &os, PERF_prof_sect *sect,
                             bool isTotal) const {
  ...
  os << std::left << std::setw(20) << sect->GetId() << "|" << std::right
  ...
  if (isTotal) {
    ...
    os << std::setw(12 - 1)
       << (100 * static_cast<double>(usecTotal) / USEC_PER_PULSE) << "%|";
  }
  os << std::setw(20 - 1)
     << (100 * static_cast<double>(usecMax) / USEC_PER_PULSE) << "%\n\r";
}

size_t PerfProfMgr::ReprBase(char *out_buf, size_t n, bool isTotal,
                             PERF_prof_sect *sect) const {
  if (!out_buf) {
    return 0;
  }
  ...
  os << std::setprecision(2) << std::fixed << "\n\r" << std::left
     ...
     << "|" << std::right << std::setw(12) << "Enter Count"
     << "|";
  if (!isTotal) {
...
PERF_prof_sect *PerfProfMgr::NewSection(const char *id) {
  PERF_prof_sect *ptr = new PERF_prof_sect(id);
  mSections[id] = ptr;
  return ptr;
}

void PerfProfMgr::ResetAll() {
  for (auto &&entry : this->mSections) {
    entry.second->PulseReset();
  }
}

void PERF_prof_sect::PulseReset() {
  mPulseEnterCount = 0;
  mPulseExitCount = 0;
  timerclear(&mPulseTotal);
  timerclear(&mPulseMax);
...
void PERF_prof_sect_enter(PERF_prof_sect *ptr) { ptr->Enter(); }

void PERF_prof_sect_exit(PERF_prof_sect *ptr) { ptr->Exit(); }

void PERF_prof_reset(void) { sProfMgr.ResetAll(); }
