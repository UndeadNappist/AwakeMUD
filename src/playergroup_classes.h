...
public:
  ...
  // Versions of setters that do not perform validity checks (avoid using unless
  ...
  // Misc methods.
  void
  audit_log(const char *msg,
            bool is_redacted); // Logs an unredacted message exactly as written.
  void audit_log_vfprintf(
      const char *format,
      ...); // Logs an unredacted message with format/args like printf.
  void secret_log(
      const char *msg); // Logs a pre-redacted message exactly as written.
  void secret_log_vfprintf(
      const char *format,
      ...); // Logs a pre-redacted message with format/args like printf.
