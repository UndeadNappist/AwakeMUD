...
extern void store_mail(long to, struct char_data *from,
                       const char *message_pointer);

// Prototypes from this file.
void perform_pgroup_grant_revoke(struct char_data *ch, char *argument,
                                 bool revoke);
const char *pgroup_print_privileges(Bitfield privileges);
void do_pgroup_promote_demote(struct char_data *ch, char *argument,
                              bool promote);
...
ACMD(do_invitations) {
  ...
  while (pgi) {
    ...
    strftime(expiration_string, 20, "%Y-%m-%d %H:%M:%S",
             localtime(&expiration));
    sprintf(buf, " '%s' (%s), which expires on %s\r\n", pgr->get_name(),
            pgr->get_alias(), expiration_string);
    pgi = pgi->next;
  }
  send_to_char(buf, ch);
}
...
ACMD(do_accept) {
  ...
  while (pgi) {
    ...
    if (strn_cmp(argument, pgr->get_name(), strlen(argument)) == 0 ||
        strn_cmp(argument, pgr->get_alias(), strlen(argument)) == 0) {
      send_to_char(ch,
                   "You accept the invitation and declare yourself a member of "
                   "'%s'.\r\n",
                   pgr->get_name());

      // Notify all online members.
      for (struct char_data *i = character_list; i; i = i->next) {
        if (!IS_NPC(i) && GET_PGROUP_MEMBER_DATA(i) &&
            GET_PGROUP(i)->get_idnum() == pgr->get_idnum()) {
          // Notify the character.
          if (pgr->is_secret() &&
              !GET_PGROUP_MEMBER_DATA(i)->privileges.AreAnySet(
                  PRIV_COCONSPIRATOR, PRIV_LEADER, ENDBIT)) {
            send_to_char(i,
                         "You notice that the membership numbers for '%s' have "
                         "gone up.\r\n",
                         pgr->get_name());
          } else {
      ...
      // TODO: Should we send mail (but account for secret status), or require
      // people to view the roster/logs?

      // Add the player to the group. This function also removes the invitation
      // and logs the character's joining (if applicable).
      pgr->add_member(ch);
...
ACMD(do_decline) {
  ...
  while (pgi) {
    ...
    // Skip over anything from a disabled group, but don't delete-- they'll
    // auto-expire on their own.
    if (pgr->is_disabled()) {
      pgi = pgi->next;
      continue;
    }
...
Playergroup *Playergroup::find_pgroup(long idnum) {
  ...
  if (!idnum) {
    log_vfprintf("SYSERR: Illegal pgroup num %ld passed to find_pgroup.",
                 idnum);
    return NULL;
  }
...
struct pgroup_cmd_struct {
  ...
  bool requires_coconspirator_if_secret;
    ...
    {"abdicate", PRIV_LEADER, do_pgroup_abdicate, FALSE, TRUE, FALSE, FALSE,
     FALSE},
    {"balance", PRIV_TREASURER, do_pgroup_balance, FALSE, TRUE, TRUE, FALSE,
     FALSE},
    ...
    {"design", PRIV_ARCHITECT, do_pgroup_design, FALSE, TRUE, TRUE, FALSE,
     FALSE},
    ...
    {"grant", PRIV_ADMINISTRATOR, do_pgroup_grant, FALSE, TRUE, TRUE, FALSE,
     TRUE},
    {"help", PRIV_NONE, do_pgroup_help, TRUE, TRUE, FALSE, FALSE, FALSE},
    {"invite", PRIV_RECRUITER, do_pgroup_invite, TRUE, TRUE, TRUE, FALSE,
     FALSE},
    {"invitations", PRIV_RECRUITER, do_pgroup_invitations, TRUE, TRUE, TRUE,
     FALSE, TRUE},
    ...
    {"privileges", PRIV_NONE, do_pgroup_privileges, TRUE, TRUE, TRUE, FALSE,
     FALSE},
    {"promote", PRIV_MANAGER, do_pgroup_promote, FALSE, TRUE, TRUE, FALSE,
     TRUE},
    {"quit", PRIV_NONE, do_pgroup_resign, TRUE, TRUE, FALSE, FALSE, FALSE},
    {"resign", PRIV_NONE, do_pgroup_resign, TRUE, TRUE, FALSE, TRUE, FALSE},
    {"revoke", PRIV_ADMINISTRATOR, do_pgroup_revoke, FALSE, TRUE, TRUE, FALSE,
     TRUE},
    {"roster", PRIV_NONE, do_pgroup_roster, TRUE, TRUE, TRUE, FALSE, TRUE},
    {"status", PRIV_NONE, do_pgroup_status, TRUE, TRUE, TRUE, TRUE, FALSE},
    {"transfer", PRIV_PROCURER, do_pgroup_transfer, FALSE, TRUE, TRUE, FALSE,
     FALSE},
...
ACMD(do_pgroup) {
  ...
  if (GET_PGROUP_MEMBER_DATA(ch)) {
    ...
    if (!GET_PGROUP(ch)) {
      ...
      log(buf);
      send_to_char("We're sorry, but your character is currently bugged. "
                   "Please log out and back in.\r\n"
                   "If that doesn't fix the problem, please reach out to the "
                   "staff for help.\r\n",
                   ch);
      return;
    }

    // Precondition: If your group is disabled, the only commands you can use
    // are LOGS, STATUS, and RESIGN.
    if (GET_PGROUP(ch)->is_disabled() &&
        !pgroup_commands[cmd_index].valid_while_group_disabled) {
      send_to_char("You can't perform that action after your group has been "
                   "disbanded.\r\n",
                   ch);
      display_pgroup_help(ch);
      return;
    ...
    // Precondition: If the command requires a kosherized group, you must be
    // part of one.
    if (!GET_PGROUP(ch)->is_founded() &&
        !pgroup_commands[cmd_index].valid_while_group_not_founded) {
      send_to_char("You need to be a member of a fully-fledged playergroup to "
                   "do that.\r\n",
                   ch);
      return;
    }
    ...
    if (GET_PGROUP(ch)->is_founded() &&
        !pgroup_commands[cmd_index].valid_while_group_is_founded) {
      // Deliberately letting this fall through-- only candidate command right
      // now is FOUND and it has its own error message.
    }

    // Precondition: You must have the appropriate privilege (or be leader) to
    // perform this command.
    if (pgroup_commands[cmd_index].privilege_required != PRIV_NONE &&
        !GET_PGROUP_MEMBER_DATA(ch)->privileges.IsSet(PRIV_LEADER)) {
      // If the group is secret, and you're not the leader, you must have the
      // COCONSPIRATOR priv to perform sensitive actions.
      if (!(GET_PGROUP_MEMBER_DATA(ch)->privileges.IsSet(
              pgroup_commands[cmd_index].privilege_required))) {
        send_to_char(
            ch, "You must be %s %s within your group to do that.\r\n",
            strchr((const char *)"aeiouyAEIOUY",
                   *pgroup_privileges[pgroup_commands[cmd_index]
                                          .privilege_required])
                ? "an"
                : "a",
    ...
    if (GET_PGROUP(ch)->is_secret() &&
        pgroup_commands[cmd_index].requires_coconspirator_if_secret) {
      if (!(GET_PGROUP_MEMBER_DATA(ch)->privileges.AreAnySet(
              PRIV_COCONSPIRATOR, PRIV_LEADER, ENDBIT))) {
        send_to_char("This group is clandestine, limiting your ability to dive "
  ...
  // Not a member of a group.
    ...
    // Precondition: If you are not part of a group, the only command you can
    // use is CREATE.
  ...
  // Precondition: You must have an accessible pocket secretary if one is
  // required.
  if (pgroup_commands[cmd_index].requires_pocket_secretary &&
      !has_valid_pocket_secretary(ch)) {
...
void do_pgroup_abdicate(struct char_data *ch, char *argument) {
  ...
  if (!*argument || str_cmp(argument, "confirm") != 0) {
    send_to_char(ch,
                 "If you're sure you want to abdicate from leadership of '%s', "
                 "type PGROUP ABCIDATE CONFIRM.\r\n",
  ...
  if (GET_PGROUP(ch)->is_secret())
    sprintf(buf, "A shadowy figure has abdicated from the leadership of '%s'.",
            pgr->get_name());
  else
    sprintf(buf, "%s has abdicated from the leadership of '%s'.",
  ...
  sprintf(buf2, "SELECT idnum FROM pfiles_playergroups WHERE `group` = %ld",
          pgr->get_idnum());
...
void do_pgroup_buy(struct char_data *ch, char *argument) {
  // TODO: Log.
  // pgroup  buy  <vehicle>  Purchase a vehicle, drawing funds from bank and
  // assigning ownership to pgroup.
  send_to_char("buy", ch);
}

void do_pgroup_contest(struct char_data *ch, char *argument) {
  // TODO: Log.
  // pgroup  contest  [confirm]      On confirm, kicks off election cycle. If
  // cycle open, joins cycle as nominee.
...
void do_pgroup_create(struct char_data *ch, char *argument) {
  ...
  if (GET_PGROUP_MEMBER_DATA(ch)) {
    send_to_char("You are already part of a playergroup. You'll need to leave "
                 "it first.\r\n",
                 ch);
    return;
  }
...
void do_pgroup_design(struct char_data *ch, char *argument) {
  ...
  /* Requirements for design:
   ...
   - PGROUP DESIGN puts you into a mode where you're essentially projecting
   (body vulnerable to attack, etc)

   Creates a new zone (or uses one that already exists if you've done this
   before) and puts you in a new empty room. Allows use of BLUEPRINT commands.
   Changes save automatically (there's no undo, just like normal building).
   Allows RETURN to return to your body.

   You can also PGROUP DESIGN TALLY|TOTAL|COST to see how much your drafted HQ
   will cost to actualize. You can then PGROUP DESIGN FINALIZE to lock it, pay
   the deposit, and send it to staff for approval.

   Staff can approve it (clones into a real zone for connection to world) or
   deny it (refunds nuyen to group, unlocks blueprint).
   */
}
...
void do_pgroup_disband(struct char_data *ch, char *argument) {
  ...
  if (!*argument || str_cmp(argument, "confirm") != 0) {
    send_to_char(ch,
                 "If you're sure you want to disband '%s', type PGROUP DISBAND "
                 "CONFIRM.\r\n",
                 GET_PGROUP(ch)->get_name());
    return;
  }
  ...
  // Read out the people who are getting kicked (in case we want to manually
  // restore later).
  sprintf(query_buf,
          "SELECT idnum, Rank, Privileges FROM pfiles_playergroups WHERE "
          "`group` = %ld ORDER BY Rank ASC",
          pgr->get_idnum());
  ...
  sprintf(buf,
          "The following characters are being kicked from '%s' (%s, %ld) due "
          "to disbanding by %s: \r\n",
          pgr->get_name(), pgr->get_alias(), pgr->get_idnum(),
          GET_CHAR_NAME(ch));
  ...
  while ((ns = results.Head())) {
    if (!strcmp(ns->data->privileges.ToString(), "0"))
      ...
      sprintf(ENDOF(buf), "%s, rank %d, privs: %s.\r\n",
              get_player_name(ns->data->idnum), ns->data->rank,
              pgroup_print_privileges(ns->data->privileges));
    }
    delete ns->data;
    results.RemoveItem(ns);
  }
  ...
  sprintf(query_buf, "DELETE FROM pfiles_playergroups WHERE `group` = %ld",
          pgr->get_idnum());
  mysql_wrapper(mysql, query_buf);

  // Delete pgroup's invitations. TODO: Notify the invitee that the invitation
  // has expired.
  sprintf(query_buf, "DELETE FROM playergroup_invitations WHERE `Group` = %ld",
          pgr->get_idnum());
  mysql_wrapper(mysql, query_buf);

  // TODO: How should we handle old logfiles?

  // Remove this group from all active players' char_data structs. Delete
  // members' pgroup_data pointers.
  for (struct char_data *i = character_list; i; i = i->next) {
    if (!IS_NPC(i) && GET_PGROUP_MEMBER_DATA(i) &&
        GET_PGROUP(i)->get_idnum() == pgr->get_idnum()) {
      // Notify the character, unless they're the person doing the disbanding.
...
void do_pgroup_donate(struct char_data *ch, char *argument) {
  // TODO: Log.
  // Wires the specified amount from your bank account to the pgroup's shell
  // accounts. pgroup  donate  <amount>  [reason]   Deposits <amount> of nuyen.
  // Reason is listed in logs.
  long amount;
  ...
  if (strlen(remainder) > MAX_PGROUP_LOG_LENGTH - 2) {
    send_to_char(ch,
                 "That reason is too long; you're limited to %d characters.",
  ...
  GET_PGROUP(ch)->audit_log_vfprintf("%s donated %ld nuyen. Reason: %s",
                                     GET_CHAR_NAME(ch), amount, remainder);
  GET_PGROUP(ch)->secret_log_vfprintf(
      "A shadowy figure donated %ld nuyen. Reason: %s", amount, remainder);
  sprintf(buf, "%s donated %ld nuyen to '%s'. Reason: %s", GET_CHAR_NAME(ch),
          amount, GET_PGROUP(ch)->get_name(), remainder);
...
void do_pgroup_found(struct char_data *ch, char *argument) {
  ...
  if (member_count < NUM_MEMBERS_NEEDED_TO_FOUND) {
    send_to_char(
        ch,
        "You need %d members to found your group, but you only have %d.\r\n",
  ...
  if (GET_NUYEN(ch) < COST_TO_FOUND_GROUP) {
    send_to_char(ch,
                 "You need to have %d nuyen on hand to found your group.\r\n",
                 COST_TO_FOUND_GROUP);
    return;
  }

  // TODO: Should this be done in a specific place or in the presence of a
  // specific NPC for RP reasons?
  ...
  GET_PGROUP(ch)->audit_log_vfprintf("%s has officially founded the group.",
                                     GET_CHAR_NAME(ch));
  GET_PGROUP(ch)->secret_log_vfprintf(
      "A shadowy figure has officially founded the group.");
}
...
void do_pgroup_invitations(struct char_data *ch, char *argument) {
  // pgroup  invitations  [revoke]  [name]  No args: Displays group's open
  // invitations. With revoke: Deletes the specified invitation.
}
...
void do_pgroup_lease(struct char_data *ch, char *argument) {
  // TODO: Log.
  // pgroup  lease  <apartment>      Lease an apartment in your pgroup's name
  // using pgroup bank funds.
...
void do_pgroup_logs(struct char_data *ch, char *argument) {
  ...
  // TODO: Potentially allow PGROUP LOGS (x) (y) to select logs between days X
  // and Y.
  int days;
  ...
  if (!*argument) {
    ...
    if (days < 1) {
      send_to_char(
          "Syntax: PGROUP LOGS [number of days of history to include]\r\n", ch);
      return;
    }
    if (days > MAX_PGROUP_LOG_READBACK) {
      send_to_char(
          ch, "You can only view the logs created in the last %d days.\r\n",
          MAX_PGROUP_LOG_READBACK);
      return;
    }
  }

  bool redacted_mode = GET_PGROUP(ch)->is_secret() &&
                       !GET_PGROUP_MEMBER_DATA(ch)->privileges.AreAnySet(
                           PRIV_COCONSPIRATOR, PRIV_LEADER, ENDBIT);

  // Select the logs between this moment and X days in the past, then display in
  // descending order.
  char querybuf[MAX_STRING_LENGTH];
  const char *query_fmt =
      "SELECT message FROM pgroup_logs"
      " WHERE "
      "  idnum = %ld"
  ...
  sprintf(querybuf, query_fmt, GET_PGROUP(ch)->get_idnum(), days,
          redacted_mode ? "TRUE" : "FALSE");
...
void do_pgroup_note(struct char_data *ch, char *argument) {
  ...
  GET_PGROUP(ch)->audit_log_vfprintf("Note from %s: %s", GET_CHAR_NAME(ch),
                                     argument);
  GET_PGROUP(ch)->secret_log_vfprintf("Note from someone: %s", argument);
  send_to_char(
      "You take a moment and type your note into your pocket secretary.\r\n",
      ch);
...
void do_pgroup_outcast(struct char_data *ch, char *argument) {
  ...
  for (vict = character_list; vict; vict = vict->next) {
    if (!IS_NPC(vict) &&
        (isname(name, GET_KEYWORDS(vict)) ||
         isname(name, GET_CHAR_NAME(vict)) || recog(ch, vict, name)))
      break;
  }
  ...
  // Ensure targeted character is part of the same group as the invoking
  // character.
  if (!(GET_PGROUP_MEMBER_DATA(vict) && GET_PGROUP(vict) &&
        GET_PGROUP(vict) == GET_PGROUP(ch))) {
  ...
  if (GET_PGROUP_MEMBER_DATA(vict)->rank >= GET_PGROUP_MEMBER_DATA(ch)->rank) {
    send_to_char(ch, "You can't do that to your %s!\r\n",
                 GET_PGROUP_MEMBER_DATA(vict)->rank >
                         GET_PGROUP_MEMBER_DATA(ch)->rank
                     ? "superiors"
                     : "peers");
    return;
  }
  ...
  // Delete pfile pgroup associations.
  sprintf(buf2, "DELETE FROM pfiles_playergroups WHERE `idnum` = %ld",
          GET_IDNUM(vict));
  mysql_wrapper(mysql, buf2);

  // Delete any old invitations that could let them come right back.
  sprintf(buf2, "DELETE FROM playergroup_invitations WHERE `idnum` = %ld",
          GET_IDNUM(vict));
  mysql_wrapper(mysql, buf2);

  // Log the action.
  GET_PGROUP(ch)->audit_log_vfprintf("%s has outcasted %s.", GET_CHAR_NAME(ch),
                                     GET_CHAR_NAME(vict));
  GET_PGROUP(ch)->secret_log_vfprintf("A shadowy figure has outcasted %s.",
                                      GET_CHAR_NAME(vict));
...
void do_pgroup_privileges(struct char_data *ch, char *argument) {
  send_to_char(ch, "You have the following privileges in '%s': %s\r\n",
               GET_PGROUP(ch)->get_name(),
               pgroup_print_privileges(GET_PGROUP_MEMBER_DATA(ch)->privileges));
}
...
void do_pgroup_resign(struct char_data *ch, char *argument) {
  ...
  if (!*argument || str_cmp(argument, "confirm") != 0) {
    send_to_char(ch,
                 "If you're sure you want to resign from '%s', type PGROUP "
                 "RESIGN CONFIRM.\r\n",
                 GET_PGROUP(ch)->get_name());
    return;
  }
...
void do_pgroup_status(struct char_data *ch, char *argument) {
  ...
  if (pgr->is_disabled()) {
    send_to_char("^RThis group has been disbanded by its leader and is no "
                 "longer operational.^n\r\n",
                 ch);
    return;
  }
  ...
  if (!pgr->is_founded()) {
    send_to_char("^YThis group is provisional, and needs to be founded with "
                 "PGROUP FOUND before it is fully operational.^n\r\n",
                 ch);
  } else {
  ...
  if (pgr->is_secret() && !GET_PGROUP_MEMBER_DATA(ch)->privileges.AreAnySet(
                              PRIV_COCONSPIRATOR, PRIV_LEADER, ENDBIT)) {
    send_to_char("This group is clandestine, limiting your ability to delve "
                 "deeper into its information.\r\n",
                 ch);
    return;
  }

  // Group is not secret, or member is privileged. Display sensitive
  // information.

  // Get membership info.
  sprintf(query_buf,
          "SELECT COUNT(*) FROM pfiles_playergroups WHERE `group` = %ld",
          pgr->get_idnum());
  mysql_wrapper(mysql, query_buf);
  MYSQL_RES *res = mysql_use_result(mysql);
...
void do_pgroup_transfer(struct char_data *ch, char *argument) {
  // TODO: Log.
  // pgroup  transfer  <vehicle>  <target>   Transfer a vehicle like
  // do_transfer.
  send_to_char("transfer", ch);
}

void do_pgroup_vote(struct char_data *ch, char *argument) {
  // PGROUP VOTE to list candidates (assuming a contest is open). PGROUP VOTE
  // FOR <name> to cast/change vote.
  send_to_char("vote", ch);
}

void do_pgroup_wire(struct char_data *ch, char *argument) {
  // TODO: Log.
  // wire <amount> <target> <reason>  Sends a given amount of nuyen to the
  // target, identifed by PC unique name. Reason must be given.
  ...
  GET_PGROUP(ch)->save_pgroup_to_db();
  if (isfile) {
    sprintf(buf, "UPDATE pfiles SET Bank=Bank+%lu WHERE idnum=%ld;", amount,
            isfile);
  ...
  GET_PGROUP(ch)->audit_log_vfprintf(
      "%s wired %lu nuyen to %s. Reason: %s", GET_CHAR_NAME(ch), amount,
      vict ? GET_CHAR_NAME(vict) : (player_name = get_player_name(isfile)),
      remainder);
  GET_PGROUP(ch)->secret_log_vfprintf(
      "A shadowy figure wired %lu nuyen to someone's account. Reason: %s",
      amount, remainder);
  sprintf(buf, "%s wired %lu nuyen to %s on behalf of '%s'. Reason: %s",
          GET_CHAR_NAME(ch), amount, vict ? GET_CHAR_NAME(vict) : player_name,
          GET_PGROUP(ch)->get_name(), remainder);
  mudlog(buf, ch, LOG_GRIDLOG, TRUE);
...
/* Help messaging for main pgroup command. Specific help on these commands
 * should be written into helpfiles (eg HELP PGROUP FOUND). */
void display_pgroup_help(struct char_data *ch) {
  ...
  for (int cmd_index = 0; *(pgroup_commands[cmd_index].cmd) != '\n';
    ...
    if (!(pgroup_commands[cmd_index].privilege_required == PRIV_NONE ||
          (GET_PGROUP_MEMBER_DATA(ch)->privileges.AreAnySet(
              pgroup_commands[cmd_index].privilege_required, PRIV_LEADER,
              ENDBIT)))) {
      continue;
    }

    // If the group is not yet founded and the command is not valid while not
    // founded, skip it.
    if (!(GET_PGROUP(ch)->is_founded() ||
          pgroup_commands[cmd_index].valid_while_group_not_founded)) {
      continue;
    ...
    // If the group has already been founded and the command is not valid while
    // founded, skip it.
    if (GET_PGROUP(ch)->is_founded() &&
        !pgroup_commands[cmd_index].valid_while_group_is_founded) {
      continue;
    ...
    // If the group is disabled and the command is not valid while the group is
    // disabled, skip it.
    if (GET_PGROUP(ch)->is_disabled() &&
        !pgroup_commands[cmd_index].valid_while_group_disabled) {
      continue;
    ...
    // If the command is 'create', don't show it (that's only valid for people
    // not in groups).
    if (pgroup_commands[cmd_index].command_pointer == do_pgroup_create) {
      continue;
    }

    // If we've gotten here, the command is assumed kosher for this character
    // and group combo-- print it in our list.
    sprintf(ENDOF(buf), " %-11s%s", pgroup_commands[cmd_index].cmd,
            cmds_written++ % 5 == 4 ? "\r\n" : "");
  }
...
void pgedit_parse(struct descriptor_data *d, const char *arg) {
  ...
  case PGEDIT_MAIN_MENU:
    ...
    case 'q':
      if (d->edit_pgroup->is_clone()) {
        ...
        GET_PGROUP(CH)->audit_log_vfprintf(
            "%s modified the group's information (name %s, alias %s, tag %s, "
            "%s).",
            GET_CHAR_NAME(CH), d->edit_pgroup->get_name(),
            d->edit_pgroup->get_alias(), d->edit_pgroup->get_tag(),
            d->edit_pgroup->is_secret() ? "clandestine" : "not clandestine");

        GET_PGROUP(CH)->secret_log_vfprintf(
            "A shadowy figure modified the group's information (name %s, alias "
            "%s, tag %s, %s).",
            d->edit_pgroup->get_name(), d->edit_pgroup->get_alias(),
        ...
        GET_PGROUP(CH)->audit_log_vfprintf(
            "%s created the group (name %s, alias %s, tag %s).",
            GET_CHAR_NAME(CH), d->edit_pgroup->get_name(),
            d->edit_pgroup->get_alias(), d->edit_pgroup->get_tag());

        GET_PGROUP(CH)->secret_log_vfprintf(
            "A shadowy figure created the group (name %s, alias %s, tag %s).",
            d->edit_pgroup->get_name(), d->edit_pgroup->get_alias(),
            d->edit_pgroup->get_tag());
  ...
  case PGEDIT_CHANGE_TAG:
    ...
    break;
  default:
    sprintf(buf, "SYSERR: Unknown edit mode %d referenced from pgedit_parse.",
            d->edit_mode);
    mudlog(buf, CH, LOG_MISCLOG, TRUE);
    log(buf);
    break;
...
// Search character's inventory and worn items for an accessible pocket
// secretary. If found, return true.
bool has_valid_pocket_secretary(struct char_data *ch) {
  ...
  for (int i = 0; i < NUM_WEARS; i++) {
    if (GET_EQ(ch, i) && GET_OBJ_SPEC(ch->equipment[i]) == pocket_sec)
      if (GET_OBJ_VAL(ch->equipment[i], 1) == 0 ||
          GET_OBJ_VAL(ch->equipment[i], 1) == GET_IDNUM(ch))
        return TRUE;
  }
...
const char *list_privs_char_can_affect(struct char_data *ch) {
  ...
  for (int priv = 0; priv < PRIV_MAX; priv++) {
    ...
    // Leaders can hand out anything; otherwise, only return things the char has
    // (except admin, which is leader-assigned-only).
    if (is_leader || (GET_PGROUP_MEMBER_DATA(ch)->privileges.IsSet(priv) &&
                      priv != PRIV_ADMINISTRATOR)) {
      sprintf(ENDOF(privstring_buf), "%s%s", is_first ? "" : ", ",
              pgroup_privileges[priv]);
      is_first = FALSE;
    }
  }
...
void perform_pgroup_grant_revoke(struct char_data *ch, char *argument,
  ...
  // Can't overflow the substring buffer if the substring buffer is always the
  ...
  // Find the index number of the requested privilege by comparing the typed
  // name with the privs table.
  ...
  case PRIV_MAX: // No privilege was found matching the string. Fail.
    send_to_char(
        ch, "'%s' is not a valid privilege. Privileges you can %s are: %s\r\n",
  ...
  case PRIV_ADMINISTRATOR: // ADMINISTRATOR cannot be handed out. Fail.
    if (!(GET_PGROUP_MEMBER_DATA(ch)->privileges.IsSet(PRIV_LEADER))) {
      send_to_char(ch,
                   "Only the leader of the group may %s that privilege.\r\n",
                   revoke ? "revoke" : "grant");
      return;
    }
    break;
  ...
  if (!(GET_PGROUP_MEMBER_DATA(ch)->privileges.AreAnySet(priv, PRIV_LEADER,
                                                         ENDBIT))) {
    send_to_char(ch,
                 "You must first be assigned that privilege before you can %s "
                 "others.\r\n",
                 revoke ? "revoke it from" : "grant it to");
    return;
  }

  // Now that we know the privilege is kosher, we can do the more expensive
  ...
  if (GET_PGROUP_MEMBER_DATA(vict)->rank >= GET_PGROUP_MEMBER_DATA(ch)->rank) {
    send_to_char(ch,
                 "You can only %s people who are lower-ranked than you.\r\n",
  ...
  if (revoke) {
    ...
    // Write to the log.
    GET_PGROUP(ch)->audit_log_vfprintf(
        "%s revoked the %s privilege from %s.", GET_CHAR_NAME(ch),
        pgroup_privileges[priv], GET_CHAR_NAME(vict));
    // No secret log for this one ("Someone has revoked X from someone else." is
    // pointless)

    // Write to the relevant characters' screens.
    send_to_char(ch, "You revoke from %s the %s privilege in '%s'.\r\n",
                 GET_CHAR_NAME(vict), pgroup_privileges[priv],
                 GET_PGROUP(ch)->get_name());
  ...
  // Grant mode.
    ...
    // Write to the log.
    GET_PGROUP(ch)->audit_log_vfprintf("%s granted %s the %s privilege.",
...
void do_pgroup_promote_demote(struct char_data *ch, char *argument,
  ...
  // Bounds check rank.
  if (!(rank = atoi(rank_string)) || rank < (promote ? 2 : 1) ||
      rank > MAX_PGROUP_RANK) {
  ...
  // Log the action.
  GET_PGROUP(ch)->audit_log_vfprintf("%s %s %s to rank %d.", GET_CHAR_NAME(ch),
                                     promote ? "promoted" : "demoted",
                                     GET_CHAR_NAME(vict), rank);

  GET_PGROUP(ch)->secret_log_vfprintf("Someone has been %s to rank %d.",
                                      promote ? "promoted" : "demoted", rank);
...
const char *pgroup_print_privileges(Bitfield privileges) {
  ...
  for (int priv = PRIV_ADMINISTRATOR; priv < PRIV_MAX; priv++) {
    if (privileges.IsSet(priv)) {
      sprintf(ENDOF(output), "%s%s", is_first ? "" : ", ",
              pgroup_privileges[priv]);
      if (priv == PRIV_LEADER)
