...
#define PRIV_COCONSPIRATOR                                                     \
  3 // In groups where membership is secret, allows viewing of roster.
...
#define PRIV_MAX                                                               \
  13 /* Not an actual priv-- used for iteration over pgroup_privileges[].      \
        Must always be equal to total number of privileges, not including      \
        priv_none of course. */
#define PRIV_NONE 10000 // No privilege required.

// Playergroup settings.
#define PGROUP_FOUNDED                                                         \
  0 // Set by PGROUP FOUND, indicates that group is fully founded/active.
#define PGROUP_DISABLED 1 // Set by PGROUP DISBAND.
#define PGROUP_SECRETSQUIRREL                                                  \
  2 // TODO: Membership is secret and requires PRIV_COCONSPIRATOR to view.
...
// Configurables.
#define NUM_MEMBERS_NEEDED_TO_FOUND                                            \
  2 // TODO: Should be 3, but decreased for testing purposes.
#define COST_TO_FOUND_GROUP                                                    \
  100000 // Nuyen that must be paid by the founding player in order to make a
         // group official.
#define PGROUP_INVITATION_LIFETIME_IN_DAYS                                     \
  7 // Number of IRL days an invitation will be valid for.
#define DEFAULT_PGROUP_LOG_LOOKUP_LENGTH                                       \
  1 // Number of IRL days to look back when PGROUP LOGS is passed no argument.

// Helper functions.
//  GET_PGROUP_MEMBER_DATA pulls up the character's membership struct, which has
//  rank, privileges, etc.
#define GET_PGROUP_MEMBER_DATA(ch) (ch)->pgroup
//  GET_PGROUP pulls the actual instantiated class of the pgroup that the
//  character is a member of. Unless you're using this to set values, make sure
//  you check for the presence of GET_PGROUP_MEMBER_DATA(ch) first to ensure the
//  pgroup exists.
#define GET_PGROUP(ch) GET_PGROUP_MEMBER_DATA(ch)->pgroup

// Maximums.
#define MAX_PGROUP_RANK 10
#define MAX_PGROUP_NAME_LENGTH                                                 \
  80 // If you change this, update your SQL tables too. SQL field length should
     // be 2x+1 this (or greater).
#define MAX_PGROUP_ALIAS_LENGTH                                                \
  20 // If you change this, update your SQL tables too. SQL field length should
     // be 2x+1 this (or greater).
#define MAX_PGROUP_LOG_LENGTH                                                  \
  256 // If you change this, update your SQL tables too. SQL field length should
      // be 2x+1 this (or greater).
#define MAX_PGROUP_LOG_READBACK                                                \
  30 // The maximum number of days into the past players can view group logs.

// Tag maximums: Only update tag-without-color, and update your SQL tables too.
// SQL field length should be MAX_PGROUP_TAG_LENGTH + 1 (or greater).
#define MAX_PGROUP_TAG_LENGTH_WITHOUT_COLOR 7
#define MAX_PGROUP_TAG_LENGTH                                                  \
  (                                                                            \
      MAX_PGROUP_TAG_LENGTH_WITHOUT_COLOR * 3 +                                \
      2) // Accounts for color codes before each letter as well as a ^n at the
         // end.
