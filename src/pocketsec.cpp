...
void wire_nuyen(struct char_data *ch, struct char_data *targ, int amount,
                long isfile) {
  GET_BANK(ch) -= amount;
  if (isfile) {
    sprintf(buf, "UPDATE pfiles SET Bank=Bank+%d WHERE idnum=%ld;", amount,
            isfile);
  ...
  sprintf(buf, "%s wired %d nuyen to %s.", GET_CHAR_NAME(ch), amount,
          targ ? GET_CHAR_NAME(targ) : (player_name = get_player_name(isfile)));
...
void pocketsec_phonemenu(struct descriptor_data *d) {
  ...
  send_to_char("\r\n[^cC^n]^call^n     [^cA^n]^cdd Name^n     [^cD^n]^celete "
               "Name^n     [^cB^n]^cack^n\r\n",
               CH);
...
void pocketsec_notemenu(struct descriptor_data *d) {
  ...
  send_to_char("\r\n[^cR^n]^cead^n     [^cA^n]^cdd Note^n     [^cD^n]^celete "
               "Note^n     [^cB^n]^cack^n\r\n",
               CH);
...
void pocketsec_menu(struct descriptor_data *d) {
  CLS(CH);
  if (!SEC->contains) {
    send_to_char("This pocket secretary has not yet been initialised. "
                 "Initialise? [Y/N]\r\n",
                 CH);
...
void pocketsec_mailmenu(struct descriptor_data *d) {
  ...
  for (mail = folder->contains; mail; mail = mail->next_content) {
    i++;
    send_to_char(CH, " %2d >%s%s\r\n", i, !GET_OBJ_VAL(mail, 0) ? " ^R" : " ",
                 GET_OBJ_NAME(mail));
  }
  send_to_char("\r\n[^cR^n]^cead Mail^n     [^cD^n]^celete Mail^n     "
               "[^cS^n]^cend mail^n     [^cB^n]^cack^n\r\n",
               CH);
