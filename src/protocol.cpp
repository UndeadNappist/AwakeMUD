...
static void ReportBug(const char *apText) {
  ...
  sprintf(bugbuf, "protocol.c bug: %s", apText);
...
static void CompressStart(descriptor_t *apDescriptor) {
   ...
   */
  ReportBug("CompressStart() in protocol.c is being called, but it doesn't do "
            "anything!\n");
}
...
static void CompressEnd(descriptor_t *apDescriptor) {
   ...
   * Otherwise you can just ignore this function.
   */
  ReportBug("CompressEnd() in protocol.c is being called, but it doesn't do "
            "anything!\n");
}
...
static const char s_Gauge1[] =
    "\005\002Health\002red\002HEALTH\002HEALTH_MAX\006";
static const char s_Gauge2[] = "\005\002Mana\002blue\002MANA\002MANA_MAX\006";
static const char s_Gauge3[] =
    "\005\002Movement\002green\002MOVEMENT\002MOVEMENT_MAX\006";
static const char s_Gauge4[] =
    "\005\002Exp TNL\002yellow\002EXPERIENCE\002EXPERIENCE_MAX\006";
...
static variable_name_t VariableNameTable[eMSDP_MAX + 1] = {
    ...
    {eMSDP_SNIPPET_VERSION, "SNIPPET_VERSION",
     NUMBER_READ_ONLY_SET_TO(SNIPPET_VERSION)},
...
static void PerformHandshake(descriptor_t *apDescriptor, char aCmd,
                             char aProtocol);
static void PerformSubnegotiation(descriptor_t *apDescriptor, char aCmd,
                                  char *apData, int aSize);
static void SendNegotiationSequence(descriptor_t *apDescriptor, char aCmd,
                                    char aProtocol);
static bool ConfirmNegotiation(descriptor_t *apDescriptor,
                               negotiated_t aProtocol, bool abWillDo,
                               bool abSendReply);
...
static void SendATCP(descriptor_t *apDescriptor, const char *apVariable,
                     const char *apValue);
...
static const char *GetAnsiColour(bool abBackground, int aRed, int aGreen,
                                 int aBlue);
static const char *GetRGBColour(bool abBackground, int aRed, int aGreen,
                                int aBlue);
...
static const char s_BoldMagenta[] =
    "\033[1;35m";                               /* Bright magenta foreground */
  ...
  if (!bInit) {
    ...
    for (i = eMSDP_NONE + 1; i < eMSDP_MAX; ++i) {
      if (VariableNameTable[i].Variable != i) {
        ReportBug(
            "MSDP: Variable table does not match the enums in the header.\n");
        break;
      }
    }
  }
    ...
    if (VariableNameTable[i].bString) {
      if (VariableNameTable[i].pDefault != NULL)
        pProtocol->pVariables[i]->pValueString =
            AllocString(VariableNameTable[i].pDefault);
...
void ProtocolInput(descriptor_t *apDescriptor, char *apData, int aSize,
                   char *apOut) {
  ...
  for (Index = 0; Index < aSize; ++Index) {
    ...
    if (CmdIndex >= MAX_PROTOCOL_BUFFER || IacIndex >= MAX_PROTOCOL_BUFFER) {
      ReportBug(
          "ProtocolInput: Too much incoming data to store in the buffer.\n");
      return;
    }
...
const char *ProtocolOutput(descriptor_t *apDescriptor, const char *apData,
                           int *apLength) {
  ...
  for (; i < MAX_OUTPUT_BUFFER && apData[j] != '\0' && !bTerminate &&
    ...
    if (apData[j] == '\t') {
      ...
      case 'E': /* light electric lime */
        ...
        break;
      case 'l': /* dark black, AKA you're a dick for using this color when most
                   clients have black backgrounds */
      ...
      case 'V': /* light violet */
        ...
        break;
      case '(': /* MXP link */
        if (!pProtocol->bBlockMXP &&
            pProtocol->pVariables[eMSDP_MXP]->ValueInt) {
          pCopyFrom = LinkStart;
          p_is_color = FALSE;
        }
        break;
        ...
        if (tolower(apData[++j]) == 'u') {
          ...
          while (apData[j] != '\0' && !bDone && bValid) {
            if (apData[j] == ']')
              ...
              Buffer[Index++] = apData[j++];
            else /* It's too long, so drop out - the colour code may still be
                    valid */
              bValid = FALSE;
          }
      ...
      if (pCopyFrom != NULL &&
          (!p_is_color || !D_PRF_FLAGGED(apDescriptor, PRF_NOCOLOR))) {
      ...
      case 'E': /* light electric lime */
        pCopyFrom = ColourRGB(apDescriptor, "F250");
        break;
      case 'l': /* black, AKA you're a dick for using this color when most
                   clients have black backgrounds */
        pCopyFrom = ColourRGB(apDescriptor, "F111");
        break;
      case 'L': /* grey */
  ...
  if (i >= MAX_OUTPUT_BUFFER) {
    i = 0;
    ReportBug(
        "ProtocolOutput: Too much outgoing data to store in the buffer.\n");
  }
  ...
  if (aMSDP > eMSDP_NONE && aMSDP < eMSDP_MAX) {
    ...
    if (VariableNameTable[aMSDP].bString) {
      ...
      int RequiredBuffer = strlen(VariableNameTable[aMSDP].pName) +
                           strlen(pProtocol->pVariables[aMSDP]->pValueString) +
                           12;
      ...
      if (RequiredBuffer >= MAX_VARIABLE_LENGTH) {
        ...
        sprintf(MSDPBuffer, "%c%c%c%c%s%c%s%c%c", IAC, SB, TELOPT_MSDP,
    ...
    {
      if (pProtocol->bMSDP) {
        sprintf(MSDPBuffer, "%c%c%c%c%s%c%d%c%c", IAC, SB, TELOPT_MSDP,
                MSDP_VAR, VariableNameTable[aMSDP].pName, MSDP_VAL,
...
void MSDPSendPair(descriptor_t *apDescriptor, const char *apVariable,
                  const char *apValue) {
  char MSDPBuffer[MAX_VARIABLE_LENGTH + 1] = {'\0'};
  ...
  if (apVariable != NULL && apValue != NULL) {
    ...
    if (RequiredBuffer >= MAX_VARIABLE_LENGTH) {
      ...
      sprintf(MSDPBuffer, "%c%c%c%c%s%c%s%c%c", IAC, SB, TELOPT_MSDP, MSDP_VAR,
              apVariable, MSDP_VAL, apValue, IAC, SE);
    } else if (pProtocol->bATCP) {
      sprintf(MSDPBuffer, "%c%c%cMSDP.%s %s%c%c", IAC, SB, TELOPT_ATCP,
...
void MSDPSendList(descriptor_t *apDescriptor, const char *apVariable,
                  const char *apValue) {
  char MSDPBuffer[MAX_VARIABLE_LENGTH + 1] = {'\0'};
  ...
  if (apVariable != NULL && apValue != NULL) {
    ...
    if (RequiredBuffer >= MAX_VARIABLE_LENGTH) {
      ...
      int i; /* Loop counter */
      sprintf(MSDPBuffer, "%c%c%c%c%s%c%c%c%s%c%c%c", IAC, SB, TELOPT_MSDP,
              MSDP_VAR, apVariable, MSDP_VAL, MSDP_ARRAY_OPEN, MSDP_VAL,
...
void MSDPSetString(descriptor_t *apDescriptor, variable_t aMSDP,
                   const char *apValue) {
  protocol_t *pProtocol = apDescriptor ? apDescriptor->pProtocol : NULL;
...
void MSDPSetTable(descriptor_t *apDescriptor, variable_t aMSDP,
                  const char *apValue) {
  protocol_t *pProtocol = apDescriptor ? apDescriptor->pProtocol : NULL;
...
void MSDPSetArray(descriptor_t *apDescriptor, variable_t aMSDP,
                  const char *apValue) {
  protocol_t *pProtocol = apDescriptor ? apDescriptor->pProtocol : NULL;
...
static void PerformHandshake(descriptor_t *apDescriptor, char aCmd,
  ...
  case (char)TELOPT_TTYPE:
    if (aCmd == (char)WILL) {
      ...
      ConfirmNegotiation(apDescriptor, eNEGOTIATED_TTYPE, FALSE,
                         pProtocol->bTTYPE);
      pProtocol->bTTYPE = FALSE;
  ...
  case (char)TELOPT_ECHO:
    if (aCmd == (char)DO) {
      ...
      ConfirmNegotiation(apDescriptor, eNEGOTIATED_ECHO, FALSE,
                         pProtocol->bECHO);
      pProtocol->bECHO = FALSE;
  ...
  case (char)TELOPT_NAWS:
    if (aCmd == (char)WILL) {
      ...
      ConfirmNegotiation(apDescriptor, eNEGOTIATED_NAWS, FALSE,
                         pProtocol->bNAWS);
      pProtocol->bNAWS = FALSE;
  ...
  case (char)TELOPT_CHARSET:
    if (aCmd == (char)WILL) {
      ...
      ConfirmNegotiation(apDescriptor, eNEGOTIATED_CHARSET, FALSE,
                         pProtocol->bCHARSET);
      pProtocol->bCHARSET = FALSE;
    } else if (aCmd == (char)DO) {
      /* Invalid negotiation, send a rejection */
  ...
  case (char)TELOPT_MSDP:
    if (aCmd == (char)DO) {
      ...
      ConfirmNegotiation(apDescriptor, eNEGOTIATED_MSDP, FALSE,
                         pProtocol->bMSDP);
      pProtocol->bMSDP = FALSE;
    } else if (aCmd == (char)WILL) {
      /* Invalid negotiation, send a rejection */
  ...
  case (char)TELOPT_MSSP:
    if (aCmd == (char)DO) {
      ...
      ConfirmNegotiation(apDescriptor, eNEGOTIATED_MSSP, FALSE,
                         pProtocol->bMSSP);
      pProtocol->bMSSP = FALSE;
    } else if (aCmd == (char)WILL) {
      /* Invalid negotiation, send a rejection */
  ...
  case (char)TELOPT_MCCP:
    if (aCmd == (char)DO) {
      ...
      ConfirmNegotiation(apDescriptor, eNEGOTIATED_MCCP, FALSE,
                         pProtocol->bMCCP);
  ...
  case (char)TELOPT_MXP:
    if (aCmd == (char)WILL || aCmd == (char)DO) {
      ...
      if (!pProtocol->bMXP) {
        /* Enable MXP. */
        const char EnableMXP[] = {(char)IAC, (char)SB, TELOPT_MXP,
                                  (char)IAC, (char)SE, '\0'};
      ...
      ConfirmNegotiation(apDescriptor, eNEGOTIATED_MXP2, FALSE,
                         pProtocol->bMXP);
      pProtocol->bMXP = FALSE;
    }
    break;
  ...
  case (char)TELOPT_ATCP:
    if (aCmd == (char)WILL) {
      ...
      ConfirmNegotiation(apDescriptor, eNEGOTIATED_ATCP, FALSE,
                         pProtocol->bATCP);
      pProtocol->bATCP = FALSE;
    } else if (aCmd == (char)DO) {
      /* Invalid negotiation, send a rejection */
...
static void PerformSubnegotiation(descriptor_t *apDescriptor, char aCmd,
  ...
  case (char)TELOPT_TTYPE:
    if (pProtocol->bTTYPE) {
      ...
      if (!strcmp(pProtocol->pVariables[eMSDP_CLIENT_ID]->pValueString,
                  "Unknown")) {
        delete[] pProtocol->pVariables[eMSDP_CLIENT_ID]->pValueString;
        pProtocol->pVariables[eMSDP_CLIENT_ID]->pValueString =
            AllocString(pClientName);
      ...
      if (pProtocol->pLastTTYPE == NULL ||
          (strcmp(pProtocol->pLastTTYPE, pClientName) &&
           strcmp(pProtocol->pVariables[eMSDP_CLIENT_ID]->pValueString,
                  pClientName))) {
        char RequestTTYPE[] = {(char)IAC, (char)SB, TELOPT_TTYPE, SEND,
                               (char)IAC, (char)SE, '\0'};
      ...
      if (PrefixString("Mudlet", pClientName)) {
        ...
        if (strlen(pClientName) > 7) {
          ...
          delete[] pProtocol->pVariables[eMSDP_CLIENT_VERSION]->pValueString;
          pProtocol->pVariables[eMSDP_CLIENT_VERSION]->pValueString =
              AllocString(pClientName + 7);

          /* Mudlet 1.1 and later supports 256 colours. */
          if (strcmp(pProtocol->pVariables[eMSDP_CLIENT_VERSION]->pValueString,
                     "1.1") >= 0) {
            pProtocol->pVariables[eMSDP_XTERM_256_COLORS]->ValueInt = 1;
            pProtocol->b256Support = eYES;
          }
    ...
    if (apDescriptor->pProtocol->Negotiated[aProtocol] != abWillDo) {
      ...
      if (abSendReply) {
        ...
        case eNEGOTIATED_TTYPE:
          SendNegotiationSequence(apDescriptor, (char)(abWillDo ? DO : DONT),
                                  TELOPT_TTYPE);
          break;
        case eNEGOTIATED_ECHO:
          SendNegotiationSequence(apDescriptor, (char)(abWillDo ? WILL : WONT),
                                  TELOPT_ECHO);
          break;
...
static void ParseMSDP(descriptor_t *apDescriptor, const char *apData) {
  ...
  while (*apData) {
    ...
    ExecuteMSDPPair(apDescriptor, Variable[MSDP_VAR - 1],
                    Variable[MSDP_VAL - 1]);
          ...
          if (MatchString(apVariable, VariableNameTable[i].pName)) {
            if (VariableNameTable[i].bString) {
              ...
              if (!VariableNameTable[i].bWriteOnce ||
                  !strcmp(apDescriptor->pProtocol->pVariables[i]->pValueString,
                          "Unknown")) {
                ...
                if (j >= VariableNameTable[i].Min) {
                  ...
                  apDescriptor->pProtocol->pVariables[i]->pValueString =
                      AllocString(pBuffer);
                }
              }
...
static void SendATCP(descriptor_t *apDescriptor, const char *apVariable,
  ...
  if (apVariable != NULL && apValue != NULL) {
    ...
    if (RequiredBuffer >= MAX_VARIABLE_LENGTH) {
      ...
      {
        sprintf(ATCPBuffer,
                "SendATCP: Variable name has a length of %d bytes (exceeds "
                "MAX_VARIABLE_LENGTH of %d).\n",
                RequiredBuffer, MAX_VARIABLE_LENGTH);
      }
      ...
      sprintf(ATCPBuffer, "%c%c%c%s %s%c%c", IAC, SB, TELOPT_ATCP, apVariable,
              apValue, IAC, SE);
    }
...
static void SendMSSP(descriptor_t *apDescriptor) {
  ...
  static MSSP_t MSSPTable[] = {
      ...
      {"LOCATION", "USA", NULL},
      {"MINIMUM AGE", "13",
       NULL}, // COPPA compliance-- we do not service those under 13.
...
static const char *GetAnsiColour(bool abBackground, int aRed, int aGreen,
  ...
  if (aRed == aGreen && aRed == aBlue && aRed < 2)
    ...
    return abBackground ? s_BackYellow
                        : aRed >= 3 ? s_BoldYellow : s_DarkYellow;
  else if (aRed == aBlue && aRed > aGreen)
    return abBackground ? s_BackMagenta
                        : aRed >= 3 ? s_BoldMagenta : s_DarkMagenta;
