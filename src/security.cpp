...
bool run_crypto_tests() {
  ...
  if (!validate_password(INSECURE_TEST_PASSWORD, hashed_password)) {
    log("FATAL ERROR: We failed to validate the initial argon2 test password "
        "after hashing.");
    exit(1);
  }
  ...
  sprintf(
      message_buf,
      "Reproducibility tests completed in %ld microseconds (approx ~%ld sec).",
      elapsed, elapsed_secs);
  log(message_buf);

  if (elapsed > (long)(ACCEPTABLE_HASH_TIME_IN_SECONDS * 1000000L)) {
    log("FATAL ERROR: Hashing time exceeded the allowable threshold. Modify "
        "security.h to increase your threshold, or lower your OPSLIMIT and "
        "MEMLIMIT values. WARNING: CHANGING OPSLIMIT OR MEMLIMIT WILL BREAK "
        "ALL YOUR OLD PASSWORDS.");
    exit(1);
  }

  // Begin test of the conversion algorithms.
  strncpy(hashed_password, CRYPT(INSECURE_TEST_PASSWORD, "TestChar"),
          MAX_PWD_LENGTH);

  // Validate crypt() vs password.
  if (!validate_password(INSECURE_TEST_PASSWORD, hashed_password)) {
    log("FATAL ERROR: validate_password() failed to match a crypt() password "
        "with its origin string.");
    exit(1);
  }

  // Check crypt() and update to argon2.
  if (!validate_and_update_password(INSECURE_TEST_PASSWORD, hashed_password)) {
    log("FATAL ERROR: validate_and_update_password() failed to validate a "
        "crypt() password.");
    exit(1);
  }
  ...
  if (strstr(hashed_password, ARGON2_HASH_PREFIX) == NULL) {
    log("FATAL ERROR: validate_and_update_password() failed to change crypt() "
        "password to argon2.");
    exit(1);
  }
  ...
  if (!validate_password(INSECURE_TEST_PASSWORD, hashed_password)) {
    log("FATAL ERROR: validate_password() failed to match an argon2() password "
        "with its origin string.");
    exit(1);
  }
  ...
  if (!validate_and_update_password(INSECURE_TEST_PASSWORD, hashed_password)) {
    log("FATAL ERROR: validate_and_update_password() failed to validate "
        "argon2() password.");
    exit(1);
  }
...
// hash_and_store_password(): Hashing is dead simple now. Writes out to second
// argument's array.
void hash_and_store_password(const char *password, char *array_to_write_to) {
  if (crypto_pwhash_str(array_to_write_to, password, strlen(password),
                        CRYPTO_OPSLIMIT, CRYPTO_MEMLIMIT) != 0) {
    /* out of memory */
...
// validate_password(): Check if a password matches the given hash. Allows
// overriding with a staff password. Returns TRUE on match, FALSE otherwise.
...
bool validate_password(const char *password, const char *hashed_password) {
  ...
  // If it matches the override password, let them in. This must be set on a
  // per-implementation basis. If this value is not set, the override is not
  // available. If set, we require that this value be at least
  // MINIMUM_STAFF_OVERRIDE_PASSWORD_LENGTH characters in length.
  ...
  if (override_password && !strcmp(password, override_password)) {
    if (strlen(override_password) < MINIMUM_STAFF_OVERRIDE_PASSWORD_LENGTH) {
      ...
      sprintf(message_buf,
              "USER ERROR: Someone attempted to use the staff override "
              "password, but said password did not meet the minimum length "
              "requirement of %d characters. Please re-set this value in "
              "environment variable %s and try again.",
              MINIMUM_STAFF_OVERRIDE_PASSWORD_LENGTH,
              STAFF_OVERRIDE_PASSWORD_ENV_VARIABLE);
      mudlog(message_buf, NULL, LOG_SYSLOG, TRUE);
      return FALSE;
    }
    // Log it loudly. The next log line after this will be the 'X has connected'
    // line.
    mudlog("^RWARNING: STAFF OVERRIDE PASSWORD USED TO LOG IN TO THE FOLLOWING "
           "CHARACTER.",
  ...
  if (strstr(hashed_password, ARGON2ID_HASH_PREFIX) != NULL)
    return crypto_pwhash_str_verify(hashed_password, password,
                                    strlen(password)) == 0;
  else if (strstr(hashed_password, ARGON2_HASH_PREFIX) != NULL) {
    /* WARNING: You are not using the currently-recommended Argon2id() hashing
     algorithm.
     Your game will probably still function, but your passwords will not be a
  ...
  /* Maintainer's note: If you see the following error, the above check is
   falling through and giving a too-long string to crypt().
   ...
   TODO: actually fix this */
    ...
    return !strncmp(CRYPT(password, hashed_password), hashed_password,
                    MAX_PWD_LENGTH);
}

// validate_and_update_password(): Check if a password matches the given hash.
// Re-hash to argon2 if needed. Returns TRUE on match, FALSE otherwise.
bool validate_and_update_password(const char *password, char *hashed_password) {
  ...
  if (strstr(hashed_password, ARGON2ID_HASH_PREFIX) == NULL) {
    mudlog("Info: Re-hashing old password to the newest algorithm.", NULL,
           LOG_SYSLOG, TRUE);
