...
#define WSPEC(name)                                                            \
  int(name)(struct char_data * ch, struct char_data * vict,                    \
            struct obj_data * weapon, int dam)
...
struct extra_descr_data {
  ...
  extra_descr_data() : keyword(NULL), description(NULL), next(NULL) {}
};
...
struct text_data {
  ...
  text_data() : keywords(NULL), name(NULL), room_desc(NULL), look_desc(NULL) {}
};
...
struct obj_flag_data {
  ...
  int availtn;
  float availdays;
  float street_index; // A multiplier on cost if bought through black or grey
                      // markets.
  int legality[3];
  long quest_id;
  int attempt; // ITEM_MONEY: Number of failed cracks. ITEM_WEAPON: Built-in
               // recoil comp.
};
...
struct obj_data {
  ...
  // No sense in initializing the value since it's memset to 0 in most
  // invocations.
  int canary;
#endif
  obj_data()
      : in_room(NULL), in_veh(NULL), ex_description(NULL), restring(NULL),
        photo(NULL), graffiti(NULL), source_info(NULL), carried_by(NULL),
        worn_by(NULL), in_obj(NULL), contains(NULL), next_content(NULL),
        targ(NULL), tveh(NULL) {}
};
/* ======================================================================= */
...
#endif
  room_direction_data()
      : general_description(NULL), keyword(NULL), exit_info(0), key(0),
        to_room(NULL), key_level(0), ward(0), idnum(0), hidden(0), material(0),
        barrier(0), condition(0), to_room_vnum(NOWHERE),
        go_into_secondperson(NULL), go_into_thirdperson(NULL),
...
struct room_data {
  ...
  struct room_direction_data
      *temporary_stored_exit[NUM_OF_DIRS]; // Stores exits that elevators or
                                           // spec procs overwrote
  ...
  room_data()
      : name(NULL), description(NULL), night_desc(NULL), ex_description(NULL),
        ...
        address(NULL), peaceful(0), func(NULL), staff_level_lock(0),
        contents(NULL), people(NULL), vehicles(NULL), watching(NULL) {
...
struct spell_data {
  ...
  spell_data() : name(NULL), next(NULL) {}
};
...
struct char_player_data {
  ...
  char_player_data()
      : char_name(NULL), background(NULL), title(NULL), pretitle(NULL),
        whotitle(NULL), prompt(NULL), matrixprompt(NULL), poofin(NULL),
        poofout(NULL), tradition(2), host(NULL) {}
};
...
struct char_point_data {
  ...
  unsigned int
      noto;   /* the reputation of the player  (karma earned via kills) */
...
struct char_special_data_saved {
  ...
  char_special_data_saved() : dirty(FALSE) {}
};
...
struct char_special_data {
  ...
  struct obj_data
      *programming; /* Program char is currently designing/programming */
  int conjure[4];
  int num_spirits;
  long idnum;
  bool nervestrike;
  ...
  char_special_data()
      : fight_veh(NULL), fighting(NULL), hunting(NULL), programming(NULL),
        position(POS_STANDING), defined_position(NULL), leave(NULL),
        arrive(NULL), subscribe(NULL), rigging(NULL), mindlink(NULL),
        spirits(NULL) {
    for (int i = 0; i < 3; i++)
      conjure[i] = 0;
...
struct player_special_data {
  ...
  player_special_data()
      : aliases(NULL), remem(NULL), last_tell(0), questnum(0),
        obj_complete(NULL), mob_complete(NULL), watching(NULL), ignored(NULL) {
...
struct mob_special_data {
  ...
  mob_special_data() : memory(NULL), alert(0), alerttime(0), lasthit(0) {
    for (int i = 0; i < 10; i++)
      mob_skills[i] = 0;
  }
};
...
struct veh_follow {
  ...
  veh_follow() : follower(NULL), next(NULL) {}
};
...
struct follow_type {
  ...
  follow_type() : follower(NULL), next(NULL) {}
};
...
struct veh_data {
  ...
  veh_data()
      : in_room(NULL), name(NULL), description(NULL), short_description(NULL),
        restring(NULL), long_description(NULL), restring_long(NULL),
        inside_description(NULL), rear_description(NULL), followers(NULL),
        following(NULL), followch(NULL), mount(NULL), idnum(0), owner(0),
        spare(0), spare2(0), dest(NULL), contents(NULL), people(NULL),
        rigger(NULL), fighting(NULL), fight_veh(NULL), next_veh(NULL),
        next_sub(NULL), prev_sub(NULL), carriedvehs(NULL), in_veh(NULL),
        towing(NULL), grid(NULL), leave(NULL), arrive(NULL), next(NULL) {
...
struct char_data {
  ...
  Pgroup_data
      *pgroup; /* Data concerning the player group this char is part of. */
  Pgroup_invitation *pgroup_invitations; /* The list of open group invitations
                                            associated with this player. */

  /* Adding a field here? If it's a pointer, add it to utils.cpp's
   * copy_over_necessary_info() to avoid breaking mdelete etc. */

  char_data()
      : in_room(NULL), was_in_room(NULL), player_specials(NULL), in_veh(NULL),
        persona(NULL), squeue(NULL), sustained(NULL), ssust(NULL),
        carrying(NULL), desc(NULL), cyberware(NULL), bioware(NULL),
        next_in_room(NULL), next(NULL), next_fighting(NULL), next_in_zone(NULL),
        next_in_veh(NULL), next_watching(NULL), followers(NULL), master(NULL),
        spells(NULL), pgroup(NULL), pgroup_invitations(NULL) {
...
struct descriptor_data {
  ...
  bool edit_convert_color_codes; /* if this is true, display color codes in
                                    descs as ^^ for copy-paste */
  ...
  descriptor_data()
      : showstr_head(NULL), showstr_point(NULL), str(NULL), output(NULL),
        ...
        snoop_by(NULL), next(NULL), invalid_command_counter(0),
        iedit_limit_edits(0), misc_data(NULL), edit_obj(NULL), edit_room(NULL),
...
struct msg_type {
  ...
  msg_type() : attacker_msg(NULL), victim_msg(NULL), room_msg(NULL) {}
};
...
struct index_data {
  ...
  index_data() : func(NULL), sfunc(NULL), wfunc(NULL) {}
};
...
struct phone_data {
  ...
  phone_data()
      : number(0), connected(0), rtg(1102), dest(NULL), phone(NULL),
        persona(NULL), next(NULL) {}
};
...
struct spirit_data {
  ...
  spirit_data() : called(0), next(NULL) {}
};

struct order_data {
  char name[50];
  void (*func)(struct char_data *ch, struct char_data *spirit,
...
struct sustain_data {
  ...
  sustain_data()
      : spell(0), subtype(0), force(0), success(0), idnum(0), time(0), drain(0),
        caster(0), focus(NULL), spirit(NULL), other(NULL), next(NULL) {}
};
...
struct spirit_sustained {
  ...
  spirit_sustained() : target(NULL), next(NULL) {}
};
...
struct combat_data {
  ...
  combat_data(struct char_data *character, struct obj_data *weap)
      : too_tall(FALSE), skill(0), tn(0), dice(0), successes(0), dam_type(0),
        is_physical(FALSE), power(0), damage_level(0), weapon_is_gun(FALSE),
        weapon_has_bayonet(FALSE), burst_count(0), recoil_comp(0),
        climbingclaws(0), fins(0), handblades(0), handrazors(0),
        improved_handrazors(0), handspurs(0), footanchors(0),
        bone_lacing_power(0), num_cyberweapons(0), ch(NULL), veh(NULL),
        weapon(NULL), magazine(NULL), gyro(NULL) {
...
struct help_data {
  ...
  help_data(const char *title_to_keep_for_sql)
      : original_title(NULL), title(NULL), body(NULL), category(0),
        links(NULL) {
