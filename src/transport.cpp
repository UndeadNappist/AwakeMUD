...
struct dest_data taxi_destinations[] = {
    ...
    {"dante", "Dante's Inferno", 32661,
     TAXI_DEST_TYPE_RESTAURANTS_AND_NIGHTCLUBS, TRUE},
    ...
    {"mitsuhama", "Mitsuhama Towers", 32722, TAXI_DEST_TYPE_CORPORATE_PARK,
     TRUE},
    ...
    // { "auburn", "Auburn", 29011 },
    {"hellhound", "Hellhound Bus Stop", 32501, TAXI_DEST_TYPE_TRANSPORTATION,
     TRUE},
    // { "lysleul", "Lysleul Plaza", 30551 },
    // { "microdeck", "Auburn Microdeck Outlet", 29010 },
    {"novatech", "Novatech Seattle", 32541, TAXI_DEST_TYPE_CORPORATE_PARK,
     TRUE},
    ...
    {"sapphire", "The Star Sapphire", 70301, TAXI_DEST_TYPE_ACCOMMODATIONS,
     TRUE},
    // { "everett", "Everett", 39263 },
    // { "beacon", "Beacon Mall Everett", 39253 },
    // { "touristville", "Touristville", 25313 },
    // { "skeleton", "The Skeleton", 25308 },
    {"junkyard", "The Tacoma Junkyard", 2070, TAXI_DEST_TYPE_AREA_OF_TOWN,
     TRUE},
#ifdef USE_PRIVATE_CE_WORLD
    {"planetary", "Planetary Corporation", 72503, TAXI_DEST_TYPE_CORPORATE_PARK,
     TRUE},
#endif
...
static struct transport_type seatac[NUM_SEATAC_STATIONS] = {
    {501, SOUTH, 3018, NORTH}, {501, EAST, 20235, WEST},
    {501, EAST, 29036, WEST},  {501, EAST, 9823, WEST},
    {501, EAST, 29036, WEST},  {501, EAST, 20235, WEST},
};
/*
...
SPECIAL(taxi);
static int process_elevator(struct room_data *room, struct char_data *ch,
                            int cmd, char *argument);
...
// Define to collapse validation logic for destinations. Input is an integer
// index in the destination list.
#define DEST_IS_VALID(destination, dest_list)                                  \
  ((dest_list)[(destination)].enabled &&                                       \
   !vnum_from_non_connected_zone((dest_list)[(destination)].vnum))

// Taxi sign: If you look at it, it prints a dynamic description instead of the
// hardcoded one.
SPECIAL(taxi_sign) {
  ...
  default:
    sprintf(buf,
            "Warning: taxi_sign spec given to object %s (%ld) that had no "
            "matching definition in transport.cpp!",
  ...
  strcpy(
      buf,
      "The keyword for each location is listed after the location name.  Say "
      "the keyword to the driver, and for a small fee, he will drive you to "
      "your destination.\r\n--------------------------------------------\r\n");

  bool is_first_printed_dest_title = TRUE;

  // Print that sweet, sweet destination-flavored goodness.
  for (unsigned int taxi_dest_type = 0; taxi_dest_type < NUM_TAXI_DEST_TYPES;
       taxi_dest_type++) {
    ...
    for (int dest_index = 0; *dest_data_list[dest_index].keyword != '\n';
         dest_index++) {
      if (DEST_IS_VALID(dest_index, dest_data_list) &&
          dest_data_list[dest_index].type == taxi_dest_type) {
    ...
    // We have something! Print the dest type's title to make the list
    // ~~fancy~~. Extra carriage return before if we're not the first.
    ...
    for (unsigned int dest_index = 0;
         *dest_data_list[dest_index].keyword != '\n'; dest_index++) {
      if (DEST_IS_VALID(dest_index, dest_data_list) &&
          dest_data_list[dest_index].type == taxi_dest_type) {
...
void open_taxi_door(struct room_data *room, int dir, struct room_data *taxi,
  ...
  memset((char *)taxi->dir_option[dir], 0, sizeof(struct room_direction_data));
  ...
  // Optionally set the taxi's room rating so it waits a few ticks before trying
  // to drive off again.
  taxi->rating = rating;
}
      ...
      if (world[j].dir_option[i]) {
        ...
        if (to->people) {
          if (j >= real_room(FIRST_PORTCAB))
            sprintf(
                buf,
                "The taxi doors slide shut and it pulls off from the curb.");
          else
            sprintf(buf, "The taxi door slams shut as its wheels churn up a "
                         "cloud of smoke.");
          act(buf, FALSE, to->people, 0, 0, TO_ROOM);
          act(buf, FALSE, to->people, 0, 0, TO_CHAR);
        }
        if (world[j].people)
          act("The door automatically closes.", FALSE, world[j].people, 0, 0,
              TO_CHAR);
      }
    if (j == LAST_CAB)
      j = FIRST_PORTCAB - 1;
...
ACMD(do_hail) {
  ...
  if (IS_ASTRAL(ch)) {
    send_to_char(
        "Magically active cab drivers...now I've heard everything...\n\r", ch);
    return;
  }
  ...
  if (AFF_FLAGS(ch).AreAnySet(AFF_SPELLINVIS, AFF_INVISIBLE, AFF_SPELLIMPINVIS,
                              AFF_IMP_INVIS, ENDBIT) ||
      GET_INVIS_LEV(ch) > 0) {
    send_to_char("A cab almost stops, but guns it at the last second, "
                 "splashing you...\n\r",
                 ch);
    return;
  }
  ...
  for (int dir = number(NORTH, UP - 1);; dir = number(NORTH, UP - 1))
    if (!ch->in_room->dir_option[dir]) {
      ...
      if (portland)
        sprintf(
            buf,
            "A nice looking red and white cab pulls up smoothly to the curb, "
            "and its door opens to the %s.",
            fulldirs[dir]);
      else
        sprintf(buf,
                "A beat-up yellow cab screeches to a halt, "
                "and its door opens to the %s.",
                fulldirs[dir]);
...
SPECIAL(taxi) {
  ...
  if (!cmd) {
    // Iterate through the cab's contents, looking for anyone that the driver
    // recognizes.
    for (temp = driver->in_room->people; temp; temp = temp->next_in_room)
      if (temp != driver && memory(driver, temp))
        break;

    // If nobody was found, clear out the driver's memory structure and wait for
    // a command.
    if (!temp) {
      GET_SPARE1(driver) = 0;
      GET_SPARE2(driver) = 0;
    ...
    case ACT_REPLY_DEST:
      if (portland)
        sprintf(say, "%s?  Sure, that will be %d nuyen.",
                port_destinations[GET_SPARE2(driver)].str,
                (int)GET_SPARE1(driver));
      else
        sprintf(say, "%s?  Yeah, sure...it'll cost ya %d nuyen, whaddya say?",
                taxi_destinations[GET_SPARE2(driver)].str,
                (int)GET_SPARE1(driver));
      do_say(driver, say, 0, 0);
      if (GET_EXTRA(driver) == 1) {
        do_say(driver,
               "But seeing as you're new around here, I'll waive my usual fee, "
               "okay?",
               0, 0);
        GET_SPARE1(driver) = 0;
      }
        ...
        for (int j = number(NORTH, NORTHWEST);; j = number(NORTH, NORTHWEST)) {
          ...
          if (!world[dest].dir_option[j]) {
            ...
            if (world[dest].people) {
              act("A taxi pulls to a stop, its door sliding open.", FALSE,
                  world[dest].people, 0, 0, TO_ROOM);
              act("A taxi pulls to a stop, its door sliding open.", FALSE,
                  world[dest].people, 0, 0, TO_CHAR);
            }
  ...
  if (!IS_NPC(ch) && memory(driver, ch) &&
      (CMD_IS("north") || CMD_IS("east") || CMD_IS("west") || CMD_IS("south") ||
       CMD_IS("ne") || CMD_IS("se") || CMD_IS("sw") || CMD_IS("nw") ||
       CMD_IS("northeast") || CMD_IS("southeast") || CMD_IS("southwest") ||
       CMD_IS("northwest")) &&
  ...
  // If you're an NPC, or if the driver's not listening in the first place--
  // bail.
  if (IS_NPC(ch) || (GET_ACTIVE(driver) != ACT_AWAIT_CMD &&
                     GET_ACTIVE(driver) != ACT_AWAIT_YESNO))
    return FALSE;
  ...
  if (CMD_IS("say") || CMD_IS("'")) {
    ...
    if (GET_ACTIVE(driver) == ACT_AWAIT_CMD)
      for (dest = 0; (portland ? *port_destinations[dest].keyword
                               : *taxi_destinations[dest].keyword) != '\n';
           dest++) {
        // Skip invalid destinations.
        if (!DEST_IS_VALID(dest,
                           portland ? port_destinations : taxi_destinations))
          continue;

        if (str_str((const char *)argument,
                    (portland ? port_destinations[dest].keyword
                              : taxi_destinations[dest].keyword))) {
          comm = CMD_TAXI_DEST;
          found = TRUE;
          break;
    ...
    if (!found) {
      if (str_str(argument, "yes") || str_str(argument, "sure") ||
          str_str(argument, "alright") || str_str(argument, "yeah") ||
          str_str(argument, "okay") || str_str(argument, "yup")) {
        comm = CMD_TAXI_YES;
  ...
  /* I would like to extend a personal and heartfelt 'fuck you' to whomever
   * thought that using the anonymously-named 'i' as both an rnum and a
   * direction was a good idea. - LS */
  if (comm == CMD_TAXI_DEST && !memory(driver, ch) &&
      real_room(GET_LASTROOM(ch)) > -1 && GET_ACTIVE(driver) == ACT_AWAIT_CMD) {
    for (int dir = NORTH; dir < UP; dir++)
      if (ch->in_room->dir_option[dir]) {
    ...
    while (temp_room) {
      int x =
          find_first_step(real_room(temp_room->number),
                          real_room((portland ? port_destinations[dest].vnum
                                              : taxi_destinations[dest].vnum)));
      if (x == -2)
        break;
      else if (x < 0) {
        temp_room = NULL;
    ...
    GET_SPARE2(driver) = portland ? port_destinations[GET_SPARE2(driver)].vnum
                                  : taxi_destinations[GET_SPARE2(driver)].vnum;
    GET_ACTIVE(driver) = ACT_DRIVING;
    ...
    for (int dir = NORTH; dir < UP; dir++)
      if (ch->in_room->dir_option[dir]) {
        ...
        act("The door shuts as the taxi begins to accelerate.", FALSE, ch, 0, 0,
            TO_ROOM);
        act("The door shuts as the taxi begins to accelerate.", FALSE, ch, 0, 0,
            TO_CHAR);
      }
...
#define DOOR world[rnum_from].dir_option[direction_from]
  if (!DOOR) {
    ...
    sprintf(buf, "Building new elevator door %s from %ld to %ld.",
            fulldirs[direction_from], world[rnum_from].number,
            world[rnum_to].number);
    log(buf);
#endif
...
#ifdef ELEVATOR_DEBUG
    sprintf(buf, "Reusing existing elevator door %s from %ld to %ld.",
            fulldirs[direction_from], world[rnum_from].number,
            world[rnum_to].number);
    log(buf);
#endif
  ...
  if (!DOOR->general_description)
    DOOR->general_description =
        str_dup("A pair of elevator doors allows access to the liftway.\r\n");
...
static void init_elevators(void) {
  ...
  if ((real_button = real_object(ELEVATOR_BUTTON_VNUM)) < 0)
    log("Elevator button object does not exist; will not load elevator call "
        "buttons.");
  if ((real_panel = real_object(ELEVATOR_PANEL_VNUM)) < 0)
    log("Elevator panel object does not exist; will not load elevator control "
        "panels.");
  ...
  for (i = 0; i < num_elevators && !feof(fl); i++) {
    ...
    if ((rnum = real_room(elevator[i].room)) > -1) {
      ...
      sprintf(buf, "Nonexistent elevator cab %ld in elevator %d. Skipping.",
              elevator[i].room, i);
      log(buf);
      continue;
    }
    ...
    if (elevator[i].num_floors > 0) {
      ...
      for (j = 0; j < elevator[i].num_floors; j++) {
        ...
        // sprintf(buf, "%s: vnum %ld, shaft %ld, doors %s.", line, room,
        // shaft_vnum, fulldirs[t[1]]); log(buf);
        ...
        if (rnum > -1) {
          ...
          if (shaft_rnum > -1) {
            ...
            make_elevator_door(shaft_rnum, rnum,
                               rev_dir[elevator[i].floor[j].doors]);
            // Flag the shaft appropriately.
            ROOM_FLAGS(&world[shaft_rnum])
                .SetBits(ROOM_NOMOB, ROOM_NOBIKE, ROOM_ELEVATOR_SHAFT,
                         ROOM_FALL, ENDBIT);
            world[shaft_rnum].rating = ELEVATOR_SHAFT_FALL_RATING;
          } else {
            sprintf(buf, "Fatal error: Nonexistent elevator shaft vnum %ld.",
                    elevator[i].floor[j].shaft_vnum);
            log(buf);
            shutdown();
          }
        } else {
          sprintf(buf, "Nonexistent elevator destination %ld -- blocking.",
                  elevator[i].floor[j].vnum);
          log(buf);
          elevator[i].floor[j].vnum = -1;
        }
      }

      // Ensure an exit from car -> landing and vice/versa exists. Store the
      // shaft's exit.
      struct room_data *car = &world[real_room(elevator[i].room)];
      struct room_data *landing =
          &world[real_room(elevator[i].floor[car->rating].vnum)];
      struct room_data *shaft =
          &world[real_room(elevator[i].floor[car->rating].shaft_vnum)];
      int dir = elevator[i].floor[car->rating].doors;

      // Ensure car->landing exit.
      make_elevator_door(real_room(landing->number), real_room(car->number),
                         dir);

      // Ensure landing->car exit.
      make_elevator_door(real_room(car->number), real_room(landing->number),
                         rev_dir[dir]);
...
static void close_elevator_doors(struct room_data *room, int num, int floor) {
  ...
  // Close the doors between landing and car, but do not destroy or replace
  // anything.
...
SPECIAL(call_elevator) {
  ...
  if (CMD_IS("push") || CMD_IS("press")) {
    skip_spaces(&argument);
    if (!*argument ||
        !(!strcasecmp("elevator", argument) || !strcasecmp("button", argument)))
      ...
      if (index < 0 || elevator[index].destination) {
        send_to_char(
            "You press the call button, but nothing seems to happen.\r\n", ch);
        return TRUE;
      }
      ...
      send_to_char(
          "You press the call button, and the small light turns on.\r\n", ch);
  ...
  if (CMD_IS("look") || CMD_IS("examine")) {
    ...
    if (!*arg || index < 0 ||
        !(!strn_cmp("panel", arg, strlen(arg)) ||
          !strn_cmp("elevator", arg, strlen(arg))))
      return FALSE;

    rnum = real_room(elevator[index].room);

    i = world[rnum].rating + 1 - elevator[index].num_floors -
        elevator[index].start_floor;
    if (i > 0)
      send_to_char(ch,
                   "The floor indicator shows that the elevator is currently "
                   "at B%d.\r\n",
                   i);
    else if (i == 0)
      ...
      send_to_char(ch,
                   "The floor indicator shows that the elevator is current at "
                   "floor %d.\r\n",
                   0 - i);
    return TRUE;
  }
  ...
  if (!cmd) {
    ...
    if (elevator[num].time_left > 0) {
      if (!elevator[num].is_moving) {
        // Someone pushed the button, but the elevator hasn't started yet. Make
        // it move.
        if (IS_SET(room->dir_option[elevator[num].floor[room->rating].doors]
                       ->exit_info,
                   EX_CLOSED))
        ...
        // Message the shaft.
        send_to_room(
            "The elevator car shudders and begins to move.\r\n",
            &world[real_room(elevator[num].floor[room->rating].shaft_vnum)]);

        // Notify the next shaft room that they're about to get a car in the
        // face.
        sprintf(buf,
                "The shaft rumbles ominously as the elevator car %s you begins "
                "to accelerate towards you.\r\n",
                elevator[num].dir == DOWN ? "above" : "below");
        send_to_room(
            buf,
            &world[real_room(
                elevator[num]
                    .floor[room->rating + (elevator[num].dir == DOWN ? 1 : -1)]
                    .shaft_vnum)]);

        // Notify all other shaft rooms about the elevator starting to move.
        for (int i = 0; i < elevator[num].num_floors; i++) {
          if (i == room->rating ||
              i == room->rating + (elevator[num].dir == DOWN ? 1 : -1))
            continue;

          send_to_room("The shaft rumbles ominously as an elevator car begins "
                       "to move.\r\n",
      ...
      /* If you fail an athletics test, you get dragged by the car, sustaining
       * impact damage and getting pulled along with the elevator. */
      for (struct char_data *vict = shaft->people; vict;
        ...
        act("^R$n is caught up in the mechanism and dragged along!^n", FALSE,
            vict, 0, 0, TO_ROOM);
        send_to_char(
            "^RThe elevator mechanism snags you and drags you along!^n\r\n",
            vict);

        char_from_room(vict);
        char_to_room(
            vict, &world[real_room(
                      elevator[num]
                          .floor[(elevator[num].dir == DOWN ? room->rating + 1
                                                            : room->rating - 1)]
                          .shaft_vnum)]);

        sprintf(buf,
                "$n ragdolls in from %s, propelled by the bulk of the moving "
                "elevator.",
                elevator[num].dir == DOWN ? "above" : "below");
        act(buf, FALSE, vict, 0, 0, TO_ROOM);

        power = MIN(4, 15 - (GET_IMPACT(vict) /
                             2)); // Base power 15 (getting pinned and dragged
                                  // by an elevator HURTS). Impact armor helps.
        success =
            success_test(GET_BOD(vict), MAX(2, power) + modify_target(vict));
        dam = convert_damage(stage(-success, power));
        damage(vict, vict, dam, TYPE_ELEVATOR, TRUE);
      }

      // Restore the exit shaft->landing. EDGE CASE: Will be NULL if the car
      // started here on boot and hasn't moved.
      if (shaft->temporary_stored_exit[dir]) {
        // We assume that it was stored by the elevator moving through here, so
        // the dir_option should be NULL and safe to overwrite.
        shaft->dir_option[dir] = shaft->temporary_stored_exit[dir];
        shaft->temporary_stored_exit[dir] = NULL;
      }
      make_elevator_door(real_room(landing->number), real_room(shaft->number),
                         dir);

      // Restore the landing->shaft exit. Same edge case and assumptions as
      // above.
      make_elevator_door(real_room(shaft->number), real_room(landing->number),
                         rev_dir[dir]);

      // Remove the elevator car's exit (entirely possible that the floor we're
      // going to has no exit in that direction).
      ...
      sprintf(
          buf,
          "A rush of air precedes the arrival of an elevator car from %s.\r\n",
          elevator[num].dir == DOWN ? "above" : "below");
      send_to_room(buf, &world[real_room(shaft->number)]);

      /* If you fail an athletics test, the elevator knocks you off the wall,
       * dealing D impact damage and triggering fall. */
      for (struct char_data *vict = shaft->people; vict;
           vict = vict->next_in_room) {
        // Nohassle imms and astral projections are immune to this bullshit.
        if (PRF_FLAGGED(vict, PRF_NOHASSLE) || IS_ASTRAL(vict))
        ...
        // Failure case: Deal damage.
        act("^R$n is crushed by the bulk of the elevator!^n", FALSE, vict, 0, 0,
            TO_ROOM);
        send_to_char("^RThe elevator grinds you against the hoistway wall with "
                     "crushing force!^n\r\n",
                     vict);
      ...
      if (elevator[num].time_left > 0) {
        ...
        // face.
        sprintf(buf,
                "A light breeze brushes by as the elevator car %s you speeds "
                "towards you.\r\n",
                elevator[num].dir == DOWN ? "above" : "below");
        send_to_room(
            buf,
    ...
    // Arrive at the target floor.
      ...
      temp = room->rating + 1 - elevator[num].num_floors -
             elevator[num].start_floor;
      if (temp > 0)
        sprintf(buf, "The elevator stops at B%d, and the doors open to the %s.",
                temp, fulldirs[elevator[num].floor[room->rating].doors]);
      else if (temp == 0)
        sprintf(buf,
                "The elevator stops at the ground floor, and the doors open to "
                "the %s.",
                fulldirs[elevator[num].floor[room->rating].doors]);
      else
        sprintf(buf,
                "The elevator stops at floor %d, and the doors open to the %s.",
      ...
      send_to_room(
          "The cable thrums with tension as the elevator comes to a graceful "
          "halt.\r\n",
          &world[real_room(elevator[num].floor[room->rating].shaft_vnum)]);
      ...
      for (int i = 0; i < elevator[num].num_floors; i++) {
        ...
        send_to_room(
            "The ominous rumblings from the shaft's guiderails fade.\r\n",
            &world[real_room(elevator[num].floor[i].shaft_vnum)]);
      }
    ...
    if (!strncmp(argument, "open", strlen(argument))) {
      if (IS_SET(room->dir_option[elevator[num].floor[room->rating].doors]
        ...
        send_to_char("You jab the OPEN button a few times, then realize the "
      ...
      if (!IS_SET(room->dir_option[elevator[num].floor[room->rating].doors]
        ...
        send_to_char("You stare blankly at the closed elevator doors in front "
                     "of you and wonder what you were thinking.\r\n",
                     ch);
      }
      return TRUE;
    }
    ...
    if (LOWER(*argument) == 'b' && (number = atoi(argument + 1)) > 0) {
      ...
      number =
          elevator[num].num_floors + elevator[num].start_floor + number - 1;
      ...
      number =
          elevator[num].num_floors + elevator[num].start_floor - 1 - number;
    } else
      number = -1;
    ...
    one_argument(argument, arg);
    if (!*arg || !(!strn_cmp("panel", arg, strlen(arg)) ||
                   !strn_cmp("elevator", arg, strlen(arg)) ||
                   !strn_cmp("buttons", arg, strlen(arg)) ||
                   !strn_cmp("control", arg, strlen(arg))))
      return FALSE;
    ...
    sprintf(ENDOF(buf), "\r\n(OPEN)%s(CLOSE)\r\n\r\n",
            PRF_FLAGGED(ch, PRF_SCREENREADER) ? "\r\n" : "  ");
    temp =
        room->rating + 1 - elevator[num].num_floors - elevator[num].start_floor;
    if (temp > 0)
      sprintf(buf + strlen(buf),
              "The floor indicator shows that the "
...
SPECIAL(escalator) { return FALSE; }
        ...
        if (GET_LASTROOM(temp) > 0 || GET_LASTROOM(temp) < -3)
          ...
          for (dir = NORTH; dir <= DOWN; dir++)
            if (world[i].dir_option[dir] && world[i].dir_option[dir]->to_room) {
              act("As you reach the end, you step off the escalator.", FALSE,
                  temp, 0, 0, TO_CHAR);
...
void process_seatac_monorail(void) {
  ...
  case 20:
    send_to_room(
        "Lights flash along the runway as the monorail approaches.\r\n",
        &world[roomnum]);
    break;
  case 1:
  case 5:
  ...
  case 7:
    ...
    break;
  case 11:
    send_to_room("A voice announces, \"Next stop: Seattle. Change here for "
                 "Downtown Shuttle\"\r\n",
                 &world[carnum]);
    break;
  case 15:
...
void process_seattle_ferry(void) {
  ...
  case 1:
  case 14:
    extend_walkway_st(ferry, seattle_ferry[ind].to, dock,
                      seattle_ferry[ind].from);
    break;
  case 4:
  case 17:
    contract_walkway_st(ferry, seattle_ferry[ind].to, dock,
                        seattle_ferry[ind].from);
    break;
  case 5:
...
void close_busdoor(int bus, int to, int room, int from) {
  ...
  delete world[room].dir_option[from];
  world[room].dir_option[from] = NULL;
  send_to_room("The bus door shuts, the driver yells \"^Wall aboard!^n\", and "
               "begins driving.\r\n",
               &world[room]);
  send_to_room("The bus door shuts, the driver yells \"^Wall aboard!^n\", and "
               "begins driving.\r\n",
...
void process_hellhound_bus(void) {
  ...
  case 0:
    send_to_room(
        "The bus pulls into the garage, and slowly moves to the platform.\r\n",
        &world[stop]);
    break;
  case 1:
  case 28:
  ...
  case 23:
    send_to_room(
        "The driver shouts from the front, \"Next stop: Portland\"\r\n",
        &world[bus]);
    break;
  case 26:
    send_to_room(
  ...
  case 49:
    send_to_room(
        "The driver shouts from the front, \"Next stop: Seattle\".\r\n",
        &world[bus]);
    break;
  }
  where++;
...
void camas_extend(int bus, int to, int room, int from) {
  ...
  send_to_room("The Lear-Cessna Platinum II smoothly lands and lays out a "
               "small stairway entrance.\r\n",
               &world[room]);
  send_to_room("The Lear-Cessna Platinum II smoothly lands and lays out a "
               "small stairway entrance.\r\n",
               &world[bus]);
...
void camas_retract(int bus, int to, int room, int from) {
  ...
  delete world[room].dir_option[from];
  world[room].dir_option[from] = NULL;
  send_to_room("The stairs retract and the Lear-Cessna Platinum II taxis along "
               "the runway before taking flight.\r\n",
               &world[room]);
  send_to_room("The stairs retract and the Lear-Cessna Platinum II taxis along "
               "the runway before taking flight.\r\n",
               &world[bus]);
...
void process_camas_ferry(void) {
  ...
  case 0:
  case 84:
    send_to_room("A Lear-Cessna Platinum II appears overhead and circles as it "
                 "moves in for a landing.\r\n",
                 &world[stop]);
    break;
  case 4:
  case 92:
  ...
  case 112:
    ...
    break;
  case 72:
    send_to_room("The Lear-Cessna Platinum II approaches a private Caribbean "
                 "island.\r\n",
                 &world[bus]);
    break;
  case 152:
    send_to_room(
        "The Lear-Cessna Platinum II approaches the Salish-Sidhe lands.\r\n",
        &world[bus]);
    break;
  }
  where++;
...
// FILEPOINTER
// Downtown Portlan, 60th Street Hospital, Gresham, 60th Street back from
// Gresham Lightrail
struct transport_type lightrail[4] = {
    {14799, SOUTH, 14701, NORTH}, // Downtown Portlan
...
void open_lightraildoor(int lightrail, int to, int room, int from) {
  ...
  send_to_room("The incoming lightrail grinds to a halt and its doors slide "
               "open with a hiss.\r\n",
               &world[room]);
...
void close_lightraildoor(int lightrail, int to, int room, int from) {
  ...
  delete world[room].dir_option[from];
  world[room].dir_option[from] = NULL;
  send_to_room("The lightrail's doors slide shut and a tone emanates around "
               "the platform, signaling its departure.\r\n",
               &world[room]);
  send_to_room("The lightrail's doors slide shut and a tone signals as it "
               "begins moving.\r\n",
               &world[lightrail]);
}
...
void process_lightrail_train(void) {
  ...
  case 0:
    send_to_room(
        "The lightrail emits a loud grind as it brakes into the station.\r\n",
        &world[stop]);
    break;
  case 1:
...
struct transport_type tacsea[2] = {{2099, SOUTH, 2007, NORTH},
...
struct transport_type victoria[2] = {{62100, NORTH, 62104, SOUTH},
...
struct transport_type sugarloaf[3] = {{62199, EAST, 62104, WEST},
...
void grenada_extend(int bus, int to, int room, int from) {
  ...
  send_to_room("The Hawker-Ridley HS-895 Skytruck docks with the platform and "
               "begins loading passengers.\r\n",
               &world[room]);
  send_to_room("The Hawker-Ridley HS-895 Skytruck docks with the platform and "
               "begins loading passengers.\r\n",
               &world[bus]);
...
void grenada_retract(int bus, int to, int room, int from) {
  ...
  delete world[room].dir_option[from];
  world[room].dir_option[from] = NULL;
  send_to_room("The airplane taxis into position on the runway before "
...
void process_grenada_plane(void) {
  ...
  case 0:
  case 84:
    send_to_room("A Hawker-Ridley HS-895 Skytruck lands on the main runway and "
                 "moves towards the departure gate.\r\n",
                 &world[stop]);
    break;
  case 4:
  case 92:
  ...
  case 72:
    send_to_room(
        "The voice of the pilot speaks over the intercom, "
...
void TransportInit() { init_elevators(); }
