...
extern char *colorize(struct descriptor_data *d, const char *str,
...
int number(int from, int to) {
  ...
  if (to > RAND_MAX) {
    char errbuf[150];
    sprintf(errbuf,
            "WARNING: Attempting to generate random number between %d and %d, "
            "but RAND_MAX is %d!",
            from, to, RAND_MAX);
...
int light_level(struct room_data *room) {
  if (!room) {
    mudlog("SYSERR: light_level() called on null room.", NULL, LOG_SYSLOG,
           TRUE);
    return LIGHT_NORMAL;
  }
  ...
  if ((time_info.hours < 6 && time_info.hours > 19) &&
      (room->vision[0] > LIGHT_MINLIGHT ||
       room->vision[0] <= LIGHT_NORMALNOLIT))
    return LIGHT_MINLIGHT;
  else
    return room->vision[0];
}
...
int modify_target_rbuf_raw(struct char_data *ch, char *rbuf,
                           int current_visibility_penalty) {
  ...
  if (GET_SUSTAINED_NUM(ch) > 0) {
    ...
    buf_mod(rbuf, "Sustain",
            (GET_SUSTAINED_NUM(ch) - GET_SUSTAINED_FOCI(ch)) * 2);
  }
  ...
  if (PLR_FLAGGED(ch, PLR_PERCEIVE)) {
    ...
    if (temp_room->vision[1] == LIGHT_MIST)
      if (CURRENT_VISION(ch) == NORMAL ||
          (CURRENT_VISION(ch) == LOWLIGHT && NATURAL_VISION(ch) == LOWLIGHT)) {
        smoke_target += 2;
        buf_mod(rbuf, "Mist", 2);
      }
    if (temp_room->vision[1] == LIGHT_LIGHTSMOKE ||
        (weather_info.sky == SKY_RAINING &&
         temp_room->sector_type != SPIRIT_HEARTH &&
    ...
    if (temp_room->vision[1] == LIGHT_HEAVYSMOKE ||
        (weather_info.sky == SKY_LIGHTNING &&
         ...
         !ROOM_FLAGGED(temp_room, ROOM_INDOORS))) {
      if (CURRENT_VISION(ch) == NORMAL ||
          (CURRENT_VISION(ch) == LOWLIGHT && NATURAL_VISION(ch) == NORMAL)) {
        smoke_target += 6;
    ...
    // The maximum visibility penalty we apply is +8 TN to avoid things like an
    // invisible person in a smoky pitch-black room getting +24 to hit TN.
    if (light_target + smoke_target + current_visibility_penalty > 8) {
      buf_mod(rbuf, "ButVisPenaltyMaxIs8",
              (8 - current_visibility_penalty) - light_target + smoke_target);
  ...
  for (struct sustain_data *sust = GET_SUSTAINED(ch); sust; sust = sust->next) {
    if (sust->caster == FALSE &&
        (sust->spell == SPELL_CONFUSION || sust->spell == SPELL_CHAOS)) {
      base_target += MIN(sust->force, sust->success);
      buf_mod(rbuf, "Confused", MIN(sust->force, sust->success));
    }
  }
  if (!(IS_ELEMENTAL(ch) || IS_SPIRIT(ch)))
    for (struct spirit_sustained *sust = SPIRIT_SUST(ch); sust;
         sust = sust->next)
      if (sust == CONFUSION) {
  ...
  if (temp_room && ROOM_FLAGGED(temp_room, ROOM_INDOORS)) {
    float heightdif =
        GET_HEIGHT(ch) / ((temp_room->z != 0 ? temp_room->z : 1) * 100);
    if (heightdif > 1) {
      base_target += 2;
...
// decapitalize a string that starts with A or An, now allows for color strings
// at the beginning
char *decapitalize_a_an(const char *source) {
  ...
  if (*(source + index) == 'A') {
    ...
    if (index < len - 1 && (*(source + index + 1) == ' ' ||
                            (*(source + index + 1) == 'n' && index < len - 2 &&
                             *(source + index + 2) == ' '))) {
      *(dest + index) = 'a';
    }
  }
...
int strn_cmp(const char *arg1, const char *arg2, int n) {
  ...
  if (arg1 == NULL || arg2 == NULL) {
    log_vfprintf("SYSERR: strn_cmp() passed a NULL pointer, %p or %p.", arg1,
                 arg2);
    return (0);
  }
...
bool has_cyberweapon(struct char_data *ch) {
  ...
  for (obj = ch->cyberware; obj; obj = obj->next_content)
    if ((GET_OBJ_VAL(obj, 0) == CYB_HANDSPUR ||
         GET_OBJ_VAL(obj, 0) == CYB_HANDRAZOR ||
         GET_OBJ_VAL(obj, 0) == CYB_HANDBLADE ||
         GET_OBJ_VAL(obj, 0) == CYB_FIN ||
         GET_OBJ_VAL(obj, 0) == CYB_CLIMBINGCLAWS) &&
        !GET_OBJ_VAL(obj, 9))
      return TRUE;
  return 0;
}
...
void mudlog(const char *str, struct char_data *ch, int log, bool file) {
  ...
  if (ch && (ch->in_room || ch->in_veh)) {
    if (ch->desc)
      sprintf(buf2, "[%5ld] (%s) ", get_ch_in_room(ch)->number,
              GET_CHAR_NAME(ch));
    else
  ...
  for (i = descriptor_list; i; i = i->next)
    if (!i->connected) {
      if (i->original)
        ...
        tch = i->character;
      if (!tch || PLR_FLAGS(tch).AreAnySet(PLR_WRITING, PLR_MAILING,
                                           PLR_EDITING, ENDBIT))
        continue;

      // We don't show log messages from imms who are invis at a level higher
      // than you, unless you're a high enough level that that doesn't matter.
      if (ch && !access_level(tch, GET_INVIS_LEV(ch)) &&
      ...
      default:
        char errbuf[500];
        sprintf(errbuf,
                "SYSERR: Attempting to display a message to log type %d, but "
                "that log type is not handled in utils.cpp's mudlog() "
                "function! Dumping to SYSLOG.",
                log);
        mudlog(errbuf, NULL, LOG_SYSLOG, TRUE);
        check_log = PRF_SYSLOG;
        break;
...
void sprint_obj_mods(struct obj_data *obj, char *result) {
  *result = 0;
  if (obj->obj_flags.bitvector.GetNumSet() > 0) {
    ...
    obj->obj_flags.bitvector.PrintBits(xbuf, MAX_STRING_LENGTH, affected_bits,
                                       AFF_MAX);
  ...
  for (int i = 0; i < MAX_OBJ_AFFECT; i++)
    if (obj->affected[i].modifier != 0) {
      ...
      sprintf(result, "%s (%+d %s)", result, obj->affected[i].modifier, xbuf);
    }
  return;
}
...
int get_speed(struct veh_data *veh) {
  int speed = 0,
      maxspeed = (int)(veh->speed * ((veh->load - veh->usedload) /
                                     (veh->load != 0 ? veh->load : 1)));
...
int negotiate(struct char_data *ch, struct char_data *tch, int comp,
              int basevalue, int mod, bool buy) {
  struct obj_data *bio;
  int cmod = 0, tmod = 0;
  ...
  for (bio = ch->bioware; bio; bio = bio->next_content)
    if (GET_OBJ_VAL(bio, 0) == BIO_TAILOREDPHEREMONES) {
      cskill +=
          GET_OBJ_VAL(bio, 2) ? GET_OBJ_VAL(bio, 1) * 2 : GET_OBJ_VAL(bio, 1);
      break;
    }
  ...
  if (num > 0) {
    if (buy)
      basevalue =
          MAX((int)(basevalue * 3 / 4), basevalue - (num * (basevalue / 20)));
    else
      basevalue =
          MIN((int)(basevalue * 5 / 4), basevalue + (num * (basevalue / 15)));
  } else {
    if (buy)
      basevalue =
          MIN((int)(basevalue * 5 / 4), basevalue + (num * (basevalue / 15)));
...
int get_skill(struct char_data *ch, int skill, int &target) {
  ...
  // TODO: Adept power Improved Ability. This ability is not currently in the
  // game, but would be factored in here. See Core p169 for details.

  // Convert NPCs so that they can use Armed Combat in place of any weapon skill
  // except unarmed combat.
  if (IS_NPC(ch) &&
      ((skill >= SKILL_ARMED_COMBAT && skill <= SKILL_CLUBS) ||
  ...
  if (GET_SKILL(ch, skill)) {
    ...
    // If their skill in this area has not been boosted, they get to add their
    // task pool up to the skill's learned level.
    if (REAL_SKILL(ch, skill) == GET_SKILL(ch, skill))
      totalskill += MIN(REAL_SKILL(ch, skill),
                        GET_TASK_POOL(ch, skills[skill].attribute));
    ...
    if (ch->cyberware) {
      ...
      for (struct obj_data *obj = ch->cyberware; obj; obj = obj->next_content)
        if (GET_OBJ_VAL(obj, 0) == CYB_MOVEBYWIRE)
          ...
          for (int i = 5; i < 10; i++)
            if (real_object(GET_OBJ_VAL(obj, i)) &&
                obj_proto[real_object(GET_OBJ_VAL(obj, i))]
                        .obj_flags.value[0] == skill)
              chip = TRUE;

      // If they have both a chipjack with the correct chip loaded and a
      // Chipjack Expert, add the rating to their skill as task pool dice (up to
      // skill max).
      if (chip && expert) {
    ...
    if (ch->bioware) {
      for (struct obj_data *bio = ch->bioware; bio; bio = bio->next_content) {
        if (GET_OBJ_VAL(bio, 0) == BIO_REFLEXRECORDER &&
            GET_OBJ_VAL(bio, 3) == skill)
          totalskill++;
    ...
    /* Enhanced Articulation: Possessors roll an additional die when making
     Success Tests involving any Combat, Physical, Technical and B/R Skills.
    ...
    if (enhan) {
      ...
      case SKILL_PILOT_TRUCK:
        // You only get the bonus for vehicle skills if you're physically
        // driving the vehicle.
    ...
    if (target >= 8)
      return 0;
    target += 4;
    sprintf(gskbuf, "$n (%s) %s(%d) = %d(%d): +4 TN", GET_CHAR_NAME(ch),
            skills[skill].name, skill, GET_SKILL(ch, skill),
            REAL_SKILL(ch, skill));
    act(gskbuf, 1, ch, NULL, NULL, TO_ROLLS);
...
bool biocyber_compatibility(struct obj_data *obj1, struct obj_data *obj2,
                            struct char_data *ch) {
  ...
  if (cyber1 && cyber2) {
    if (GET_OBJ_VAL(cyber1, 0) != CYB_EYES)
      ...
      case CYB_FILTRATION:
        if (GET_OBJ_VAL(cyber1, 3) == GET_OBJ_VAL(cyber2, 3)) {
          send_to_char(
              "You already have this type of filtration installed.\r\n", ch);
          return FALSE;
        }
        break;
      case CYB_DATAJACK:
        if (GET_OBJ_VAL(cyber2, 0) == CYB_EYES &&
            IS_SET(GET_OBJ_VAL(cyber2, 3), EYE_DATAJACK)) {
      ...
      case CYB_VCR:
        if (GET_OBJ_VAL(cyber2, 0) == CYB_BOOSTEDREFLEXES) {
          send_to_char("Vehicle Control Rigs are not compatible with Boosted "
                       "reflexes.\r\n",
                       ch);
          return FALSE;
        }
      ...
      case CYB_BOOSTEDREFLEXES:
        if (GET_OBJ_VAL(cyber2, 0) == CYB_VCR) {
          send_to_char("Boosted reflexes is not compatible with Vehicle "
                       "Control Rigs.\r\n",
                       ch);
          return FALSE;
        }
      ...
      case CYB_WIREDREFLEXES:
        if (GET_OBJ_VAL(cyber2, 0) == CYB_WIREDREFLEXES ||
            GET_OBJ_VAL(cyber2, 0) == CYB_MOVEBYWIRE ||
            GET_OBJ_VAL(cyber2, 0) == CYB_BOOSTEDREFLEXES) {
          send_to_char(
              "You already have reaction enhancing cyberware installed.\r\n",
              ch);
          return FALSE;
        }
        if (GET_OBJ_VAL(cyber1, 0) == CYB_MOVEBYWIRE &&
            GET_OBJ_VAL(cyber2, 0) == CYB_REACTIONENHANCE) {
          send_to_char(
              "Move-by-wire is not compatible with reaction enhancers.\r\n",
              ch);
          return FALSE;
        }
        break;
      ...
      case CYB_ORALDART:
      case CYB_ORALGUN:
        if (GET_OBJ_VAL(cyber2, 0) == CYB_ORALSLASHER ||
            GET_OBJ_VAL(cyber2, 0) == CYB_ORALDART ||
            GET_OBJ_VAL(cyber2, 0) == CYB_ORALGUN) {
      ...
      case CYB_DERMALSHEATHING:
        if (GET_OBJ_VAL(cyber2, 0) == CYB_DERMALPLATING ||
            GET_OBJ_VAL(cyber2, 0) == CYB_DERMALSHEATHING) {
      ...
      case CYB_REACTIONENHANCE:
        if (GET_OBJ_VAL(cyber2, 0) == CYB_MOVEBYWIRE) {
          send_to_char(
              "Reaction enhancers are not compatible with Move-by-wire.\r\n",
              ch);
          return FALSE;
        }
        break;
      }
    if (IS_SET(GET_OBJ_VAL(cyber1, 3), EYE_DATAJACK) &&
        GET_OBJ_VAL(cyber2, 0) == CYB_DATAJACK) {
      send_to_char("You already have a datajack installed.\r\n", ch);
      return FALSE;
    }
    if (GET_OBJ_VAL(cyber2, 0) == CYB_EYES &&
        GET_OBJ_VAL(cyber1, 0) == CYB_EYES)
      for (int bit = EYE_CAMERA; bit <= EYE_ULTRASOUND; bit *= 2) {
        if (IS_SET(GET_OBJ_VAL(cyber2, 3), bit) &&
            IS_SET(GET_OBJ_VAL(cyber1, 3), bit)) {
          send_to_char("You already have eye modifications with this option "
                       "installed.\r\n",
                       ch);
          return FALSE;
        }
        if (bit >= EYE_OPTMAG1 && bit <= EYE_OPTMAG3 &&
            IS_SET(GET_OBJ_VAL(cyber1, 3), bit))
          if (IS_SET(GET_OBJ_VAL(cyber2, 3), EYE_ELECMAG1) ||
              IS_SET(GET_OBJ_VAL(cyber2, 3), EYE_ELECMAG2) ||
              IS_SET(GET_OBJ_VAL(cyber2, 3), EYE_ELECMAG3)) {
            send_to_char("Optical magnification is not compatible with "
                         "electronic magnification.\r\n",
                         ch);
            return FALSE;
          }
        if (bit >= EYE_ELECMAG1 && bit <= EYE_ELECMAG3 &&
            IS_SET(GET_OBJ_VAL(cyber1, 3), bit))
          if (IS_SET(GET_OBJ_VAL(cyber2, 3), EYE_OPTMAG1) ||
              IS_SET(GET_OBJ_VAL(cyber2, 3), EYE_OPTMAG2) ||
              IS_SET(GET_OBJ_VAL(cyber2, 3), EYE_OPTMAG3)) {
            send_to_char("Optical magnification is not compatible with "
                         "electronic magnification.\r\n",
                         ch);
            return FALSE;
    ...
    switch (GET_OBJ_VAL(cyber1, 0)) {
    case CYB_FILTRATION:
      if (GET_OBJ_VAL(bio1, 0) == BIO_TRACHEALFILTER &&
          GET_OBJ_VAL(cyber1, 3) == FILTER_AIR) {
        send_to_char("Air filtration cyberware is not compatible with a "
                     "Tracheal Filter.\r\n",
                     ch);
        return FALSE;
      }
      if (GET_OBJ_VAL(bio1, 0) == BIO_DIGESTIVEEXPANSION &&
          GET_OBJ_VAL(cyber1, 3) == FILTER_INGESTED) {
        send_to_char("Tracheal Filtration cyberware is not compatible with "
                     "Digestive Expansion.\r\n",
                     ch);
        return FALSE;
      }
    ...
    case CYB_MOVEBYWIRE:
      ...
      if (GET_OBJ_VAL(bio1, 0) == BIO_SYNAPTICACCELERATOR) {
        send_to_char(
            "Synaptic Accelerators are not compatible with Move-By-Wire.\r\n",
            ch);
        return FALSE;
      }
      if (GET_OBJ_VAL(bio1, 0) == BIO_SUPRATHYROIDGLAND) {
        send_to_char(
            "Suprathyroid Glands are not compatible with Move-By-Wire.\r\n",
            ch);
        return FALSE;
      }
      break;
    case CYB_WIREDREFLEXES:
      if (GET_OBJ_VAL(bio1, 0) == BIO_SYNAPTICACCELERATOR) {
        send_to_char("Your Synaptic Accelerator is not compatible with Wired "
                     "Reflexes.\r\n",
                     ch);
        return FALSE;
      }
      break;
    case CYB_EYES:
      if (GET_OBJ_VAL(bio1, 0) == BIO_CATSEYES ||
          GET_OBJ_VAL(bio1, 0) == BIO_NICTATINGGLAND) {
        send_to_char(
            "Bioware and cyberware eye modifications aren't compatible.\r\n",
            ch);
        return FALSE;
      }
      break;
    case CYB_MUSCLEREP:
      if (GET_OBJ_VAL(bio1, 0) == BIO_MUSCLEAUG ||
          GET_OBJ_VAL(bio1, 0) == BIO_MUSCLETONER) {
        send_to_char("Muscle replacement isn't compatible with Muscle "
                     "Augmentation or Toners.\r\n",
                     ch);
        return FALSE;
      }
      break;
    ...
    case CYB_DERMALSHEATHING:
      if (GET_OBJ_VAL(bio1, 0) == BIO_ORTHOSKIN) {
        send_to_char(
            "Orthoskin is not compatible with Dermal Plating or Sheathing.\r\n",
            ch);
        return FALSE;
      }
      break;
...
void reduce_abilities(struct char_data *vict) {
  ...
  for (i = 0; i < ADEPT_NUMPOWER; i++)
    if (GET_POWER_TOTAL(vict, i) > GET_MAG(vict) / 100) {
      ...
      send_to_char(vict,
                   "Your loss in magic makes you feel less "
                   "skilled in %s.\r\n",
                   adept_powers[i]);
    }
...
void magic_loss(struct char_data *ch, int magic, bool msg) {
  ...
  if (GET_REAL_MAG(ch) < 100) {
    ...
    sprintf(buf, "UPDATE pfiles SET Tradition=%d WHERE idnum=%ld;",
            TRAD_MUNDANE, GET_IDNUM(ch));
    mysql_wrapper(mysql, buf);
    for (int i = 0; i < NUM_WEARS; i++)
      if (GET_EQ(ch, i) && GET_OBJ_TYPE(GET_EQ(ch, i)) == ITEM_FOCUS &&
          GET_OBJ_VAL(GET_EQ(ch, i), 2) == GET_IDNUM(ch))
        GET_OBJ_VAL(GET_EQ(ch, i), 2) = GET_OBJ_VAL(GET_EQ(ch, i), 4) =
            GET_OBJ_VAL(GET_EQ(ch, i), 9) = 0;
...
bool has_kit(struct char_data *ch, int type) {
  for (struct obj_data *o = ch->carrying; o; o = o->next_content)
    if (GET_OBJ_TYPE(o) == ITEM_WORKSHOP && GET_OBJ_VAL(o, 0) == type &&
        GET_OBJ_VAL(o, 1) == TYPE_KIT)
      return TRUE;

  if (GET_EQ(ch, WEAR_HOLD))
    if (GET_OBJ_TYPE(GET_EQ(ch, WEAR_HOLD)) == ITEM_WORKSHOP &&
        GET_OBJ_VAL(GET_EQ(ch, WEAR_HOLD), 0) == type &&
        GET_OBJ_VAL(GET_EQ(ch, WEAR_HOLD), 1) == TYPE_KIT)
      return TRUE;
...
struct obj_data *find_workshop(struct char_data *ch, int type) {
  ...
  // If they're in a valid room, return the room's workshop field for MAXIMUM
  // EFFICIENCY.
  if (ch->in_room)
    return ch->in_room->best_workshop[type];

  // If we've gotten here, they must be in a vehicle. Iterate through and find
  // the best candidate.
  for (struct obj_data *o = ch->in_veh->contents; o; o = o->next_content) {
    if (GET_OBJ_TYPE(o) == ITEM_WORKSHOP && GET_WORKSHOP_TYPE(o) == type) {
      if (GET_WORKSHOP_GRADE(o) == TYPE_FACILITY) {
        // Jackpot! Facilities are the best option, so we can terminate early
...
bool _is_workshop_valid(struct obj_data *obj) {
  ...
  if (GET_OBJ_TYPE(obj) != ITEM_WORKSHOP) {
    sprintf(
        buf,
        "SYSERR: Non-workshop item '%s' (%ld) passed to workshop functions.",
  ...
  if (!(GET_WORKSHOP_GRADE(obj) == TYPE_SHOP && GET_WORKSHOP_IS_SETUP(obj)) &&
      GET_WORKSHOP_GRADE(obj) != TYPE_FACILITY) {
    // For this to be true, the item is a kit. We're not going to throw an error
    // over that, but it's not a valid workshop.
    return FALSE;
  }
...
void remove_workshop_from_room(struct obj_data *obj) {
  ...
  // Iterate through all items in the room, looking for other valid
  // workshops/facilities of this type.
  for (struct obj_data *o = obj->in_room->contents; o; o = o->next_content) {
    if (o != obj && GET_OBJ_TYPE(o) == ITEM_WORKSHOP &&
        GET_WORKSHOP_TYPE(o) == GET_WORKSHOP_TYPE(obj)) {
      ...
      case TYPE_FACILITY:
        ...
        return;
      case TYPE_SHOP:
        // The value of best_workshop is either a workshop or null, so no harm
        // in setting it to another workshop.
        obj->in_room->best_workshop[GET_WORKSHOP_TYPE(obj)] = o;
        break;
      case TYPE_KIT:
        break;
      default:
        sprintf(buf,
                "SYSERR: Invalid workshop type %d found for object '%s' (%ld).",
                GET_WORKSHOP_GRADE(o), GET_OBJ_NAME(o), GET_OBJ_VNUM(o));
        mudlog(buf, NULL, LOG_SYSLOG, TRUE);
        break;
      }
    }
...
struct obj_data *get_mount_weapon(struct obj_data *mount) {
  if (mount == NULL) {
    mudlog("SYSERR: Attempting to retrieve weapon for nonexistent mount.", NULL,
           LOG_SYSLOG, TRUE);
    return NULL;
  }

  for (struct obj_data *contains = mount->contains; contains;
       contains = contains->next_content) {
...
struct obj_data *stop_manning_weapon_mounts(struct char_data *ch,
                                            bool send_message) {
  if (AFF_FLAGGED(ch, AFF_MANNING)) {
    ...
    // Clean up the mount's data (stop it from pointing to character; remove
    // targets)
    mount->worn_by = NULL;
    mount->targ = NULL;
    mount->tveh = NULL;
...
struct obj_data *get_mount_manned_by_ch(struct char_data *ch) {
  ...
  if (!ch) {
    mudlog("SYSERR: Attempting to get mount manned by NULL character.", NULL,
           LOG_SYSLOG, TRUE);
    // No mount returned.
    return NULL;
  }
  ...
  // Require that they be in a vehicle. Error and clear their flag if they're
  // not.
  if (!ch->in_veh) {
    sprintf(buf,
            "SYSERR: Attempting to get mount manned by %s, but %s is not in "
            "any vehicle.",
            GET_CHAR_NAME(ch), HSHR(ch));
    mudlog(buf, ch, LOG_SYSLOG, TRUE);

    // Clean up their mount info. We don't use stop_manning..() because it calls
  ...
  for (struct obj_data *mount = ch->in_veh->mount; mount;
       mount = mount->next_content) {
    if (mount->worn_by == ch) {
  ...
  sprintf(buf,
          "SYSERR: Attempting to get mount manned by %s, but the mount does "
          "not exist in %s vehicle.",
          GET_CHAR_NAME(ch), HSHR(ch));
  mudlog(buf, ch, LOG_SYSLOG, TRUE);
...
void store_message_to_history(struct descriptor_data *d, int channel,
                              const char *mallocd_message) {
  // We use our very own message buffer to ensure we'll never overwrite whatever
  // buffer the caller is using.
  static char log_message[256];

  // Precondition: No screwy pointers. Removed warning since we can be passed
  // NPC descriptors (which we ignore).
  if (d == NULL || mallocd_message == NULL) {
    // mudlog("SYSERR: Null descriptor or message passed to
    // store_message_to_history.", NULL, LOG_SYSLOG, TRUE);
    return;
  }

  // Precondition: Channel must be a valid index (0 ≤ channel < number of
  // channels defined in awake.h).
  if (channel < 0 || channel >= NUM_COMMUNICATION_CHANNELS) {
    sprintf(log_message,
            "SYSERR: Channel %d is not within bounds 0 <= channel < %d.",
            channel, NUM_COMMUNICATION_CHANNELS);
  ...
  if (d->message_history[channel].NumItems() > NUM_MESSAGES_TO_RETAIN) {
    // We're over the amount. Remove the tail, making sure we delete the
    // contents.
...
void delete_message_history(struct descriptor_data *d) {
  ...
  if (d == NULL) {
    mudlog("SYSERR: Null descriptor passed to delete_message_history.", NULL,
           LOG_SYSLOG, TRUE);
    return;
  }

  nodeStruct<const char *> *temp = NULL;
  // For each channel in history, delete all messages (just keep nuking head
  // until it's NULL).
...
void terminate_mud_process_with_message(const char *message, int error_code) {
  sprintf(buf,
          "FATAL ERROR: The MUD has encountered a terminal error (code %d) and "
          "will now halt. The message given was as follows: %s",
          error_code, message);
  mudlog(buf, NULL, LOG_SYSLOG, TRUE);
  log(buf);
  exit(error_code);
...
struct room_data *get_veh_in_room(struct veh_data *veh) {
  ...
  if (!veh->in_room) {
    sprintf(errbuf,
            "SYSERR: get_veh_in_room called on veh %s, but it's not in a room "
            "or vehicle!",
            GET_VEH_NAME(veh));
    mudlog(errbuf, NULL, LOG_SYSLOG, TRUE);
  }
...
struct room_data *get_ch_in_room(struct char_data *ch) {
  ...
  sprintf(errbuf,
          "SYSERR: get_ch_in_room called on char %s, but they're not in a room "
          "or vehicle!",
          GET_NAME(ch));
  mudlog(errbuf, ch, LOG_SYSLOG, TRUE);
...
struct room_data *get_obj_in_room(struct obj_data *obj) {
  ...
  // Below this line is frantic flailing to avoid passing back NULL (which tends
  // to crash the game).
  if (obj->in_obj)
  ...
  sprintf(errbuf,
          "SYSERR: get_obj_in_room called on obj %s, but it's not in a room or "
          "vehicle!",
          GET_OBJ_NAME(obj));
  mudlog(errbuf, NULL, LOG_SYSLOG, TRUE);
...
bool invis_ok(struct char_data *ch, struct char_data *vict) {
  ...
  // Ultrasound pierces all invis as long as it's not blocked by silence or
  // stealth.
  ...
  // Standard invis is pierced by thermographic vision, which is default on
  // vehicles.
  if (IS_AFFECTED(vict, AFF_INVISIBLE)) {
    return CURRENT_VISION(ch) == THERMOGRAPHIC || AFF_FLAGGED(ch, AFF_RIG) ||
...
// Given a vnum, searches all objects and nested containers in the given
// container for the first match and returns it.
struct obj_data *find_matching_obj_in_container(struct obj_data *container,
                                                vnum_t vnum) {
  ...
  // Check each item in this container. If it's a match, return it; otherwise,
  // check its contents.
  for (struct obj_data *contents = container->contains; contents;
...
bool attach_attachment_to_weapon(struct obj_data *attachment,
                                 struct obj_data *weapon,
                                 struct char_data *ch) {
  if (!attachment || !weapon) {
    if (ch)
      send_to_char(
          ch, "Sorry, something went wrong. Staff have been notified.\r\n");
    mudlog("SYSERR: NULL weapon or attachment passed to "
           "attach_attachment_to_weapon().",
  ...
  if (GET_OBJ_TYPE(attachment) != ITEM_GUN_ACCESSORY) {
    if (ch)
      ...
      sprintf(buf,
              "SYSERR: Attempting to attach non-attachment '%s' (%ld) to '%s' "
              "(%ld).",
  ...
  if (GET_OBJ_TYPE(weapon) != ITEM_WEAPON ||
    ...
    if (ch)
      ...
      sprintf(buf,
              "SYSERR: Attempting to attach '%s' (%ld) to non-gun '%s' (%ld).",
              GET_OBJ_NAME(attachment), GET_OBJ_VNUM(attachment),
              GET_OBJ_NAME(weapon), GET_OBJ_VNUM(weapon));
      mudlog(buf, ch, LOG_SYSLOG, TRUE);
    }
  ...
  if (GET_ACCESSORY_TYPE(attachment) == ACCESS_SMARTGOGGLE) {
    if (ch)
      ...
      sprintf(
          buf,
          "SYSERR: Attempting to attach smartgoggle '%s' (%ld) to '%s' (%ld).",
          GET_OBJ_NAME(attachment), GET_OBJ_VNUM(attachment),
          GET_OBJ_NAME(weapon), GET_OBJ_VNUM(weapon));
      mudlog(buf, ch, LOG_SYSLOG, TRUE);
    }
  ...
  if (((GET_ACCESSORY_ATTACH_LOCATION(attachment) ==
        ACCESS_ACCESSORY_LOCATION_TOP) &&
       (GET_WEAPON_ATTACH_TOP_VNUM(weapon) > 0)) ||
      ((GET_ACCESSORY_ATTACH_LOCATION(attachment) ==
        ACCESS_ACCESSORY_LOCATION_BARREL) &&
       (GET_WEAPON_ATTACH_BARREL_VNUM(weapon) > 0)) ||
      ((GET_ACCESSORY_ATTACH_LOCATION(attachment) ==
        ACCESS_ACCESSORY_LOCATION_UNDER) &&
       (GET_WEAPON_ATTACH_UNDER_VNUM(weapon) > 0))) {
    if (ch) {
      send_to_char(
          ch, "You cannot mount more than one attachment to the %s of %s.\r\n",
          gun_accessory_locations[GET_ACCESSORY_ATTACH_LOCATION(attachment)],
          GET_OBJ_NAME(weapon));
    } else {
      sprintf(
          buf,
          "SYSERR: Attempting to attach '%s' (%ld) to already-filled %s "
          "location on '%s' (%ld).",
          GET_OBJ_NAME(attachment), GET_OBJ_VNUM(attachment),
          gun_accessory_locations[GET_ACCESSORY_ATTACH_LOCATION(attachment)],
          GET_OBJ_NAME(weapon), GET_OBJ_VNUM(weapon));
  ...
  if (((GET_ACCESSORY_ATTACH_LOCATION(attachment) ==
    ...
    if (ch) {
      send_to_char(
          ch, "%s isn't compatible with %s-mounted attachments.\r\n",
          CAP(GET_OBJ_NAME(weapon)),
          gun_accessory_locations[GET_ACCESSORY_ATTACH_LOCATION(attachment)]);
    } else {
      sprintf(
          buf,
          "SYSERR: Attempting to attach '%s' (%ld) to unacceptable %s location "
          "on '%s' (%ld).",
          GET_OBJ_NAME(attachment), GET_OBJ_VNUM(attachment),
          gun_accessory_locations[GET_ACCESSORY_ATTACH_LOCATION(attachment)],
          GET_OBJ_NAME(weapon), GET_OBJ_VNUM(weapon));
  ...
  if (GET_ACCESSORY_ATTACH_LOCATION(attachment) <
    ...
    sprintf(
        buf,
        "SYSERR: Accessory attachment location %d out of range for '%s' (%ld).",
        GET_ACCESSORY_ATTACH_LOCATION(attachment), GET_OBJ_NAME(attachment),
  ...
  if (GET_ACCESSORY_TYPE(attachment) == ACCESS_SILENCER ||
      GET_ACCESSORY_TYPE(attachment) == ACCESS_SOUNDSUPP) {
    // Weapon cannot be a revolver or shotgun.
    if (GET_WEAPON_ATTACK_TYPE(weapon) == WEAP_REVOLVER ||
        GET_WEAPON_ATTACK_TYPE(weapon) == WEAP_SHOTGUN) {
      if (ch) {
        send_to_char(ch, "%ss can't use silencers or suppressors.\r\n",
                     CAP(weapon_type[GET_WEAPON_ATTACK_TYPE(weapon)]));
      } else {
        sprintf(buf,
                "SYSERR: Attempting to attach silencer '%s' (%ld) to %s '%s' "
                "(%ld).",
                GET_OBJ_NAME(attachment), GET_OBJ_VNUM(attachment),
    ...
    if (GET_ACCESSORY_TYPE(attachment) == ACCESS_SILENCER) {
      ...
      if (WEAPON_CAN_USE_FIREMODE(weapon, MODE_BF) ||
          WEAPON_CAN_USE_FIREMODE(weapon, MODE_FA)) {
        if (ch) {
          send_to_char(ch,
                       "%s would tear your silencer apart-- it needs a sound "
                       "suppressor.\r\n",
                       CAP(GET_OBJ_NAME(weapon)));
        } else {
          sprintf(buf,
                  "SYSERR: Attempting to attach silencer '%s' (%ld) to BF/FA "
                  "weapon '%s' (%ld).",
                  GET_OBJ_NAME(attachment), GET_OBJ_VNUM(attachment),
                  GET_OBJ_NAME(weapon), GET_OBJ_VNUM(weapon));
          mudlog(buf, ch, LOG_SYSLOG, TRUE);
    ...
    if (GET_ACCESSORY_TYPE(attachment) == ACCESS_SOUNDSUPP) {
      ...
      if (!(WEAPON_CAN_USE_FIREMODE(weapon, MODE_BF) ||
        ...
        if (ch) {
          send_to_char(ch,
                       "Sound suppressors are too heavy-duty for %s-- it needs "
                       "a silencer.\r\n",
                       CAP(GET_OBJ_NAME(weapon)));
        } else {
          sprintf(buf,
                  "SYSERR: Attempting to attach suppressor '%s' (%ld) to "
                  "non-BF/FA weapon '%s' (%ld).",
                  GET_OBJ_NAME(attachment), GET_OBJ_VNUM(attachment),
                  GET_OBJ_NAME(weapon), GET_OBJ_VNUM(weapon));
          mudlog(buf, ch, LOG_SYSLOG, TRUE);
        }
  ...
  // Transfer the first (and only the first) applies-affect from the attachment
  // to the weapon.
  if (attachment->affected[0].modifier != 0) {
    ...
    if (!successfully_modified) {
      ...
      sprintf(
          buf,
          "WARNING: '%s' (%ld) attempted to attach '%s' (%ld) to '%s' (%ld), "
          ...
          ch ? GET_IDNUM(ch) : -1, GET_OBJ_NAME(attachment),
          GET_OBJ_VNUM(attachment), GET_OBJ_NAME(weapon), GET_OBJ_VNUM(weapon),
  ...
  GET_OBJ_COST(weapon) =
      MAX(0, GET_OBJ_COST(weapon) + GET_OBJ_COST(attachment));

  // Update the weapon's aff flags.
  for (int waff_index = 0;
       acceptable_weapon_attachment_affects[waff_index] != -1; waff_index++) {
    if (attachment->obj_flags.bitvector.IsSet(
            acceptable_weapon_attachment_affects[waff_index])) {
      weapon->obj_flags.bitvector.SetBit(
          acceptable_weapon_attachment_affects[waff_index]);
    }
  }

  // Update the weapon's attach location to reflect this item.
  GET_OBJ_VAL(weapon, GET_ACCESSORY_ATTACH_LOCATION(attachment) + 7) =
      GET_OBJ_VNUM(attachment);
  ...
  // Let the caller handle trashing the object (assuming they didn't just pass
  // us a proto reference).

  return TRUE;
}

struct obj_data *unattach_attachment_from_weapon(int location,
                                                 struct obj_data *weapon,
                                                 struct char_data *ch) {
  if (!weapon) {
    if (ch)
      send_to_char(
          ch, "Sorry, something went wrong. Staff have been notified.\r\n");
    mudlog("SYSERR: NULL weapon passed to unattach_attachment_from_weapon().",
           NULL, LOG_SYSLOG, TRUE);
    return NULL;
  }
  ...
  if (location < ACCESS_LOCATION_TOP || location > ACCESS_LOCATION_UNDER) {
    if (ch)
      send_to_char(
          ch, "Sorry, something went wrong. Staff have been notified.\r\n");
    sprintf(buf,
            "SYSERR: Attempt to unattach item from invalid location %d on '%s' "
            "(%ld).",
  ...
  if (GET_OBJ_TYPE(weapon) != ITEM_WEAPON) {
    if (ch)
      ...
      sprintf(buf,
              "SYSERR: Attempting to unattach something from non-weapon '%s' "
              "(%ld).",
              GET_OBJ_NAME(weapon), GET_OBJ_VNUM(weapon));
      mudlog(buf, ch, LOG_SYSLOG, TRUE);
    }
  ...
  struct obj_data *attachment =
      read_object(GET_WEAPON_ATTACH_LOC(weapon, location), VIRTUAL);
  ...
  if (!attachment) {
    ...
    sprintf(buf,
            "SYSERR: Attempting to unattach invalid vnum %d from %s of weapon "
            "'%s' (%ld).",
            GET_WEAPON_ATTACH_LOC(weapon, location),
            gun_accessory_locations[location - 7], GET_OBJ_NAME(weapon),
            GET_OBJ_VNUM(weapon));
    mudlog(buf, ch, LOG_SYSLOG, TRUE);
  ...
  if (GET_ACCESSORY_TYPE(attachment) == ACCESS_GASVENT) {
    if (ch) {
      send_to_char(ch,
                   "%s is permanently attached to %s and can't be removed.\r\n",
                   GET_OBJ_NAME(attachment), GET_OBJ_NAME(weapon));
      return NULL;
    }
    // We assume the coder knows what they're doing when unattaching a gasvent.
    // They may proceed.
  }

  // Remove the first (and only the first) affect of the attachment from the
  // weapon.
  if (attachment->affected[0].modifier != 0) {
    bool successfully_modified = FALSE;
    for (int index = 0; index < MAX_OBJ_AFFECT; index++) {
      if (weapon->affected[index].location ==
              attachment->affected[0].location &&
          weapon->affected[index].modifier ==
              attachment->affected[0].modifier) {
    ...
    if (!successfully_modified) {
      sprintf(
          buf,
          "WARNING: '%s' (%ld) unattached '%s' (%ld) from '%s' (%ld), but the "
          "gun was missing the attachment's affect. Something needs revising.",
          ch ? GET_CHAR_NAME(ch) : "An automated process",
          ch ? GET_IDNUM(ch) : -1, GET_OBJ_NAME(attachment),
          GET_OBJ_VNUM(attachment), GET_OBJ_NAME(weapon), GET_OBJ_VNUM(weapon));
      mudlog(buf, ch, LOG_SYSLOG, TRUE);
  ...
  for (int waff_index = 0;
       acceptable_weapon_attachment_affects[waff_index] != -1; waff_index++) {
    // If we would remove one, first look at the other attachments (if any) and
    // make sure they don't have it too.
    if (attachment->obj_flags.bitvector.IsSet(
            acceptable_weapon_attachment_affects[waff_index])) {
      bool found_duplicate = FALSE;
      for (int attach_loc = ACCESS_LOCATION_TOP;
           attach_loc <= ACCESS_LOCATION_UNDER; attach_loc++) {
      ...
      if (!found_duplicate)
        weapon->obj_flags.bitvector.RemoveBit(
            acceptable_weapon_attachment_affects[waff_index]);
    }
  }
...
void copy_over_necessary_info(struct char_data *original,
...
char *double_up_color_codes(const char *string) {
  ...
  if (strlen(string) * 2 + 1 > sizeof(doubledbuf)) {
    mudlog("SYSERR: Size of string passed to double_up_color_codes exceeds max "
           "size; aborting process.",
           NULL, LOG_SYSLOG, TRUE);
    return NULL;
  }
...
// Wipes out all the various fiddly bits so we don't have to remember to do it
// every time.
void clear_editing_data(struct descriptor_data *d) {
  // This is distinct from free_editing_structs()! That one purges out memory,
  // this just unsets flags.
  d->edit_number = 0;
  PLR_FLAGS(d->character).RemoveBit(PLR_EDITING);
  d->edit_convert_color_codes = FALSE;

  // We're setting things to NULL here. If you don't want to leak memory, clean
  // it up beforehand.
  d->edit_room = NULL;
}

// Sets a character's skill, with bounds. Assumes that you've already deducted
// the appropriate cost.
void set_character_skill(struct char_data *ch, int skill_num, int new_value,
                         bool send_message) {
  char msgbuf[500];

  if (!ch) {
    mudlog("SYSERR: NULL character passed to set_character_skill.", ch,
           LOG_SYSLOG, TRUE);
    return;
  }
  ...
  if (skill_num < SKILL_ATHLETICS || skill_num >= MAX_SKILLS) {
    sprintf(msgbuf,
            "SYSERR: Invalid skill number %d passed to set_character_skill.",
            skill_num);
    mudlog(msgbuf, ch, LOG_SYSLOG, TRUE);
    return;
  }

  if (new_value < 0 || (!access_level(ch, LVL_BUILDER) &&
                        new_value > MAX_SKILL_LEVEL_FOR_MORTS)) {
    sprintf(msgbuf,
            "SYSERR: Attempting to assign skill level %d to %s, which exceeds "
            "range 0 <= x <= %d.",
            new_value, GET_CHAR_NAME(ch),
            access_level(ch, LVL_BUILDER) ? MAX_SKILL_LEVEL_FOR_IMMS
                                          : MAX_SKILL_LEVEL_FOR_MORTS);
    mudlog(msgbuf, ch, LOG_SYSLOG, TRUE);
    return;
  }

  if (new_value == REAL_SKILL(ch, skill_num)) {
    // This is not an error condition (think restoring an imm and skillsetting
    // everything); just silently fail.
    return;
  }
  ...
  if (send_message) {
    ...
    if (skills[skill_num].type == SKILL_TYPE_ACTIVE) {
      if (new_value == 0) {
        ...
        send_to_char(ch, "^gYou've practiced so much that you can act without "
                         "thinking about it.^n\r\n");
      } else if (new_value == 7) {
        send_to_char(ch, "^gYou are considered an expert in your field.^n\r\n");
      } else if (new_value == 8) {
        send_to_char(ch,
                     "^GYour talents at %s are considered world-class.^n\r\n",
                     skills[skill_num].name);
      } else {
        send_to_char(
            ch, "^GYou further hone your talents towards perfection.^n\r\n");
      }
    }
    // Knowledge skill messaging.
      ...
      if (new_value == 0) {
        ...
        send_to_char(ch,
                     "^cYou have a dedicated knowledge of that area.^n\r\n");
      } else if (new_value == 4) {
        ...
        send_to_char(
            ch, "^GYou further hone your knowledge towards perfection.^n\r\n");
      }
    }
  }
...
char *how_good(int skill, int rank) {
  static char buf[256];
  sprintf(buf, " (%s)",
          skill_rank_name(rank, skills[skill].type == SKILL_TYPE_KNOWLEDGE));
  return buf;
}
...
// Un-nests contained objects until it figures out who's actually carrying the
// object (if anyone).
...
// Un-nests contained objects until it figures out who's actually wearing the
// object (if anyone).
...
// Finds the object's holder (either carrying or wearing it or its parent object
// recursively).
...
// Creates a NEW loggable string from an object. YOU MUST DELETE [] THE OUTPUT
// OF THIS.
char *generate_new_loggable_representation(struct obj_data *obj) {
  ...
  if (!obj) {
    strcpy(log_string,
           "SYSERR: Null object passed to generate_loggable_representation().");
    mudlog(log_string, NULL, LOG_SYSLOG, TRUE);
    return str_dup(log_string);
  }

  sprintf(log_string, "(obj %ld) %s%s", GET_OBJ_VNUM(obj), obj->text.name,
  ...
  if (obj->contains) {
    ...
    for (struct obj_data *temp = obj->contains; temp;
      ...
      sprintf(buf3, " %s%s%s",
              ...
              representation, temp->next_content ? ";" : "");
  ...
  case ITEM_MONEY:
    // The only time we'll ever hit perform_give with money is if it's a
    // credstick.
  ...
  case ITEM_DECK_ACCESSORY:
    ...
    if (GET_OBJ_VAL(obj, 0) == TYPE_PARTS) {
      sprintf(ENDOF(log_string), ", containing %d nuyen worth of %s",
              GET_OBJ_COST(obj),
              GET_OBJ_VAL(obj, 1) ? "optical chips" : "parts");
    }
    break;
  case ITEM_MAGIC_TOOL:
    // Summoning materials.
    if (GET_OBJ_VAL(obj, 0) == TYPE_SUMMONING) {
      sprintf(ENDOF(log_string),
              ", containing %d nuyen worth of summoning materials",
              GET_OBJ_COST(obj));
    }
    break;
  case ITEM_GUN_AMMO:
    // A box of ammunition.
    sprintf(ENDOF(log_string), ", containing %d units of %s %s ammo",
            GET_OBJ_VAL(obj, 0), ammo_type[GET_OBJ_VAL(obj, 2)].name,
...
void purgelog(struct veh_data *veh) {
  ...
  sprintf(
      internal_buffer,
      "- Writing purgelog for vehicle %s (vnum %ld, idnum %ld, owned by %ld).",
      GET_VEH_NAME(veh), GET_VEH_VNUM(veh), veh->idnum, veh->owner);
  mudlog(internal_buffer, NULL, LOG_PURGELOG, TRUE);

  // Log its mounts.
  for (struct obj_data *mount = veh->mount; mount;
       mount = mount->next_content) {
  ...
  for (int x = 0; x < NUM_MODS; x++) {
    ...
    sprintf(internal_buffer, "- Mod attached to the %s: %s", mod_name[x],
            representation);
    mudlog(internal_buffer, NULL, LOG_PURGELOG, TRUE);
    delete[] representation;
  }
...
// Copy Source into Dest, replacing the target substring in Source with the
// specified replacement. Requirement: dest's max size must be greater than
// source's current size plus all the replacements being done in it.
char *replace_substring(char *source, char *dest, const char *replace_target,
  ...
  for (unsigned long i = 0; i < strlen(source); i++) {
    ...
    if (source[i] == *(replace_target_ptr)) {
      ...
      if (!*(++replace_target_ptr)) {
        // If we have, we've matched the full thing. Plop the replacement into
        // dest and reset.
      ...
      // They're not equal. If we've incremented RTP, we know we've skipped
      // characters, so it's time to unwind that.
      for (unsigned long diff = replace_target_ptr - replace_target; diff > 0;
           diff--) {
