...
bool attach_attachment_to_weapon(struct obj_data *attachment,
                                 struct obj_data *weapon, struct char_data *ch);
...
#define CREATE(result, type, number)                                           \
  do {                                                                         \
    if ((number) * sizeof(type) <= 0)                                          \
      log_vfprintf("SYSERR: Zero bytes or less requested at %s:%d.", __FILE__, \
                   __LINE__);                                                  \
...
#define IS_PROJECT(ch)                                                         \
  (IS_NPC(ch) && ch->desc && ch->desc->original &&                             \
   PLR_FLAGS(ch->desc->original).IsSet(PLR_PROJECT) && GET_MOB_VNUM(ch) == 22)
...
#define IS_DUAL(ch)                                                            \
  ((IS_NPC(ch) && MOB_FLAGS(ch).IsSet(MOB_DUAL_NATURE)) ||                     \
   (!IS_NPC(ch) &&                                                             \
    (PLR_FLAGS(ch).IsSet(PLR_PERCEIVE) || access_level(ch, LVL_ADMIN))))
...
#define D_PRF_FLAGGED(d, flag)                                                 \
  ((d)->original                                                               \
       ...
       : ((d)->character ? PRF_FLAGS((d)->character).IsSet((flag)) : FALSE))
#define ROOM_FLAGGED(loc, flag)                                                \
  ((loc) ? (loc)->room_flags.IsSet((flag)) : FALSE)
...
#define IS_WATER(room)                                                         \
  (SECT((room)) == SPIRIT_LAKE || SECT((room)) == SPIRIT_RIVER ||              \
   SECT((room)) == SPIRIT_SEA)

#define IS_DARK(room) (light_level((room)) == ITEM_FULLDARK)
#define IS_LIGHT(room)                                                         \
  (light_level((room)) <= LIGHT_NORMALNOLIT ||                                 \
   light_level((room)) == LIGHT_PARTLIGHT)
#define IS_LOW(room)                                                           \
  (light_level((room)) == LIGHT_MINLIGHT ||                                    \
   light_level((room)) == LIGHT_PARTLIGHT)

#define GET_ROOM_NAME(room) ((room) ? (room)->name : "(null room name)")
#define GET_ROOM_DESC(room)                                                    \
  ((room) ? ((room)->night_desc && weather_info.sunlight == SUN_DARK           \
                 ? (room)->night_desc                                          \
                 : (room)->description)                                        \
          : "(null room desc)")
...
#define GET_ROOM_VNUM(room) ((vnum_t)((room) ? (room)->number : NOWHERE))
...
#define GET_VEH_NAME(veh)                                                      \
  (decapitalize_a_an((veh)->restring ? (veh)->restring                         \
                                     : (veh)->short_description))
#define GET_VEH_DESC(veh)                                                      \
  ((veh)->restring_long ? (veh)->restring_long : (veh)->long_description)
...
#define GET_CHAR_NAME(ch)                                                      \
  (((ch)->desc && (ch)->desc->original)                                        \
       ? (ch)->desc->original->player.char_name                                \
       : ((ch)->player.char_name ? (ch)->player.char_name : GET_NAME((ch))))
...
#define GET_DESC_LEVEL(d)                                                      \
  ((d)->original ? GET_LEVEL((d)->original)                                    \
                 : ((d)->character ? GET_LEVEL((d)->character) : 0))
...
#define GET_BIOOVER(ch)                                                        \
  (-((int)((GET_ESS((ch)) + 300) - GET_INDEX((ch))) / 100))
...
#define GET_SPELLS(ch)                                                         \
  ((ch)->desc && (ch)->desc->original ? (ch)->desc->original->spells           \
                                      : (ch)->spells)
...
#define GET_QUEST(ch)                                                          \
  ((ch)->desc && (ch)->desc->original                                          \
       ? (ch)->desc->original->player_specials->questnum                       \
...
#define GET_PROMPT(ch)                                                         \
  ((PLR_FLAGGED((ch), PLR_MATRIX) ? (ch)->player.matrixprompt                  \
                                  : (ch)->player.prompt))
#define GET_ALIASES(ch)                                                        \
  ((ch)->desc && (ch)->desc->original                                          \
       ? (ch)->desc->original->player_specials->aliases                        \
...
#define RIG_VEH(ch, veh)                                                       \
  ((veh) = ((ch)->char_specials.rigging ? (ch)->char_specials.rigging          \
                                        : (ch)->in_veh));
...
#define GET_SKILL(ch, i)                                                       \
  ((ch)->char_specials.saved.skills[i][1] > 0                                  \
       ? (ch)->char_specials.saved.skills[i][1]                                \
       : (ch)->char_specials.saved.skills[i][0])
#define REAL_SKILL(ch, i)                                                      \
  ((ch)->char_specials.saved.skills[i][1] > 0                                  \
       ? 0                                                                     \
       : (ch)->char_specials.saved.skills[i][0])
// SET_SKILL is used only in medit.cpp for NPCs. Set char skills with
// utils.cpp's set_character_skill().
#define SET_SKILL(ch, i, pct)                                                  \
  {                                                                            \
    (ch)->char_specials.saved.skills[i][0] = pct;                              \
    GET_SKILL_DIRTY_BIT((ch)) = TRUE;                                          \
  }
#define GET_POWER(ch, i)                                                       \
  ((ch)->char_specials.saved.powers[i][1]                                      \
       ? MIN((ch)->char_specials.saved.powers[i][1],                           \
             (ch)->char_specials.saved.powers[i][0])                           \
       : 0)
...
#define CAN_CARRY_N(ch)                                                        \
  (8 + GET_QUI(ch) + (GET_REAL_LEVEL(ch) >= LVL_BUILDER ? 50 : 0))
#define AWAKE(ch) (GET_POS(ch) > POS_SLEEPING && GET_QUI(ch) > 0)
#define CAN_SEE_IN_DARK(ch)                                                    \
  ((IS_ASTRAL(ch) || IS_DUAL(ch) || CURRENT_VISION(ch) == THERMOGRAPHIC ||     \
    PRF_FLAGGED((ch), PRF_HOLYLIGHT)))
#define GET_BUILDING(ch) ((ch)->char_specials.programming)
#define IS_WORKING(ch)                                                         \
  ((AFF_FLAGS(ch).AreAnySet(AFF_PROGRAM, AFF_DESIGN, AFF_PART_BUILD,           \
                            AFF_PART_DESIGN, AFF_PILOT, AFF_RIG, AFF_BONDING,  \
                            AFF_CONJURE, AFF_PACKING, AFF_LODGE, AFF_CIRCLE,   \
                            AFF_SPELLDESIGN, AFF_AMMOBUILD, ENDBIT)))
#define STOP_WORKING(ch)                                                       \
  {                                                                            \
    AFF_FLAGS((ch)).RemoveBits(AFF_PROGRAM, AFF_DESIGN, AFF_PART_BUILD,        \
                               AFF_PART_DESIGN, AFF_BONDING, AFF_CONJURE,      \
                               AFF_PACKING, AFF_LODGE, AFF_CIRCLE,             \
                               AFF_SPELLDESIGN, AFF_AMMOBUILD, ENDBIT);        \
 ...
 */
#define VALID_OBJ_RNUM(obj)                                                    \
  (GET_OBJ_RNUM(obj) <= top_of_objt && GET_OBJ_RNUM(obj) != NOTHING)
...
#define GET_OBJ_VNUM(obj)                                                      \
  (VALID_OBJ_RNUM(obj) ? obj_index[GET_OBJ_RNUM(obj)].vnum : NOTHING)
...
#define GET_OBJ_SPEC(obj)                                                      \
  (VALID_OBJ_RNUM(obj) ? obj_index[GET_OBJ_RNUM(obj)].func : NULL)
#define GET_WSPEC(obj)                                                         \
  ((obj)->item_number >= 0 ? (obj_index[GET_OBJ_RNUM(obj)].wfunc) : NULL)
...
#define IS_GUN(type)                                                           \
  ((((type) >= WEAP_HOLDOUT) && ((type) < WEAP_GREN_LAUNCHER)) ||              \
   (type) == WEAP_REVOLVER)
...
#define ANA(obj)                                                               \
  (strchr((const char *)"aeiouyAEIOUY", *(obj)->text.keywords) ? "An" : "A")
#define SANA(obj)                                                              \
  (strchr((const char *)"aeiouyAEIOUY", *(obj)->text.keywords) ? "an" : "a")

/* Various macros building up to CAN_SEE */

#define HOLYLIGHT_OK(sub)                                                      \
  (GET_REAL_LEVEL(sub) >= LVL_BUILDER && PRF_FLAGGED((sub), PRF_HOLYLIGHT))

#define LIGHT_OK_ROOM_SPECIFIED(sub, provided_room)                            \
  (IS_ASTRAL(sub) || IS_DUAL(sub) || CURRENT_VISION((sub)) == THERMOGRAPHIC || \
   HOLYLIGHT_OK(sub) || IS_LIGHT(provided_room) ||                             \
   !((light_level(provided_room) == LIGHT_MINLIGHT ||                          \
      light_level(provided_room) == LIGHT_FULLDARK) &&                         \
     CURRENT_VISION((sub)) == NORMAL))

#define LIGHT_OK(sub)                                                          \
  ((IS_ASTRAL(sub) || IS_DUAL(sub) || CURRENT_VISION(sub) == THERMOGRAPHIC ||  \
    HOLYLIGHT_OK(sub) || IS_LIGHT(get_ch_in_room(sub))) ||                     \
   !((light_level(get_ch_in_room(sub)) == LIGHT_MINLIGHT ||                    \
      light_level(get_ch_in_room(sub)) == LIGHT_FULLDARK) &&                   \
     CURRENT_VISION(sub) == NORMAL))

#define SELF(sub, obj) ((sub) == (obj))

#define SEE_ASTRAL(sub, obj)                                                   \
  (!IS_ASTRAL(obj) || IS_ASTRAL(sub) || IS_DUAL(sub) ||                        \
   AFF_FLAGGED(obj, AFF_MANIFEST))
...
#define CAN_SEE_ROOM_SPECIFIED(sub, obj, room_specified)                       \
  (!(MOB_FLAGGED(obj, MOB_TOTALINVIS) && GET_LEVEL(sub) < LVL_BUILDER) &&      \
   (SELF((sub), (obj)) ||                                                      \
    (SEE_ASTRAL((sub), (obj)) &&                                               \
     LIGHT_OK_ROOM_SPECIFIED(sub, room_specified) && invis_ok(sub, obj) &&     \
...
#define INVIS_OK_OBJ(sub, obj)                                                 \
  (!IS_OBJ_STAT((obj), ITEM_INVISIBLE) ||                                      \
   IS_AFFECTED((sub), AFF_DETECT_INVIS) || IS_ASTRAL(sub) || IS_DUAL(sub) ||   \
   HOLYLIGHT_OK(sub))
...
#define CAN_SEE_OBJ(sub, obj)                                                  \
  (LIGHT_OK(sub) && INVIS_OK_OBJ((sub), (obj)) && CAN_SEE_CARRIER((sub), (obj)))
...
#define PERS2(ch, vict)                                                        \
  ((GET_INVIS_LEV(ch) <= 0 || access_level(vict, GET_INVIS_LEV(ch)))           \
       ? GET_NAME(ch)                                                          \
       : "Someone")

#define OBJS(obj, vict)                                                        \
  (CAN_SEE_OBJ((vict), (obj)) ? GET_OBJ_NAME((obj)) : "something")

#define OBJN(obj, vict)                                                        \
  (CAN_SEE_OBJ((vict), (obj)) ? fname((obj)->text.keywords) : "something")

// Accepts characters or vehicles.
#define EXIT(thing, door)                                                      \
  (thing->in_room ? thing->in_room->dir_option[door] : NULL)

#define LOCK_LEVEL(ch, obj, door)                                              \
  ((obj) ? GET_OBJ_VAL(obj, 4)                                                 \
...
#define CAN_GO(ch, door)                                                       \
  (EXIT(ch, door) &&                                                           \
   ...
   !(IS_ASTRAL(ch) &&                                                          \
     IS_SET(EXIT(ch, door)->exit_info, EX_ASTRALLY_WARDED)) &&                 \
   !(ROOM_FLAGGED(EXIT(ch, door)->to_room, ROOM_STAFF_ONLY) &&                 \
...
// This guarantees that the pointer will be null after deletion (something
// that's plagued this codebase for years).
#define DELETE_AND_NULL_ARRAY(target)                                          \
  {                                                                            \
    delete[](target);                                                          \
    (target) = NULL;                                                           \
  }
#define DELETE_AND_NULL(target)                                                \
  {                                                                            \
    delete (target);                                                           \
    (target) = NULL;                                                           \
  }
...
#define DELETE_ARRAY_IF_EXTANT(target)                                         \
  {                                                                            \
    if ((target))                                                              \
      delete[](target);                                                        \
    (target) = NULL;                                                           \
  }
#define DELETE_IF_EXTANT(target)                                               \
  {                                                                            \
    if ((target))                                                              \
      delete (target);                                                         \
    (target) = NULL;                                                           \
...
#define GET_PART_DESIGN_COMPLETION(part)                                       \
  (GET_OBJ_VAL((part), 3)) // 0 = done, -1 = required ?
...
#define GET_WEAPON_ATTACH_LOC(weapon, loc)                                     \
  (((loc) >= ACCESS_LOCATION_TOP && (loc) <= ACCESS_LOCATION_UNDER)            \
       ? GET_OBJ_VAL((weapon), (loc))                                          \
       : 0)

#define WEAPON_CAN_USE_FIREMODE(weapon, mode)                                  \
  (IS_SET(GET_WEAPON_POSSIBLE_FIREMODES(weapon), 1 << (mode)))
...
#define GET_CYBERWARE_FLAGS(cyberware)                                         \
  (GET_OBJ_VAL((cyberware), 3)) // CYBERWEAPON_RETRACTABLE, CYBERWEAPON_IMPROVED
#define GET_CYBERWARE_LACING_TYPE(cyberware)                                   \
  (GET_OBJ_VAL((cyberware), 3)) // Yes, this is also value 3. Great design here.
...
#define GET_CYBERDECK_FREE_STORAGE(deck)                                       \
  (GET_CYBERDECK_TOTAL_STORAGE((deck)) - GET_CYBERDECK_USED_STORAGE((deck)))
...
#define IS_DAMTYPE_PHYSICAL(type)                                              \
  !((type) == TYPE_HIT || (type) == TYPE_BLUDGEON || (type) == TYPE_PUNCH ||   \
    (type) == TYPE_TASER || (type) == TYPE_CRUSH || (type) == TYPE_POUND)

// If d->edit_convert_color_codes is true, doubles up ^ marks to not print color
// codes to the user in the specified string. Uses a static buf, so only one
// invocation per sprintf()!
#define DOUBLE_UP_COLOR_CODES_IF_NEEDED(str)                                   \
  (d ? (d->edit_convert_color_codes ? double_up_color_codes((str)) : (str))    \
     : (str))
